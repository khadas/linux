...
#undef pr_fmt
#define pr_fmt(fmt) "a4_audio_clocks: " fmt
...
static const char *const mclk_parent_names[] = {
	"null", "null", "null", "rtc_pll", "hifi_pll",
...
static struct clk_gate *a4_audio_clk_gates[] = {
	&audio_tdmina,
	&audio_tdminlb,
	&audio_tdmouta,
	&audio_tdmoutb,
...
static struct clk_gate *a4_audio2_clk_gates[] = {
	&audio2_ddr_arb,
	&audio2_pdm,
	&audio2_tdminc,
	&audio2_toddrc,
...
static struct clk_hw *a4_audio_clk_hws[] = {
...
static int a4_clk_gates_init(struct clk **clks, void __iomem *iobase)
{
	int clkid;

	if (ARRAY_SIZE(a4_audio_clk_gates) != CLKID_AUDIO_GATE_MAX) {
		pr_err("check clk gates number\n");
		return -EINVAL;
	}

	for (clkid = 0; clkid < CLKID_AUDIO_GATE_MAX; clkid++) {
		a4_audio_clk_gates[clkid]->reg = iobase;
		clks[clkid] = clk_register(NULL, a4_audio_clk_hws[clkid]);
...
static int a4_clk2_gates_init(struct clk **clks, void __iomem *iobase)
{
	int clkid;

	if (ARRAY_SIZE(a4_audio2_clk_gates)
	...
	for (clkid = CLKID_AUDIO_GATE_MAX;
		...
		a4_audio2_clk_gates[clkid - CLKID_AUDIO_GATE_MAX]->reg = iobase;
		clks[clkid] = clk_register(NULL, a4_audio_clk_hws[clkid]);
		WARN_ON(IS_ERR_OR_NULL(clks[clkid]));
	}
...
static int a4_clks_init(struct clk **clks, void __iomem *iobase)
{
...
static int a4_clks2_init(struct clk **clks, void __iomem *iobase)
{
...
struct audio_clk_init a4_audio_clks_init = {
	.clk_num   = NUM_AUDIO_CLKS,
	.clk_gates = a4_clk_gates_init,
	.clks      = a4_clks_init,
	.clk2_gates = a4_clk2_gates_init,
	.clks2      = a4_clks2_init,
};
