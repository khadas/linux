...
#undef pr_fmt
#define pr_fmt(fmt) "t3_audio_clocks: " fmt
...
static struct clk_gate *t3_audio_clk_gates[] = {
	&audio_ddr_arb,
	&audio_pdm,
	&audio_tdmina,
	&audio_tdminb,
...
static struct clk_hw *t3_audio_clk_hws[] = {
...
static int t3_clk_gates_init(struct clk **clks, void __iomem *iobase)
{
	int clkid;

	if (ARRAY_SIZE(t3_audio_clk_gates) != MCLK_BASE) {
	...
	for (clkid = 0; clkid < MCLK_BASE; clkid++) {
		unsigned long offset =
			(unsigned long)t3_audio_clk_gates[clkid]->reg;
		t3_audio_clk_gates[clkid]->reg =
			(void __iomem *)((unsigned long)iobase + offset);
		clks[clkid] = clk_register(NULL, t3_audio_clk_hws[clkid]);
...
static int t3_clks_init(struct clk **clks, void __iomem *iobase)
{
...
struct audio_clk_init t3_audio_clks_init = {
	.clk_num   = NUM_AUDIO_CLKS,
	.clk_gates = t3_clk_gates_init,
	.clks      = t3_clks_init,
};
