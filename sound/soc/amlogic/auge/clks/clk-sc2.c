...
#undef pr_fmt
#define pr_fmt(fmt) "sc2_audio_clocks: " fmt
...
CLOCK_GATE(audio_reserved1, AUD_ADDR_OFFSET(EE_AUDIO_CLK_GATE_EN0), 23, sys_clk);
CLOCK_GATE(audio_reserved2, AUD_ADDR_OFFSET(EE_AUDIO_CLK_GATE_EN0), 24, sys_clk);
CLOCK_GATE(audio_reserved3, AUD_ADDR_OFFSET(EE_AUDIO_CLK_GATE_EN0), 25, sys_clk);
...
CLOCK_GATE(audio_reserved4, AUD_ADDR_OFFSET(EE_AUDIO_CLK_GATE_EN0), 30, sys_clk);
CLOCK_GATE(audio_reserved5, AUD_ADDR_OFFSET(EE_AUDIO_CLK_GATE_EN0), 31, sys_clk);
...
static struct clk_gate *sc2_audio_clk_gates[] = {
	&audio_ddr_arb,
	&audio_pdm,
	&audio_tdmina,
	&audio_tdminb,
...
static struct clk_hw *sc2_audio_clk_hws[] = {
...
static int sc2_clk_gates_init(struct clk **clks, void __iomem *iobase)
{
	int clkid;

	if (ARRAY_SIZE(sc2_audio_clk_gates) != MCLK_BASE) {
	...
	for (clkid = 0; clkid < MCLK_BASE; clkid++) {
		unsigned long offset =
			(unsigned long)sc2_audio_clk_gates[clkid]->reg;
		sc2_audio_clk_gates[clkid]->reg =
			(void __iomem *)((unsigned long)iobase + offset);
		clks[clkid] = clk_register(NULL, sc2_audio_clk_hws[clkid]);
...
static int sc2_clks_init(struct clk **clks, void __iomem *iobase)
{
...
struct audio_clk_init sc2_audio_clks_init = {
	.clk_num   = NUM_AUDIO_CLKS,
	.clk_gates = sc2_clk_gates_init,
	.clks      = sc2_clks_init,
};
