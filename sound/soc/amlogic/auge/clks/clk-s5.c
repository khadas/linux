...
#undef pr_fmt
#define pr_fmt(fmt) "s5_audio_clocks: " fmt
...
static const char *const mclk_parent_names[] = {
	"hifi1_pll", "nna_pll", "mpll2", "mpll3", "hifi_pll",
...
CLOCK_GATE(audio_tdmind, AUD_ADDR_OFFSET(EE_AUDIO_CLK_GATE_EN1), 11, sys_clk);
CLOCK_GATE(audio_tdmoutd, AUD_ADDR_OFFSET(EE_AUDIO_CLK_GATE_EN1), 12, sys_clk);
CLOCK_GATE(audio_pcpd_mon_a, AUD_ADDR_OFFSET(EE_AUDIO_CLK_GATE_EN1), 15, sys_clk);
CLOCK_GATE(audio_pcpd_mon_b, AUD_ADDR_OFFSET(EE_AUDIO_CLK_GATE_EN1), 16, sys_clk);


static struct clk_gate *s5_audio_clk_gates[] = {
	&audio_ddr_arb,
	&audio_pdm,
	&audio_tdmina,
	&audio_tdminb,
...
static struct clk_hw *s5_audio_clk_hws[] = {
	...
	[CLKID_AUDIO_GATE_RESERVED1]   = &audio_toram.hw,
	...
	[CLKID_AUDIO_GATE_TDMIND]	   = &audio_tdmind.hw,
	[CLKID_AUDIO_GATE_TDMOUTD]	   = &audio_tdmoutd.hw,
	[CLKID_AUDIO_GATE_PCPD_A]	   = &audio_pcpd_mon_a.hw,
	[CLKID_AUDIO_GATE_PCPD_B]	   = &audio_pcpd_mon_b.hw,
};

static int s5_clk_gates_init(struct clk **clks, void __iomem *iobase)
{
	int clkid;

	if (ARRAY_SIZE(s5_audio_clk_gates) != MCLK_BASE) {
	...
	for (clkid = 0; clkid < MCLK_BASE; clkid++) {
		unsigned long offset =
			(unsigned long)s5_audio_clk_gates[clkid]->reg;
		s5_audio_clk_gates[clkid]->reg =
			(void __iomem *)((unsigned long)iobase + offset);
		clks[clkid] = clk_register(NULL, s5_audio_clk_hws[clkid]);
...
static int s5_clks_init(struct clk **clks, void __iomem *iobase)
{
...
struct audio_clk_init s5_audio_clks_init = {
	.clk_num   = NUM_AUDIO_CLKS,
	.clk_gates = s5_clk_gates_init,
	.clks      = s5_clks_init,
};
