...
typedef int (*func_noml)(struct asoc_simple_priv *priv,
			 struct device_node *np,
			 struct device_node *codec,
			 struct link_info *li, bool is_top);
typedef int (*func_dpcm)(struct asoc_simple_priv *priv,
			 struct device_node *np,
			 struct device_node *codec,
			 struct link_info *li, bool is_top);
	...
	do {
		...
		for_each_child_of_node(node, np) {
			...
			if (dpcm_selectable &&
				...
				ret = func_d(priv, np, codec, li, is_top);
			/* else normal sound */
			else
				ret = func_n(priv, np, codec, li, is_top);
