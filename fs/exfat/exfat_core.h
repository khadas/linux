...
struct FS_FUNC_T {
	s32 (*alloc_cluster)(struct super_block *sb,
			     s32 num_alloc, struct CHAIN_T *p_chain);
	void (*free_cluster)(struct super_block *sb,
			      struct CHAIN_T *p_chain, s32 do_relse);
	 s32 (*count_used_clusters)(struct super_block *sb);

	 s32 (*init_dir_entry)(struct super_block *sb,
			       struct CHAIN_T *p_dir, s32 entry, u32 type,
			       u32 start_clu, u64 size);
	 s32 (*init_ext_entry)(struct super_block *sb,
			       struct CHAIN_T *p_dir, s32 entry,
			       s32 num_entries, struct UNI_NAME_T *p_uniname,
	...
	void (*delete_dir_entry)(struct super_block *sb,
				  struct CHAIN_T *p_dir, s32 entry, s32 offset,
				  s32 num_entries);
...
struct ENTRY_SET_CACHE_T {
	...
	/* flag in stream entry.
	   01 for cluster chain, 03 for contig. clusteres. */
	s32 alloc_flag;
	u32 num_entries;
...
s32 ffsGetVolInfo(struct super_block *sb, struct VOL_INFO_T *info);
s32 ffsSyncVol(struct super_block *sb, s32 do_sync);

/* file management functions */
s32 ffsLookupFile(struct inode *inode, char *path, struct FILE_ID_T *fid);
s32 ffsCreateFile(struct inode *inode, char *path, u8 mode,
		  struct FILE_ID_T *fid);
s32 ffsReadFile(struct inode *inode, struct FILE_ID_T *fid, void *buffer,
		u64 count, u64 *rcount);
s32 ffsWriteFile(struct inode *inode, struct FILE_ID_T *fid, void *buffer,
		 u64 count, u64 *wcount);
s32 ffsTruncateFile(struct inode *inode, u64 old_size, u64 new_size);
s32 ffsMoveFile(struct inode *old_parent_inode, struct FILE_ID_T *fid,
		struct inode *new_parent_inode, struct dentry *new_dentry);
s32 ffsRemoveFile(struct inode *inode, struct FILE_ID_T *fid);
s32 ffsSetAttr(struct inode *inode, u32 attr);
s32 ffsGetStat(struct inode *inode, struct DIR_ENTRY_T *info);
s32 ffsSetStat(struct inode *inode, struct DIR_ENTRY_T *info);
s32 ffsMapCluster(struct inode *inode, s32 clu_offset, u32 *clu);

/* directory management functions */
s32 ffsCreateDir(struct inode *inode, char *path, struct FILE_ID_T *fid);
s32 ffsReadDir(struct inode *inode, struct DIR_ENTRY_T *dir_ent);
s32 ffsRemoveDir(struct inode *inode, struct FILE_ID_T *fid);
...
void fat_get_entry_time(struct DENTRY_T *p_entry,
			struct TIMESTAMP_T *tp, u8 mode);
void exfat_get_entry_time(struct DENTRY_T *p_entry,
			  struct TIMESTAMP_T *tp, u8 mode);
void fat_set_entry_time(struct DENTRY_T *p_entry,
			struct TIMESTAMP_T *tp, u8 mode);
void exfat_set_entry_time(struct DENTRY_T *p_entry,
			  struct TIMESTAMP_T *tp, u8 mode);
s32 fat_init_dir_entry(struct super_block *sb,
		       struct CHAIN_T *p_dir, s32 entry, u32 type,
		       u32 start_clu, u64 size);
...
void init_ext_entry(struct EXT_DENTRY_T *ep,
		    s32 order, u8 chksum, u16 *uniname);
void init_file_entry(struct FILE_DENTRY_T *ep, u32 type);
void init_strm_entry(struct STRM_DENTRY_T *ep, u8 flags,
		     u32 start_clu, u64 size);
void init_name_entry(struct NAME_DENTRY_T *ep, u16 *uniname);
void fat_delete_dir_entry(struct super_block *sb,
			  struct CHAIN_T *p_dir, s32 entry, s32 order,
			  s32 num_entries);
void exfat_delete_dir_entry(struct super_block *sb, struct CHAIN_T *p_dir,
			    s32 entry, s32 order, s32 num_entries);

s32 find_location(struct super_block *sb, struct CHAIN_T *p_dir,
		  s32 entry, u32 *sector, s32 *offset);
struct DENTRY_T *get_entry_with_sector(struct super_block *sb,
				       u32 sector, s32 offset);
struct DENTRY_T *get_entry_in_dir(struct super_block *sb,
				  struct CHAIN_T *p_dir, s32 entry,
				  u32 *sector);
...
s32 get_num_entries_and_dos_name(struct super_block *sb,
...
s32 resolve_path(struct inode *inode, char *path,
		 struct CHAIN_T *p_dir, struct UNI_NAME_T *p_uniname);
...
s32 rename_file(struct inode *inode, struct CHAIN_T *p_dir, s32 old_entry,
		struct UNI_NAME_T *p_uniname, struct FILE_ID_T *fid);
s32 move_file(struct inode *inode, struct CHAIN_T *p_olddir,
	      s32 oldentry, struct CHAIN_T *p_newdir,
	      struct UNI_NAME_T *p_uniname, struct FILE_ID_T *fid);

/* sector read/write functions */
s32 sector_read(struct super_block *sb, u32 sec,
		struct buffer_head **bh, s32 read);
s32 sector_write(struct super_block *sb, u32 sec,
		 struct buffer_head *bh, s32 sync);
s32 multi_sector_read(struct super_block *sb, u32 sec,
		      struct buffer_head **bh, s32 num_secs, s32 read);
s32 multi_sector_write(struct super_block *sb, u32 sec,
		       struct buffer_head *bh, s32 num_secs, s32 sync);
