/* Some of the source code in this file came from
* "linux/fs/fat/file.c","linux/fs/fat/inode.c"
* and "linux/fs/fat/misc.c".  */
/*
 *  linux/fs/fat/file.c
 *
			...
			bool excl)
{
	...
	/* timestamp is already written,
	   so mark_inode_dirty() is unnecessary. */
			...
			struct inode *new_dir, struct dentry *new_dentry)
{
	...
	err =
	    FsMoveFile(old_dir, &(EXFAT_I(old_inode)->fid), new_dir,
		       new_dentry);
	if (err) {
		if (err == FFS_PERMISSIONERR)
			err = -EPERM;
...
static int exfat_cont_expand(struct inode *inode, loff_t size)
{
	...
	if (IS_SYNC(inode)) {
		err =
		    filemap_fdatawrite_range(mapping, start, start + count - 1);
		...
		if (!err)
			err =
			    filemap_fdatawait_range(mapping, start,
						    start + count - 1);
	}
	return err;
}
			       ...
			       struct inode *inode, umode_t *mode_ptr)
{
	...
	if (exfat_mode_can_hold_ro(inode)) {
		/* Of the w bits, either all
		   (subject to umask) or none must be present. */
		if ((perm & S_IWUGO) && ((perm & S_IWUGO) != (S_IWUGO & ~mask)))
			return -EPERM;
	} else {
		/* If exfat_mode_can_hold_ro(inode)
		   is false, can't change w bits. */
	...
	*mapped_blocks = 0;
	...
	if ((p_fs->vol_type == FAT12) || (p_fs->vol_type == FAT16)) {
		if (inode->i_ino == EXFAT_ROOT_INO) {
			if (sector <
			    (p_fs->
			     dentries_in_root >> (p_bd->sector_size_bits -
						  DENTRY_SIZE_BITS))) {
...
static int exfat_statfs(struct dentry *dentry, struct kstatfs *buf)
{
	...
	if (p_fs->used_clusters == (u32)~0) {
		...
		if (p_fs->dev_ejected)
			DPRINTK("[EXFAT] statfs on device is ejected\n");
	}
...
static int exfat_fill_super(struct super_block *sb, void *data, int silent)
{
	...
	if (ret) {
		if (!silent)
			DPRINTK(KERN_ERR "[EXFAT] FsMountVol failed\n");
	...
	if (sbi->fs_info.vol_type != EXFAT) {
		...
		if (!sbi->nls_disk) {
			DPRINTK(KERN_ERR "[EXFAT] Codepage %s not found\n",
				buf);
			goto out_fail2;
		}
	}
	...
	if (!sb->s_root) {
		DPRINTK(KERN_ERR "[EXFAT] Getting the root inode failed\n");
		goto out_fail2;
	}
...
static int __init init_exfat(void)
{
	...
	DPRINTK(KERN_INFO "exFAT: Version %s\n", EXFAT_VERSION);
