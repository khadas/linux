...
static struct BUF_CACHE_T *FAT_cache_find(struct super_block *sb, u32 sec);
static struct BUF_CACHE_T *FAT_cache_get(struct super_block *sb, u32 sec);
...
static struct BUF_CACHE_T *buf_cache_find(struct super_block *sb, u32 sec);
static struct BUF_CACHE_T *buf_cache_get(struct super_block *sb, u32 sec);
...
{
	struct FS_INFO_T *p_fs = (struct FS_INFO_T *)&(EXFAT_SB(sb)->fs_info);

	int i;

	/* LRU list */
	p_fs->FAT_cache_lru_list.next =
		(struct BUF_CACHE_T *)&p_fs->FAT_cache_lru_list;
	p_fs->FAT_cache_lru_list.prev = p_fs->FAT_cache_lru_list.prev;
	...
	for (i = 0; i < FAT_CACHE_SIZE; i++) {
		...
		p_fs->FAT_cache_array[i].prev =
		p_fs->FAT_cache_array[i].next = NULL;
		push_to_mru(&(p_fs->FAT_cache_array[i]),
				&p_fs->FAT_cache_lru_list);
	}

	p_fs->buf_cache_lru_list.next =
		(struct BUF_CACHE_T *)&p_fs->buf_cache_lru_list;
	p_fs->buf_cache_lru_list.prev = p_fs->buf_cache_lru_list.prev;
	...
	for (i = 0; i < FAT_CACHE_HASH_SIZE; i++) {
		...
		p_fs->FAT_cache_hash_list[i].hash_next =
			(struct BUF_CACHE_T *)&(p_fs->FAT_cache_hash_list[i]);
		p_fs->FAT_cache_hash_list[i].hash_prev =
			p_fs->FAT_cache_hash_list[i].prev;
	}
	...
	for (i = 0; i < BUF_CACHE_HASH_SIZE; i++) {
		...
		p_fs->buf_cache_hash_list[i].hash_next =
			(struct BUF_CACHE_T *)&(p_fs->buf_cache_hash_list[i]);
		p_fs->buf_cache_hash_list[i].hash_prev =
			p_fs->buf_cache_hash_list[i].prev;
	}
...
static s32 __FAT_read(struct super_block *sb, u32 loc, u32 *content)
{
	...
	struct FS_INFO_T *p_fs = (struct FS_INFO_T *)&(EXFAT_SB(sb)->fs_info);
	struct BD_INFO_T *p_bd = (struct BD_INFO_T *)&(EXFAT_SB(sb)->bd_info);
...
static struct BUF_CACHE_T *FAT_cache_find(struct super_block *sb, u32 sec)
{
	...
	for (bp = hp->hash_next; bp != hp; bp = bp->hash_next) {
		if ((bp->drv == p_fs->drv) && (bp->sec == sec)) {
			...
			DPRINTK("It will make system panic.\n");
	...
	struct BUF_CACHE_T *bp)
{
	...
	off = (bp->sec + (bp->sec >> p_fs->sectors_per_clu_bits)) &
			(FAT_CACHE_HASH_SIZE-1);
		...
		struct BUF_CACHE_T *bp)
{
	...
	p_fs = (struct FS_INFO_T *)&(EXFAT_SB(sb)->fs_info);
	off = (bp->sec + (bp->sec >> p_fs->sectors_per_clu_bits)) &
			(BUF_CACHE_HASH_SIZE-1);
