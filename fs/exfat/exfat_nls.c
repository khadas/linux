...
static s32  convert_uni_to_ch(struct nls_table *nls,
	u8 *ch, u16 uni, s32 *lossy);
static s32  convert_ch_to_uni(struct nls_table *nls,
	u16 *uni, u8 *ch, s32 *lossy);
...
void nls_uniname_to_dosname(struct super_block *sb,
	...
	struct UNI_NAME_T *p_uniname, s32 *p_lossy)
{
	...
	u8 *p_cstring, struct UNI_NAME_T *p_uniname)
{
	...
	if (nls == NULL) {
		len = utf16s_to_utf8s(uniname, MAX_NAME_LENGTH,
			UTF16_HOST_ENDIAN, p_cstring, MAX_NAME_LENGTH);
		p_cstring[len] = 0;
		return;
	}
	...
	struct UNI_NAME_T *p_uniname, u8 *p_cstring, s32 *p_lossy)
{
	...
	*(++end_of_name) = '\0';
	...
	if (strcmp((char *) p_cstring, ".") &&
		strcmp((char *) p_cstring, "..")) {
