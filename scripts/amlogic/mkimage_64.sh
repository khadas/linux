...
#
# Build kernel and dtb for arm64
...
find_defconfig()
{
	cnt=$(find ${DEFDIR} -name ${1}*_defconfig -type f -print | wc -l)

	def=$(find ${DEFDIR} -name ${1}*_defconfig -type f -print)
...
build_dtb()
{
	for d in ${ALLDTS}; do
		...
		make ARCH=${ARCH} CROSS_COMPILE=${CROSS_COMPILE} ${d%.dts}.dtb
...
{
	echo -e "\nExamples:"
	echo -e "  # build kernel using ${DEFCONFIG}"
	...
	echo -e "  # build kernel using defconfig with prefix <meson64>\c"
	echo -e " specified by the option -k"
	echo -e "\033[32;1m  $PRGNAME -k meson64\033[0m\n"
	...
	echo -e "\033[32;1m  $PRGNAME -k meson64 -d p320 -c\033[0m\n"
}
