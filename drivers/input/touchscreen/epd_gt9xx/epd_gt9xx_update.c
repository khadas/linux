 ...
 *      V1.6: 
 *          1. replace guitar_client with epd_gtp_i2c_connect_client;
...
//extern void epd_gtp_reset_guitar(struct i2c_client *client, s32 ms);
//extern s32  epd_gtp_send_cfg(struct i2c_client *client);
//extern s32 epd_gtp_read_version(struct i2c_client *, u16* );
//extern struct i2c_client * epd_gtp_i2c_connect_client;
//extern void epd_gtp_irq_enable(struct goodix_ts_data *ts);
//extern void epd_gtp_irq_disable(struct goodix_ts_data *ts);
//extern s32 epd_gtp_i2c_read_dbl_check(struct i2c_client *, u16, u8 *, int);
...
#if GTP_ESD_PROTECT
extern void epd_gtp_esd_switch(struct i2c_client *, s32);
#endif

#if GTP_COMPATIBLE_MODE
s32 epd_gtp_gup_fw_download_proc(void *dir, u8 dwn_mode);
#endif
/*******************************************************
Function:
...
static s32 gup_init_panel(struct goodix_ts_data *ts)
{
    ...
    {
        ret = epd_gtp_i2c_read_dbl_check(ts->client, GTP_REG_SENSOR_ID, &sensor_id, 1);
        if (SUCCESS == ret)
        {
            if (sensor_id >= 0x06)
            {
    ...
    ts->epd_gtp_cfg_len = cfg_info_len[sensor_id];
    
    if (ts->epd_gtp_cfg_len < GTP_CONFIG_MIN_LENGTH)
    {
    ...
    ret = epd_gtp_i2c_read_dbl_check(ts->client, GTP_REG_CONFIG_DATA, &opr_buf[0], 1);
    ...
    memcpy(&config[GTP_ADDR_LENGTH], send_cfg_buf[sensor_id], ts->epd_gtp_cfg_len);
    ...
    check_sum = 0;
    for (i = GTP_ADDR_LENGTH; i < ts->epd_gtp_cfg_len; i++)
    {
        check_sum += config[i];
    }
    config[ts->epd_gtp_cfg_len] = (~check_sum) + 1;

    GTP_DEBUG_FUNC();
    ret = epd_gtp_send_cfg(ts->client);
    if (ret < 0)
    {
        GTP_ERROR("Send config error.");
    }
    epd_gtp_read_version(ts->client, &version);
    msleep(10);
    return 0;
}
#endif
...
static u8 gup_get_ic_fw_msg(struct i2c_client *client)
{
    ...
    ret = epd_gtp_i2c_read_dbl_check(client, GUP_REG_HW_INFO, &buf[GTP_ADDR_LENGTH], 4);
    if (FAIL == ret)
    {
    ...
    // step3:get pid & vid
    ret = epd_gtp_i2c_read_dbl_check(client, GUP_REG_PID_VID, &buf[GTP_ADDR_LENGTH], 6);
    if (FAIL == ret)
    {
...
static void gup_leave_update_mode(struct goodix_ts_data *ts)
{
    ...
    epd_gtp_reset_guitar(epd_gtp_i2c_connect_client, 20);
}
...
static s8 gup_update_config(struct i2c_client *client)
{
    ...
    chip_cfg_len = ts->epd_gtp_cfg_len;
...
static u8 gup_check_update_file(struct i2c_client *client, st_fw_head* fw_head, u8* path)
{
    ...
    else
    {
        ...
        update_msg.fw_total_len = sizeof(epd_gtp_default_FW) - FW_HEAD_LENGTH;
        if (sizeof(epd_gtp_default_FW) < (FW_HEAD_LENGTH+FW_SECTION_LENGTH*4+FW_DSP_ISP_LENGTH+FW_DSP_LENGTH+FW_BOOT_LENGTH))
        {
            printk(" <<-GTP-ERROR->>    haha INVALID epd_gtp_default_FW, check your gt9xx_firmware.h file!   sizeof(epd_gtp_default_FW)=%d\n", sizeof(epd_gtp_default_FW));
            return FAIL;           
        }
        GTP_DEBUG("Firmware actual size: %d(%dK)", update_msg.fw_total_len, update_msg.fw_total_len/1024);
        memcpy(fw_head, &epd_gtp_default_FW[0], FW_HEAD_LENGTH);
    
        //check firmware legality
        fw_checksum = 0;
        for(i=0; i< update_msg.fw_total_len; i+=2)
        {
            fw_checksum += (epd_gtp_default_FW[FW_HEAD_LENGTH + i] << 8) + epd_gtp_default_FW[FW_HEAD_LENGTH + i + 1];
        }
        ...
        if (got_file_flag & CFG_FILE_READY)
        {
            ret = gup_update_config(epd_gtp_i2c_connect_client);
            if(ret <= 0)
            {
...
static u8 gup_load_section_file(u8 *buf, u32 offset, u16 length, u8 set_or_end)
{
    ...
    if (got_file_flag == HEADER_FW_READY)
    {
        ...
        {
            memcpy(buf, &epd_gtp_default_FW[FW_HEAD_LENGTH + offset], length);
        }
        else    //seek end
        {
            memcpy(buf, &epd_gtp_default_FW[update_msg.fw_total_len + FW_HEAD_LENGTH - offset], length);
        }
        return SUCCESS;
    }
#endif
...
static s32 gup_update_proc(void *dir)
{
    ...
    {
        return epd_gtp_gup_fw_download_proc(dir, GTP_FL_FW_BURN);
    }
#endif

    update_msg.file = NULL;
    ret = gup_check_update_file(epd_gtp_i2c_connect_client, &fw_head, (u8*)dir);     //20121211
    if(FAIL == ret)
    {
    ...
    ret = gup_get_ic_fw_msg(epd_gtp_i2c_connect_client);
    if(FAIL == ret)
    {
    ...
    ts->enter_update = 1;
    epd_gtp_irq_disable(ts);
#if GTP_ESD_PROTECT
    epd_gtp_esd_switch(ts->client, SWITCH_OFF);
#endif
    ret = gup_enter_update_mode(epd_gtp_i2c_connect_client);
    if(FAIL == ret)
    {
    ...
    while(retry++ < 5)
    {
        ...
        ret = gup_burn_dsp_isp(epd_gtp_i2c_connect_client);
        if(FAIL == ret)
        {
        ...
        show_len = 20;
        ret = gup_burn_fw_ss51(epd_gtp_i2c_connect_client);
        if(FAIL == ret)
        {
        ...
        show_len = 30;
        ret = gup_burn_fw_dsp(epd_gtp_i2c_connect_client);
        if(FAIL == ret)
        {
        ...
        show_len = 40;
        ret = gup_burn_fw_boot(epd_gtp_i2c_connect_client);
        if(FAIL == ret)
        {
        ...
        ret = gup_burn_fw_boot_isp(epd_gtp_i2c_connect_client);
        if (FAIL == ret)
        {
        ...
        show_len = 60;
        ret = gup_burn_fw_link(epd_gtp_i2c_connect_client);
        if (FAIL == ret)
        {
        ...
        show_len = 70;
        ret = gup_burn_fw_gwake(epd_gtp_i2c_connect_client);
        if (FAIL == ret)
        {
        ...
        ret = gup_burn_fw_finish(epd_gtp_i2c_connect_client);
        if (FAIL == ret)
        {
...
update_fail:
    ...
    if (SUCCESS == update_ret)
    {
        ...
        else
        {
            ...
            ret = epd_gtp_send_cfg(epd_gtp_i2c_connect_client);
            if (ret < 0)
            {
    ...
    ts->enter_update = 0;
    epd_gtp_irq_enable(ts);
    
#if GTP_ESD_PROTECT
    epd_gtp_esd_switch(ts->client, SWITCH_ON);
#endif
...
//extern s32 epd_gtp_fw_startup(struct i2c_client *client);
...
u8 epd_gtp_gup_check_fs_mounted(char *path_name)
{
...
s32 epd_gtp_i2c_write_bytes(struct i2c_client *client, u16 addr, u8 *buf, s32 len)
{
    s32 ret = 0;
    s32 write_bytes = 0;
    s32 retry = 0;
...
s32 epd_gtp_i2c_read_bytes(struct i2c_client *client, u16 addr, u8 *buf, s32 len)
{
    s32 ret = 0;
    s32 read_bytes = 0;
    s32 retry = 0;
...
{
    u8 buf;
    epd_gtp_i2c_read_bytes(epd_gtp_i2c_connect_client, addr, &buf, 1);

    buf = (buf & (~((u8)1 << bit))) | ((u8)val << bit);

    epd_gtp_i2c_write_bytes(epd_gtp_i2c_connect_client, addr, &buf, 1);
}
...
static void gup_clk_count_init(s32 bCh, s32 bCNT)
{
    ...
    buf = 0;
    epd_gtp_i2c_write_bytes(epd_gtp_i2c_connect_client, _bRW_MISCTL__MEA_MODE, &buf, 1);
    //_bRW_MISCTL__MEA_SRCSEL = 8 + bCh; //From GIO1
    buf = 8 + bCh;
    epd_gtp_i2c_write_bytes(epd_gtp_i2c_connect_client, _bRW_MISCTL__MEA_SRCSEL, &buf, 1);
    //_wRW_MISCTL__MEA_MAX_NUM = bCNT; //Set the Measure Counts = 1
    buf = bCNT;
    epd_gtp_i2c_write_bytes(epd_gtp_i2c_connect_client, _wRW_MISCTL__MEA_MAX_NUM, &buf, 1);
...
static u32 gup_clk_count_get(void)
{
    ...
    {
        epd_gtp_i2c_read_bytes(epd_gtp_i2c_connect_client, _bRO_MISCTL__MEA_RDY, buf, 1);
        ready = buf[0];
    }
    ...
    gup_bit_write(_fRW_MISCTL__MEA, 0, 0);
    epd_gtp_i2c_read_bytes(epd_gtp_i2c_connect_client, _dRO_MISCTL__MEA_VAL, buf, 4);
...
static u8 gup_clk_dac_setting(int dac)
{
    ...
    epd_gtp_i2c_read_bytes(epd_gtp_i2c_connect_client, _wRW_MISCTL__RG_DMY, &buf1, 1);
    epd_gtp_i2c_read_bytes(epd_gtp_i2c_connect_client, _bRW_MISCTL__RG_OSC_CALIB, &buf2, 1);

    buf1 = (buf1 & 0xFFCF) | ((dac & 0x03) << 4);
    buf2 = (dac >> 2) & 0x3f;

    epd_gtp_i2c_write_bytes(epd_gtp_i2c_connect_client, _wRW_MISCTL__RG_DMY, &buf1, 1);
    epd_gtp_i2c_write_bytes(epd_gtp_i2c_connect_client, _bRW_MISCTL__RG_OSC_CALIB, &buf2, 1);
    
    return 0;
}

static u8 epd_gtp_gup_clk_calibration_pin_select(s32 bCh)
{
    s32 i2c_addr;
...
static void gup_sys_clk_init(void)
{
    ...
    buf = 0;
    epd_gtp_i2c_write_bytes(epd_gtp_i2c_connect_client, _bRW_MISCTL__RG_LDO_A18_PWD, &buf, 1);
    //_bRW_MISCTL__RG_BG_PWD = 0; //DrvMISCTL_BG_PowerON
    buf = 0;
    epd_gtp_i2c_write_bytes(epd_gtp_i2c_connect_client, _bRW_MISCTL__RG_BG_PWD, &buf, 1);
    //_bRW_MISCTL__RG_CLKGEN_PWD = 0; //DrvMISCTL_CLKGEN_PowerON
    buf = 0;
    epd_gtp_i2c_write_bytes(epd_gtp_i2c_connect_client, _bRW_MISCTL__RG_CLKGEN_PWD, &buf, 1);
    ...
    buf = 1;
    epd_gtp_i2c_write_bytes(epd_gtp_i2c_connect_client, _bRW_MISCTL__OSC_CK_SEL, &buf, 1);
}

s32 epd_gtp_gup_clk_calibration(void)
{
    u8 buf;
    //u8 trigger;
    s32 i;
    ...
    epd_gtp_i2c_write_bytes(epd_gtp_i2c_connect_client, _rRW_MISCTL__SWRST_B0_, &buf, 1);
    ...
    gup_sys_clk_init();
    epd_gtp_gup_clk_calibration_pin_select(1);//use GIO1 to do the calibration
    ...
    //-- ouput clk to GPIO 4
    buf = 0x00;
    epd_gtp_i2c_write_bytes(epd_gtp_i2c_connect_client, 0x41FA, &buf, 1);
    buf = 0x00;
    epd_gtp_i2c_write_bytes(epd_gtp_i2c_connect_client, 0x4104, &buf, 1);
    buf = 0x00;
    epd_gtp_i2c_write_bytes(epd_gtp_i2c_connect_client, 0x4105, &buf, 1);
    buf = 0x00;
    epd_gtp_i2c_write_bytes(epd_gtp_i2c_connect_client, 0x4106, &buf, 1);
    buf = 0x01;
    epd_gtp_i2c_write_bytes(epd_gtp_i2c_connect_client, 0x4107, &buf, 1);
    buf = 0x06;
    epd_gtp_i2c_write_bytes(epd_gtp_i2c_connect_client, 0x41F8, &buf, 1);
    buf = 0x02;
    epd_gtp_i2c_write_bytes(epd_gtp_i2c_connect_client, 0x41F9, &buf, 1);
#endif
...
static s32 gup_burn_fw_proc(struct i2c_client *client, u16 start_addr, s32 start_index, s32 burn_len)
{
    ...
    ret = epd_gtp_i2c_write_bytes(client, start_addr, (u8*)&epd_gtp_default_FW_fl[FW_HEAD_LENGTH + start_index], burn_len);
    if (ret < 0)
    {
...
static s32 gup_check_and_repair(struct i2c_client *client, u16 start_addr, s32 start_index, s32 chk_len)
{
    ...
    while ((chk_len > 0) && (chked_times < GTP_CHK_FW_MAX))
    {
		...
		if ((FW_HEAD_LENGTH + start_index + cmp_len) > sizeof(epd_gtp_default_FW_fl)) {
			GTP_ERROR("Check failed, buffer overflow\n");
			break;
		}
        ret = epd_gtp_i2c_read_bytes(client, cmp_addr, chk_cmp_buf, cmp_len);
        if (ret < 0)
        {
            chk_fail = 1;
            break;
		...
		for (i = 0; i < cmp_len; i++)
        {
            if (chk_cmp_buf[i] != epd_gtp_default_FW_fl[FW_HEAD_LENGTH + start_index +i])
            {
                chk_fail = 1;
                epd_gtp_i2c_write_bytes(client, cmp_addr+i, &epd_gtp_default_FW_fl[FW_HEAD_LENGTH + start_index + i], cmp_len-i);
                GTP_ERROR("Check failed index: %d(%d != %d), redownload chuck", i, chk_cmp_buf[i], 
                        epd_gtp_default_FW_fl[FW_HEAD_LENGTH + start_index +i]);
                break;
            }
        }
        if (chk_fail == 1)
...
static s32 gup_prepare_fl_fw(char *path, st_fw_head *fw_head)
{
    ...
    s32 i = 0;
    s32 timeout = 0;
    struct goodix_ts_data *ts = i2c_get_clientdata(epd_gtp_i2c_connect_client);
    ...
    if (update_msg.fw_total_len != sizeof(epd_gtp_default_FW_fl))
    {
		GTP_ERROR(
				  "Inconsistent fw size. default size: %d(%dK), file size: %d(%dK)",
				  (unsigned int)sizeof(epd_gtp_default_FW_fl),
				  (unsigned int)sizeof(epd_gtp_default_FW_fl) / 1024,
    ...
    update_msg.file->f_op->llseek(update_msg.file, 0, SEEK_SET);
    ret = update_msg.file->f_op->read(update_msg.file, (char*)epd_gtp_default_FW_fl, 
                             update_msg.fw_total_len + FW_HEAD_LENGTH,
...
static u8 gup_check_update_file_fl(struct i2c_client *client, st_fw_head* fw_head, char* path)
{
    ...
    memcpy(fw_head, epd_gtp_default_FW_fl, FW_HEAD_LENGTH);
    GTP_INFO("FILE HARDWARE INFO: %02x%02x%02x%02x", fw_head->hw_info[0], fw_head->hw_info[1], fw_head->hw_info[2], fw_head->hw_info[3]);
    GTP_INFO("FILE PID: %s", fw_head->pid);
    ...
    {
        fw_checksum += (epd_gtp_default_FW_fl[i] << 8) + epd_gtp_default_FW_fl[i+1];
    }
    ret = SUCCESS;
...
s32 epd_gtp_gup_fw_download_proc(void *dir, u8 dwn_mode)
{
    ...
    ret = gup_check_update_file_fl(epd_gtp_i2c_connect_client, &fw_head, (char *)dir);
    show_len = 10;
    ...
    ret = gup_enter_update_mode_fl(epd_gtp_i2c_connect_client);
    show_len = 20;
    if (FAIL == ret)
    {
    ...
    while (retry++ < 5)
    {
        ret = gup_download_fw_ss51(epd_gtp_i2c_connect_client, dwn_mode);
        show_len = 60;
        if (FAIL == ret)
        {
        ...
        ret = gup_download_fw_dsp(epd_gtp_i2c_connect_client, dwn_mode);
        show_len = 80;
        if (FAIL == ret)
        {
