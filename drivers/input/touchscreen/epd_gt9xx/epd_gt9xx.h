...
#ifndef _EPD_GOODIX_GT9XX_H_
#define _EPD_GOODIX_GT9XX_H_
...
#define GTP_HEADER_FW_UPDATE  0    // auto update fw by epd_gtp_default_FW in gt9xx_firmware.h, function together with GTP_AUTO_UPDATE
...
struct goodix_ts_data {
    ...
    u8  green_wake_mode;
    u8  enter_update;
    u8  epd_gtp_is_suspend;
    u8  epd_gtp_rawdiff_mode;
    u8  epd_gtp_cfg_len;
    u8  fixed_cfg;
    u8  fw_error;
    u8  pnl_init_error;
...
#define GTP_I2C_NAME                "Epd-Goodix-TS"
...
#define GTP_BAK_REF_PATH                "/data/epd_gtp_ref.bin"
#define GTP_MAIN_CLK_PATH               "/data/epd_gtp_clk.bin"
...
s32 epd_gtp_fw_startup(struct i2c_client *client);
s32 epd_gtp_i2c_read_dbl_check(struct i2c_client *client, u16 addr, u8 *rxbuf, int len);
void epd_gtp_irq_disable(struct goodix_ts_data *ts);
void epd_gtp_irq_enable(struct goodix_ts_data *ts);
s32 epd_gtp_read_version(struct i2c_client *client, u16 *version);
void epd_gtp_reset_guitar(struct i2c_client *client, s32 ms);
s32 epd_gtp_send_cfg(struct i2c_client *client);
