 ...
 *      V1.2:
 *          modify epd_gtp_reset_guitar,slot report,tracking_id & 0x0F. By Andrew, 2012/10/15
 *      V1.4:
...
static u8 epd_gtp_change_x2y = TRUE;
static u8 epd_gtp_x_reverse = FALSE;
static u8 epd_gtp_y_reverse = TRUE;
static struct input_dev *wake_key_dev = NULL;
static bool is_sleeped;

static const char *goodix_ts_name = "epd-goodix-ts";
...
static s8 epd_gtp_i2c_test(struct i2c_client *client);
void epd_gtp_reset_guitar(struct i2c_client *client, s32 ms);
s32 epd_gtp_send_cfg(struct i2c_client *client);
static void epd_gtp_int_sync(s32 ms, struct goodix_ts_data *ts);
...
#if GTP_ESD_PROTECT
static struct delayed_work epd_gtp_esd_check_work;
static struct workqueue_struct * epd_gtp_esd_check_workqueue = NULL;
static void epd_gtp_esd_check_func(struct work_struct *);
static s32 epd_gtp_init_ext_watchdog(struct i2c_client *client);
void epd_gtp_esd_switch(struct i2c_client *, s32);
#endif

//*********** For GT9XXF Start **********//
#if GTP_COMPATIBLE_MODE
//extern s32 epd_gtp_i2c_read_bytes(struct i2c_client *client, u16 addr, u8 *buf, s32 len);
//extern s32 epd_gtp_i2c_write_bytes(struct i2c_client *client, u16 addr, u8 *buf, s32 len);
//extern s32 epd_gtp_gup_clk_calibration(void);
//extern s32 epd_gtp_gup_fw_download_proc(void *dir, u8 dwn_mode);
//extern u8 epd_gtp_gup_check_fs_mounted(char *path_name);

static void epd_gtp_recovery_reset(struct i2c_client *client);
static s32 epd_gtp_esd_recovery(struct i2c_client *client);
//s32 epd_gtp_fw_startup(struct i2c_client *client);
static s32 epd_gtp_main_clk_proc(struct goodix_ts_data *ts);
static s32 epd_gtp_bak_ref_proc(struct goodix_ts_data *ts, u8 mode);
...
static s8 epd_gtp_enter_doze(struct goodix_ts_data *ts);
#endif
...
*********************************************************/
static s32 epd_gtp_i2c_read(struct i2c_client *client, u8 *buf, s32 len)
{
    ...
    if((retries >= 5))
    {
        ...
        { 
            epd_gtp_recovery_reset(client);
        }
        else
    #endif
        {
            epd_gtp_reset_guitar(client, 10);  
        }
    }
    return ret;
}
...
*********************************************************/
static s32 epd_gtp_i2c_write(struct i2c_client *client,u8 *buf,s32 len)
{
    struct i2c_msg msg;
    s32 ret = -1;
    s32 retries = 0;
...
s32 epd_gtp_i2c_read_dbl_check(struct i2c_client *client, u16 addr, u8 *rxbuf, int len)
{
    ...
    while (retry++ < 3)
    {
        ...
        buf[0] = (u8)(addr >> 8);
        buf[1] = (u8)(addr & 0xFF);
        epd_gtp_i2c_read(client, buf, len + 2);
        
        memset(confirm_buf, 0xAB, 16);
        confirm_buf[0] = (u8)(addr >> 8);
        confirm_buf[1] = (u8)(addr & 0xFF);
        epd_gtp_i2c_read(client, confirm_buf, len + 2);
...
s32 epd_gtp_send_cfg(struct i2c_client *client)
{
    ...
    {
        ret = epd_gtp_i2c_write(client, config , GTP_CONFIG_MAX_LENGTH + GTP_ADDR_LENGTH);
        if (ret > 0)
        {
            break;
        }
...
Output:
    None.
*********************************************************/
static void epd_gtp_touch_down(struct goodix_ts_data* ts,s32 id,s32 x,s32 y,s32 w)
{
	if (epd_gtp_change_x2y)
		GTP_SWAP(x, y);

	if (!bgt911 && !bgt970) {
		if (epd_gtp_x_reverse)
			x = ts->abs_x_max - x;

		if (epd_gtp_y_reverse)
			y = ts->abs_y_max - y;
	}
...
Output:
    None.
*********************************************************/
static void epd_gtp_touch_up(struct goodix_ts_data* ts, s32 id)
{
#if GTP_ICS_SLOT_REPORT
    input_mt_slot(ts->input_dev, id);
...
static void epd_gtp_pen_init(struct goodix_ts_data *ts)
{
    s32 ret = 0;
...
static void epd_gtp_pen_down(s32 x, s32 y, s32 w, s32 id)
{
...
static void epd_gtp_pen_up(s32 id)
{
    struct goodix_ts_data *ts = i2c_get_clientdata(epd_gtp_i2c_connect_client);
...
static void goodix_ts_work_func(struct work_struct *work)
{
    ...
    if (DOZE_ENABLED == doze_status)
    {               
        ret = epd_gtp_i2c_read(epd_gtp_i2c_connect_client, doze_buf, 3);
        ...
        {     
                ...
                )
            {
                ...
                // clear 0x814B
                doze_buf[2] = 0x00;
                epd_gtp_i2c_write(epd_gtp_i2c_connect_client, doze_buf, 3);
			}
    ...
    ret = epd_gtp_i2c_read(ts->client, point_data, 12);
    if (ret < 0)
    {
    ...
    {
        ret = epd_gtp_i2c_read(ts->client, rqst_buf, 3);
        if (ret < 0)
        {
        ...
        case GTP_RQST_CONFIG:
            ...
            else
            {
                ...
                epd_gtp_i2c_write(ts->client, rqst_buf, 3);
        ...
        case GTP_RQST_BAK_REF:
            ...
            ret = epd_gtp_bak_ref_proc(ts, GTP_BAK_REF_SEND);
            if (SUCCESS == ret)
            {
                rqst_buf[2] = GTP_RQST_RESPONDED;
                epd_gtp_i2c_write(ts->client, rqst_buf, 3);
        ...
        case GTP_RQST_RESET:
            ...
            epd_gtp_recovery_reset(ts->client);
            break;
            
        case GTP_RQST_MAIN_CLOCK:
            GTP_INFO("Request for main clock.");
            ts->rqst_processing = 1;
            ret = epd_gtp_main_clk_proc(ts);
            if (FAIL == ret)
            {
    ...
    if (touch_num > 1)
    {
        ...
        ret = epd_gtp_i2c_read(ts->client, buf, 2 + 8 * (touch_num - 1)); 
    ...
    if (pre_touch || touch_num)
    {
        ...
        if(touch_num)
        {
            ...
            if ((id & 0x80))  
            {
                ...
                epd_gtp_pen_down(input_x, input_y, input_w, 0);
                pre_pen = 1;
                pre_touch = 0;
                pen_active = 1;
            }    
        ...
        for (i = 0; i < GTP_MAX_TOUCH; i++)
        {
            ...
            if ((touch_index & (0x01<<i)))
            {
                ...
                epd_gtp_touch_down(ts, id, input_x, input_y, input_w);
                pre_touch |= 0x01 << i;
...
exit_work_func:
    if(!ts->epd_gtp_rawdiff_mode)
    {
        ret = epd_gtp_i2c_write(ts->client, end_cmd, 3);
        if (ret < 0)
        {
...
void epd_gtp_reset_guitar(struct i2c_client *client, s32 ms)
{
    ...
    epd_gtp_int_sync(50, ts);  
#if GTP_ESD_PROTECT
    epd_gtp_init_ext_watchdog(client);
#endif
}
...
static s8 epd_gtp_enter_doze(struct goodix_ts_data *ts)
{
    ...
    while(retry++ < 5)
    {
        ...
        ret = epd_gtp_i2c_write(ts->client, i2c_control_buf, 3);
        if (ret < 0)
        {
...
Output:
    Executive outcomes.
       1: succeed, otherwise failed.
*******************************************************/
static s8 epd_gtp_enter_sleep(struct goodix_ts_data * ts)
{
    s8 ret = -1;
    s8 retry = 0;
    ...
    if (CHIP_TYPE_GT9F == ts->chip_type)
    {
        ...
        ret = epd_gtp_i2c_read(ts->client, status_buf, 3);
        if (ret < 0)
        {
        ...
        {
            ret = epd_gtp_bak_ref_proc(ts, GTP_BAK_REF_STORE);
            if (FAIL == ret)
            {
...
*******************************************************/
static s8 epd_gtp_wakeup_sleep(struct goodix_ts_data * ts)
{
    u8 retry = 0;
    s8 ret = -1;
    ...
    if (CHIP_TYPE_GT9F == ts->chip_type)
    {
        ...
        for (retry = 0; retry < 10; ++retry)
        {
            // hold ss51 & dsp
            opr_buf[2] = 0x0C;
            ret = epd_gtp_i2c_write(ts->client, opr_buf, 3);
            if (FAIL == ret)
            {
            ...
            opr_buf[2] = 0x00;
            ret = epd_gtp_i2c_read(ts->client, opr_buf, 3);
            if (FAIL == ret)
            {
            ...
            ret = epd_gtp_fw_startup(ts->client);
            if (FAIL == ret)
            {
                GTP_ERROR("failed to startup GT9XXF, process recovery");
                epd_gtp_esd_recovery(ts->client);
            }
            break;
        }
        if (retry >= 10)
    ...
    while(retry++ < 5)
    {
        epd_gtp_reset_guitar(ts->client, 20);
    ...
    while(retry++ < 10)
    {
        ...
        epd_gtp_irq_disable(ts);
        epd_gtp_reset_guitar(ts->client, 10);
        epd_gtp_irq_enable(ts);
        ...
        ret = epd_gtp_i2c_test(ts->client);
        if (ret > 0)
        {
            GTP_INFO("GTP wakeup sleep.");
        ...
        #if (!GTP_GESTURE_WAKEUP)
            {
                epd_gtp_int_sync(25, ts);
            #if GTP_ESD_PROTECT
                epd_gtp_init_ext_watchdog(ts->client);
            #endif
            }
        #endif
...
#if GTP_DRIVER_SEND_CFG
static s32 epd_gtp_get_info(struct goodix_ts_data *ts)
{
    u8 opr_buf[6] = {0};
    s32 ret = 0;
    ...
    ret = epd_gtp_i2c_read(ts->client, opr_buf, 6);
    if (ret < 0)
    {
        return FAIL;
    }
...
Output:
    Executive outcomes.
        0: succeed, otherwise: failed
*******************************************************/
static s32 epd_gtp_init_panel(struct goodix_ts_data *ts)
{
    s32 ret = -1;
#if GTP_DRIVER_SEND_CFG
    s32 i = 0;
    ...
    u8 *send_cfg_buf[] = {epd_gtp_dat_10_1, cfg_info_group2, cfg_info_group3,
                            cfg_info_group4, cfg_info_group5, cfg_info_group6};
    u8 cfg_info_len[] = { CFG_GROUP_LEN(epd_gtp_dat_10_1),
    ...
    if(m89or101){
	    if (ts->cfg_file_num) {
		    send_cfg_buf[0] = epd_gtp_dat_8_9_1;
		    cfg_info_len[0] =  CFG_GROUP_LEN(epd_gtp_dat_8_9_1);
	    } else {
		    send_cfg_buf[0] = epd_gtp_dat_8_9;
		    cfg_info_len[0] =  CFG_GROUP_LEN(epd_gtp_dat_8_9);
	    }
    }
    
    if (bgt911) {
    	send_cfg_buf[0] = epd_gtp_dat_gt11;
		cfg_info_len[0] =  CFG_GROUP_LEN(epd_gtp_dat_gt11);
    }

    if (bgt9110) {
	    send_cfg_buf[0] = epd_gtp_dat_gt9110;
	    cfg_info_len[0] =  CFG_GROUP_LEN(epd_gtp_dat_gt9110);
    }

	if (bgt9111) {
		send_cfg_buf[0] = epd_gtp_dat_gt9111;
		cfg_info_len[0] =  CFG_GROUP_LEN(epd_gtp_dat_gt9111);
	}

	if (bgt970) {
		send_cfg_buf[0] = epd_gtp_dat_9_7;
		cfg_info_len[0] = CFG_GROUP_LEN(epd_gtp_dat_9_7);
	}

	if (bgt910) {
		send_cfg_buf[0] = epd_gtp_dat_7;
		cfg_info_len[0] = CFG_GROUP_LEN(epd_gtp_dat_7);
	}
...
#endif
    {
        ret = epd_gtp_i2c_read_dbl_check(ts->client, 0x41E4, opr_buf, 1);
        if (SUCCESS == ret) 
        {
            if (opr_buf[0] != 0xBE)
            {
    ...
    GTP_INFO("CTP_CONFIG_GROUP%d used, config length: %d", sensor_id + 1, ts->epd_gtp_cfg_len);
    
    if (ts->epd_gtp_cfg_len < GTP_CONFIG_MIN_LENGTH)
    {
        GTP_ERROR("Config Group%d is INVALID CONFIG GROUP(Len: %d)! NO Config Sent! You need to check you header file CFG_GROUP section!", sensor_id+1, ts->epd_gtp_cfg_len);
        ts->pnl_init_error = 1;
        return -1;
    }
    ...
    ts->epd_gtp_cfg_len = GTP_CONFIG_MAX_LENGTH;
    ret = epd_gtp_i2c_read(ts->client, config, ts->epd_gtp_cfg_len + GTP_ADDR_LENGTH);
    if (ret < 0)
    {
...
static ssize_t gt91xx_config_read_proc(struct file *file, char __user *page, size_t size, loff_t *ppos)
{
    ...
    epd_gtp_i2c_read(epd_gtp_i2c_connect_client, temp_data, GTP_CONFIG_MAX_LENGTH + 2);
    for (i = 0 ; i < GTP_CONFIG_MAX_LENGTH ; i++)
    {
...
s32 epd_gtp_read_version(struct i2c_client *client, u16* version)
{
    ...
    ret = epd_gtp_i2c_read(client, buf, sizeof(buf));
    if (ret < 0)
    {
...
static s8 epd_gtp_i2c_test(struct i2c_client *client)
{
    ...
    while(retry++ < 5)
    {
        ret = epd_gtp_i2c_read(client, test, 3);
        if (ret > 0)
        {
            return ret;
        }
...
Output:
    Executive outcomes.
        >= 0: succeed, < 0: failed
*******************************************************/
static s8 epd_gtp_request_io_port(struct goodix_ts_data *ts)
{
    s32 ret = 0;
...
Output:
    Executive outcomes.
        0: succeed, -1: failed.
*******************************************************/
static s8 epd_gtp_request_irq(struct goodix_ts_data *ts)
{
    s32 ret = -1;
...
Output:
    Executive outcomes.
        0: succeed, otherwise: failed.
*******************************************************/
static s8 epd_gtp_request_input_dev(struct i2c_client *client,
                                struct goodix_ts_data *ts)
{
    s8 ret = -1;
    s8 phys[32];
...
s32 epd_gtp_fw_startup(struct i2c_client *client)
{
    ...
    //init sw WDT
	opr_buf[0] = 0xAA;
	ret = epd_gtp_i2c_write_bytes(client, 0x8041, opr_buf, 1);
    if (ret < 0)
    {
        return FAIL;
    }
    ...
    //release SS51 & DSP
    opr_buf[0] = 0x00;
    ret = epd_gtp_i2c_write_bytes(client, 0x4180, opr_buf, 1);
    if (ret < 0)
    {
        return FAIL;
    }
    ...
    //check fw run status
    ret = epd_gtp_i2c_read_bytes(client, 0x8041, opr_buf, 1);
    if (ret < 0)
    {
        return FAIL;
    }
    ...
    else
    {
        ...
        opr_buf[0] = 0xAA;
        epd_gtp_i2c_write_bytes(client, 0x8041, opr_buf, 1);
        return SUCCESS;
    }
}
...
static s32 epd_gtp_esd_recovery(struct i2c_client *client)
{
    ...
    for (retry = 0; retry < 5; retry++)
    {
        ret = epd_gtp_gup_fw_download_proc(NULL, GTP_FL_ESD_RECOVERY); 
        if (FAIL == ret)
        {
...
{
#if GTP_ESD_PROTECT
    epd_gtp_esd_switch(client, SWITCH_OFF);
#endif
    GTP_DEBUG_FUNC();
    
    epd_gtp_esd_recovery(client); 
    
#if GTP_ESD_PROTECT
    epd_gtp_esd_switch(client, SWITCH_ON);
#endif
}
...
static s32 epd_gtp_bak_ref_proc(struct goodix_ts_data *ts, u8 mode)
{
    ...
    ret = epd_gtp_gup_check_fs_mounted("/data");
    if (FAIL == ret)
    {
        ts->ref_chk_fs_times++;
    ...
    case GTP_BAK_REF_SEND:
        ...
        ret = epd_gtp_i2c_write_bytes(ts->client, GTP_REG_BAK_REF, p_bak_ref, ts->bak_ref_len);
        if (FAIL == ret)
        {
    ...
    case GTP_BAK_REF_STORE:
        ...
        ret = epd_gtp_i2c_read_bytes(ts->client, GTP_REG_BAK_REF, p_bak_ref, ts->bak_ref_len);
        if (ret < 0)
        {
...
static s32 epd_gtp_verify_main_clk(u8 *p_main_clk)
{
    u8 chksum = 0;
...
static s32 epd_gtp_main_clk_proc(struct goodix_ts_data *ts)
{
	 ...
	 */
	ret = epd_gtp_verify_main_clk(p_main_clk);
	if (ret == FAIL) {
    ...
    epd_gtp_esd_switch(ts->client, SWITCH_OFF);
#endif
    ret = epd_gtp_gup_clk_calibration();
    epd_gtp_esd_recovery(ts->client);
...
update_main_clk:
    ret = epd_gtp_i2c_write_bytes(ts->client, GTP_REG_MAIN_CLK, p_main_clk, 6);
    if (FAIL == ret)
    {
...
static s32 epd_gtp_gt9xxf_init(struct i2c_client *client)
{
    s32 ret = 0;
    
    ret = epd_gtp_gup_fw_download_proc(NULL, GTP_FL_FW_BURN); 
    if (FAIL == ret)
    {
        return FAIL;
    }
    
    ret = epd_gtp_fw_startup(client);
    if (FAIL == ret)
    {
        return FAIL;
    }
...
static void epd_gtp_get_chip_type(struct goodix_ts_data *ts)
{
    u8 opr_buf[10] = {0x00};
    s32 ret = 0;
    ...
    ret = epd_gtp_i2c_read_dbl_check(ts->client, GTP_REG_CHIP_TYPE, opr_buf, 10);
...
static int goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (val == 89) {
		m89or101 = TRUE;
		epd_gtp_change_x2y = TRUE;
		epd_gtp_x_reverse = FALSE;
		epd_gtp_y_reverse = TRUE;
	} else if (val == 101) {
		m89or101 = FALSE;
		epd_gtp_change_x2y = TRUE;
		epd_gtp_x_reverse = TRUE;
		epd_gtp_y_reverse = FALSE;
	} else if (val == 911) {
		m89or101 = FALSE;
		bgt911 = TRUE;
		epd_gtp_change_x2y = TRUE;
    ...
    ts->epd_gtp_rawdiff_mode = 0;

    ret = epd_gtp_request_io_port(ts);
    if (ret < 0)
    {
...
/*
    ...
    else//DPT
    {
		...
		epd_gtp_x_reverse = TRUE;//FALSE;
		epd_gtp_y_reverse = TRUE;
    }
 */
#if GTP_COMPATIBLE_MODE
    epd_gtp_get_chip_type(ts);
    
    if (CHIP_TYPE_GT9F == ts->chip_type)
    {
        ret = epd_gtp_gt9xxf_init(ts->client);
        if (FAIL == ret)
        {
    ...
    ret = epd_gtp_read_version(client, &version_info);
    if (ret < 0)
    {
        GTP_ERROR("Read version failed.");
    }
    
    ret = epd_gtp_init_panel(ts);
    if (ret < 0)
    {
    ...
    ret = epd_gtp_request_input_dev(client, ts);
    if (ret < 0)
    {
        GTP_ERROR("GTP request input dev failed");
    }
    ret = epd_gtp_request_irq(ts); 
    if (ret < 0)
    {
    ...
    if (of_property_read_bool(np, "wakeup-source"))
    {
        ...
        wake_key_dev->name = "epd_touch_key";
...
#if GTP_ESD_PROTECT
    destroy_workqueue(epd_gtp_esd_check_workqueue);
#endif
...
#if GTP_ESD_PROTECT
s32 epd_gtp_i2c_read_no_rst(struct i2c_client *client, u8 *buf, s32 len)
{
    struct i2c_msg msgs[2];
    s32 ret=-1;
    s32 retries = 0;
...
s32 epd_gtp_i2c_write_no_rst(struct i2c_client *client,u8 *buf,s32 len)
{
    struct i2c_msg msg;
    s32 ret = -1;
    s32 retries = 0;
...
Output:
    void
*********************************************************/
void epd_gtp_esd_switch(struct i2c_client *client, s32 on)
{
    struct goodix_ts_data *ts;
    ...
    if (SWITCH_ON == on)     // switch on esd 
    {
        if (!ts->esd_running)
        {
            ...
            queue_delayed_work(epd_gtp_esd_check_workqueue, &epd_gtp_esd_check_work, ts->clk_tick_cnt);
        }
        else
        {
            spin_unlock(&ts->esd_lock);
    ...
    else    // switch off esd
    {
        if (ts->esd_running)
        {
            ...
            cancel_delayed_work_sync(&epd_gtp_esd_check_work);
        }
        else
        {
            spin_unlock(&ts->esd_lock);
...
static s32 epd_gtp_init_ext_watchdog(struct i2c_client *client)
{
    ...
    return epd_gtp_i2c_write_no_rst(client, opr_buffer, 3);
}
...
Output:
    None.
*******************************************************/
static void epd_gtp_esd_check_func(struct work_struct *work)
{
    s32 i;
    s32 ret = -1;
    struct goodix_ts_data *ts = NULL;
    ...
    for (i = 0; i < 3; i++)
    {
        ret = epd_gtp_i2c_read_no_rst(ts->client, esd_buf, 4);
        ...
        { 
            if ((esd_buf[2] == 0xAA) || (esd_buf[3] != 0xAA))
            {
                ...
                epd_gtp_i2c_read_no_rst(ts->client, chk_buf, 4);
            ...
            else 
            {
                ...
                esd_buf[2] = 0xAA; 
                epd_gtp_i2c_write_no_rst(ts->client, esd_buf, 3);
                break;
            }
        }
    }
    if (i >= 3)
    {
        ...
        {        
            ...
            else
            {
                ...
                esd_buf[3] = 0x01;
                esd_buf[4] = 0x01;
                epd_gtp_i2c_write_no_rst(ts->client, esd_buf, 5);
                msleep(50);
                epd_gtp_esd_recovery(ts->client);
            }
        }
    ...
    #endif
        {
            ...
            epd_gtp_i2c_write_no_rst(ts->client, esd_buf, 5);
            msleep(50);
            epd_gtp_reset_guitar(ts->client, 50);
            msleep(50);
            epd_gtp_send_cfg(ts->client);
        }
    }
...
#if GTP_ESD_PROTECT
    INIT_DELAYED_WORK(&epd_gtp_esd_check_work, epd_gtp_esd_check_func);
    epd_gtp_esd_check_workqueue = create_workqueue("epd_gtp_esd_check");
#endif
