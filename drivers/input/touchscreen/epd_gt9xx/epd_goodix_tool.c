...
extern void epd_gtp_irq_disable(struct goodix_ts_data *);
extern void epd_gtp_irq_enable(struct goodix_ts_data *);
...
ssize_t goodix_tool_write(struct file *filp, const char __user *buff, size_t len, loff_t *off)
{
    ...
    {
        epd_gtp_irq_disable(i2c_get_clientdata(gt_client));
        
    #if GTP_ESD_PROTECT
        epd_gtp_esd_switch(gt_client, SWITCH_OFF);
    #endif
    }
    else if (9 == cmd_head.wr) //enable irq!
    {
        epd_gtp_irq_enable(i2c_get_clientdata(gt_client));

    #if GTP_ESD_PROTECT
        epd_gtp_esd_switch(gt_client, SWITCH_ON);
    #endif
    }
    else if(17 == cmd_head.wr)
    {
        ...
        if(cmd_head.data[GTP_ADDR_LENGTH])
        {
            ...
            ts->epd_gtp_rawdiff_mode = true;
        }
        else
        {
            ts->epd_gtp_rawdiff_mode = false;
