...
int fts_cal_disable_touch_flag=0;
/* fts_log_level interface */
static ssize_t fts_cal_disable_touch_show(
    struct device *dev, struct device_attribute *attr, char *buf)
{
    int count = 0;
    /*struct input_dev *input_dev = fts_data->input_dev;*/
    /*int ret = -1;*/
     printk("cal=====%d\n",fts_cal_disable_touch_flag);

    return count;
}

static ssize_t fts_cal_disable_touch_store(
    struct device *dev,
    struct device_attribute *attr, const char *buf, size_t count)
{
    int value = 0;
    ...
    /*int ret = -1;*/
     ...
     mutex_lock(&input_dev->mutex);
    sscanf(buf, "%d", &value);
    fts_cal_disable_touch_flag= value;
...
int fts_disable_key_flag=0;
/* fts_log_level interface */
static ssize_t fts_disable_key_show(
    struct device *dev, struct device_attribute *attr, char *buf)
{
    /*int count = 0;*/
    /*struct input_dev *input_dev = fts_data->input_dev;*/
    /*int ret = -1;*/
     printk("touch key=====%d\n",fts_disable_key_flag);

    return sprintf(buf, "%d\n", fts_disable_key_flag);
}

static ssize_t fts_disable_key_store(
    ...
    struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    mutex_lock(&input_dev->mutex);
    sscanf(buf, "%d", &value);
    fts_disable_key_flag= value;

    printk("touch key===store==%d\n",value);
...
static DEVICE_ATTR(fts_cal_disable_touch, S_IRUGO | S_IWUSR, fts_cal_disable_touch_show, fts_cal_disable_touch_store);
static DEVICE_ATTR(fts_disable_key, S_IRUGO | S_IWUSR, fts_disable_key_show, fts_disable_key_store);
...
static struct attribute *fts_attributes[] = {
    ...
    &dev_attr_fts_cal_disable_touch.attr,
    &dev_attr_fts_disable_key.attr,
    NULL
};
