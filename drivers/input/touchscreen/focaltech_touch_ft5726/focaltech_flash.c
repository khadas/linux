...
static u8 g_fw_file2[] = {
#include FTS_UPGRADE_FW2_FILE
};

static u8 g_fw_file3[] = {
//#include FTS_UPGRADE_FW3_FILE
};


static struct upgrade_module module_list[] = {
    {FTS_MODULE_ID, FTS_MODULE_NAME, g_fw_file, sizeof(g_fw_file)},
    {FTS_MODULE2_ID, FTS_MODULE2_NAME, g_fw_file2, sizeof(g_fw_file2)},
    {FTS_MODULE3_ID, FTS_MODULE3_NAME, g_fw_file3, sizeof(g_fw_file3)},
};

static struct upgrade_func *g_upgrade_func_list[] = {
    &upgrade_func_ft5822u,
};

static struct fts_upgrade *g_fwupgrade;
...
int fts_fwupg_enter_into_boot(void)
{
    ...
    bool fwvalid = false;
    bool state = false;
    struct fts_upgrade *upg = g_fwupgrade;
...
static bool fts_fwupg_need_upgrade(struct fts_upgrade *upg)
{
    ...
    if (fwvalid) {
        ...
        printk("fts ===fw version in tp:%x, host:%x", fw_ver_in_tp, fw_ver_in_host);
...
//return true;
#endif
        FTS_INFO("fts fw version in tp:%x, host:%x", fw_ver_in_tp, fw_ver_in_host);
        if (fw_ver_in_tp < fw_ver_in_host) {
		printk("fts need upgrade!!!!!!111\n");
            return true;
        }
    } else {
        FTS_INFO("fts invalid, need upgrade fw");
        return true;
    }
...
int fts_fwupg_init(struct fts_ts_data *ts_data)
{
    ...
    int j = 0;
    int ic_stype = 0;
    struct upgrade_func *func = g_upgrade_func_list[0];
    int func_count = sizeof(g_upgrade_func_list) / sizeof(g_upgrade_func_list[0]);
    ...
    g_fwupgrade = (struct fts_upgrade *)kzalloc(sizeof(*g_fwupgrade), GFP_KERNEL);
    if (NULL == g_fwupgrade) {
    ...
    if (1 == func_count) {
        g_fwupgrade->func = func;
    } else {
        for (i = 0; i < func_count; i++) {
            func = g_upgrade_func_list[i];
...
int fts_fwupg_exit(struct fts_ts_data *ts_data)
{
    FTS_FUNC_ENTER();
    if (g_fwupgrade) {
        if (g_fwupgrade->fw_from_request) {
            vfree(g_fwupgrade->fw);
            g_fwupgrade->fw = NULL;
        }
