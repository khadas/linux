...
extern int fts_disable_key_flag;
...
//bool fts_irq_suspend=true;
//EXPORT_SYMBOL_GPL(fts_irq_suspend);

//extern int gt9xx_detect_flag;

//extern bool fts_irq_enable_flag;
int fts_tp_5946=0;
...
static int fts_get_chip_types1(
    struct fts_ts_data *ts_data,

    u8 id_h, u8 id_l, bool fw_valid)
{
    int i = 0;
    struct ft_chip_t ctype[] = FTS_CHIP_TYPE_MAPPING1;
    u32 ctype_entries = sizeof(ctype) / sizeof(struct ft_chip_t);

	printk("id_h=====0x%x  id_l=====0x%x",id_h,id_l);
    if ((0x0 == id_h) || (0x0 == id_l)) {
        FTS_ERROR("id_h/id_l is 0");
        return -EINVAL;
    }

    printk("verify id:0x%02x%02x", id_h, id_l);
    for (i = 0; i < ctype_entries; i++) {
        if (VALID == fw_valid) {
            if ((id_h == ctype[i].chip_idh) && (id_l == ctype[i].chip_idl))
                break;
...
static int fts_get_ic_information(struct fts_ts_data *ts_data)
{
    ...
    if (ret < 0) {
        FTS_ERROR("can't get ic fw_ver\n");
    }
    printk("fts tp is 5436===ver===0x%x\n",fw_ver);

    return 0;
}

static int fts_get_ic_information1(struct fts_ts_data *ts_data)
{
    int ret = 0;
    int cnt = 0;
    u8 chip_id[2] = { 0 };
    u8 fw_ver = 0;
    ts_data->ic_info.is_incell = FTS_CHIP_IDC1;
    ts_data->ic_info.hid_supported = FTS_HID_SUPPORTTED1;

    printk("fts tp is 5946\n");
    do {
        ret = fts_read_reg(FTS_REG_CHIP_ID, &chip_id[0]);
        ret = fts_read_reg(FTS_REG_CHIP_ID2, &chip_id[1]);
        if ((ret < 0) || (0x0 == chip_id[0]) || (0x0 == chip_id[1])) {
            printk("chip id read invalid, read:0x%02x%02x",
                      chip_id[0], chip_id[1]);
        } else {
            ret = fts_get_chip_types1(ts_data, chip_id[0], chip_id[1], VALID);
            if (!ret)
                break;
            else
                FTS_DEBUG("TP not ready, read:0x%02x%02x",
    ...
    if ((cnt * INTERVAL_READ_REG) >= TIMEOUT_READ_REG) {
        ...
        ret = fts_get_chip_types1(ts_data, chip_id[0], chip_id[1], INVALID);
        if (ret < 0) {
            FTS_ERROR("can't get ic informaton");
            return ret;
        }
    ...
    printk("fts tp is 5946===ver===0x%x\n",fw_ver);
    ...
    if (va_reported) {
        ...
        if (EVENT_NO_DOWN(data) || (!touchs)) {
            ...
            if(data->key_state==1){
               ...
               data->key_state &= ~(1 << 0);
               FTS_DEBUG("after [B]Points All Up send key up again!!----------------");
	    }
        } else {
...
#endif
#ifdef FTS_ROTATE_90
input_set_abs_params(input_dev, ABS_MT_POSITION_X, 0, pdata->y_max, 0, 0);
input_set_abs_params(input_dev, ABS_MT_POSITION_Y, 0, pdata->x_max, 0, 0);
#else
input_set_abs_params(input_dev, ABS_MT_POSITION_X, 0, TPD_RES_X, 0, 0);
input_set_abs_params(input_dev, ABS_MT_POSITION_Y, 0, TPD_RES_Y, 0, 0);
#endif
...
static int fts_ts_probe_entry(struct fts_ts_data *ts_data)
{
	...
	ret =fts_get_ic_information1(ts_data);
	if(ret)
	{
	 FTS_ERROR("fts_get_ic_information1, not focal IC------------------------, unregister driver");
           goto err_irq_req;
	}
    }
...
static int fts_ts_ebc_suspend(struct tp_device *tp_d)
{
    int ret = 0;
    struct fts_ts_data *ts_data = fts_data;

    printk("[FTS]%s\n",__func__);
    ret = fts_ts_suspend(ts_data->dev);

    return ret;
}

static int fts_ts_ebc_resume(struct tp_device *tp_d)
{
    int ret = 0;
    struct fts_ts_data *ts_data = fts_data;

    printk("[FTS]%s\n",__func__);
    ret = fts_ts_resume(ts_data->dev);
...
static int fts_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    ts_data->tp.tp_resume = fts_ts_ebc_resume;
...
static int fts_core_pm_suspend(struct device *dev)
{
    printk("[FTS]%s\n",__func__);
    if (device_may_wakeup(&fts_data->client->dev)) {
        //printk("%s Device MAY wakeup\n", __func__);
        if (!enable_irq_wake(fts_data->irq))
            printk("%s,enable_irq_wake\n",__func__);
        //cd->irq_wake = 1;
    } else {
        printk( "%s Device MAY NOT wakeup\n", __func__);
    }

    return 0;
}

static int fts_core_pm_resume(struct device *dev)
{
    printk("[FTS]%s\n",__func__);
    if (device_may_wakeup(&fts_data->client->dev)) {
        //printk("%s Device MAY wakeup\n", __func__);
        disable_irq_wake(fts_data->irq);
        printk("%s,disable_irq_wake\n",__func__);
        //cd->irq_wake = 1;
    } else {
        printk( "%s Device MAY NOT wakeup\n", __func__);
    }
