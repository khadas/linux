...
static int maybe_focaltech __initdata = 1;
...
#endif
				input_report_abs(data->input_dev, ABS_MT_POSITION_X, data->pdata->y_max - events[i].y);
				input_report_abs(data->input_dev, ABS_X, data->pdata->y_max - events[i].y);
			} else {
#if defined(CONFIG_TOUCHSCREEN_FTS_5726_P1)
				input_report_abs(data->input_dev, ABS_MT_POSITION_X, data->pdata->x_max - events[i].x);
				input_report_abs(data->input_dev, ABS_X, data->pdata->x_max - events[i].x);
...
static int fts_input_init(struct fts_ts_data *ts_data)
{
	...
	if (pdata->rotation) {
		...
		input_set_abs_params(input_dev, ABS_X, pdata->y_min, pdata->y_max, 0, 0);
		input_set_abs_params(input_dev, ABS_Y, pdata->x_min, pdata->x_max, 0, 0);
	} else {
...
static int fts_parse_dt(struct device *dev, struct fts_ts_platform_data *pdata)
{
	...
	if (!ret) {
		...
		FTS_INFO("rotation = %d", pdata->rotation);
	} else {
		pdata->rotation = 1;
		FTS_ERROR("rotation = %d", pdata->rotation);
	}
