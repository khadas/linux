...
*******************************************************************************/
#define FOCALTECH_APK_NODE_INFO  "File Version of  focaltech_flash_ft8716.c:  V1.0.0 2016-03-21"

#define FTS_REG_FT8716_VENDOR_ID 		0xA8

/*******************************************************************************
* Static function prototypes
*******************************************************************************/
static int  fts_ic_program_ft8716_write_pram( u8* pbt_buf, u32 dw_lenth);
static int fts_ic_program_ft8716_upgrade( u8 *pbt_buf, u32 dw_lenth);
static int fts_ft8716_get_i_file(u8 *file_buff, int *file_len);
...
/************************************************************************
*   Name: fts_ic_program_ft8716_upgrade
* Brief:  fw upgrade
...
static int fts_ft8716_get_i_file(u8 *file_buff, int *file_len)
{
	...
	{
		i_ret = fts_read_reg(FTS_REG_FT8716_VENDOR_ID, &uc_vender_id);
		if(i_ret < 0)
		{
...
int fts_ft8716_upgrade_with_i_file(void)
{
	u8 *pbt_buf = NULL;
	int i_ret=0;
	int fw_len = 0;
	...
	/*get i file*/
	i_ret = fts_ft8716_get_i_file(pbt_buf, &fw_len);
	if(i_ret < 0)
	{
		return -1;
	}
	 ...
	 /* call the write pram function */
    	i_ret = fts_ic_program_ft8716_write_pram( BUFFER_PRAM_BOOT_FILE, g_len_pram_boot_file);
	...
	i_ret = fts_ic_program_ft8716_upgrade( pbt_buf, fw_len);
...
int fts_ft8716_get_i_file_version(void)
{
	u8 *pbt_buf = NULL;
	int i_ret=0;
	int fw_len = 0;
...
int fts_ft8716_upgrade_with_bin_file(u8 *file_buff, int file_len)
{
	int i_ret=0;
	...
	/*call upgrade function*/
	i_ret = fts_ic_program_ft8716_upgrade( file_buff, file_len);
