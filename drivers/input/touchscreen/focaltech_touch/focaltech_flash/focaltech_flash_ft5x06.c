...
*******************************************************************************/
#define FOCALTECH_APK_NODE_INFO  "File Version of  focaltech_flash_ft5x06.c:  V1.0.0 2016-03-21"

#define FTS_REG_FT5X06_VENDOR_ID 		0xA8
/*******************************************************************************
* Static function prototypes
*******************************************************************************/
static int fts_ic_program_ft5x06_upgrade( u8 *pbt_buf, u32 dw_lenth);
static int fts_ft5x06_get_i_file(u8 *file_buff, int *file_len);
...
/************************************************************************
*   Name: fts_ic_program_ft5x06_upgrade
* Brief:  fw upgrade
...
static int fts_ft5x06_get_i_file(u8 *file_buff, int *file_len)
{
	...
	{
		i_ret = fts_read_reg(FTS_REG_FT5X06_VENDOR_ID, &uc_vender_id);
		if(i_ret < 0)
		{
...
int fts_ft5x06_upgrade_with_i_file(void)
{
	u8 *pbt_buf = NULL;
	int i_ret=0;
	int fw_len = 0;
	...
	/*get i file*/
	i_ret = fts_ft5x06_get_i_file(pbt_buf, &fw_len);
	if(i_ret < 0)
	{
		return -1;
	}
	...
	i_ret = fts_ic_program_ft5x06_upgrade( pbt_buf, fw_len);
	
	return i_ret;
}
int fts_ft5x06_get_i_file_version(void)
{
	u8 *pbt_buf = NULL;
	int i_ret=0;
	int fw_len = 0;
...
int fts_ft5x06_upgrade_with_bin_file(u8 *file_buff, int file_len)
{
	u8 *pbt_buf = NULL;
	int i_ret=0;
	int fw_len = 0;
