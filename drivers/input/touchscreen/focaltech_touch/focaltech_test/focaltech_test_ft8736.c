...
boolean FT8736_StartTest()
{
	...
	for(iItemCount = 0; iItemCount < g_TestItemNum; iItemCount++)
	{
		...
		///////////////////////////////////////////////////////FT8736_CHANNEL_NUM_TEST
		if(Code_FT8736_CHANNEL_NUM_TEST == g_stTestItem[ucDevice][iItemCount].ItemCode
			)
		{
			ReCode = FT8736_TestItem_ChannelsTest(&bTempResult);
			if(ERROR_CODE_OK != ReCode || (!bTempResult))
			{
				bTestResult = false;
				g_stTestItem[ucDevice][iItemCount].TestResult = RESULT_NG;
		...
		///////////////////////////////////////////////////////FT8736_NOISE_TEST
		if(Code_FT8736_NOISE_TEST == g_stTestItem[ucDevice][iItemCount].ItemCode
			)
		{

			ReCode = FT8736_TestItem_NoiseTest(&bTempResult);
			if(ERROR_CODE_OK != ReCode || (!bTempResult))
			{
				bTestResult = false;
				g_stTestItem[ucDevice][iItemCount].TestResult = RESULT_NG;
...
/************************************************************************
* Name: FT8736_TestItem_ChannelsTest
* Brief:  Check whether TP can enter Factory Mode, and do some thing
* Input: none
* Output: none
* Return: Comm Code. Code = 0x00 is OK, else fail.
...
unsigned char FT8736_TestItem_ChannelsTest(bool * bTestResult)
{
	...
	if(ReCode == ERROR_CODE_OK)
	{
		if((g_stCfg_FT8736_BasicThreshold.ChannelNumTest_ChannelXNum == g_stSCapConfEx.ChannelXNum)
			&& (g_stCfg_FT8736_BasicThreshold.ChannelNumTest_ChannelYNum == g_stSCapConfEx.ChannelYNum)
			&& (g_stCfg_FT8736_BasicThreshold.ChannelNumTest_KeyNum == g_stSCapConfEx.KeyNum))
		{
			* bTestResult = true;
			FTS_TEST_DBG("\n\nGet channels: (CHx: %d, CHy: %d, Key: %d), Set channels: (CHx: %d, CHy: %d, Key: %d)",
				g_stSCapConfEx.ChannelXNum, g_stSCapConfEx.ChannelYNum, g_stSCapConfEx.KeyNum, 
				g_stCfg_FT8736_BasicThreshold.ChannelNumTest_ChannelXNum, g_stCfg_FT8736_BasicThreshold.ChannelNumTest_ChannelYNum, g_stCfg_FT8736_BasicThreshold.ChannelNumTest_KeyNum);
...
/************************************************************************
* Name: FT8736_TestItem_NoiseTest
...
unsigned char FT8736_TestItem_NoiseTest(bool* bTestResult)
{
	...
	iNoiseFrames = g_stCfg_FT8736_BasicThreshold.NoiseTest_Frames;
	...
	if(NT_AvgData == g_stCfg_FT8736_BasicThreshold.NoiseTest_NoiseMode)
	{
	...
	else if(NT_MaxData == g_stCfg_FT8736_BasicThreshold.NoiseTest_NoiseMode)
	{
	...
	else if(NT_MaxDevication == g_stCfg_FT8736_BasicThreshold.NoiseTest_NoiseMode)
	{
	...
	else if(NT_DifferData == g_stCfg_FT8736_BasicThreshold.NoiseTest_NoiseMode)
	{
		//Caculate the Avg Value of all nodes
		memset(m_NoiseData, 0, sizeof(m_NoiseData));
	...
	if( ReCode != ERROR_CODE_OK ) goto TEST_ERR;
		...
		iMinValue = 0;
		iMaxValue = g_stCfg_FT8736_BasicThreshold.NoiseTest_Coefficient * chNoiseValue * 32 / 100;
		FTS_TEST_DBG("\n");
		for(iRow = 0;iRow < (g_stSCapConfEx.ChannelXNum + 1);iRow++)
		{
			for(iCol = 0;iCol < g_stSCapConfEx.ChannelYNum;iCol++)
...
unsigned char FT8736_TestItem_CbTest(bool* bTestResult)
{
	...
	iMinValue = g_stCfg_FT8736_BasicThreshold.CbTest_Min;
	iMaxValue = g_stCfg_FT8736_BasicThreshold.CbTest_Max;	
	for(iRow = 0;iRow < (g_stSCapConfEx.ChannelXNum + 1);iRow++)
	{
		for(iCol = 0;iCol < g_stSCapConfEx.ChannelYNum;iCol++)
		{
