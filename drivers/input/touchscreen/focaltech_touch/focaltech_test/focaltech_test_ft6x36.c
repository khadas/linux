...
#define C6X36_CHANNEL_NUM	0x0A	//1 Byte	读写(RW)	TP_Channel_Num		VA区通道个数 最大值为63
#define C6X36_KEY_NUM		0x0B	//1 Byte	读写(RW)	TP_Key_Num		虚拟按键独立通道个数 最大值为63

#define C6X36_CB_ADDR_W 0x32/*0x32*/	//1 Byte	读写(RW)	CB_addr		普通模式CB--地址		
#define C6X36_CB_ADDR_R 0x33/*0x32*/	//1 Byte	读写(RW)	CB_addr		普通模式CB--地址	
#define C6X36_CB_BUF  0x39/*0x33*/	//1 Byte	只读(RO)	CB_buf		一通道对应2字节，因此长度是 2*N							
#define C6X36_RAWDATA_ADDR	0x34/*0x34*/	//1 Byte	读写(RW)	RawData_addr		Rawdata--地址							
#define C6X36_RAWDATA_BUF	0x35

#define C6206_FACTORY_TEST_MODE			0xAE	//0：正常工厂模式,1：量产测试工厂模式1（使用单端+0级防水扫描）,2：量产测试工厂模式2（使用单端+不防水扫描）
...
{	
	...
	{
		RawDataMin = g_stCfg_SCap_DetailThreshold.RawDataTest_Min[i];//取详细阈值
		RawDataMax = g_stCfg_SCap_DetailThreshold.RawDataTest_Max[i];//取详细阈值
...
unsigned char FT6X36_TestItem_CbTest(bool * bTestResult)
{
	...
	////////////////////////////单端防水
	FTS_TEST_DBG("Proof_Level0 CB Test...\r\n");
	for(i = 0; i < 3; i++)
	{
	...
	else
	{
		...
		for(i = 0; i < g_ScreenSetParam.iChannelsNum + g_ScreenSetParam.iKeyNum; i++)
		{
			...
			iCbMin = g_stCfg_SCap_DetailThreshold.CbTest_Min[i];//取详细阈值
			iCbMax = g_stCfg_SCap_DetailThreshold.CbTest_Max[i];//取详细阈值
	...
	////<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<收集测试数据，存入CSV文件
	//GetCiDataMsg();//收集Ci Data，存入CSV文件
...
unsigned char FT6X36_TestItem_DeltaCbTest(unsigned char * bTestResult)
{
	...
	//GetDeltaCiDataMsg();//收集Delta Ci Data，存入CSV文件
	return 0;
}
