...
//mipi tp-EP0510M09 edp tp-EP0660M09
	dev_info(&client->dev,
	...
	//desc = of_get_named_gpio_flags(node,"interrupt_pin", 0, NULL);
	//interrupt_pin = desc_to_gpio(desc);
	//client->irq = gpio_to_irq(interrupt_pin);

	error = of_property_read_string(node, "interrupt_pin", &value);
	if (error) {
		printk("hlm no interrupt_pin");
		return -1;
	} else {
		interrupt_pin = of_get_named_gpio_flags(node, "interrupt_pin", 0, NULL);
		printk("hlm interrupt_pin: %d.\n", interrupt_pin);
		error = gpio_request(interrupt_pin, "ft5x06");
		printk("hlm gpio_request ret: %d.\n", error);
	}	
	client->irq = gpio_to_irq(interrupt_pin);
...
static const struct edt_i2c_chip_data edt_ft5626_data = {
	.max_support_points = 10,
	.x_resolution = 1080,
	.y_resolution = 1920,
};
...
static const struct of_device_id edt_ft5x06_of_match[] = {
	...
	{ .compatible = "edt,edt-ft5626", .data = &edt_ft5626_data },
	{ /* sentinel */ }
};
