...
static int cts_init_fwdata(struct cts_device *cts_dev)
{
    ...
    cts_err("pc_cols: %ld", fwdata->pc_cols);
    cts_err("pc_rows: %ld", fwdata->pc_rows);
    cts_err("pr_cols: %ld", fwdata->pr_cols);
    cts_err("pr_rows: %ld", fwdata->pr_rows);
    cts_err("pc_cols_used: %ld", fwdata->pc_cols_used);
    cts_err("pc_rows_used: %ld", fwdata->pc_rows_used);
    cts_err("pr_cols_used: %ld", fwdata->pr_cols_used);
    cts_err("pr_rows_used: %ld", fwdata->pr_rows_used);
...
void cts_firmware_upgrade_work(struct work_struct *work)
{
    ...
    ret = cts_request_firmware(cts_dev, hwid, CTS_DEV_FWID_ANY, 0);
    if (ret) {
        cts_err("Get firmware failed!!!, Please add builtin or bin firmware!");
        return;
    }

    cts_lock_device(cts_dev);
    if (cts_dev->fwdata.version >= FIRMWARE_VERSION(cts_dev->firmware)) {
        cts_info("Do not update, (curr) 0x%04x >= 0x%04x (local)",
                cts_dev->fwdata.version, FIRMWARE_VERSION(cts_dev->firmware));
        cts_set_program_addr(cts_dev);
