...
static struct clk_regmap p1_mclk_pre_od = {
	.data = &(struct clk_regmap_div_data){
...
static struct clk_regmap p1_mclk_pll = {
	.data = &(struct clk_regmap_div_data){
...
static const char * const p1_gen_clk_parent_names[] = {
...
static struct clk_regmap p1_gen_sel = {
	...
	.hw.init = &(struct clk_init_data){
		.name = "gen_sel",
		.ops = &clk_regmap_mux_ops,
		.parent_names = p1_gen_clk_parent_names,
		.num_parents = ARRAY_SIZE(p1_gen_clk_parent_names),
	},
};
...
static struct clk_hw_onecell_data p1_hw_onecell_data = {
	.hws = {
		...
		[CLKID_MCLK_PRE]			= &p1_mclk_pre_od.hw,
		[CLKID_MCLK_PLL]			= &p1_mclk_pll.hw,
...
static int meson_p1_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < p1_hw_onecell_data.num; i++) {
		...
		dev_err(dev, "registering %d  %s\n", i,
		    p1_hw_onecell_data.hws[i]->init->name);
