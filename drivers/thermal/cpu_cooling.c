	...
	for (i = 0, j = 0; table[i].frequency != CPUFREQ_TABLE_END; i++) {
		...
		if (property == GET_LEVEL) {
			...
			if (descend && input >= freq) {
				if ((input > freq) && j)
					*output = (j - 1);
				else if ((input > freq) && !j)
					*output = 0;
				else
					*output = j;
				if (input != freq)
					pr_debug("%s %ld to level:%d, %s:%d\n",
						 "descend, fit ", input,
						 *output, "next freq", freq);
				return 0;
				 ...
				 unsigned long cooling_state)
{
	...
	if ((cpufreq_device->cpufreq_state == cooling_state) &&
	    (cpufreq_device->cpufreq_state == cur_state)) {
		return 0;
	} else {
		/*
		 ...
		 */
		pr_debug("%s %s:%d, cur_state:%ld, cooling_state:%ld\n",
			 __func__, "state is not same, cpufreq_state",
			 cpufreq_device->cpufreq_state,
			 cur_state, cooling_state);
	}
