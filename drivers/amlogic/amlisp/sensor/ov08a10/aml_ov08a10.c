...
#define OV08A10_GAIN_H       0x3508
#define OV08A10_GAIN_L       0x3509
#define OV08A10_EXPOSURE_H   0x3501
...
static int ov08a10_set_gain(struct ov08a10 *ov08a10, u32 value)
{
	...
	Gain_H = (value >> 8) & 0xFF;
	Gain_L = value & 0xFF;
	dev_err(ov08a10->dev, "ov08a10 write gain = 0x%x \n", value);
	ret = ov08a10_write_reg(ov08a10, OV08A10_GAIN_H,Gain_H);
	if (ret)
		dev_err(ov08a10->dev, "Unable to write gainH\n");

	ret = ov08a10_write_reg(ov08a10, OV08A10_GAIN_L,Gain_L);
	if (ret)
		dev_err(ov08a10->dev, "Unable to write gainL\n");

	ov08a10_read_reg(ov08a10, OV08A10_GAIN_H, &Gain_H);
	ov08a10_read_reg(ov08a10, OV08A10_GAIN_L, &Gain_L);
	dev_err(ov08a10->dev, "ov08a10 read gain = 0x%x \n", (Gain_H << 8) | Gain_L);
	return ret;
}
...
static int ov08a10_set_exposure(struct ov08a10 *ov08a10, u32 value)
{
	...
	Expo_H = (value >> 8) & 0xFF;
	Expo_L = value & 0xFF;
	//dev_err(ov08a10->dev, "ov08a10 write exposure = 0x%x \n", value);
	//dev_err(ov08a10->dev, "ov08a10 write exposure = 0x%x \n", (Expo_H << 8) | Expo_L);
	mutex_lock(&ov08a10->lock);
	ret = ov08a10_write_reg(ov08a10, OV08A10_EXPOSURE_H,Expo_H);
	if (ret)
		dev_err(ov08a10->dev, "Unable to write Expo_H\n");

	ret = ov08a10_write_reg(ov08a10, OV08A10_EXPOSURE_L,Expo_L);
	if (ret)
		dev_err(ov08a10->dev, "Unable to write Expo_L\n");

	ret = ov08a10_read_reg(ov08a10, OV08A10_EXPOSURE_H, &Expo_H);
	if (ret)
		dev_err(ov08a10->dev, "Unable to read Expo_H\n");

	ret = ov08a10_read_reg(ov08a10, OV08A10_EXPOSURE_L, &Expo_L);
	if (ret)
		dev_err(ov08a10->dev, "Unable to read Expo_L\n");
	mutex_unlock(&ov08a10->lock);
	//dev_err(ov08a10->dev, "ov08a10 read exposure = 0x%x \n", (Expo_H << 8) | Expo_L);
	return ret;
}
...
static int ov08a10_parse_power(struct ov08a10 *ov08a10)
{
	...
	int reset_pin = -1;
	int pwdn_pin = -1;
	reset_pin = of_get_named_gpio(ov08a10->dev->of_node, "reset", 0);
	if (reset_pin < 0) {
	...
	pwdn_pin = of_get_named_gpio(ov08a10->dev->of_node, "pwdn", 0);
	if (pwdn_pin < 0) {
