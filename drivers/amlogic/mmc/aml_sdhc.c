...
*/
int aml_sdhc_read_response(struct mmc_host *mmc,
...
void aml_sdhc_prepare_dma(struct amlsd_host *host,
...
/*
* set host->clkc_w for 8bit emmc write cmd
* as it would fail on TXFIFO EMPTY,
...
void aml_sdhc_set_clkc(struct mmc_host *mmc)
{
	...
	sdhc_dbg(AMLSD_DBG_IOS,
		"vclkc %x pdata->clkc %x, clk_div %d\n",
		vclkc, pdata->clkc, clkc->clk_div);

	writel(*(u32 *)clkc, host->base + SDHC_CLKC);
}

void aml_sdhc_start_cmd(struct mmc_host *mmc,
	struct mmc_request *mrq)
{
	struct amlsd_platform *pdata = mmc_priv(mmc);
	struct amlsd_host *host = pdata->host;
...
void aml_sdhc_request_done(struct mmc_host *mmc,
	struct mmc_request *mrq)
{
	struct amlsd_platform *pdata = mmc_priv(mmc);
	struct amlsd_host *host = pdata->host;
	...
	/* this should be the last action, otherwise things may wrong */
...
void aml_sdhc_timeout(unsigned long data)
{
	...
	sdhc_err("%s: Timeout Cmd%d arg %08x Xfer %d Bytes,
		host->xfer_step=%d, host->cmd_is_stop=%d,
		pdata->port=%d,
		SDHC_CLKC=%#x, pdata->clkc=%#x,
		ctrl->dat_type=%d, pdata->width=%d\n",
	...
	if (pdata->port != MESON_SDIO_PORT_XC_A) {
		...
		if ((pdata->port == MESON_SDIO_PORT_XC_B)
			&& (pdata->gpio_power != 0))
			sdhc_err("power_on_pin=%d\n",
				amlogic_get_value(pdata->gpio_power,
					MODULE_NAME));
#endif
	...
	/*read response, if error, set -EILSEQ,
	then retry in aml_sdhc_request_done*/
	...
	*/
	...
	/*timeout in CMD12(send stop),
	return CRC error(-EILSEQ), should retry*/
	if (ista->resp_no_busy && mrq->data
		&& mrq->data->blocks > 1) {
...
#define	 PORT_SDIO		   PORT_SDHC_A
/* SDIO devices(for SDIO-WIFI) */
#define	 PORT_BLOCK		  PORT_SDHC_B
/* Block devices(for SD/MMC card) */
#define	 PORT_BASE		   PORT_SDHC_C
/* Base block device (for TSD/eMMC) */
/*sdhc controller does not support wifi now, return*/
int aml_sdhc_check_unsupport_cmd(struct mmc_host *mmc,
	struct mmc_request *mrq)
{
	struct amlsd_platform *pdata = mmc_priv(mmc);
...
static irqreturn_t aml_sdhc_irq(int irq, void *dev_id)
{
	...
	if (victl & vista) {
		...
		if ((ista->resp_err_crc && !mrq->data) ||
			(ista->data_err_crc && mrq->data
				&& mrq->data->blksz == 512)) {
			mrq->cmd->error = -EILSEQ;
				sdhc_err("%s CRC ERR Cmd %d Ista 0x%x,
					Xfer %d Bytes\n",
					pdata->pinname,
					host->mrq->cmd->opcode, vista,
					mrq->data?mrq->data->blksz*
						mrq->data->blocks:0);
			goto req_done;
		}
		...
		if (ista->data_1pack_ok || ista->data_xfer_ok) {
			...
			spin_unlock_irqrestore(&host->mrq_lock, flags);
			return IRQ_WAKE_THREAD;
		} else if (ictl->resp_ok && ista->resp_ok) {
			/*cmd without data*/
			host->xfer_step = XFER_IRQ_TASKLET_CMD;
...
irqreturn_t aml_sdhc_data_thread(int irq, void *data)
{
	...
	case XFER_IRQ_TASKLET_DATA:
		...
		/*enable busy turn ready irq bit,
		sdhc controller would send stop automatically*/
		if (host->mrq->data->blocks > 1)
			aml_sdhc_enable_imask(host,
				ICTL_DAT0_TURN_READY);
		else{
	...
	case XFER_IRQ_TASKLET_CMD:
		if (!host->mrq->data) {
			del_timer(&host->timeout_tlist);
			aml_sdhc_read_response(host->mmc,
				host->mrq->cmd);
	...
	case XFER_IRQ_TASKLET_BUSY:
		...
		aml_sdhc_clear_fifo(host);
		host->mrq->cmd->error = 0;
		xfer_bytes = host->mrq->data->blksz*
...
4. set clock divide
*/
static void aml_sdhc_set_clk_rate(struct mmc_host *mmc,
	unsigned int clk_ios)
{
	...
	*/
	...
	sdhc_dbg(AMLSD_DBG_IOS,
		"Clk IOS %d, Clk Src %d, Host Max Clk %d\n",
	...
	pdata->clkc = vclkc;
	/* pr_info("%s pdata->clkc %x, clk_div %d\n",
	...
	for (i = 0; i < MMC_MAX_DEVICE; i++) {
		...
		if (pdata->port == PORT_SDHC_C) {
			if (is_emmc_exist(host)) {
				...
				pr_info("[%s]: there is not eMMC/tsd,
					skip sdhc_c dts config!\n", __func__);
