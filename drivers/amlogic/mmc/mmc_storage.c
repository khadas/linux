/*
 * drivers/amlogic/mmc/mmc_storage.c
 *
...
void print_storage_node_info(void)
{
	...
	list_for_each_entry(node, &storage_node_list, storage_list) {
		store_msg(
		"node->offset_addr %llx , node->timestamp %d ,node->valid_node_flag %d",
...
static int check_data(void *cmp)
{
	...
	if (head->checksum != mmc_checksum(
		(unsigned char *)&(head->data[0]),
		MMC_STORAGE_AREA_VALID_SIZE)) {
...
static int storage_check(struct mmc_card *device, void *buf)
{
	...
	if (ret) {
		store_msg(
		"mmc read storage check magic name failed and ret = %d\n",
			ret);
...
int mmc_storage_rw_kernel(struct mmc_card *device,
struct storage_node_t *storage_node,
unsigned char *databuf, int len, int direct)
{
	...
	if (direct == 0) {
		while (size > 0) {
			ret = mmc_read_internal(device,
				start_blk, force_size, databuf);

			if (ret != 0)
				store_msg(
				"mmc_storage_rw_kernel : read storage at block %d failed",
				start_blk);
		...
		while (size > 0) {
			ret = mmc_write_internal(device, start_blk,
				force_size, databuf);
...
int mmc_storage_read(struct mmc_card *device, unsigned char *buf, int len)
{
	...
	list_for_each_entry(storage_node, &storage_node_list, storage_list) {
		if ((storage_node != NULL)
			&& (storage_node->valid_node_flag == 1)) {

			memset((unsigned char *)storage_data, 0x0,

			sizeof(struct mmc_storage_head_t));

			ret = mmc_storage_rw_kernel(
				device, storage_node,
			...
			if (!valid_node_failed)
				store_msg(
				"mmc read storage ok at :  %llx",
				storage_node->offset_addr);
		}
	}
	...
	if (valid_node_failed) {
		valid_node_failed = 0;
		list_for_each_entry(storage_node,
		&storage_node_list, storage_list) {
			if ((storage_node != NULL)
				&& (storage_node->valid_node_flag == 0)) {

				memset((unsigned char *)storage_data,
					0x0, sizeof(struct mmc_storage_head_t));

				ret = mmc_storage_rw_kernel(
				device, storage_node,
				(unsigned char *)storage_data, read_len, 0);
				...
				if (ret) {
					store_msg
					("storage check free node failed");
					valid_node_failed  = 1;
				}
...
int mmc_storage_write(struct mmc_card *device, unsigned char *buf, int len)
{
	...
	storage_data = kmalloc(
		sizeof(struct mmc_storage_head_t), GFP_KERNEL);
	if (storage_data == NULL) {
	...
	storage_data->magic_checksum = mmc_checksum(
		(unsigned char *)&storage_data->magic[0],
		MMC_STORAGE_MAGIC_SIZE);
	...
	show_data_buf(&storage_data->data[0]);
#endif
	storage_data->checksum = mmc_checksum(
		(unsigned char *)&storage_data->data[0],
		MMC_STORAGE_AREA_VALID_SIZE);
	...
	list_for_each_entry(storage_node,
		...
		if ((storage_node != NULL)
			&& (storage_node->valid_node_flag == 1))
			storage_data->timestamp = storage_node->timestamp + 1;
	}
	...
	list_for_each_entry(storage_node, &storage_node_list, storage_list) {
		if ((storage_node != NULL) &&
			...
			write_failed_flag = 0;
			ret = mmc_storage_rw_kernel(device, storage_node,
				(unsigned char *)storage_data, write_len, 1);
			if (ret) {
				store_msg(
					"storage write part %d failed at %llx",
					part_num, storage_node->offset_addr);
				write_failed_flag = 1;
			}
			part_num++;
			...
			if (!write_failed_flag) {
				...
				store_msg(
					"storage write part %d success at %llx",
					part_num, storage_node->offset_addr);
			}
		}
	}
...
int mmc_storage_init(struct mmc_card *device)
{
	...
	for (cnt_num = 0; cnt_num < part_num; cnt_num++) {
		...
		ret = mmc_storage_rw_kernel(device, &tmp_node,
			(unsigned char *)data_buf, MMC_STORAGE_AREA_SIZE, 0);
		if (ret) {
			store_msg("storage read failed");
		...
		if (ret) {
			store_msg(
				"storage check : invalid storage in addr %llx",
				 addr);
		} else{
			mmc_storage->secure_valid = 1;
			if (cnt_num == 0) {
	...
	if (mmc_storage->secure_valid == 1) {
		...
		if (part0_node.valid_flag && part1_node.valid_flag) {
			if (part0_node.timestamp >= part1_node.timestamp) {
				memcpy(mmc_storage->valid_node, &part0_node,
					sizeof(struct storage_node_t));
				memcpy(mmc_storage->free_node, &part1_node,
					sizeof(struct storage_node_t));
			} else{
				memcpy(mmc_storage->valid_node, &part1_node,
					sizeof(struct storage_node_t));
				memcpy(mmc_storage->free_node, &part0_node,
					sizeof(struct storage_node_t));
			}
			mmc_storage->valid_node->valid_node_flag = 1;
			mmc_storage->free_node->valid_node_flag = 0;

			list_add_tail(&mmc_storage->valid_node->storage_list,
				&storage_node_list);
			list_add_tail(&mmc_storage->free_node->storage_list,
				&storage_node_list);

			store_msg(
				"mmc  storage node0  addr = %llx and node1 addr = %llx",
				part0_node.offset_addr, part1_node.offset_addr);
			...
			part1_node.offset_addr = MMC_STORAGE_OFFSET
				+ MMC_STORAGE_AREA_SIZE;
			...
			store_msg("mmc storage node0 in mmc addr = %llx",
				part0_node.offset_addr);
			...
			store_msg("mmc storage node1 in mmc addr = %llx",
				part1_node.offset_addr);
		}
	} else
...
int mmc_storage_check(struct mmc_card *device)
{
	...
	if (mmc_storage->secure_valid == 0) {
		...
		mmc_storage->free_node->offset_addr = MMC_STORAGE_OFFSET
			+ MMC_STORAGE_AREA_SIZE;
...
static int storage_buf_malloc(struct mmc_card *device)
{
	...
	mmc_storage->valid_node = kmalloc(
		sizeof(struct storage_node_t), GFP_KERNEL);
...
int mmc_secure_storage_ops(unsigned char *buf, unsigned int len, int wr_flag)
{
	...
	if (len > MMC_STORAGE_AREA_VALID_SIZE) {
		store_msg(
			"mmc secure storage write fail,len 0x%x is bigger than 0x%x,%s:%d",
			len, MMC_STORAGE_AREA_VALID_SIZE, __func__, __LINE__);
		return -1;
	}
