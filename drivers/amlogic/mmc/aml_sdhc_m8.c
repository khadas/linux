...
for (n = 0; n <= clkc->clk_div; n++) {
	...
	/*  pr_info("Tuning result: rx_phase=%d\n", rx_phase); */
	/* sdhc_err("Tuning result:
	rx_phase=%d, tune_phase=%#x\n",
...
static int aml_sdhc_execute_tuning(struct mmc_host *mmc, u32 opcode)
{
	...
	/* if ((pdata->port != PORT_SDHC_A)
		&& (pdata->port != PORT_SDHC_C))
		{ // debug */
	/* return 0; */
	/* } */
	...
	/*  pr_info("Tuning already, tune_phase=0x%x\n", pdata->tune_phase); */
	/* return 0; */
	/* } */
...
void aml_sdhc_start_cmd(struct amlsd_platform *pdata, struct mmc_request *mrq)
{
	...
	if (mrq->data && (mrq->data->flags & MMC_DATA_WRITE)) {
		...
		if (stat->txfifo_cnt != 0) {
			...
			/* mmc_hostname(host->mmc),
			mrq->cmd->opcode, stat->txfifo_cnt, i); */
			break;
			}
			udelay(1);
		}
...
static void aml_sdhc_timeout(struct work_struct *work)
{
	 ...
	 /* u32 vista = readl(host->base + SDHC_ISTA); */
	 /* struct sdhc_ista* ista =
	 (struct sdhc_ista*)&vista; */
...
timeout_handle:
	...
	/*read response, if error,
	set -EILSEQ, then retry in
	aml_sdhc_request_done*/
	/* if(R1_STATUS(host->mrq->cmd->resp[0])
		& R1_COM_CRC_ERROR ){ */
	...
	host->mrq->cmd->opcode, */
		/* host->mrq->cmd->resp[0], */
		/* mrq->data?mrq->
		data->blksz*mrq->data->blocks:0); */
	/* goto req_done; */
	/* } */


	/*
	* response crc error & data
	crc error with xfer size >= 512,
	* ignore crc error with data transfer
	less then 512, for example 64B
	*/
...
/* req_done: */
	...
	MMC_SEND_TUNING_BLOCK_HS200)) {
		/* if((host->mrq->cmd->opcode != SD_IO_RW_DIRECT)
		&& (host->mrq->cmd->opcode !=
		SD_IO_RW_EXTENDED) */
...
if (host->mrq->data->flags & MMC_DATA_READ) {
	...
	if (i == STAT_POLL_TIMEOUT) /* error */
		sdhc_err(
		"Warning:DMA state is wrong! SDHC_ESTA=0x%x\n", esta);
	...
	for (i = 0; i < STAT_POLL_TIMEOUT; i++) {
		...
		if (get_cpu_type() ==
			...
			/* dmc_sts =
			aml_read_reg32(P_MMC_CHAN_STS); */
			dmc_sts = 1;/* (dmc_sts >> 11)&1; */
			} else{
				/* dmc_sts =
				aml_read_reg32(P_DMC_CHAN_STS); */
				dmc_sts = 1;/* (dmc_sts >> 15)&1; */
				}
/* #endif */
				if (dmc_sts)
					break;
...
/* #endif */
			...
			mrq->data->sg_len, host->bn_buf, xfer_bytes, 0);
	...
	if (delayed_work_pending(&host->timeout))
		...
		/* do not send stop for sdio wifi case */
		if (host->mrq->stop && aml_card_type_mmc(pdata)
			&& pdata->is_in && (host->mrq->cmd->opcode !=
			MMC_SEND_TUNING_BLOCK) && (host->mrq->cmd->opcode
			!= MMC_SEND_TUNING_BLOCK_HS200)) {
			/* if((host->mrq->cmd->opcode != SD_IO_RW_DIRECT)
			&& (host->mrq->cmd->opcode != SD_IO_RW_EXTENDED) */
			/* && (!mmc_card_removed(pdata->mmc->card))
			&& (!mrq->data)){ */
			aml_sdhc_send_stop(host);
		} else{
...
const char *buf, size_t count)
{
	int i;
	/* struct amlsd_host *host =
c	ontainer_of(class, struct amlsd_host, debug); */
	i = sscanf(buf, "%x", &sdhc_debug_flag);
