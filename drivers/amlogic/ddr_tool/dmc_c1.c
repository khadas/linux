...
static void check_violation(struct dmc_monitor *mon, void *data)
{
	...
	for (i = 1; i < 4; i += 2) {
		...
		if (port == 16)
			port = subport + 9;
		pr_info(DMC_TAG ", addr:%08lx, s:%08lx, ID:%s, sub:%d, c:%ld, d:%p\n",
...
static int c1_dmc_mon_set(struct dmc_monitor *mon)
{
	...
	dmc_rw(DMC_PROT0_STA, add, DMC_WRITE);
	add = mon->addr_end & PAGE_MASK;
	dmc_rw(DMC_PROT0_EDA, add, DMC_WRITE);

	dmc_rw(DMC_PROT0_CTRL, mon->device, DMC_WRITE);
...
{
	dmc_rw(DMC_PROT0_STA,   0, DMC_WRITE);
	dmc_rw(DMC_PROT0_EDA,   0, DMC_WRITE);
	dmc_rw(DMC_PROT0_CTRL,  0, DMC_WRITE);
	dmc_rw(DMC_PROT0_CTRL1, 0, DMC_WRITE);
