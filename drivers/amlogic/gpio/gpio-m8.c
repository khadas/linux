	...
	if (!ret) {
		for (i = 0; i < sizeof(gpio_to_pin[offset])/sizeof(gpio_to_pin[offset][0]); i++) {
			if (gpio_reg[i] != NONE) {
				...
				writel_relaxed((readl_relaxed(p_pin_mux_reg_addr[reg]) & (~(1<<bit))), p_pin_mux_reg_addr[reg]);
				pr_debug("clr reg=%d,bit =%d\n", reg, bit);
			}
		}
	}
	return ret;
...
int gpio_amlogic_to_irq(struct gpio_chip *chip, unsigned offset)
{
	...
	if (offset > gpio_max)
		return -1;
	aml_clrset_reg32_bits(gpio_interrupt_base+GPIO_EDGE, 0x10001<<irq_bank, type[irq_type]<<irq_bank);
	pr_info(" reg:%p,clearmask=%x,setmask=%x\n", ((gpio_interrupt_base+GPIO_EDGE)), 0x10001<<irq_bank, (aml_read_reg32(gpio_interrupt_base+GPIO_EDGE)>>irq_bank)&0x10001);
	/*select pin*/
	reg = irq_bank < 4?(gpio_interrupt_base+GPIO_SELECT_0_3):(gpio_interrupt_base+GPIO_SELECT_4_7);
	start_bit = (irq_bank&3)*8;
	aml_clrset_reg32_bits(reg, 0xff<<start_bit, amlogic_pins[offset].num<<start_bit);
	pr_info("reg:%p,clearmask=%x,set pin=%d\n", (reg), 0xff<<start_bit, (aml_read_reg32(reg)>>start_bit)&0xff);
	/*set filter*/
	start_bit = (irq_bank)*4;
	aml_clrset_reg32_bits((gpio_interrupt_base+GPIO_FILTER_NUM), 0x7<<start_bit, filter<<start_bit);
	pr_info("reg:%p,clearmask=%x,setmask=%x\n", ((gpio_interrupt_base+GPIO_FILTER_NUM)), 0x7<<start_bit, (aml_read_reg32((gpio_interrupt_base+GPIO_FILTER_NUM))>>start_bit)&0x7);
	return 0;
}
...
int gpio_amlogic_get(struct gpio_chip *chip, unsigned offset)
{
	...
	return readl_relaxed(p_gpio_input_addr[reg]) >> (bit);
}
	...
	if (offset == GPIO_BSD_EN) {
		...
		if (value)
			if (is_meson_m8_cpu())
				...
				aml_set_reg32_mask(p_pull_up_addr[2], 1<<0);
		else
			if (is_meson_m8_cpu())
				aml_clr_reg32_mask(p_gpio_output_addr[0], 1<<31);
			else
				aml_clr_reg32_mask(p_pull_up_addr[2], 1<<0);
		aml_clr_reg32_mask(p_gpio_output_addr[0], 1<<30);
		return 0;
	}
	if (offset == GPIO_TEST_N) {
		if (value)
			aml_set_reg32_mask(p_gpio_oen_addr[0], 1<<31);
		else
			aml_clr_reg32_mask(p_gpio_oen_addr[0], 1<<31);
		aml_set_reg32_mask(ao_secure0_base, 1);
		return 0;
	}
	if (value) {
...
void	gpio_amlogic_set(struct gpio_chip *chip, unsigned offset, int value)
{
	...
	pr_debug("==%s==%d\n", __func__, __LINE__);
	if (value)
		writel_relaxed((readl_relaxed(p_gpio_output_addr[reg]) | (1<<bit)), p_gpio_output_addr[reg]);
	else
		writel_relaxed((readl_relaxed(p_gpio_output_addr[reg]) & (~(1<<bit))), p_gpio_output_addr[reg]);
}
...
static int amlogic_gpio_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < 4; i++) {
		p_gpio_oen_addr[i] = p;
		p = p+4;
		p_gpio_output_addr[i] = p;
		p = p+4;
		p_gpio_input_addr[i] = p;
		p = p+4;
	}
	p = of_iomap(np, 1);
	p_gpio_oen_addr[4] = p;
	p_gpio_output_addr[4] = p;
	p_gpio_input_addr[4] = p+4;
	for (i = 0; i < 5; i++)
		pr_debug("p_gpio_oen_addr[%d]=%p,p_gpio_output_addr[%d]=%p,p_gpio_input_addr[%d]=%p\n", i, p_gpio_oen_addr[i],
		i, p_gpio_output_addr[i], i, p_gpio_input_addr[i]);
	ao_secure0_base = of_iomap(np, 2);
	gpio_interrupt_base = of_iomap(np, 3);
	dev_info(&pdev->dev, "Probed amlogic GPIO driver\n");
#if 0
	int i, j;
	for (i = 0; i <= GPIO_TEST_N; i++) {
		for (j = 0; j < 5; j++)
		pr_info("%d,%d\t", GPIO_REG(gpio_to_pin[i][j]), GPIO_BIT(gpio_to_pin[i][j]));
	pr_info("\n");
	}
#endif
#ifdef gpio_dump
	int i;
	for (i = 0; i < GPIO_TEST_N; i++)
		pr_info("%s,amlogic_pins[%d]=%d,%d,out en reg=%x,bit=%d,out val reg=%x,bit=%d,input reg=%x,bit=%d\n",
		amlogic_pins[i].name, i, amlogic_pins[i].num,
		m8_gpio_name_to_num(amlogic_pins[i].name),
	...
	for (i = 0; i < GPIO_TEST_N; i++) {
		...
		pr_info("%s \t,pull up en reg:%x \t,enbit:%d \t,==,pull up reg:%x \t,bit:%d \t\n",
			amlogic_pins[i].name,
	...
	for (i = 0; i < GPIO_TEST_N; i++) {
		...
		pr_info("%s \t,output en reg:%x \t,enbit:%d \t\n",
			amlogic_pins[i].name,
