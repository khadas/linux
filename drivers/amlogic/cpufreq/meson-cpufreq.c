						...
						unsigned int relation)
{
	...
	if (fix_syspll) {
		...
		ret        = cpufreq_frequency_table_target(
						policy, freq_table, target_freq,
						CPUFREQ_RELATION_H, &index);
		if (ret >= 0) {
			freq_new = freq_table[index].frequency;
			target_freq = freq_new;
		} else
			pr_err("input frequent :%d cannot found in frequent table, ret:%d\n",
				   target_freq, ret);
	}
#endif
	...
	/* if moving to lower freq, lower the voltage after
	 * lowering freq This should be done after CPUFREQ_PRECHANGE,
	 * which will adjust lpj and affect our udelays.
	 */
...
#ifdef CONFIG_FIX_SYSPLL
	if (fix_syspll) {
		/* select fix pll table if syspll_fixed is enabled in dts */
		cpufreq_frequency_table_get_attr(
						 meson_freq_table_fix_syspll,
						 policy->cpu);
	} else {
#endif
...
static int __init meson_cpufreq_probe(struct platform_device *pdev)
{
	...
	if (pdev->dev.of_node) {
		...
		if (err) {
			pr_err("%s:SYSPLL request to be fixed\n", __func__);
			fix_syspll = 1;
		}
	}
#endif
	if (pdev->dev.of_node) {
		...
		if (prop)
			...
			pr_debug("meson_cpufreq: no voltage_control prop\n");
	}
#endif
