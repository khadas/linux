/*
 * drivers/amlogic/power/axp_power/axp20-mfd.h
 *
...
static int  axp20_init_chip(struct axp_mfd_chip *chip)
{
	uint8_t chip_id;
	...
	if (err) {
		pr_info("[AXP20-MFD] try to read chip id failed!\n");
		return err;
	}
	...
	if (err) {
		pr_info("[AXP20-MFD] try to clear irq failed!\n");
		return err;
	}
