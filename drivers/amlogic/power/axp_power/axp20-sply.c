/*
 * drivers/amlogic/power/axp_power/axp20-sply.c
 *
...
static void axp_read_adc(struct axp20_supply *, struct axp_adc_res *);
static int  axp20_update_status(struct aml_charger *);
static void axp_charger_update(struct axp20_supply *);

static int axp_io_state;
...
int axp_gpio_set_io(int gpio, int io_state)
{
	...
	struct device *dev;
	if (!axp) {
		pr_info("[AXP] driver has not ready now, wait...\n");
		return -ENODEV;
	}
	dev = axp_get_dev();
	if (io_state == 1) {
		...
		switch (gpio) {
		/*
		 * for out put, set pin to high-z state, and remember to a
		 * variable in order to prevent unstable state on GPIO.
		 */
		case 0:
			r = axp_update(dev, AXP20_GPIO0_CFG, 0x07, 0x07);
			break;
		case 1:
			r = axp_update(dev, AXP20_GPIO1_CFG, 0x07, 0x07);
			break;
		case 2:
			r = axp_update(dev, AXP20_GPIO2_CFG, 0x07, 0x07);
			break;
		case 3:
			r = axp_update(dev, AXP20_GPIO3_CFG, 0x04, 0x04);
			break;
		default:
			return -ENXIO;
		}
		...
		switch (gpio) {
		case 0:
			r = axp_update(dev, AXP20_GPIO0_CFG, 0x02, 0x07);
			break;
		case 1:
			r = axp_update(dev, AXP20_GPIO1_CFG, 0x02, 0x07);
			break;
		case 2:
			r = axp_update(dev, AXP20_GPIO2_CFG, 0x02, 0x07);
			break;
		case 3:
			r = axp_update(dev, AXP20_GPIO3_CFG, 0x04, 0x04);
			break;
...
int axp_gpio_get_io(int gpio, int *io_state)
{
	...
	switch (gpio) {
	case 0:
		axp_read(dev, AXP20_GPIO0_CFG, &val);
		return check_io012(gpio, val & 0x07, io_state);
	case 1:
		axp_read(dev, AXP20_GPIO1_CFG, &val);
		return check_io012(gpio, val & 0x07, io_state);
	case 2:
		axp_read(dev, AXP20_GPIO2_CFG, &val);
		return check_io012(gpio, val & 0x07, io_state);
	case 3:
		axp_read(dev, AXP20_GPIO3_CFG, &val);
		val &= 0x04;
		if (val == 0x0)
			*io_state = 1;
		else
...
int axp_gpio_set_value(int gpio, int value)
{
	...
	if (value) {
		switch (gpio) {
		case 0:
			r = axp_update(dev, AXP20_GPIO0_CFG, 0x01, 0x07);
			break;
		case 1:
			r = axp_update(dev, AXP20_GPIO1_CFG, 0x01, 0x07);
			break;
		case 2:
			/* Need extern pull up resistor */
			r = axp_update(dev, AXP20_GPIO2_CFG, 0x01, 0x07);
			break;
		case 3:
			/* Need extern pull up resistor */
			r = axp_update(dev, AXP20_GPIO3_CFG, 0x02, 0x07);
			break;
		default:
			break;
		}
		...
		switch (gpio) {
		case 0:
			r = axp_update(dev, AXP20_GPIO0_CFG, 0x00, 0x07);
			break;
		case 1:
			r = axp_update(dev, AXP20_GPIO1_CFG, 0x00, 0x07);
			break;
		case 2:
			r = axp_update(dev, AXP20_GPIO2_CFG, 0x00, 0x07);
			break;
		case 3:
			r = axp_update(dev, AXP20_GPIO3_CFG, 0x00, 0x07);
			break;
		default:
			break;
		}
...
int axp_gpio_get_value(int gpio, int *value)
{
	...
	if (io_state) {
		switch (gpio) {
		case 0:
			ret = axp_read(dev, AXP20_GPIO0_CFG, &val);
			*value = val & 0x01;
			break;
		case 1:
			ret = axp_read(dev, AXP20_GPIO1_CFG, &val);
			*value = val & 0x01;
			break;
		case 2:
			ret = axp_read(dev, AXP20_GPIO2_CFG, &val);
			*value = val & 0x01;
			break;
		case 3:
			ret = axp_read(dev, AXP20_GPIO3_CFG, &val);
			val &= 0x02;
			*value = val >> 1;
			break;
		default:
		...
		switch (gpio) {
		case 0:
			ret = axp_read(dev, AXP20_GPIO012_STATE, &val);
			val &= 0x10;
			*value = val >> 4;
			break;
		case 1:
...
static int axp_set_charge(struct axp20_supply *supply)
{
	...
	if (axp_pmu_battery->pmu_init_chgvol < 4150000) {
		/* target charge voltage set to 4.10V */
		val &= ~(0x03 << 5);
	} else if (axp_pmu_battery->pmu_init_chgvol < 4200000) {
		/* target charge voltage set to 4.15V */
		val |=  (0x01 << 5);
	} else if (axp_pmu_battery->pmu_init_chgvol < 4360000) {
		/* target charge voltage set to 4.20V */
		val |=  (0x02 << 5);
	} else {
		/* target charge voltage set to 4.36V */
		val |=  (0x03 << 5);
	}
	...
	if (axp_pmu_battery->pmu_init_chgend_rate == 10) {
		/* set charge end rate */
		val &= ~(1 << 4);
	} else
		val |=  (1 << 4);

	if (axp_pmu_battery->pmu_init_chg_enabled) {
		/* enable charger */
		val |=  (1 << 7);
	} else {
		AXP_PMU_DBG("%s, do you sure not open charger?\n",
	...
	if (tmp < 40) {
		/* set pre-charge time out */
		tmp = 40;
	} else if (tmp > 70)
		tmp = 70;
	val |= ((((tmp - 40) / 10) << 6) & 0xc0);
	tmp = axp_pmu_battery->pmu_init_chg_csttime;
	if (tmp < 360)
		tmp = 360;
	else if (tmp > 720)
		tmp = 720;
...
static void axp_battery_check_status(struct axp20_supply *supply,
...
int axp_set_charge_current(int chgcur)
{
	...
	if (chgcur >= 300000 && chgcur <= 1800000) {
		tmp = ((chgcur - 200001) / 100000 & 0x0f) | 0x80;
		axp_update(g_axp20_supply->master, AXP20_CHARGE_CONTROL1,
			   tmp, 0x8F);
	} else if (chgcur == 0) {
		axp_clr_bits(g_axp20_supply->master,
			     AXP20_CHARGE_CONTROL1, 0x80);
	} else {
...
int axp_charger_set_usbcur_limit_extern(int usbcur_limit)
{
	...
	case 900:
		...
		break;
	default:
		AXP_PMU_DBG("%s=%d, not in 0,100,500,900. please check!\n",
			    "usbcur_limit", usbcur_limit);
		return -1;
		break;
	}
...
static int axp_battery_adc_set(struct axp20_supply *supply)
{
	...
	if (freq < 25 || freq > 200) {
		AXP_PMU_DBG("%s, invalid %s:%d, we use 100Hz as default\n",
			    __func__, "pmu_init_adc_freqc", freq);
		freq = 100;
	}
...
{
	if (axp_pmu_battery->pmu_usbvol_limit) {
		/* limit VBUS voltage */
		axp_set_usb_voltage_limit(axp_pmu_battery->pmu_usbvol);
	} else {
		/* not limit */
		axp_clr_bits(supply->master, AXP20_CHARGE_VBUS, (1<<6));
	}

	axp_set_noe_delay(axp_pmu_battery->pmu_pwrnoe_time);
	axp_set_pek_on_time(axp_pmu_battery->pmu_pekon_time);
	axp_set_pek_long_time(axp_pmu_battery->pmu_peklong_time);
	axp_set_pek_off_en(axp_pmu_battery->pmu_pekoff_en);
	axp_set_pwrok_delay(axp_pmu_battery->pmu_pwrok_time);
	axp_set_pek_off_time(axp_pmu_battery->pmu_pekoff_time);
	axp_set_charge_current(axp_pmu_battery->pmu_init_chgcur);
	axp_set_ot_power_off_en(axp_pmu_battery->pmu_intotp_en);
	axp_set_poweroff_voltage(axp_pmu_battery->pmu_pwroff_vol);
}
			  ...
			  struct device_attribute *attr, char *buf)
{
	...
	supply  = container_of(battery, struct axp20_supply, batt);
				 ...
				 struct device_attribute *attr, char *buf)
{
	...
	curve   = axp_pmu_battery->pmu_bat_curve;
	for (i = 0; i < 16; i++) {
...
{
	return sprintf(buf,
		       "Amlogic AXP202 driver version is %s, build time:%s\n",
...
#ifdef CONFIG_AMLOGIC_USB
int axp202_otg_change(struct notifier_block *nb, unsigned long value, void *p)
{
	/*
...
int axp202_usb_charger(struct notifier_block *nb, unsigned long value, void *p)
{
	int curr;
	...
	curr = axp_pmu_battery->pmu_usbcur;
	switch (value) {
	case USB_BC_MODE_DISCONNECT:	/* disconnect */
	case USB_BC_MODE_SDP:		/* pc */
		if (axp_pmu_battery && axp_pmu_battery->pmu_usbcur_limit) {
			/* limit usb current */
			axp_charger_set_usbcur_limit_extern(curr);
		}
		break;
	...
	case USB_BC_MODE_CDP:		/* PC + charger */
		if (axp_pmu_battery) {
			...
			axp_charger_set_usbcur_limit_extern(-1);
		}
		break;
...
int axp20_get_coulomb(struct aml_charger *charger)
{
	struct  axp20_supply *supply;
	uint8_t temp[8];
	int64_t Value1, Value2, rValue;
	int	 m;

	supply = container_of(charger, struct axp20_supply, aml_charger);
	m = axp_get_freq() * 480;
	axp_reads(supply->master, AXP_CCHAR3_RES, 8, temp);
	Value1 = ((temp[0] << 24) + (temp[1] << 16) + (temp[2] << 8) + temp[3]);
	Value2 = ((temp[4] << 24) + (temp[5] << 16) + (temp[6] << 8) + temp[7]);
...
static void axp_read_adc(struct axp20_supply *supply, struct axp_adc_res *adc)
{
	uint8_t tmp[8];
...
static int axp20_update_status(struct aml_charger *charger)
{
	struct  axp20_supply *supply;
	uint8_t        val[2];
	uint16_t       tmp;
	static uint8_t usb_cur_limit_canceled;
	 ...
	 */
	if (charger->usb_valid && !axp_pmu_battery->pmu_usbcur_limit &&
	    !usb_cur_limit_canceled) {
		axp_set_bits(supply->master, AXP20_CHARGE_VBUS, 0x03);
...
static void axp_charger_update(struct axp20_supply *supply)
{
	struct aml_charger *charger = &supply->aml_charger;
	uint8_t v[2] = {};
...
static void axp_charging_monitor(struct work_struct *work)
{
	...
	if (!charger->ext_valid) {
		/* clear charge LED when extern power is removed */
	...
	if ((charger->rest_vol - pre_rest_cap) ||
		...
		if (in_early_suspend &&
		   (pre_chg_status != charger->ext_valid)) {
			...
			AXP_PMU_DBG("%s, %s in early suspend, wake up now\n",
				    __func__,
...
static void axp_earlysuspend(struct early_suspend *h)
{
	...
	axp_set_charge_current(axp_pmu_battery->pmu_suspend_chgcur);
#endif
	if (axp_pmu_battery)
...
#if defined(CONFIG_AXP_CHGCHANGE)
	axp_set_charge_current(axp_pmu_battery->pmu_resume_chgcur);
#endif
	if (axp_pmu_battery) {
		early_power_status = supply->aml_charger.ext_valid;
...
static int axp_battery_probe(struct platform_device *pdev)
{
	...
	b->technology	      = axp_pmu_battery->pmu_battery_technology;
	b->voltage_max_design = axp_pmu_battery->pmu_init_chgvol;
	b->energy_full_design = axp_pmu_battery->pmu_battery_cap;
	b->voltage_min_design = charger->ocv_empty * 1000;
	b->use_for_apm	      = 1;
	b->name		      = axp_pmu_battery->pmu_battery_name;
	...
	axp_write(supply->master, AXP20_APS_WARNING1, 0x7A);
	...
	schedule_delayed_work(&supply->work, supply->interval);
#ifdef CONFIG_AMLOGIC_USB
	if (axp202_charger_job.flag) {
		/* do later job for usb charger detect */
	...
	if (axp_pmu_battery->pmu_ntc_enable) {
		...
		axp_write(supply->master, TS_CURRENT_REG, val);
		...
		axp_write(supply->master, POWER20_VLTF_CHGSET, val);
		...
		axp_write(supply->master, POWER20_VHTF_CHGSET, val);
	}
...
static int axp20_suspend(struct platform_device *dev, pm_message_t state)
{
	...
	if (early_power_status != supply->aml_charger.ext_valid) {
		AXP_PMU_DBG("%s, %s, prev:%x, now:%x, exit suspend process\n",
			    __func__,
...
static void axp20_shutdown(struct platform_device *dev)
{
	...
	axp_set_charge_current(axp_pmu_battery->pmu_shutdown_chgcur);
}
