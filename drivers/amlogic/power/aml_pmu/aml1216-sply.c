/*
 * drivers/amlogic/power/aml_pmu/aml1216-sply.c
 *
...
static int aml1216_set_full_charge_voltage(int voltage);
...
#endif
		if (ocv_voltage > 3950) {
			pr_info("%s, otp_version:%d, ocv:%d, %s.\n",
...
static void aml1216_otg_work_fun(struct work_struct *work)
{
	...
	AML1216_INFO("%s, OTG value:%d, is_short:%d\n", __func__,
		     aml1216_otg_value,
		     g_aml1216_init->vbus_dcin_short_connect);
...
int aml1216_otg_change(struct notifier_block *nb, unsigned long value, void *p)
{
	if (!g_aml1216_supply) {
		AML1216_INFO("%s, driver isn't ready, do it later\n", __func__);
...
int aml1216_usb_charger(struct notifier_block *nb, unsigned long value, void *p)
{
	int curr;
	if (!g_aml1216_supply) {
		AML1216_INFO("%s, driver isn't ready, do it later\n", __func__);
	...
	usb_bc_mode = value;
	curr = aml1216_battery->pmu_usbcur;
	switch (value) {
	case USB_BC_MODE_SDP:   /* pc */
		...
		if (aml1216_battery && aml1216_battery->pmu_usbcur_limit) {
			/* limit usb current */
			aml1216_set_usb_current_limit(curr);
		}
		break;
	...
	case USB_BC_MODE_CDP:	/* PC + charger */
		if (aml1216_battery) {
			...
			aml1216_set_usb_current_limit(-1);
		}
		break;
...
static ssize_t pmu_reg16_show(struct device *dev, struct device_attribute *attr,
			      char *buf)
{
	return printf_usage();
}
...
{
	return sprintf(buf,
		       "AML PMU AML1216 driver version is %s, build time:%s\n",
...
int aml1216_dump_all_register(char *buf)
{
	...
	if (!buf) {
		pr_info("[AML1216] DUMP ALL REGISTERS:\n");
		for (i = 0; i < 24; i++) {
			aml1216_reads(i*16, val, 16);
			pr_info("0x%03x - %03x: ", i * 16, i * 16 + 15);
				  ...
				  struct device_attribute *attr, char *buf)
{
	...
	size += sprintf(buf + size, "%s",
			"[AML1216] DUMP ALL REGISTERS OVER!\n");
	return size;
}
			     ...
			     struct device_attribute *attr, char *buf)
{
	...
	supply = container_of(battery, struct aml1216_supply, batt);
	api = aml_pmu_get_api();
...
int aml1216_cal_ocv(int ibat, int vbat, int dir)
{
	...
	rdc = aml1216_battery->pmu_battery_rdc;
	if (dir == CHARGER_CHARGING && aml1216_battery) {
		/* charging */
		result = vbat - (ibat * rdc) / 1000;
	} else if (dir == CHARGER_DISCHARGING && aml1216_battery) {
		/* discharging */
		result = vbat + (ibat * rdc) / 1000;
	} else
		result = vbat;
...
static int aml1216_update_state(struct aml_charger *c)
{
	uint8_t val;
	int  vsys_voltage;
	...
	c->ibat = aml1216_get_battery_current();
	if (val & 0x18) {
		if (c->ibat >= 20)
	...
	c->vbat = aml1216_get_battery_voltage();
	c->ocv  = aml1216_cal_ocv(c->ibat, c->vbat, c->charge_status);

	ocv_voltage = c->ocv;
	if (val & 0x40) {
		AML1216_INFO("%s, charge timeout, val:0x%02x, reset charger\n",
			     __func__, val);
		aml1216_set_charge_enable(0);
		msleep(1000);
		aml1216_set_charge_enable(1);
	...
	if (aml1216_get_otp_version() == 0) {
		if (((vsys_voltage > c->vbat) &&
		     (vsys_voltage - c->vbat < 500)) ||
		     (c->vbat > 3950)) {
			pr_info("%s, %s, vsys:%d, vbat:%d\n",
				 __func__,
				 "vsys is not large, or vbat is too large",
				 vsys_voltage, c->vbat);
			aml1216_set_charge_enable(0);
		} else
			aml1216_set_charge_enable(1);
...
static void aml1216_charging_monitor(struct work_struct *work)
{
	...
	if (charger->rest_vol <= 0 &&
		...
		if (over_discharge_cnt >= 5) {
			AML1216_ERR("%s, %s, force system power off\n",
				    __func__,
	...
	if ((charger->rest_vol - pre_rest_cap) ||
		...
		if (in_early_suspend &&
		   (pre_pwr_status != charger->ext_valid)) {
			...
			AML_PMU_INFO("%s, %s early suspend, wake up now\n",
				     __func__,
				     "usb power status changed in ");
			/* assume power key pressed */
			input_report_key(aml1216_power_key, KEY_POWER, 1);
...
static void aml1216_earlysuspend(struct early_suspend *h)
{
	...
	int cur;
	if (aml1216_battery) {
		cur = aml1216_battery->pmu_suspend_chgcur;
		aml1216_set_charging_current(cur);
	}
...
static void aml1216_lateresume(struct early_suspend *h)
{
	...
	if (aml1216_battery) {
		cur = aml1216_battery->pmu_resume_chgcur;
		aml1216_set_charging_current(cur);
		input_report_key(aml1216_power_key, KEY_POWER, 0);
		input_sync(aml1216_power_key);
	}
	early_power_status = supply->aml_charger.ext_valid;
	in_early_suspend = 0;
...
static void aml1216_irq_work_func(struct work_struct *work)
{
	...
	/* TODO: add code here */
	supply = container_of(work, struct aml1216_supply, irq_work);
...
static int aml1216_suspend(struct platform_device *dev, pm_message_t state)
{
	...
	if (early_power_status != supply->aml_charger.ext_valid) {
		AML1216_DBG("%s, power status changed, prev:%x, now:%x\n",
