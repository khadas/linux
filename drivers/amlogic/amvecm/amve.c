...
/*vlock is support eq_after gxbb,but which is usefull only for tv chip
...
{
	unsigned int i;
	vlock_log = kmalloc(vlock_log_size * sizeof(struct vlock_log_s *),
		GFP_KERNEL);
	if (vlock_log == NULL) {
		pr_info("%s:vlock_log kmalloc fail!!!\n", __func__);
		return;
	}
