...
static u32 dbg_nal_skip_flag =
	0;	/* bit[0], skip vps; bit[1], skip sps; bit[2], skip pps */
...
buf[0]: 1, when stop, release cma buffer if blackout is 1;
...
static void init_buf_list(struct hevc_state_s *hevc)
{
	...
	if (buf_alloc_size > 0) {
		...
		if (debug) {
			pr_info
			("init_buf_list (width %d height %d):\n",
	...
	for (i = 0; i < max_buf_num; i++) {
		...
		if (debug) {
			pr_info("Buffer %d: start_adr %lx size %lx\n", i,
...
static int config_pic(struct hevc_state_s *hevc, struct PIC_s *pic)
{
	...
	/* int MV_MEM_UNIT=lcu_size_log2==6 ? 0x100 :
	   lcu_size_log2==5 ? 0x40 : 0x10; */
	int MV_MEM_UNIT =
		lcu_size_log2 == 6 ? 0x200 : lcu_size_log2 == 5 ? 0x80 : 0x20;
	int mc_buffer_size_u_v = lcu_total * lcu_size * lcu_size / 2;
	/* 64k alignment */
	int mc_buffer_size_u_v_h = (mc_buffer_size_u_v + 0xffff) >> 16;
	int mpred_mv_end =
	...
	if ((hevc->work_space_buf->mpred_mv.buf_start +
		 (((pic->index + 1) * lcu_total) * MV_MEM_UNIT))
			<= mpred_mv_end) {
		for (i = 0; i < hevc->buf_num; i++) {
			y_adr = ((m_BUF[i].free_start_adr + 0xffff)
					>> 16) << 16;	/* 64k alignment */
			if ((y_adr + ((mc_buffer_size_u_v_h << 16) * 3)) <=
				(m_BUF[i].start_adr + m_BUF[i].size)) {
				m_BUF[i].free_start_adr =
					y_adr + ((mc_buffer_size_u_v_h
								<< 16) * 3);
				break;
			}
		}
		if (i < hevc->buf_num) {
			...
			pic->mc_y_adr = y_adr;
			pic->mc_u_v_adr =
				y_adr + ((mc_buffer_size_u_v_h << 16) << 1);
			...
			if (debug) {
				...
				pr_info
				("mc_u_v_adr %lx mpred_mv_wr_start_adr %lx\n",
		...
		for (i = 0; i < MAX_REF_PIC_NUM; i++) {
			if (m_PIC[i].alloc_pages != NULL
				...
				dma_release_from_contiguous(cma_dev,
						...
						cma_page_count);
				pr_info("release cma buffer[%d] (%d %x)\n", i,
					   m_PIC[i].cma_page_count,
					   (unsigned)m_PIC[i].alloc_pages);
...
static void init_pic_list(struct hevc_state_s *hevc)
{
	...
	if (debug) {
		pr_info("[Buffer Management] init_pic_list (%d %d):\n",
			   pic_width, pic_height);
	}
	/* WRITE_VREG(HEVCD_MPP_ANC2AXI_TBL_CONF_ADDR, 0x0); */

	for (i = 0; i < MAX_REF_PIC_NUM; i++) {
		if (((i + 1) * (mc_buffer_size_u_v_h << 16) * 3) >
			hevc->mc_buf->buf_size) {
			if (use_cma)
				hevc->use_cma_flag = 1;
			else {
				if (debug) {
		...
		memset(&(m_PIC[i]), 0, sizeof(struct PIC_s));
		m_PIC[i].index = i;
		m_PIC[i].mpred_mv_wr_start_addr =
			hevc->work_space_buf->mpred_mv.buf_start +
			((i * lcu_total) * MV_MEM_UNIT);

		if (hevc->use_cma_flag) {
			if ((m_PIC[i].cma_page_count != 0)
				&& (m_PIC[i].alloc_pages != NULL)
				&& (m_PIC[i].cma_page_count !=
					PAGE_ALIGN((mc_buffer_size_u_v_h <<
							16) * 3) /
					PAGE_SIZE)) {
				dma_release_from_contiguous(cma_dev,
						m_PIC[i].
			...
			if (m_PIC[i].alloc_pages == NULL) {
				m_PIC[i].cma_page_count =
					PAGE_ALIGN((mc_buffer_size_u_v_h << 16)
							* 3) / PAGE_SIZE;
				m_PIC[i].alloc_pages =
					dma_alloc_from_contiguous(cma_dev,
							m_PIC[i].
				...
				m_PIC[i].mc_y_adr =
					page_to_phys(m_PIC[i].alloc_pages);
				m_PIC[i].mc_u_v_adr =
					m_PIC[i].mc_y_adr +
					((mc_buffer_size_u_v_h << 16) << 1);
				pr_info("allocate cma buffer[%d] (%d,%x,%x)\n",
					   i, m_PIC[i].cma_page_count,
					   (unsigned)m_PIC[i].alloc_pages,
					   (unsigned)m_PIC[i].mc_y_adr);
			} else {
				pr_info("reuse cma buffer[%d] (%d,%x,%x)\n", i,
					   m_PIC[i].cma_page_count,
					   (unsigned)m_PIC[i].alloc_pages,
			...
			m_PIC[i].alloc_pages = NULL;
			m_PIC[i].mc_y_adr =
				hevc->mc_buf->buf_start +
				i * (mc_buffer_size_u_v_h << 16) * 3;
			m_PIC[i].mc_u_v_adr =
				m_PIC[i].mc_y_adr +
				((mc_buffer_size_u_v_h << 16) << 1);
		}
		...
		if ((((m_PIC[i].mc_y_adr + ((mc_buffer_size_u_v_h << 16) * 3)) >
			  mc_buffer_end) && (m_PIC[i].alloc_pages == NULL))
			||
			((m_PIC[i].mpred_mv_wr_start_addr +
			  (lcu_total * MV_MEM_UNIT)) > mpred_mv_end)) {
			if (debug) {
				pr_info
				("Max mc buffer or mpred_mv buffer is used\n");
			}
		...
		/* WRITE_VREG(HEVCD_MPP_ANC2AXI_TBL_CMD_ADDR,
		   m_PIC[i].mc_y_adr|(m_PIC[i].mc_canvas_y<<8)|0x1); */
		/* WRITE_VREG(HEVCD_MPP_ANC2AXI_TBL_CMD_ADDR,
		   m_PIC[i].mc_u_v_adr|(m_PIC[i].mc_canvas_u_v<<8)|0x1); */

		if (debug & H265_DEBUG_BUFMGR) {
			pr_info
			("Buffer %d: canv_y %x  canv_u_v %x mc_y_adr %lx ",
			 i, m_PIC[i].mc_canvas_y,
			 m_PIC[i].mc_canvas_u_v, m_PIC[i].mc_y_adr);
			 pr_info("mc_u_v_adr %lx mpred_mv_wr_start_addr %lx\n",
			 m_PIC[i].mc_u_v_adr,
			 m_PIC[i].mpred_mv_wr_start_addr);
		}
	}
...
static void dump_pic_list(struct hevc_state_s *hevc)
{
	...
	for (i = 0; i < MAX_REF_PIC_NUM; i++) {
		...
		pr_info("output_ready:%d, mv_wr_start %lx\n",
...
static void set_canvas(struct PIC_s *pic)
{
	...
	canvas_config(pic->y_canvas_index, pic->mc_y_adr, ALIGN(pic->width, 64),
				  ALIGN(pic->height, 32), CANVAS_ADDR_NOWRAP,
				  CANVAS_BLKMODE_64X32);
	canvas_config(pic->uv_canvas_index, pic->mc_u_v_adr,
				  ALIGN(pic->width, 64),
				  ALIGN(pic->height >> 1, 32),
