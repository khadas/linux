...
/*      apb_write_reg(DEMOD_CFG_BASE,0x00000007);*/
	...
	/* reset*/
	/*dvbc_reset();*/
	apb_write_reg(QAM_BASE + 0x4,
	apb_read_reg(QAM_BASE + 0x4) & ~(1 << 4));
	/* disable fsm_en*/
	apb_write_reg(QAM_BASE + 0x4,
	apb_read_reg(QAM_BASE + 0x4) & ~(1 << 0));
	/* Sw disable demod*/
	apb_write_reg(QAM_BASE + 0x4,
	apb_read_reg(QAM_BASE + 0x4) | (1 << 0));
	/* Sw enable demod*/
	...
	/* max_frq_off = (400KHz * 2^29) / (AD=28571 * symbol_rate=6875)*/
