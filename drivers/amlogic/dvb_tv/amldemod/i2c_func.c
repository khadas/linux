...
static void sw_i2c_setscl(struct aml_demod_i2c *adap, unsigned val)
{
	...
	*(u32 *)(adap->scl_oe) = oe;
	...
	scl = *(u32 *)(adap->scl_out);
	if (val)
		scl |= (1 << (adap->scl_bit));
	else
		scl &= ~(1 << (adap->scl_bit));
	*(u32 *)(adap->scl_out) = scl;
}
...
static void sw_i2c_setsda(struct aml_demod_i2c *adap, unsigned val)
{
	...
	*(u32 *)(adap->sda_oe) = oe;
	...
	sda = *(u32 *)(adap->sda_out);
	if (val)
		sda |= (1 << (adap->sda_bit));
	else
		sda &= ~(1 << (adap->sda_bit));
	*(u32 *)(adap->sda_out) = sda;
}
...
static int aml_i2c_sw_test_bus(struct aml_demod_i2c *adap, char *name)
{
	...
	scl = sw_i2c_getscl(adap);
	if (sda) {
		pr_dbg("%s: SDA stuck high!\n", name);
		goto bailout;
	}
	if (!scl) {
		pr_dbg("%s: SCL unexpected low while pull SDA low!\n", name);
		goto bailout;
	}
	...
	scl = sw_i2c_getscl(adap);
	if (!sda) {
		pr_dbg("%s: SDA stuck low!\n", name);
		goto bailout;
	}
	if (!scl) {
		pr_dbg("%s: SCL unexpected low while pulling SDA high!\n",
		       name);
		goto bailout;
	}
	...
	scl = sw_i2c_getscl(adap);
	if (scl) {
		pr_dbg("%s: SCL stuck high!\n", name);
		goto bailout;
	}
	if (!sda) {
		pr_dbg("%s: SDA unexpected low while pull SCL low!\n", name);
		goto bailout;
	}
	...
	scl = sw_i2c_getscl(adap);
	if (!scl) {
		pr_dbg("%s: SCL stuck low!\n", name);
		goto bailout;
	}
	if (!sda) {
		pr_dbg("%s: SDA unexpected low while pulling SCL high!\n",
		       name);
		goto bailout;
	}
	if (adap->debug)
		       ...
		       unsigned char addr, int retries)
{
	...
	for (i = 0; i <= retries; i++) {
		...
		if (ret == 1 || i == retries)
			break;
		if (adap->debug)
			pr_dbg("emitting stop condition\n");
		i2c_stop(adap);
		udelay(adap->udelay);
		if (adap->debug)
			pr_dbg("emitting start condition\n");
		i2c_start(adap);
	}
	if (i && ret && adap->debug)
		pr_dbg("Used %d tries to %s client at 0x%02x: %s\n",
...
static int sendbytes(struct aml_demod_i2c *adap, struct i2c_msg *msg)
{
	...
	while (count > 0) {
		...
		if ((retval > 0) || (nak_ok && (retval == 0))) {
			...
			pr_dbg("Error: sendbytes: NAK bailout.\n");
			return -EIO;
		} else {
			pr_dbg("Error: sendbytes: error %d\n", retval);
			return retval;
		}
	}
	return wrcount;
...
static int bit_doAddress(struct aml_demod_i2c *adap, struct i2c_msg *msg)
{
	...
	if (flags & I2C_M_TEN) {
		...
		if ((ret != 1) && !nak_ok) {
			pr_dbg("Error: died at extended address code\n");
			return -EREMOTEIO;
		}
		...
		if ((ret != 1) && !nak_ok) {
			...
			pr_dbg("Error: died at 2nd address code\n");
			return -EREMOTEIO;
		}
		if (flags & I2C_M_RD) {
			if (adap->debug)
				pr_dbg("emitting repeated start condition\n");
			i2c_start(adap);
			...
			if ((ret != 1) && !nak_ok) {
				pr_dbg
				    ("Error: died at repeated address code\n");
				return -EREMOTEIO;
			}
		}
...
static int aml_i2c_sw_bit_xfer(struct aml_demod_i2c *adap,
			       struct i2c_msg msgs[], int num)
{
	...
	for (i = 0; i < num; i++) {
		...
		if (!(pmsg->flags & I2C_M_NOSTART)) {
			...
			if ((ret != 0) && !nak_ok) {
				pr_dbg("NAK from device addr 0x%02x msg #%d\n",
				       msgs[i].addr, i);
				goto bailout;
			}
		}
		      ...
		      int num)
{
	...
	/*pr_dbg("adap->scl_oe[%x], adap->i2c_priv[%p], adap->i2c_id[%x]\n",
	...
	if (adap->scl_oe) {
		...
		if (adap->i2c_priv) {
			/*pr_dbg("msgs[%p], num[%x]\n", msgs, num);*/
			ret =
			    i2c_transfer((struct i2c_adapter *)adap->i2c_priv,
					 msgs, num);
		} else {
			pr_dbg("i2c error, no valid i2c\n");
		}
	}
	return ret;
}
