...
#include "pinctrl-m8.h"
#include <linux/of_address.h>
#include <linux/amlogic/gpio-amlogic.h>
#include "pinconf-amlogic.h"
DEFINE_MUTEX(spi_nand_mutex);
void __iomem *p_pull_up_addr[6];
void __iomem *p_pin_mux_reg_addr[11];
void __iomem *p_pull_upen_addr[6];
...
static int  m8_pmx_probe(struct platform_device *pdev)
{
	...
	p = of_iomap(np, 0);
	for (i = 0; i < 10; i++) {
		p_pin_mux_reg_addr[i] = p;
		p = p+4;
	}
	p_pin_mux_reg_addr[10] = of_iomap(np, 1);
	p = of_iomap(np, 2);
	for (i = 0; i < 5; i++) {
		p_pull_upen_addr[i] = p;
		p = p+4;
	}
	p = of_iomap(np, 3);
	for (i = 0; i < 5; i++) {
	...
	p = of_iomap(np, 4);
	p_pull_upen_addr[5] = p;
	p_pull_up_addr[5] = p;
	for (i = 0; i < 11; i++)
		pr_debug("p_pin_mux_reg_addr[%d]=%p\n", i, p_pin_mux_reg_addr[i]);
	for (i = 0; i < 6; i++) {
		pr_debug("p_pull_upen_addr[%d]=%p\n", i, p_pull_upen_addr[i]);
		pr_debug("p_pull_up_addr[%d]=%p\n", i, p_pull_up_addr[i]);
	}
