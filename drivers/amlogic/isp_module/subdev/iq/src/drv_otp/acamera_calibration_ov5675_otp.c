...
#define ACAMERA_OTP_TOTAL_SIZE (4374)
//#define ACAMERA_OTP_DUMP
...
#define ACAMERA_OTP_WB_START 2048
#define ACAMERA_OTP_WB_SIZE  18
#define ACAMERA_OTP_WB_CHECKSUM 2066

#define ACAMERA_OTP_LSC_3000_R_START 3605
#define ACAMERA_OTP_LSC_3000_G_START 3861
#define ACAMERA_OTP_LSC_3000_B_START 4117
#define ACAMERA_OTP_LSC_3000_CHECKSUM 4373


#define ACAMERA_OTP_LSC_4000_R_START 2836
#define ACAMERA_OTP_LSC_4000_G_START 3092
#define ACAMERA_OTP_LSC_4000_B_START 3348
#define ACAMERA_OTP_LSC_4000_CHECKSUM 3604


#define ACAMERA_OTP_LSC_5000_R_START 2067
#define ACAMERA_OTP_LSC_5000_G_START 2323
#define ACAMERA_OTP_LSC_5000_B_START 2579
#define ACAMERA_OTP_LSC_5000_CHECKSUM 2835
...
#define AMZ_USE_LSC_COEFFICIENT_TRANSFORM 0
#define AMZ_USE_LSC_OTP_RATIOS            1
#define AMZ_USE_AWB_EXTENDED_CALIBRATION  1
#define AMZ_ACAMERA_OTP_NUM_AWB_LIGHT_CAL 3

#define ACAMERA_MAX_INT16  (65535)
#define SENSOR_CHIP_ID 0x1
...
static uint16_t sensor_get_id( acamera_sbus_t *p_sbus )
{
    ...
    sensor_id = acamera_sbus_read_u8(p_sbus, 0x0020);

    LOG(LOG_CRIT, "%s: OV5675 \n", __func__);

    if (sensor_id != SENSOR_CHIP_ID) {
        LOG(LOG_CRIT, "%s: Failed to read sensor id:0x%x\n", __func__,sensor_id);
        return 0xFF;
    }
    LOG(LOG_CRIT, "%s: OV5675 success \n", __func__);
    return 0;
}
    ...
    if ( p_sbus != NULL ) {
        if ( (otp_start + otp_size - 1) < ACAMERA_OTP_TOTAL_SIZE ) {
            if ( buffer != NULL && buffer_size >= otp_size ) {
                ...
                for ( offset = otp_start ; offset < otp_size; offset ++ ) {
                    uint8_t data = acamera_sbus_read_u8( p_sbus,  offset ) ;
...
static int32_t sensor_otp_validate( uint8_t* p_otp, uint32_t size, uint16_t checksum_target) {
    ...
    // only lower one byte are counted
    checksum %= 0xFF;
...
#if AMZ_USE_LSC_COEFFICIENT_TRANSFORM
#if AMZ_USE_LSC_OTP_RATIOS
#include "amz_lsc_tgd.inc"
#endif
#else
...
static int32_t acamera_lsc_transform( uint8_t* otp_buf, uint32_t otp_length, uint8_t* lsc_buf, uint32_t lsc_length ) {
    ...
    #ifdef ACAMERA_OTP_DUMP
    char lsc_to_string[300]={'0'};
    memset((void *)lsc_to_string, 0, sizeof(lsc_to_string));
    for (i = 0; i < ACAMERA_ARM_LSC_DIM; i++) {

        for (j = 0; j < ACAMERA_ARM_LSC_DIM; j++) {
            sprintf(lsc_to_string + j*4,"%3d ",arm_lsc[i][j]);
        }
        printk(KERN_ERR"%s \n",lsc_to_string);
    }
    #endif
    ...
    for (i = 0; i < ACAMERA_ARM_LSC_DIM; i++) {
            ...
            if (arm_lsc[ACAMERA_ARM_LSC_DIM-1][i] < arm_lsc[ACAMERA_ARM_LSC_DIM-2][i]) arm_lsc[ACAMERA_ARM_LSC_DIM-1][i] = arm_lsc[ACAMERA_ARM_LSC_DIM-2][i];
            if (arm_lsc[i][ACAMERA_ARM_LSC_DIM-1] < arm_lsc[i][ACAMERA_ARM_LSC_DIM-2]) arm_lsc[i][ACAMERA_ARM_LSC_DIM-1] = arm_lsc[i][ACAMERA_ARM_LSC_DIM-2];
    }
    #ifdef ACAMERA_OTP_DUMP
    memset((void *)lsc_to_string, 0, sizeof(lsc_to_string));
    for (i = 0; i < ACAMERA_OTP_LSC_DIM-1; i++) {

        for (j = 0; j < ACAMERA_OTP_LSC_DIM-1; j++) {
            sprintf(lsc_to_string + j*4,"%3d ",otp_lsc[i][j]);
        }
        printk(KERN_ERR"%s \n",lsc_to_string);
    }
    memset((void *)lsc_to_string, 0, sizeof(lsc_to_string));
...
static int32_t acamera_wb_component_update( uint8_t* p_otp, uint32_t awb_start, uint32_t awb_size, uint16_t* wb_ptr, uint32_t wb_length) {
    int32_t result = 0 ;
    uint8_t checksum = p_otp[ACAMERA_OTP_WB_CHECKSUM];
    // validate only data before high byte of the checksum.
    result = sensor_otp_validate(p_otp + ACAMERA_OTP_WB_START, ACAMERA_OTP_WB_SIZE, checksum);
    if (result != 0) {
        LOG(LOG_CRIT, "WB otp checksum failed");
        return result;
    }
    if ( p_otp != NULL && awb_size != 0 && wb_ptr != NULL && wb_length != 0 ) {
        ...
        #ifdef ACAMERA_OTP_DUMP
        LOG(LOG_CRIT, "OWB DEBUG: output WB  R/G:0x%x 0x%x 0x%x B/G:0x%x ", wb_ptr[0], wb_ptr[1], wb_ptr[2], wb_ptr[3]);
        #endif
    } else {
        result = -1;
...
#if AMZ_USE_AWB_EXTENDED_CALIBRATION
    // Warning:  using this option requires that the corresponding
    //  c->calibrations[CALIBRATION_STATIC_WB]->ptr  (== wp_ptr)
    // contains the three extra rows for the quartets of rggb measurements stored in the eeprom

    if ((wb_length/sizeof(wb_ptr[0])) < 16) {
        LOG(LOG_CRIT, "OWB DEBUG: AMZ: length=%d (< 16). Skipping wb initialization", wb_length);
        return result;
    }

    if ((wb_length/sizeof(wb_ptr[0])) >= 16 && result != -1) {
    // The eeprom data is also required by the custom AWB, no data conversion is required
    // for custom AWB algorithm, simply store average values from eeprom.
        int i,awb_offset = awb_start;
        for (i = 0; i < AMZ_ACAMERA_OTP_NUM_AWB_LIGHT_CAL; i++) {

            // At wb_ptr[0][]: Amlogic data is stored
            wb_ptr[(i+1)*4 + 0] = ((uint16_t)p_otp[awb_offset+1] << 8) | (p_otp[awb_offset+0]); // for R
            wb_ptr[(i+1)*4 + 1] = ((uint16_t)p_otp[awb_offset+3] << 8) | (p_otp[awb_offset+2]); // for Gr
            wb_ptr[(i+1)*4 + 2] = wb_ptr[(i+1)*4 + 1]; // Gb is same as Gr
            wb_ptr[(i+1)*4 + 3] = ((uint16_t)p_otp[awb_offset+5] << 8) | (p_otp[awb_offset+4]); // for B
            awb_offset += 6; // RGB values Stored as 2 bytes
        }
        LOG(LOG_DEBUG, "OWB DEBUG: AMZ: %d,%d  %d,%d  %d,%d",
            wb_ptr[(0+1)*4 + 0], wb_ptr[(0+1)*4 + 3],
            wb_ptr[(1+1)*4 + 0], wb_ptr[(1+1)*4 + 3],
            wb_ptr[(2+1)*4 + 0], wb_ptr[(2+1)*4 + 3]);
    }
#endif
 ...
 *   @return 0 on success, otherwise return -1
 */
int32_t acamera_calibration_ov5675_otp( ACameraCalibrations *c ) {
    int32_t result = 0 ;
    ...
    if ( result == 0 ) {
        ...
        if ( result == 0 ) {
            ...
            uint8_t checksum = otp_memory[ACAMERA_OTP_WB_CHECKSUM];
               int ret = sensor_otp_validate(otp_memory + ACAMERA_OTP_WB_START, ACAMERA_OTP_WB_SIZE, checksum);
            if (ret != 0) {
                LOG(LOG_CRIT, "WB otp checksum failed");
            } else {
...
#if AMZ_USE_LSC_COEFFICIENT_TRANSFORM
#if AMZ_USE_LSC_OTP_RATIOS
            result |= amz_lsc_tgd(otp_memory, c);
            LOG(LOG_CRIT, "amz_lsc_tgd: result=%d  CALL", result);
#else
            LOG(LOG_CRIT, "amz_lsc_tgd: result=%d  NOCALL", result);
#endif
#else
            checksum = otp_memory[ACAMERA_OTP_LSC_3000_CHECKSUM];
            ret = sensor_otp_validate(otp_memory + ACAMERA_OTP_LSC_3000_R_START, ACAMERA_OTP_LSC_SIZE*3, checksum);
            if (ret != 0) {
                result |= 1;
                LOG(LOG_CRIT, "LSC 3000 otp checksum failed");
            } else {
                // read LSC table for 3000k
                #ifdef ACAMERA_OTP_DUMP
                LOG(LOG_CRIT, "LSC 3000 otp:");
                LOG(LOG_CRIT, "LSC 3000 otp R:");
                #endif
            ...
            checksum = otp_memory[ACAMERA_OTP_LSC_4000_CHECKSUM];
            ret = sensor_otp_validate(otp_memory + ACAMERA_OTP_LSC_4000_R_START, ACAMERA_OTP_LSC_SIZE*3, checksum);
            if (ret != 0) {
                result |= 1;
                LOG(LOG_CRIT, "LSC 4000 otp checksum failed");
            } else {
                // read LSC table for 4000k
                #ifdef ACAMERA_OTP_DUMP
                LOG(LOG_CRIT, "LSC 4000 otp:");
                LOG(LOG_CRIT, "LSC 4000 otp R:");
                #endif
            ...
            if (ret != 0) {
                result |= 1;
                LOG(LOG_CRIT, "LSC 5000 otp checksum failed");
            } else {
                // read LSC table for 5000k
                #ifdef ACAMERA_OTP_DUMP
                LOG(LOG_CRIT, "LSC 5000 otp:");
                LOG(LOG_CRIT, "LSC 5000 otp R:");
                #endif
