...
static uint32_t _calibration_iridix_ev_lim_no_str[] = {5500000, 2500000}; //3574729

static uint8_t _calibration_ae_correction[] = {135, 135, 135, 135, 135, 128, 100, 70, 70, 70, 70, 70};

static uint32_t _calibration_ae_exposure_correction[] = {5375, 15739, 30778, 60282, 120186, 240325, 534334, 1396501, 2906388, 5683332, 9991231, 15360142}; //500,157778,500325,632161,1406400,6046465 //23282 - Max Lab Exposure
...
// these values track the LSC gains. (0=1x 64=3x)
static uint8_t _calibration_sinter_radial_lut[] = {0,0,0,1,2,3,4,5,7,9,11,13,16,19,23,27,31,35,39,43,47,51,54,57,59,61,62,63,64,64,64,64};

static uint16_t _calibration_sinter_radial_params[] = {
    1,    // rm_enable
    2592/2, // rm_centre_x
    1944/2, // rm_centre_y
    818   // rm_off_centre_mult: round((2^31)/((rm_centre_x^2)+(rm_centre_y^2)))
};
...
static uint32_t _calibration_iridix8_strength_dk_enh_control[] = {
    25,      // dark_prc
    99,      // bright_prc
    320,     // min_dk: minimum dark enhancement
    330,    // max_dk: maximum dark enhancement
    ...
    35,       // min_str: iridix strength in percentage //26
    65,      // max_str: iridix strength in percentage: 50 = 1x gain. 100 = 2x gain                //50
...
static uint16_t _calibration_ae_control_HDR_target[][2] = {
    {0 * 256, 135}, // HDR AE target should not be higher than LDR target
    {1 * 256, 135},
    {2 * 256, 110},
    {3 * 256, 100},
    {4 * 256, 90},
...
static uint16_t _calibration_ae_zone_wght_hor[] = {1,1,2,2,3,4,5,6,7,9,10,12,13,14,15,16,16,16,16,15,14,13,12,10,9,7,6,5,4,3,2,2,1,1};
static uint16_t _calibration_ae_zone_wght_ver[] = {10,11,11,12,12,13,13,14,14,15,15,15,15,16,16,16,16,16,16,16,16,15,15,15,15,14,14,13,13,12,12,11,11,10};
...
// 60% eh / 40% sRGB
#define GAMMA_NORMAL  \
0,233,412,554,679,791,895,991,1081,1167,1248,1325,1399,1469,1536,1601,1663,1722,1780,1835,1888,1940,1989,2037,2084,2129,2172,2215,2256,2296,2334,2372,2409,2444,2479,2513,2546,2578,2610,2640,2670,2700,2728,2756,2784,2811,2837,2863,2888,2913,2937,2961,2984,3007,3030,3052,3074,3095,3116,3137,3157,3177,3197,3216,3235,3254,3273,3291,3309,3327,3345,3362,3379,3396,3412,3429,3445,3461,3477,3492,3508,3523,3538,3553,3568,3583,3597,3611,3625,3639,3653,3667,3680,3694,3707,3720,3733,3746,3759,3771,3784,3796,3809,3821,3833,3845,3857,3868,3880,3892,3903,3914,3926,3937,3948,3959,3970,3981,3992,4002,4013,4023,4034,4044,4055,4065,4075,4085,4095

// 40% eh / 60% sRGB
#define GAMMA_LOW  \
0,271,454,596,717,825,924,1015,1099,1179,1255,1326,1394,1459,1522,1581,1639,1694,1747,1799,1848,1896,1942,1987,2031,2073,2114,2154,2193,2231,2268,2304,2339,2373,2407,2439,2471,2502,2533,2563,2592,2621,2649,2677,2704,2730,2756,2782,2807,2832,2856,2880,2904,2927,2950,2972,2994,3016,3038,3059,3080,3100,3121,3141,3161,3180,3200,3219,3237,3256,3274,3293,3311,3328,3346,3363,3381,3398,3414,3431,3448,3464,3480,3496,3512,3528,3543,3558,3574,3589,3604,3619,3633,3648,3662,3677,3691,3705,3719,3733,3747,3760,3774,3787,3801,3814,3827,3840,3853,3866,3879,3892,3904,3917,3929,3942,3954,3966,3978,3990,4002,4014,4026,4037,4049,4061,4072,4084,4095
...
static uint32_t _calibration_custom_settings_context[][4] = {
    ...
    //mesh shading
    { 0x1abfc, 0x1f1fe449L, 0xffffffff,4},//shading mesh scale 0x1f1fe445L, 0x1f1fe449L
    //Demosaic
    { 0x1ae7c, 0xa0a08eb4L, 0xffffffff,4 }, // UU Slope/VA Slope/AA Slope/VH Slope
    ...
    { 0x1aeb4, 0x1fa4L, 0x1fff,2 }, //min_d_strength
    { 0x1aeb8, 0x1fa4L, 0x1fff,2 }, //min_ud_strength
    { 0x1aecc, 0x1900190L, 0x1fff1fff,4 }, //max_ud_strength/max_d_strength
    //fr sharpen
    { 0x1c090, 0x3200320L, 0x3fff3fff,4 }, //Clip Str Min [13:0]/Clip Str Max [13:0]
    //CNR
    { 0x1b0d0, 0x1L, 0x1,1 }, //square_root_enable
    { 0x1b0f0, 0x3e8L, 0xfff,2 },// global_offset
    { 0x1b0f4, 0xafc8L, 0x0,2 }, //global_slope
    { 0x1b0fc, 0x320L, 0xfff,2 }, //uv_seg1_offset
    { 0x1b100, 0xfa00L, 0x0,2 }, //uv_seg1_slope
    { 0x1b138, 0xc8L, 0xfff,2 }, //uv_var1_offset
    { 0x1b13c, 0x3e80L, 0x0,2 }, //uv_var1_slope
    { 0x1b144, 0xc8L, 0xfff,2 }, //uv_var2_offset
    { 0x1b148, 0x3e80L, 0x0,2 }, //uv_var2_slope
    //sinter noise pofile
...
static uint32_t _calibration_demosaic_rgb_extension_control[][13] = {
    {0*256, 150, 16, 160, 150, 30, 70, 7000, 7000, 4000, 100, 8000, 8000},
    {1*256, 150, 16, 162, 175, 50, 100, 7000, 7000, 4000, 100, 8000, 10000},
    {2*256, 150, 16, 166, 200, 50, 170, 7000, 7000, 4000, 100, 8000, 10000},
    {3*256, 150, 16, 172, 225, 100, 170, 7000, 7000, 4000, 80, 8000, 11000},
    {4*256, 150, 32, 172, 250, 100, 170, 7000, 5000, 4000, 70, 8000, 12000},
    {5*256, 150, 32, 172, 250, 100, 170, 7000, 6000, 4000, 50, 8000, 12000},
    {6*256, 150, 32, 172, 250, 100, 170, 7000, 6000, 4000, 50, 8000, 12000},
    {7*256, 150, 32, 172, 250, 100, 170, 7000, 6000, 4000, 40, 8000, 12000},
    {8*256, 150, 32, 172, 250, 100, 170, 7000, 6000, 4000, 40, 8000, 12000},
};
...
static uint32_t _calibration_fr_sharpen_extension_control[][6] = {
    {0*256, 10, 10, 1000, 1000, 1700},
    {1*256, 10, 30, 1000, 1000, 1700},
    {2*256, 10, 50, 1000, 1000, 1700},
    {3*256, 10, 50, 1000, 1000, 1700},
    {4*256, 10, 50, 1000, 1000, 1700},
    {5*256, 10, 50, 1000, 1000, 1700},
    {6*256, 10, 50, 1000, 1000, 1700},
    {7*256, 10, 50, 1000, 1000, 1700},
    {8*256, 10, 50, 1000, 1000, 1700},
};
...
static uint32_t _calibration_cnr_extension_control[][20] = {
    {0*256, 2048, 0, 50, 4000, 0, 50, 4000, 0, 50, 4000, 0, 50, 4000, 0, 150, 65535, 0, 150, 65535},
    {1*256, 2048, 0, 50, 4000, 0, 50, 4000, 0, 50, 4000, 0, 50, 4000, 0, 150, 65535, 0, 150, 65535},
    {2*256, 2048, 0, 50, 4000, 0, 50, 4000, 0, 50, 4000, 0, 50, 4000, 0, 150, 65535, 0, 150, 65535},
    {3*256, 2048, 0, 50, 3000, 0, 50, 3000, 0, 50, 3000, 0, 50, 3000, 0, 150, 65535, 0, 150, 65535},
    {4*256, 2048, 0, 50, 3000, 0, 50, 3000, 0, 50, 3000, 0, 50, 3000, 0, 200, 65535, 0, 200, 65535},
    {5*256, 2048, 0, 50, 3000, 0, 50, 3000, 0, 50, 3000, 0, 50, 3000, 0, 200, 65535, 0, 200, 65535},
    {6*256, 2048, 0, 50, 3000, 0, 50, 3000, 0, 50, 3000, 0, 50, 3000, 0, 200, 65535, 0, 200, 65535},
    {7*256, 2048, 0, 50, 3000, 0, 50, 3000, 0, 50, 3000, 0, 50, 3000, 0, 200, 65535, 0, 200, 65535},
    {8*256, 2048, 0, 50, 3000, 0, 50, 3000, 0, 50, 3000, 0, 50, 3000, 0, 200, 65535, 0, 200, 65535},
};

// { GAIN_LOG2_ID*256, svariance, bright_pr, contrast, white_level }
static uint32_t _calibration_iridix_extension_control[][5] = {
    {0 * 256, 5, 250, 235, 850000},
    {1 * 256, 5, 250, 235, 800000},
    {2 * 256, 5, 250, 235, 700000},
    {3 * 256, 5, 250, 235, 550000},
    {4 * 256, 5, 250, 235, 400000},
    {5 * 256, 5, 250, 235, 400000},
    {6 * 256, 5, 250, 235, 400000},
    {7 * 256, 5, 250, 235, 400000},
};
...
uint32_t get_calibrations_dynamic_linear_ov5675( ACameraCalibrations *c )
{
    uint32_t result = 0;
    if ( c != 0 ) {
