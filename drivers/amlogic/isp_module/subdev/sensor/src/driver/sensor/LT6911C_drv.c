...
static uint16_t sensor_get_id( void *ctx )
{
    ...
    acamera_sbus_write_u8(&p_ctx->sbus, 0xff, 0x80);
    acamera_sbus_write_u8(&p_ctx->sbus, 0xee, 0x01);

    acamera_sbus_write_u8(&p_ctx->sbus, 0xff, 0xa0);
    sensor_id |= acamera_sbus_read_u8(&p_ctx->sbus, 0) << 8;
    sensor_id |= acamera_sbus_read_u8(&p_ctx->sbus, 1);
    ...
    LOG(LOG_CRIT, "%s: read lt6911 id %x\n", __func__, sensor_id);
...
void sensor_deinit_lt6911( void *ctx )
{
...
{
    sensor_context_t *p_ctx = ctx;
    uint32_t htotal = 0, hactive = 0;
    uint32_t vtotal = 0, vactive = 0;

    acamera_sbus_write_u8(&p_ctx->sbus, 0xff, 0xd2);
    htotal |= acamera_sbus_read_u8(&p_ctx->sbus, 0x89) << 8;
    htotal |= acamera_sbus_read_u8(&p_ctx->sbus, 0x8A);

    hactive |= acamera_sbus_read_u8(&p_ctx->sbus, 0x8B) << 8;
    hactive |= acamera_sbus_read_u8(&p_ctx->sbus, 0x8C);

    LOG(LOG_CRIT, "%s: read lt6911 htotal %x, hactive %x\n", __func__, htotal, hactive);

    vtotal |= acamera_sbus_read_u8(&p_ctx->sbus, 0x9e) << 8;
    vtotal |= acamera_sbus_read_u8(&p_ctx->sbus, 0x9f);

    vactive |= acamera_sbus_read_u8(&p_ctx->sbus, 0x96) << 8;
    vactive |= acamera_sbus_read_u8(&p_ctx->sbus, 0x97);

    supported_modes[0].resolution.width = hactive * 2;
    supported_modes[0].resolution.height = vactive;
...
static sensor_context_t *sensor_global_parameter(void* sbp)
{
    ...
    sensor_ctx.param.isp_context_seq.seq_table_max = 0;
...
//--------------------Initialization------------------------------------------------------------
void sensor_init_lt6911( void **ctx, sensor_control_t *ctrl, void *sbp )
{
...
int sensor_detect_lt6911( void* sbp)
{
    int ret = 0;
    sensor_ctx.sbp = sbp;
    sensor_bringup_t* sensor_bp = (sensor_bringup_t*) sbp;
    ...
    sensor_ctx.sbus.mask = SBUS_MASK_SAMPLE_8BITS | SBUS_MASK_ADDR_8BITS | SBUS_MASK_ADDR_SWAP_BYTES;
    sensor_ctx.sbus.control = 0;
    sensor_ctx.sbus.bus = 0;
    sensor_ctx.sbus.device = SENSOR_DEV_ADDRESS;
    acamera_sbus_init( &sensor_ctx.sbus, sbus_i2c );
    ...
    pr_info("sensor_detect_lt6911:%d\n", ret);
