...
static void sensor_set_mode( void *ctx, uint8_t mode )
{
    ...
    default:
     ...
     if ((param->modes_table[mode].exposures == 2) && (param->modes_table[mode].fps == 30 * 256)) {
        p_ctx->s_fps = 30;
        p_ctx->vmax = 1220;
    } else {
...
void sensor_init_imx307( void **ctx, sensor_control_t *ctrl, void* sbp)
{
    ...
    *ctx = &s_ctx;
    ...
    s_ctx.param.isp_context_seq.seq_num= SENSOR_IMX307_CONTEXT_SEQ;
