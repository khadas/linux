...
static uint32_t sensor_power_on( void *ctx )
{
    sensor_context_t *p_ctx = ctx;
    uint16_t ret_value = 0xFFFF;
    if ( p_ctx != NULL ) {
        ...
        settings.ctx_num = ctx_num;
        if ( sd != NULL && ctx_num < FIRMWARE_CONTEXT_NUMBER ) {
            int rc = v4l2_subdev_call( sd, core, ioctl, SOC_SENSOR_POWER_ON, &settings );
            if ( rc == 0 ) {
                ret_value = settings.args.general.val_out;
            } else {
                LOG( LOG_ERR, "Failed to power on. rc = %d", rc );
            }
        } else {
            LOG( LOG_CRIT, "SOC sensor subdev pointer is NULL" );
        }
...
static uint32_t sensor_power_off( void *ctx )
{
    sensor_context_t *p_ctx = ctx;
    uint16_t ret_value = 0xFFFF;
    if ( p_ctx != NULL ) {
        ...
        settings.ctx_num = ctx_num;
        if ( sd != NULL && ctx_num < FIRMWARE_CONTEXT_NUMBER ) {
            int rc = v4l2_subdev_call( sd, core, ioctl, SOC_SENSOR_POWER_OFF, &settings );
            if ( rc == 0 ) {
                ret_value = settings.args.general.val_out;
            } else {
                LOG( LOG_ERR, "Failed to power off. rc = %d", rc );
            }
        } else {
            LOG( LOG_CRIT, "SOC sensor subdev pointer is NULL" );
        }
...
void sensor_init_v4l2( void **ctx, sensor_control_t *ctrl )
{
    ...
    if ( ctx_counter < FIRMWARE_CONTEXT_NUMBER ) {
        ...
        ctrl->power_on = sensor_power_on;
        ctrl->power_off = sensor_power_off;
