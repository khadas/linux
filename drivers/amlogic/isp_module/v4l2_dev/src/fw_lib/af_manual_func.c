...
void af_read_stats_data( AF_fsm_ptr_t p_fsm )
{
    ...
    p_fsm->zones_horiz = zones_horiz;
    p_fsm->zones_vert = zones_vert;
    ...
    for ( y = 0; y < zones_vert; y++ ) {
        ...
        for ( x = 0; x < zones_horiz; x++ ) {
            ...
            p_fsm->zone_raw_statistic[full_inx][0] = stats[full_inx][0];
            p_fsm->zone_raw_statistic[full_inx][1] = stats[full_inx][1];
        }
    }
