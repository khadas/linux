...
{
    acamera_context_t *p_ctx = (acamera_context_t *)(&g_firmware.fw_ctx[acamera_get_api_context()]);
...
int32_t acamera_init( acamera_settings *settings, uint32_t ctx_num )
{
    ...
    if ( result == 0 ) {
        ...
        if (p_id != ACAMERA_ISP_ID_PRODUCT_DEFAULT) {
            LOG(LOG_CRIT, "Failed to read profuct id\n");
            return 0;
        }
        LOG(LOG_INFO, "Success get product id: 0x%08x\n", p_id);
...
{
    if (port == 0xff) {
        system_dma_copy_sg( g_firmware.dma_chan_isp_config, ISP_CONFIG_PING, SYS_DMA_TO_DEVICE, NULL, acamera_get_api_context() );
        system_dma_copy_sg( g_firmware.dma_chan_isp_config, ISP_CONFIG_PONG, SYS_DMA_TO_DEVICE, NULL, acamera_get_api_context() );
...
int32_t acamera_interrupt_handler()
{
    ...
    LOG( LOG_DEBUG, "IRQ MASK is %d", irq_mask );
                ...
                if ( irq_bit == ISP_INTERRUPT_EVENT_ISP_START_FRAME_START ) {
                    ...
                    // we must finish all previous processing before scheduling new dma
                    {
                        ...
                        if ( acamera_isp_isp_global_ping_pong_config_select_read( 0 ) == ISP_CONFIG_PONG ) {
                            LOG( LOG_DEBUG, "Current config is pong" );
                            //            |^^^^^^^^^|
                            // next --->  |  PING   |
                            //            |_________|
                    ...
                    // unhandled irq
                    LOG( LOG_DEBUG, "Unhandled interrupt bit %d", irq_bit );
                }
            }
            irq_bit--;
        }
