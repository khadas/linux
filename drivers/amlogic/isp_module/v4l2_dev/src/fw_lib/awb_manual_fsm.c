...
int AWB_fsm_get_param( void *fsm, uint32_t param_id, void *input, uint32_t input_size, void *output, uint32_t output_size )
{
    ...
    case FSM_PARAM_GET_AWB_ZONE_WEIGHT: {
        if ( !output) {
            LOG( LOG_ERR, "Invalid param, param_id: %d.", param_id );
            rc = -1;
            break;
        }
        awb_get_zone_weight(p_fsm, (uint8_t *)output);
        break;
    }
...
int AWB_fsm_set_param( void *fsm, uint32_t param_id, void *input, uint32_t input_size )
{
	...
	case FSM_PARAM_SET_AWB_PRESET:
        ...
        p_fsm->wb_log2[0] = p_new->wb_log2[0];
        p_fsm->wb_log2[1] = p_new->wb_log2[1];
        p_fsm->wb_log2[2] = p_new->wb_log2[2];
        p_fsm->wb_log2[3] = p_new->wb_log2[3];	
		
        p_fsm->wb[0] = p_new->wb[0];
