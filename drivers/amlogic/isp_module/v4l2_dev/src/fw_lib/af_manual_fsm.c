...
int AF_fsm_get_param( void *fsm, uint32_t param_id, void *input, uint32_t input_size, void *output, uint32_t output_size )
{
    ...
    case FSM_PARAM_GET_AF_LENS_STATUS:
        ...
        *(int32_t *)output = p_fsm->lens_driver_ok;
        break;
    case FSM_PARAM_GET_AF_STATS_INFO:
        if ( !output || output_size != sizeof( fsm_param_af_stats_info_t ) ) {
            LOG( LOG_ERR, "Invalid param, param_id: %d.", param_id );
            rc = -1;
            break;
        }

        fsm_param_af_stats_info_t *p_af_stats_info = (fsm_param_af_stats_info_t *)output;

        p_af_stats_info->stats = (uint32_t *)p_fsm->zone_raw_statistic;
        p_af_stats_info->zones_size = p_fsm->zones_horiz * p_fsm->zones_vert;
