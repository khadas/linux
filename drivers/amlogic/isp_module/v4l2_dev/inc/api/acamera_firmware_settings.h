...
typedef struct _acamera_settings {
    ...
    uint32_t (*get_calibrations)( uint32_t ctx_num,void * sensor_arg,ACameraCalibrations *, char* s_name ) ;  // must be initialized to provide calibrations. Must be provided.
    ...
    tframe_t* sc0_frames ;                                              // frames to be used for the sc0 dma writer
    uint32_t  sc0_frames_number ;                                       // number of frames for sc0 pipe
    void (*callback_sc0)( uint32_t ctx_num, tframe_t * tframe, const metadata_t *metadata ) ; // callback on every sc0 output frame. can be null if there is no sc0 output
    tframe_t* sc1_frames ;                                              // frames to be used for the sc1 dma writer
    uint32_t  sc1_frames_number ;                                       // number of frames for sc1 pipe
    void (*callback_sc1)( uint32_t ctx_num, tframe_t * tframe, const metadata_t *metadata ) ; // callback on every SC1 output frame. can be null if there is no sc0 output
    tframe_t* sc2_frames ;                                              // frames to be used for the sc0 dma writer
    uint32_t  sc2_frames_number ;                                       // number of frames for sc0 pipe
    void (*callback_sc2)( uint32_t ctx_num, tframe_t * tframe, const metadata_t *metadata ) ; // callback on every SC2 output frame. can be null if there is no sc0 output
    tframe_t* sc3_frames ;                                              // frames to be used for the sc0 dma writer
    uint32_t  sc3_frames_number ;                                       // number of frames for sc0 pipe
    void (*callback_sc3)( uint32_t ctx_num, tframe_t * tframe, const metadata_t *metadata ) ; // callback on every SC3 output frame. can be null if there is no sc0 output
} acamera_settings ;
