/*
 * drivers/amlogic/amlnf/include/hw_ctrl.h
 *
...
static inline uint amlnf_read_reg32(void __iomem *_reg)
{
	return __raw_readl(_reg);
};

static inline void amlnf_write_reg32(void __iomem *_reg,
	const uint _value)
{
	__raw_writel(_value, _reg);
};

static inline void amlnf_set_reg32_bits(void __iomem *_reg,
	const uint _value,
	const uint _start,
	const uint _len)
{
...
static inline void amlnf_clrset_reg32_bits(void __iomem *_reg,
	const uint clr,
	const uint set)
{
...
static inline uint amlnf_get_reg32_bits(void __iomem *_reg,
	const uint _start,
	const uint _len)
{
...
static inline void amlnf_set_reg32_mask(void __iomem *_reg,
	const uint _mask)
{
	__raw_writel((__raw_readl(_reg) | (_mask)), _reg);
}

static inline void amlnf_clr_reg32_mask(void __iomem *_reg,
	const uint _mask)
{
	__raw_writel((__raw_readl(_reg) & (~(_mask))), _reg);
}
...
#define nfc_set_bits(host, reg) \
	{\
		...
		value |= ((unsigned)((_value)&((1L<<(_len))-1)) << (_start));\
		__raw_writel(value, (host)->reg_base + P_##reg);\
	}
*/
...
#define NFC_SET_DADDR(host, a) \
	(AMLNF_WRITE_REG((host)->reg_base + P_NAND_DADR, (unsigned int)a))
#define NFC_SET_IADDR(host, a) \
	(AMLNF_WRITE_REG((host)->reg_base + P_NAND_IADR, (unsigned int)a))
#define NFC_SET_SADDR(host, a) \
	(AMLNF_WRITE_REG((host)->reg_base + P_NAND_SADR, (unsigned int)a))
...
#define NFC_SET_CFG(host, val) \
	(AMLNF_WRITE_REG((host)->reg_base + P_NAND_CFG, (unsigned int)val))
