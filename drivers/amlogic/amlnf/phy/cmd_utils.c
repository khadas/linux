/*
 * drivers/amlogic/amlnf/phy/cmd_utils.c
 *
...
static int erase_env_protect(struct amlnand_chip *aml_chip, int blk)
{
	...
	struct nand_arg_info *nand_bbtinfo = &(aml_chip->nand_bbtinfo);
	struct nand_arg_info *shipped_bbtinfo = &aml_chip->shipped_bbtinfo;
	struct nand_arg_info *config_msg = &aml_chip->config_msg;
	...
	unsigned int offset, pages_per_blk;
	int ret = 0;
...
int  amlnf_erase_ops(uint64_t off, uint64_t erase_len, unsigned char scrub_flag)
{
	...
	u32 erase_shift, write_shift, writesize, erasesize, pages_per_blk;
