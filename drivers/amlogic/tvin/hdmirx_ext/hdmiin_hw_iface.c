/*
 * drivers/amlogic/tvin/hdmirx_ext/hdmiin_hw_iface.c
 *
...
int __hw_get_cable_status(void)
{
	...
	if ((hdrv) && (hdrv->hw_ops.get_cable_status))
		return hdrv->hw_ops.get_cable_status();
...
int __hw_get_signal_status(void)
{
	...
	if ((hdrv) && (hdrv->hw_ops.get_signal_status))
		return hdrv->hw_ops.get_signal_status();
...
int __hw_get_input_port(void)
{
	...
	if ((hdrv) && (hdrv->hw_ops.get_input_port))
		return hdrv->hw_ops.get_input_port();
...
void __hw_set_input_port(int port)
{
	...
	if ((hdrv) && (hdrv->hw_ops.set_input_port))
		return hdrv->hw_ops.set_input_port(port);
...
int __hw_get_video_timming(struct video_timming_s *ptimming)
{
	...
	if ((hdrv) && (hdrv->hw_ops.get_video_timming))
		return hdrv->hw_ops.get_video_timming(ptimming);
...
int __hw_is_hdmi_mode(void)
{
	...
	if ((hdrv) && (hdrv->hw_ops.is_hdmi_mode))
		return hdrv->hw_ops.is_hdmi_mode();
...
int __hw_get_audio_sample_rate(void)
{
	...
	if ((hdrv) && (hdrv->hw_ops.get_audio_sample_rate))
		return hdrv->hw_ops.get_audio_sample_rate();
...
int __hw_debug(char *buf)
{
	...
	if ((hdrv) && (hdrv->hw_ops.debug))
		return hdrv->hw_ops.debug(buf);
...
char *__hw_get_chip_id(void)
{
	...
	if ((hdrv) && (hdrv->hw_ops.get_chip_version))
		return hdrv->hw_ops.get_chip_version();
...
int __hw_init(void)
{
	...
	if ((hdrv) && (hdrv->hw_ops.init))
		return hdrv->hw_ops.init(hdrv);
