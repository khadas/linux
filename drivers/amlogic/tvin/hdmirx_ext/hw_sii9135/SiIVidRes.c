...
static WORD GetVSyncFreq(WORD wHtot, WORD wVtot)
{
	...
	HDMIIN_PD("%s: RX_SYS_CTRL1_ADDR(0x%02x)=0x%02x\n",
...
{
	HDMIIN_PD("sii9135 vmode:\n");
	if (SearchRes) {
		if (SearchRes == 2)
			HDMIIN_PD("Video Mode has polarity mismatch");
		if (SiI_Inf.bGlobStatus & SiI_RX_GlobalHDMI_Detected)
			HDMIIN_PD("HDMI MODE\n");
		else {
			HDMIIN_PD("DVI MODE ");
			HDMIIN_PD("v. mode detected, AVI will be ignored\n");
		}
		if (Index < PC_BASE)
			HDMIIN_PD("CE Space:");
		else
			HDMIIN_PD("PC Space:");
		HDMIIN_PD(" Mode ROM-%i",
		...
		if (VModeTables[Index].ModeId.SubMode) {
			HDMIIN_PD(" Sub mode: %i",
				VModeTables[Index].ModeId.SubMode);
		}
		HDMIIN_PD(" Resolution: %i x %i@%i\n",
