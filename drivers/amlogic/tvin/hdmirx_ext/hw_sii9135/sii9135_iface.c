/*
 * drivers/amlogic/tvin/hdmirx_ext/hw_sii9135/sii9135_iface.c
 *
 ...
 *
*/
/* implement of functions of hw_ops of hdmiin_dev_s. */
...
void __sii9135_dump_video_timming(void)
{
	...
	if (__sii9135_get_video_timming(&timming) == -1) {
		HDMIIN_PE("[%s] get video timming failed\n", __func__);
		return;
	}
...
/*	pr_info("get para: %s %s %s %s!\n",argv[0],argv[1],argv[2],argv[3]);*/
	...
	case 5: /* reset, for test reset_gpio */
		...
		__plat_gpio_output(&hdmiin_drv->hw.reset_gpio, value);
		pr_info("reset_gpio is set to %d\n", (unsigned int)value);
		break;
	case 6: /* tt, for loop test of hdmiin i2c */
...
void hdmiin_register_hw_sii9135_ops(struct hdmiin_drv_s *hdrv)
{
	...
	hdrv->hw_ops.set_input_port        = __sii9135_set_input_port;
	hdrv->hw_ops.get_video_timming     = __sii9135_get_video_timming;
	hdrv->hw_ops.is_hdmi_mode          = __sii9135_is_hdmi_mode;
	hdrv->hw_ops.get_audio_sample_rate = __sii9135_get_audio_sample_rate;
