...
BYTE siiVideoModeDetection(BYTE *, struct SyncInfoType_s *);
BOOL siiCheckOutOfRangeConditions(struct SyncInfoType_s *);
BYTE siiSetVidResDependentVidOutputFormat(BYTE, BYTE, BYTE);
void siiGetSyncInfo(struct SyncInfoType_s *);
BOOL siiCheckSyncDetect(void);
BOOL siiCheckClockDetect(void);
BYTE siiGetPixClockFromTables(BYTE);
BOOL siiCompareNewOldSync(struct SyncInfoType_s *, struct SyncInfoType_s *);
