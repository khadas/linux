/*
 * drivers/amlogic/tvin/hdmirx_ext/platform_iface.c
 *
...
int __plat_hrtimer_expired(void)
{
	...
	HDMIIN_PD("hdmiin hrtimer remove\n");
		...
		int addr_len, int data_len)
{
	...
	if (hdrv == NULL) {
		HDMIIN_PE("[%s] hdmiin driver is null !\n", __func__);
		return -1;
	}
	if (hdrv->hw.i2c_client == NULL) {
		HDMIIN_PE("[%s] invalid i2c_client !\n", __func__);
		return -1;
	}

	ret = i2c_transfer(hdrv->hw.i2c_client->adapter, msg, 2);
	if (ret < 0) {
		HDMIIN_PE("[%s]: %d, dev_addr = 0x%x, reg = 0x%x\n",
		...
		unsigned char *pvalue)
{
	...
	*pvalue = buf[0];
	...
	if (ret < 0) {
		HDMIIN_PE("[%s]: dev_addr = 0x%x, reg = 0x%x, ret = %d\n",
...
int __plat_i2c_init(struct hdmiin_hw_s *phw, unsigned char dev_i2c_addr)
{
	...
	if (hdmiin_debug_print)
		HDMIIN_PD("[%s]: i2c addr = 0x%x\n", __func__, dev_i2c_addr);

	memset(&board_info, 0x0, sizeof(board_info));
	strncpy(board_info.type, HDMIIN_DEV_NAME, I2C_NAME_SIZE);
...
{
	if (gpio->flag == 0) {
		HDMIIN_PW("[%s] gpio is not regisetered\n", __func__);
		return;
	}
	if (IS_ERR(gpio->gpio)) {
		HDMIIN_PW("gpio %s: %p, err: %ld\n",
			gpio->name, gpio->gpio, PTR_ERR(gpio->gpio));
		return;
	}
	gpiod_direction_output(gpio->gpio, ((value == 0) ? 0 : 1));
	HDMIIN_PD("gpio %s: %p, value: %d\n",
