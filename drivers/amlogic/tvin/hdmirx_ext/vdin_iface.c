/*
 * drivers/amlogic/tvin/hdmirx_ext/vdin_iface.c
 *
...
static int __vdin_set_pinmux(int flag)
{
	...
	if (flag) { /* enable pinmux */
		if (hdrv->hw.pinmux_flag == 0) {
			/* request pinmux */
			hdrv->hw.pin = devm_pinctrl_get_select(hdrv->dev,
				"aml_hdmiin_dvp");
			if (IS_ERR(hdrv->hw.pin))
				HDMIIN_PE("hdmiin pinmux set error\n");
			else
				hdrv->hw.pinmux_flag = 1;
		} else {
			HDMIIN_PW("hdmiin pinmux is already selected\n");
		}
	} else { /* disable pinmux */
		if (hdrv->hw.pinmux_flag) {
			...
			HDMIIN_PW("hdmiin pinmux is already released\n");
		}
	}
...
void hdmiin_stop_vdin(struct hdmiin_vdin_s *info)
{
	if (info->started == 0)
		return;
	...
	HDMIIN_PD("[%s] stop vdin\n", __func__);
}

void hdmiin_start_vdin(struct hdmiin_vdin_s *info, int width, int height,
		int frame_rate, int field_flag)
{
	...
	HDMIIN_PD("[%s]: width=%d, height=%d, frame_rate=%d, field_flag=%d\n",
	...
	if (__hw_get_video_timming(&timming) == -1) {
		HDMIIN_PD("[%s] get video timming failed!\n", __func__);
		return;
	}

	if ((width <= 0) || (height <= 0) || (frame_rate <= 0)) {
		HDMIIN_PD("[%s] invalid size or frame_rate\n", __func__);
		return;
	}
	...
	*/
	...
	__vdin_set_pinmux(1);
	if (hdrv->vops) {
		HDMIIN_PD("[%s] begin start_tvin_service()\n", __func__);
		hdrv->vops->start_tvin_service(0, &para);
		info->started = 1;
	} else {
		HDMIIN_PE("[%s] v4l2_ops is NULL\n", __func__);
	}
...
void hdmiin_start_vdin_mode(unsigned int mode)
{
	struct hdmiin_drv_s *hdrv = hdmiin_get_driver();
	unsigned int height = 0, width = 0, frame_rate = 0, field_flag = 0;

	HDMIIN_PD("[%s], start with mode = %d\n", __func__, mode);
	switch (mode) {
	case CEA_480I60:
		width = 1440;
		height = 480;
	...
	case CEA_1080P50:
		...
		break;
	default:
		HDMIIN_PE("[%s], invalid video mode = %d!\n", __func__, mode);
		return;
	}

	hdmiin_start_vdin(&hdrv->vdin, width, height, frame_rate, field_flag);
