/*
 * drivers/amlogic/tvin/vdin_ctrl/vdin_ctrl_drv.c
 *
...
#define HDMI_DEBUG()  pr_info("VDINCTRL DEBUG: %s [%d]", __func__, __LINE__)
...
#define pr_dbg(fmt, args...)   pr_info("vdin_ctrl: " fmt, ## args)
#define pr_error(fmt, args...) pr_info "vdin_ctrl: " fmt, ## args)
...
static int is_hdmi_mode;
static int horz_active;
static int vert_active;
static int is_interlace;
static int vfreq;

static int audio_status;
static int audio_sample_freq;
static int audio_channel_alloc;
...
static struct vdinctrl_dev vdinctrl_device;
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (vdin_enable != vdinctrl_device.vdin_enable) {
		...
		if (vdinctrl_device.vdin_enable == 1) {
			/* start_vdin(getHDMIRXHorzActive(),
			 *		 *  getHDMIRXVertActive(),
			 *		  10000, IsHDMIRXInterlace()); */
...
static ssize_t show_poweron(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", vdinctrl_device.fe_enable);
}

static ssize_t store_poweron(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	size_t r;
	int val;
...
static ssize_t store_dbg(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	char tmpbuf[128];
	int i = 0;
...
fe_ctrl_task_handle(void *data)
{
	...
	if (pre_state != vdinctrl_device.state) {
		pr_info("[VDIN CTRL State] %s -> %s\n",
			state_name[pre_state],
			state_name[vdinctrl_device.state]);
	}

	/* msleep(10);  change for code style warnning */
	msleep(20);
...
{
	struct vdinctrl_dev *vdin_ctrl_in_devp;

	/* Get the per-device structure that contains this cdev */
	vdin_ctrl_in_devp = container_of(node->i_cdev,
			struct vdinctrl_dev, cdev);
...
{
	/* struct vdinctrl_dev *vdin_ctrl_in_devp = file->private_data; */
...
static int vdinctrl_ioctl(struct inode *node, struct file *file,
...
static const struct file_operations vdinctrl_fops = {
	.owner    = THIS_MODULE,
	.open     = vdinctrl_open,
...
static int vdinctrl_probe(struct platform_device *pdev)
{
	...
	/* vdinctrl_dev = device_create(vdinctrl_class,
	 * NULL, vdinctrl_id, "vdin_ctrl%d", 0); */
	vdinctrl_dev = device_create(vdinctrl_class, NULL,
			vdinctrl_id, NULL, "vdin_ctrl%d", 0);
	...
	vdinctrl_device.task = kthread_run(fe_ctrl_task_handle,
			&vdinctrl_device, "kthread_hdmi");

	if (r < 0) {
		pr_info(KERN_ERR "vdin_ctrl: register switch dev failed\n");
		return r;
	}
...
static struct platform_device *vdin_ctrl_device;
