...
#define AUTO_CLK_VS_CNT                 10
/* 10 // get stable BD readings after n+1 frames */
#define AUTO_PHASE_VS_CNT               2
/* get stable AP readings after n+1 frames */
...
#define VGA_AUTO_TRY_COUNTER            300
/* vga max adjust counter, 3 seconds */
/* divide window into 7*7 sub-windows &
make phase detection on 9 sub-windows */
/* ------- */
/* -*-*-*- */
/* ------- */
/* -*-*-*- */
...
#define TVAFE_ADC_RESET_MAX_CNT         3
/* ADC reset max counter, avoid mode */
/* detection error sometimes for component */
#define __BAR_DET_ORI 0
enum bar_det_status_e {
	BAR_INIT,
	BAR_PRE_DET,
	BAR_TUNE,
	BAR_SUCCESS,
...
static enum bar_det_status_e b_status;
...
static bool adc_dbg_en;
module_param(adc_dbg_en, bool, 0664);
MODULE_PARM_DESC(adc_dbg_en,
	"enable/disable adc auto adj debug message");

static bool adc_parm_en;
module_param(adc_parm_en, bool, 0664);
MODULE_PARM_DESC(adc_parm_en, "enable/disable set adc parm message");
static bool adc_fmt_chg_dbg;
module_param(adc_fmt_chg_dbg, bool, 0664);
MODULE_PARM_DESC(adc_fmt_chg_dbg,
	"enable/disable adc fmt change debug message");
...
static bool enable_dphase;
module_param(enable_dphase, bool, 0644);
MODULE_PARM_DESC(enable_dphase,
	"turn on/off the different phase config");

static unsigned short comp_phase[TVIN_SIG_FMT_COMP_MAX -
		TVIN_SIG_FMT_VGA_THRESHOLD];
...
MODULE_PARM_DESC(bar_init_cnt,
	"init_cnt:wait for stabilization(unit:vs)");
...
MODULE_PARM_DESC(bar_width,
	"bar det: to decrease when after old border det");
 ...
 */
static void tvafe_adc_set_frame_skip_number(
			struct tvafe_adc_s *adc, unsigned int frame_number)
{
	unsigned long flags;
...
{
	return	R_APB_BIT(TVFE_DVSS_INDICATOR1, NOSIG_BIT, NOSIG_WID) ?
			true : false;
}
/*
 ...
 */
inline bool tvafe_adc_fmt_chg(struct tvin_parm_s *parm,
					struct tvafe_adc_s *adc)
{
	...
	unsigned int   h_cnt_offset = 0, h_cnt_offset1 = 0,
					v_cnt_offset = 0, v_cnt_offset1 = 0;
	...
	if ((port >= TVIN_PORT_VGA0) && (port <= TVIN_PORT_VGA7)) {
		...
		tmp0 = min(tmp0, tmp1);
		hs_cnt_offset = abs((signed int)hw_info->hs_cnt -
					(signed int)tmp0);
		hw_info->hs_cnt = tmp0;
		...
		tmp0 = min(tmp0, tmp1);
		vs_cnt_offset = abs((signed int)hw_info->vs_width -
					(signed int)tmp0);
		hw_info->vs_width = tmp0;
		/* h_cnt */
		tmp0 = R_APB_BIT(TVFE_SYNCTOP_INDICATOR4,
				SAM_HCNT_BIT, SAM_HCNT_WID);
		h_cnt_offset = abs((signed int)hw_info->h_cnt -
					(signed int)tmp0);
		/* h_cnt_offset = abs((signed int)tvin_vga_fmt_tbl[fmt-
		TVIN_SIG_FMT_VGA_512X384P_60HZ_D147].h_cnt -
		(signed int)tmp0); */
		hw_info->h_cnt = tmp0;
		/* v_cnt */
		tmp0 = R_APB_BIT(TVFE_SYNCTOP_INDICATOR4,
				SAM_VCNT_BIT, SAM_VCNT_WID);
		v_cnt_offset = abs((signed int)hw_info->v_total -
					(signed int)tmp0);
		/* v_cnt_offset = abs((signed int)tvin_vga_fmt_tbl[fmt-
		TVIN_SIG_FMT_VGA_512X384P_60HZ_D147].v_total-
		(signed int)tmp0); */
		hw_info->v_total = tmp0;


	} else if ((port >= TVIN_PORT_COMP0) &&
	(port <= TVIN_PORT_COMP7)) {
		...
		if (adc->hs_sog_sw_cnt > TVIN_FMT_CHK_HS_SOG_DLY_CNT) {
			...
			/* the difference between h_cnt
			from sog and h_cnt from hs_out2 */
			h_cnt_offset1 = abs((signed int)hw_info->h_cnt -
					(signed int)tmp1);
			hw_info->h_cnt = tmp0;
			...
			/* pr_info("[tvafe..] %s: hw:%d,
			hw.hcnt:%d fmt is:%s\n",__func__, tmp0, */
			/* hw_info->h_cnt, tvin_sig_fmt_str(fmt)); */
			/* if the diff exceed max cnt,
			reset adc for abnormal format */
			if (h_cnt_offset > TVIN_FMT_CHK_COMP_RST_MAX_CNT) {

#if 0
				/* avoid mode detection error for component */
				if (devp->adc.adc_reset_cnt++ <
					TVAFE_ADC_RESET_MAX_CNT) {
			...
			if ((status == TVIN_SIG_STATUS_STABLE) &&
				(fmt_info_p)) {
				hw_info->v_total = fmt_info_p->v_total;
				tmp1 = R_APB_BIT(TVFE_SOG_MON_INDICATOR2,
						SOG_VTOTAL_BIT, SOG_VTOTAL_WID);
				/*during vsync sog cnt
				is double of vsync width*/
				tmp1 -= hw_info->vs_width;
				v_cnt_offset1 =
					abs((signed int)hw_info->v_total -
					(signed int)tmp1);
				/* chose the correct v_total
				from sog and v_hs */
 ...
 */
inline enum tvin_sig_fmt_e tvafe_adc_search_mode(
			struct tvin_parm_s *parm, struct tvafe_adc_s *adc)
{
	...
	if ((port >= TVIN_PORT_VGA0) && (port <= TVIN_PORT_VGA7)) {
		...
		for (index = index_min; index < index_max; index++) {
			...
			if (tvin_vga_fmt_tbl[index-
				TVIN_SIG_FMT_VGA_512X384P_60HZ_D147].h_cnt == 0)
				continue;
			if (abs((signed int)hw_info->h_cnt -
				(signed int)tvin_vga_fmt_tbl[index-
				TVIN_SIG_FMT_VGA_512X384P_60HZ_D147].h_cnt) >
				tvin_vga_fmt_tbl[index-
			TVIN_SIG_FMT_VGA_512X384P_60HZ_D147].h_cnt_offset)
				continue;
			if (abs((signed int)hw_info->v_total -
				(signed int)tvin_vga_fmt_tbl[index-
			TVIN_SIG_FMT_VGA_512X384P_60HZ_D147].v_total) >
				tvin_vga_fmt_tbl[index-
			TVIN_SIG_FMT_VGA_512X384P_60HZ_D147].v_cnt_offset)
				continue;
			if (abs((signed int)hw_info->hs_cnt -
				(signed int)tvin_vga_fmt_tbl[index-
			TVIN_SIG_FMT_VGA_512X384P_60HZ_D147].hs_cnt) >
				tvin_vga_fmt_tbl[index-
			TVIN_SIG_FMT_VGA_512X384P_60HZ_D147].hs_cnt_offset)
				continue;
			if (abs((signed int)hw_info->vs_width -
				(signed int)tvin_vga_fmt_tbl[index-
			TVIN_SIG_FMT_VGA_512X384P_60HZ_D147].vs_width) >
				vs_width_thr)
				continue;
			if ((hw_info->hs_pol ==
				tvin_vga_fmt_tbl[index-
			TVIN_SIG_FMT_VGA_512X384P_60HZ_D147].hs_pol) &&
				(hw_info->vs_pol == tvin_vga_fmt_tbl[index-
			TVIN_SIG_FMT_VGA_512X384P_60HZ_D147].vs_pol))
				break;
		}
		...
		for (index = index_min; index < index_max; index++) {
			...
			if (tvin_comp_fmt_tbl[index-
				TVIN_SIG_FMT_COMP_480P_60HZ_D000].h_cnt == 0)
				continue;
			if (abs((signed int)hw_info->h_cnt -
				(signed int)tvin_comp_fmt_tbl[index-
				TVIN_SIG_FMT_COMP_480P_60HZ_D000].h_cnt) <=
				tvin_comp_fmt_tbl[index-
				TVIN_SIG_FMT_COMP_480P_60HZ_D000].h_cnt_offset){

				if (abs((signed int)hw_info->v_total -
					(signed int)tvin_comp_fmt_tbl[index-
				TVIN_SIG_FMT_COMP_480P_60HZ_D000].v_total) <=
					tvin_comp_fmt_tbl[index-
				TVIN_SIG_FMT_COMP_480P_60HZ_D000].v_cnt_offset){
	...
	if (adc_dbg_en)
		pr_info("[tvafe..] %s h_cnt= %d, v_total= %d, hs_cnt=%d, vs_width= %d, hs_pol= %d, vs_pol= %d\n",
			__func__, hw_info->h_cnt, hw_info->v_total,
			hw_info->hs_cnt,
...
static void tvafe_adc_set_bd_window(enum tvin_sig_fmt_e fmt)
{
	...
	/* tmp += tvin_vga_fmt_tbl[fmt-
	TVIN_SIG_FMT_VGA_512X384P_60HZ_D147].h_active +
	ADC_WINDOW_H_OFFSET + ADC_WINDOW_H_OFFSET; */
	tmp =
	...
	/* tmp += tvin_vga_fmt_tbl[fmt-
	TVIN_SIG_FMT_VGA_512X384P_60HZ_D147].v_active +
	ADC_WINDOW_V_OFFSET + ADC_WINDOW_V_OFFSET; */
	tmp =
	tvin_vga_fmt_tbl[fmt-TVIN_SIG_FMT_VGA_512X384P_60HZ_D147].v_total - 1;
 ...
 */
static void tvafe_adc_set_ap_window(enum tvin_sig_fmt_e fmt,
					unsigned char idx)
{
	unsigned int hh = tvin_vga_fmt_tbl[fmt-
		TVIN_SIG_FMT_VGA_512X384P_60HZ_D147].h_active /
		VGA_AUTO_PHASE_H_WIN;
	unsigned int vv = tvin_vga_fmt_tbl[fmt-
		TVIN_SIG_FMT_VGA_512X384P_60HZ_D147].v_active /
		VGA_AUTO_PHASE_V_WIN;
	unsigned int hs = tvin_vga_fmt_tbl[fmt-
		TVIN_SIG_FMT_VGA_512X384P_60HZ_D147].hs_width +
	tvin_vga_fmt_tbl[fmt-TVIN_SIG_FMT_VGA_512X384P_60HZ_D147].hs_bp +
		(((idx%3) << 1) + 1)*hh;
	unsigned int he = hs + hh - 1;
	unsigned int vs = tvin_vga_fmt_tbl[fmt-
		TVIN_SIG_FMT_VGA_512X384P_60HZ_D147].vs_width +
	tvin_vga_fmt_tbl[fmt-TVIN_SIG_FMT_VGA_512X384P_60HZ_D147].vs_bp +
		(((idx/3) << 1) + 1)*vv;
 ...
 */
void tvafe_vga_set_v_pos(unsigned int vs,
		unsigned int ve, enum tvin_scan_mode_e scan_mode)
{
	...
	W_APB_BIT(TVFE_DEG_VEVEN, vs + offset,
			DEG_VSTART_EVEN_BIT, DEG_VSTART_EVEN_WID);
	W_APB_BIT(TVFE_DEG_VEVEN, ve + offset,
...
static void tvafe_vga_border_detect_init(enum tvin_sig_fmt_e fmt)
{
	...
	/* line_thr = 1/16 of h_active (valid pixels >
	line_thr => valid line) */
	W_APB_BIT(TVFE_BD_MUXCTRL3, (tvin_vga_fmt_tbl[fmt-
	TVIN_SIG_FMT_VGA_512X384P_60HZ_D147].h_active)>>5
	/*(tvin_fmt_tbl[fmt].h_active)>>4*/,
 ...
 */
static void tvafe_vga_auto_phase_init(enum tvin_sig_fmt_e fmt,
						unsigned char idx)
{
	/* disable auto phase */
...
static void tvafe_vga_get_h_border(struct tvafe_vga_auto_s *vga_auto)
{
	...
	if (adc_dbg_en)
		...
		__func__, r_left_hcnt, g_left_hcnt, b_left_hcnt,
		r_right_hcnt, g_right_hcnt, b_right_hcnt);
...
static void tvafe_vga_get_v_border(struct tvafe_vga_auto_s *vga_auto)
{
	...
	r_top_vcnt = R_APB_BIT(TVFE_AP_INDICATOR13,
				BD_R_TOP_VCNT_BIT, BD_R_TOP_VCNT_WID);
	r_bot_vcnt = R_APB_BIT(TVFE_AP_INDICATOR13,
				BD_R_BOT_VCNT_BIT, BD_R_BOT_VCNT_WID);
	g_top_vcnt = R_APB_BIT(TVFE_AP_INDICATOR15,
				BD_G_TOP_VCNT_BIT, BD_G_TOP_VCNT_WID);
	g_bot_vcnt = R_APB_BIT(TVFE_AP_INDICATOR15,
				BD_G_BOT_VCNT_BIT, BD_G_BOT_VCNT_WID);
	b_top_vcnt = R_APB_BIT(TVFE_AP_INDICATOR17,
				BD_B_TOP_VCNT_BIT, BD_B_TOP_VCNT_WID);
	b_bot_vcnt = R_APB_BIT(TVFE_AP_INDICATOR17,
				BD_B_BOT_VCNT_BIT, BD_B_BOT_VCNT_WID);
	if (adc_dbg_en)
		pr_info("[tvafe..]%s r_start %u.g_start %u.b_start%u.r_end %u.g_end %u.b_end%u.\n",
			__func__, r_top_vcnt,
			g_top_vcnt, b_top_vcnt, r_bot_vcnt,
			g_bot_vcnt, b_bot_vcnt);
 ...
 */
static void tvafe_vga_auto_clock_handler(enum tvin_sig_fmt_e fmt,
						struct tvafe_adc_s *adc)
{
	unsigned int clk = 0;
	signed int diff = 0;
	...
	case VGA_CLK_INIT:
		...
		/* tvafe_vga_set_clock(tvin_vga_fmt_tbl[tvinfo->fmt -
		TVIN_SIG_FMT_VGA_512X384P_60HZ_D147].h_total-1);
		//set spec clock value */
	...
	case VGA_CLK_ROUGH_ADJ:
		...
		if (vga_auto->vs_cnt > AUTO_CLK_VS_CNT) {
			...
			/* calculate new clk */
			clk = ((
			(clk * (unsigned int)tvin_vga_fmt_tbl[fmt-
		TVIN_SIG_FMT_VGA_512X384P_60HZ_D147].h_active) << 8) /
		(vga_auto->border.hend - vga_auto->border.hstart + 1) +
		128) >> 8;
			if (adc_dbg_en)
				pr_info("[tvafe..] %s: auto clock start, init_clk=%d\n",
				__func__, clk);
			/* if clk is too far from spec,
			then return error */
			if ((clk >
				((tvin_vga_fmt_tbl[fmt-
			TVIN_SIG_FMT_VGA_512X384P_60HZ_D147].h_total -
			1) + (tvin_vga_fmt_tbl[fmt-
			TVIN_SIG_FMT_VGA_512X384P_60HZ_D147].h_total >>
				MAX_AUTO_CLOCK_ORDER))) ||
				(clk < ((tvin_vga_fmt_tbl[fmt-
			TVIN_SIG_FMT_VGA_512X384P_60HZ_D147].h_total -
			1) - (tvin_vga_fmt_tbl[fmt-
			TVIN_SIG_FMT_VGA_512X384P_60HZ_D147].h_total >>
				MAX_AUTO_CLOCK_ORDER)))
	...
	case VGA_CLK_FINE_ADJ:
		...
		if (vga_auto->vs_cnt > AUTO_CLK_VS_CNT) {
			...
			tvafe_vga_get_h_border(vga_auto);
			/* get diff */
			diff = (signed int)vga_auto->border.hend -
			(signed int)vga_auto->border.hstart + (signed int)1 -
			(signed int)tvin_vga_fmt_tbl[fmt-
			TVIN_SIG_FMT_VGA_512X384P_60HZ_D147].h_active;
			/* get clk */
			clk = tvafe_vga_get_clock();
			if (!diff)
			...
			if (diff > 0) {
				if (vga_auto->adj_dir == 1) {
					if (clk > (tvin_vga_fmt_tbl[fmt-
			TVIN_SIG_FMT_VGA_512X384P_60HZ_D147].h_total - 1)) {
						tvafe_vga_auto_clock_adj(clk,
							diff);
					}
					vga_auto->clk_state = VGA_CLK_END;
				} else{
	...
	case VGA_CLK_EXCEPTION: /* stop auto */
		...
		if (adc_dbg_en)
			...
			__func__);
		tvafe_vga_set_clock(tvin_vga_fmt_tbl[fmt-
		TVIN_SIG_FMT_VGA_512X384P_60HZ_D147].h_total - 1);
		/* set spec clock value */
		tvafe_adc_set_frame_skip_number(adc, 2);
		tvafe_vga_border_detect_disable();
...
enum black_det_e tvin_vdin_get_bar(bool is_left, ushort v_active)
{
	uint val1 = 0, val2 = 0;
	enum black_det_e ret = UNKNOWN;
	if (is_left) {
		val1 = rd_bits(0, VDIN_BLKBAR_IND_LEFT1_CNT,
			BLKBAR_LEFT1_CNT_BIT, BLKBAR_LEFT1_CNT_WID);
		val2 = rd_bits(0, VDIN_BLKBAR_IND_LEFT2_CNT,
			BLKBAR_LEFT2_CNT_BIT, BLKBAR_LEFT2_CNT_WID);
	} else{
		val2 = rd_bits(0, VDIN_BLKBAR_IND_RIGHT1_CNT,
			BLKBAR_RIGHT1_CNT_BIT, BLKBAR_RIGHT1_CNT_WID);
		val1 = rd_bits(0, VDIN_BLKBAR_IND_RIGHT2_CNT,
	...
	if (abs(val1-val2) > v_active/black_bar_v2) {
		...
		if (val1+val2 >= 2*min(v_active*black_bar_v1/100,
			v_active-black_bar_white)) {
			ret = IS_BLACK;
		} else{
			ret = IS_VALID;
		}
...
void tvin_vdin_bar_detect(enum tvin_sig_fmt_e fmt,
			struct tvafe_adc_s *adc)
{
	const struct tvin_format_s *fmt_info = tvin_get_fmt_info(fmt);
	enum black_det_e l_status = UNKNOWN, r_status = UNKNOWN;
	static signed short l_step, r_step;
	static  unsigned int barhstart, barhend;
	static uint dir;
	static uint pre_status, cur_status;
	static uint time_out;
	...
	case BAR_INIT:
		...
		cur_status = 0;
		pre_status = 0;
		barhstart = rd_bits(0, VDIN_BLKBAR_H_START_END,
			BLKBAR_HSTART_BIT, BLKBAR_HSTART_WID);
		barhend   = rd_bits(0, VDIN_BLKBAR_H_START_END,
	...
	case BAR_TUNE:
		...
		if ((adc->vga_parm.hpos_step +
			(r_step - l_step) < (signed short)1-
			(signed short)fmt_info->hs_bp)
			|| (adc->vga_parm.hpos_step +
			(r_step - l_step) > (signed short)(fmt_info->h_total-
		fmt_info->hs_width-fmt_info->hs_bp-fmt_info->h_active-1))) {
			if (adc_dbg_en)
		...
		wr(0, VDIN_BLKBAR_H_START_END,
			(barhend-l_step)|(barhstart+r_step)<<16);
		cur_status = UNKNOWN;
		pre_status = UNKNOWN;
		b_status = BAR_TUNE;
		break;
...
void tvin_vdin_H_bar_detect(enum tvin_sig_fmt_e fmt,
					struct tvafe_adc_s *adc)
{
	unsigned int barhstart = 0, barhend = 0, hleft1 = 0,
					hleft2 = 0, hright1 = 0, hright2 = 0;
	const struct tvin_format_s *fmt_info_p;
	...
	hleft1 = rd_bits(0, VDIN_BLKBAR_IND_LEFT1_CNT,
		BLKBAR_LEFT1_CNT_BIT, BLKBAR_LEFT1_CNT_WID);
	hleft2 = rd_bits(0, VDIN_BLKBAR_IND_LEFT2_CNT,
		BLKBAR_LEFT2_CNT_BIT, BLKBAR_LEFT2_CNT_WID);

		if (hleft1 >= (fmt_info_p->v_active>>black_bar_v1) &&
			(hleft2 <= fmt_info_p->v_active>>black_bar_v2)) {
				adc->cmd_status = TVAFE_CMD_STATUS_SUCCESSFUL;
				if (cnt == 4)
	...
	if (adc->cmd_status == TVAFE_CMD_STATUS_SUCCESSFUL) {
		...
		wr(0, VDIN_BLKBAR_H_START_END, barhend|barhstart<<16);
	}
	if (adc_dbg_en)
		pr_info("[vdin..]%s:cnt %d hleft1 cnt %u, hleft2 cnt %u, hright1 cnt %u,hright2 cnt %u,barhend %u,barhstart %u.\n",
		__func__, cnt, hleft1, hleft2, hright1,
		hright2, barhend, barhstart);
}
/*
...
void tvin_vdin_bbar_init(enum tvin_sig_fmt_e fmt)
{
	...
	b_status = BAR_INIT;
	/* disable reset */
	wr_bits(0, VDIN_BLKBAR_CTRL0, 0,
			BLKBAR_DET_SOFT_RST_N_BIT, BLKBAR_DET_SOFT_RST_N_WID);

	wr_bits(0, VDIN_BLKBAR_CTRL0, white_thr_level,
			BLKBAR_BLK_LVL_BIT, BLKBAR_BLK_LVL_WID);
	wr_bits(0, VDIN_BLKBAR_CTRL0, 2,
		BLKBAR_DIN_SEL_BIT, BLKBAR_DIN_SEL_WID);
	wr(0, VDIN_SCIN_HEIGHTM1, (fmt_info_p->v_active - 1));

	wr_bits(0, VDIN_BLKBAR_CTRL0, 1,
			BLKBAR_SW_STAT_EN_BIT, BLKBAR_SW_STAT_EN_WID);
	wr_bits(0, VDIN_BLKBAR_CTRL0, 1,
		BLKBAR_H_WIDTH_BIT, BLKBAR_H_WIDTH_WID);
	wr_bits(0, VDIN_BLKBAR_H_START_END, (fmt_info_p->h_active - 1),
			BLKBAR_HEND_BIT, BLKBAR_HEND_WID);
	wr_bits(0, VDIN_BLKBAR_H_START_END, 0,
			BLKBAR_HSTART_BIT, BLKBAR_HSTART_WID);
	/* win_ve */
	wr_bits(0, VDIN_BLKBAR_V_START_END, (fmt_info_p->v_active - 1),
			BLKBAR_VEND_BIT, BLKBAR_VEND_WID);
	wr_bits(0, VDIN_BLKBAR_V_START_END, 0,
			BLKBAR_VSTART_BIT, BLKBAR_VSTART_WID);
	wr_bits(0, VDIN_BLKBAR_CTRL0, 1, BLKBAR_DET_TOP_EN_BIT,
			BLKBAR_DET_TOP_EN_WID);

	wr_bits(0, VDIN_BLKBAR_CTRL0, 1, BLKBAR_DET_SOFT_RST_N_BIT,
 ...
 */
static void tvafe_vga_auto_phase_handler(enum tvin_sig_fmt_e fmt,
							struct tvafe_adc_s *adc)
{
	...
	if (!fmt_info) {
		...
		case VGA_PHASE_INIT:
			...
			vga_parm->phase =
				(unsigned short)vga_auto->ap_pha_index;
			...
			if (++vga_auto->adj_cnt > VGA_AUTO_TRY_COUNTER) {
				...
				if (vga_auto->vs_cnt > AUTO_PHASE_VS_CNT) {
					...
					if (vga_auto->ap_max_diff < sum) {
						...
						vga_auto->ap_winmax_index =
							vga_auto->ap_win_index;
					}
					if (unlikely(++vga_auto->ap_win_index >
						VGA_PHASE_WIN_INDEX_MAX)){

						tvafe_adc_set_ap_window(fmt,
						vga_auto->ap_winmax_index);
						vga_auto->ap_max_diff = 0;
						vga_auto->phase_state =
							VGA_PHASE_ADJ;
		...
		case VGA_PHASE_ADJ:
			if (++vga_auto->adj_cnt > VGA_AUTO_TRY_COUNTER) {
				...
				if (vga_auto->vs_cnt > AUTO_PHASE_VS_CNT) {
					...
					sum = tvafe_vga_get_ap_diff();
					if (adc_dbg_en)
						pr_info("sum=%d,vga_auto->ap_pha_index=%d\n",
						sum, vga_auto->ap_pha_index);
					if (vga_auto->ap_max_diff <= sum) {

						vga_auto->ap_max_diff = sum;
						vga_auto->ap_phamax_index =
							vga_auto->ap_pha_index;
					}
					if (++vga_auto->ap_pha_index >
						VGA_ADC_PHASE_MAX) {

						/* tvafe_vga_set_phase(vga_auto
						->ap_phamax_index); */
						/* tvafe_adc_digital_reset();
						//added for phase
						abnormal bug. */
						vga_parm->phase =
				(unsigned short)vga_auto->ap_phamax_index;
						/* enable border detect */
						tvafe_vga_auto_phase_disable();
					tvafe_vga_border_detect_enable();
					tvafe_vga_border_detect_init(fmt);
						vga_auto->phase_state =
							VGA_PHASE_END;
					if (adc_dbg_en)
						pr_info("End:sum=%d,vga_auto->ap_phamax_index=%d\n",
		...
		case VGA_PHASE_END: /* auto position */
			...
			if (adc_dbg_en)
				pr_info("[tvafe..] %s:border detect end ! ve: %d,vs: %d,he: %d,hs: %d\n",
					__func__, vga_auto->border.vend,
					vga_auto->border.vstart,
			vga_auto->border.hend, vga_auto->border.hstart);
			if (((vga_auto->border.hend - vga_auto->border.hstart +
			1) > fmt_info->h_active) && (vga_auto->border.hend >=
			(fmt_info->hs_front+fmt_info->hs_width +
			fmt_info->hs_bp + fmt_info->h_active - 10))) {
				hs = fmt_info->hs_width + fmt_info->hs_bp;
				he = hs + fmt_info->h_active - 1;
			} else if (vga_auto->border.hstart <
			(fmt_info->hs_width + fmt_info->hs_bp)) {
				hs = vga_auto->border.hstart;
				he = hs + fmt_info->h_active - 1;
			} else if (((vga_auto->border.hend -
				vga_auto->border.hstart + 1) >=
				fmt_info->h_active) ||
				(vga_auto->border.hend > (fmt_info->hs_width +
				fmt_info->hs_bp + fmt_info->h_active - 1))) {
			...
			if (((vga_auto->border.vend - vga_auto->border.vstart +
			1) >= fmt_info->v_active) || (vga_auto->border.vend >
			(fmt_info->vs_width + fmt_info->vs_bp +
			fmt_info->v_active - 1))) {
				ve = vga_auto->border.vend;
				vs = ve - fmt_info->v_active + 1;
			} else if (vga_auto->border.vstart <
				(fmt_info->vs_width + fmt_info->vs_bp)) {
			...
			tvafe_vga_set_v_pos(vs, ve,
			tvin_vga_fmt_tbl[tvinfo->fmt -
			TVIN_SIG_FMT_VGA_512X384P_60HZ_D147].
			scan_mode);*/
 ...
 */
void tvafe_vga_auto_adjust_handler(struct tvin_parm_s *parm,
		struct tvafe_adc_s *adc)
{
	enum tvin_port_e port = parm->port;
	enum tvin_sig_fmt_e fmt = parm->info.fmt;
...
void tvafe_adc_configure(enum tvin_sig_fmt_e fmt)
{
	...
	} else if (fmt < TVIN_SIG_FMT_COMP_MAX &&
			fmt > TVIN_SIG_FMT_VGA_THRESHOLD){
		/* Component formats */

		buff_t =
		adc_component_table[fmt-TVIN_SIG_FMT_COMP_480P_60HZ_D000];
	...
	if (fmt > TVIN_SIG_FMT_VGA_MAX &&
		fmt < TVIN_SIG_FMT_COMP_MAX && enable_dphase){

		W_APB_BIT(ADC_REG_56,
			comp_phase[fmt-TVIN_SIG_FMT_VGA_THRESHOLD - 1],
				CLKPHASEADJ_BIT, CLKPHASEADJ_WID);
	}
	...
	aml_write_cbus(PERIPHS_PIN_MUX_0 ,
				aml_read_cbus(PERIPHS_PIN_MUX_0)&0xcff0ffdf);
	aml_write_cbus(PERIPHS_PIN_MUX_1 ,
				aml_read_cbus(PERIPHS_PIN_MUX_1)&0xfc017fff);
	aml_write_cbus(PERIPHS_PIN_MUX_2 ,
				aml_read_cbus(PERIPHS_PIN_MUX_2)&0xe001ffff);
	aml_write_cbus(PERIPHS_PIN_MUX_3 ,
				aml_read_cbus(PERIPHS_PIN_MUX_3)&0xfc000000);
	aml_write_cbus(PERIPHS_PIN_MUX_4 ,
				aml_read_cbus(PERIPHS_PIN_MUX_4)&0xff8007ff);
	aml_write_cbus(PERIPHS_PIN_MUX_6 ,
				aml_read_cbus(PERIPHS_PIN_MUX_6)&0xffffffbf);
	aml_write_cbus(PERIPHS_PIN_MUX_7 ,
				aml_read_cbus(PERIPHS_PIN_MUX_7)&0xff00003f);
	aml_write_cbus(PERIPHS_PIN_MUX_10,
				aml_read_cbus(PERIPHS_PIN_MUX_10)&0xffffffb3);
	/* enable TVFE_TEST mux on test pins 0~27 & 30 */
	WRITE_CBUS_REG(PERIPHS_PIN_MUX_9 , 0x4fffffff);/*  */
#endif
}

void tvafe_adc_comphase_pr(void)
{
	int i = 0;
	for (i = 0; i < TVIN_SIG_FMT_COMP_MAX-TVIN_SIG_FMT_VGA_THRESHOLD-1; i++)
		pr_info("[tvafe..] %s phase 0x%x.\n",
		tvin_sig_fmt_str(i+TVIN_SIG_FMT_VGA_MAX+1), comp_phase[i]);
}

void tvafe_adc_set_deparam(struct tvin_parm_s *parm,
		struct tvafe_adc_s *adc)
{
	enum tvin_sig_fmt_e fmt = parm->info.fmt;
	/* tvafe_vga_set_clock(tvin_vga_fmt_tbl[fmt -
	TVIN_SIG_FMT_VGA_512X384P_60HZ_D147].h_total - 1); */
	tvafe_vga_set_h_pos(tvin_vga_fmt_tbl[fmt-
	TVIN_SIG_FMT_VGA_512X384P_60HZ_D147].hs_width +
	tvin_vga_fmt_tbl[fmt-TVIN_SIG_FMT_VGA_512X384P_60HZ_D147].hs_bp,
	tvin_vga_fmt_tbl[fmt-TVIN_SIG_FMT_VGA_512X384P_60HZ_D147].hs_width +
	tvin_vga_fmt_tbl[fmt-TVIN_SIG_FMT_VGA_512X384P_60HZ_D147].hs_bp +
	tvin_vga_fmt_tbl[fmt-TVIN_SIG_FMT_VGA_512X384P_60HZ_D147].h_active - 1);
	tvafe_vga_set_v_pos(tvin_vga_fmt_tbl[fmt-
	TVIN_SIG_FMT_VGA_512X384P_60HZ_D147].vs_bp +
	tvin_vga_fmt_tbl[fmt-TVIN_SIG_FMT_VGA_512X384P_60HZ_D147].vs_width,
	tvin_vga_fmt_tbl[fmt-TVIN_SIG_FMT_VGA_512X384P_60HZ_D147].vs_bp +
	tvin_vga_fmt_tbl[fmt-TVIN_SIG_FMT_VGA_512X384P_60HZ_D147].vs_width +
	tvin_vga_fmt_tbl[fmt-TVIN_SIG_FMT_VGA_512X384P_60HZ_D147].v_active - 1,
	tvin_vga_fmt_tbl[fmt-TVIN_SIG_FMT_VGA_512X384P_60HZ_D147].scan_mode);
	if (adc_parm_en)
 ...
 */
void tvafe_adc_set_param(struct tvin_parm_s *parm,
				struct tvafe_adc_s *adc)
{
	enum tvin_sig_fmt_e fmt = parm->info.fmt;
	struct tvafe_vga_parm_s *vga_parm = &adc->vga_parm;
	...
	data = (signed int)(vga_parm->clk_step);
/* * (tvin_fmt_tbl[fmt].h_total - 1)>>12);  //htotal/2^12 */
	tmp = (unsigned int)((signed int)(tvin_vga_fmt_tbl[fmt-
	TVIN_SIG_FMT_VGA_512X384P_60HZ_D147].h_total - 1) + data);
	...
	data = (signed int)tvin_vga_fmt_tbl[fmt-
		TVIN_SIG_FMT_VGA_512X384P_60HZ_D147].hs_bp;
	if (step + data < 0) {

		hs = tvin_vga_fmt_tbl[fmt-
			TVIN_SIG_FMT_VGA_512X384P_60HZ_D147].hs_width
			+ (unsigned int)((data + step)%8 + 8);
		he = (unsigned int)((signed int)tvin_vga_fmt_tbl[fmt-
			TVIN_SIG_FMT_VGA_512X384P_60HZ_D147].hs_width +
			(signed int)tvin_vga_fmt_tbl[fmt-
			TVIN_SIG_FMT_VGA_512X384P_60HZ_D147].h_active
				+ data + step) - 1;
		...
		hs = (unsigned int)((signed int)tvin_vga_fmt_tbl[fmt-
			TVIN_SIG_FMT_VGA_512X384P_60HZ_D147].hs_width +
			data + step);
		he = hs + tvin_vga_fmt_tbl[fmt-
			TVIN_SIG_FMT_VGA_512X384P_60HZ_D147].h_active - 1;
		/* avoid he is bigger than h total */
		he = he > (tvin_vga_fmt_tbl[fmt-
		TVIN_SIG_FMT_VGA_512X384P_60HZ_D147].h_total - 1) ?
		(tvin_vga_fmt_tbl[fmt-
		TVIN_SIG_FMT_VGA_512X384P_60HZ_D147].h_total - 1):he;
	}
	if (adc_parm_en)
	...
	if (step + data - vbp_offset < 0) {
		...
		vs = tvin_vga_fmt_tbl[fmt-
			TVIN_SIG_FMT_VGA_512X384P_60HZ_D147].vs_width +
			vbp_offset;
		ve = (unsigned int)((signed int)vs +
			(signed int)tvin_vga_fmt_tbl[fmt-
			TVIN_SIG_FMT_VGA_512X384P_60HZ_D147].v_active +
			data + step - vbp_offset) - 1;
	 /* avoid ve is bigger than v total */
	} else if ((1 == tvin_vga_fmt_tbl[fmt-
		TVIN_SIG_FMT_VGA_512X384P_60HZ_D147].vs_front) && (0 < step)) {

		vs = (unsigned int)((signed int)tvin_vga_fmt_tbl[fmt-
			TVIN_SIG_FMT_VGA_512X384P_60HZ_D147].vs_width +
			data + 1);
		ve = vs + tvin_vga_fmt_tbl[fmt-
			TVIN_SIG_FMT_VGA_512X384P_60HZ_D147].v_active - 1;

	} else if (step + 1 > tvin_vga_fmt_tbl[fmt-
		TVIN_SIG_FMT_VGA_512X384P_60HZ_D147].vs_front){

		vs = (unsigned int)((signed int)tvin_vga_fmt_tbl[fmt-
			TVIN_SIG_FMT_VGA_512X384P_60HZ_D147].vs_width + data +
			tvin_vga_fmt_tbl[fmt-
			TVIN_SIG_FMT_VGA_512X384P_60HZ_D147].vs_front - 1);
		ve = vs + tvin_vga_fmt_tbl[fmt-
			TVIN_SIG_FMT_VGA_512X384P_60HZ_D147].v_active - 1;
		if (adc_parm_en)
			pr_info("[tvafe..] %s: vstep %u>vs_front enable %u cut window.\n",
			__func__, step, tvin_vga_fmt_tbl[fmt-
			TVIN_SIG_FMT_VGA_512X384P_60HZ_D147].vs_front);
	...
	if (adc_parm_en)
		...
		__func__, step, vs, ve);
	tvafe_vga_set_v_pos(vs, ve, tvin_vga_fmt_tbl[fmt-
		TVIN_SIG_FMT_VGA_512X384P_60HZ_D147].scan_mode);
 ...
 */
void tvafe_adc_get_param(struct tvin_parm_s *parm,
struct tvafe_adc_s *adc)
{
	enum tvin_sig_fmt_e fmt = parm->info.fmt;
	struct tvafe_vga_parm_s *vga_parm = &adc->vga_parm;
	...
	if (adc_parm_en)
		...
		__func__, vga_parm->clk_step, vga_parm->phase,
		vga_parm->hpos_step, vga_parm->vpos_step);
