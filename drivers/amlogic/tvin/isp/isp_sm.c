...
static unsigned int capture_debug;
...
static unsigned int af_sm_dg;

/* volatile struct isp_ae_to_sensor_s ae_sens; */

struct isp_ae_to_sensor_s ae_sens;

static inline int isp_ae_find_step(struct cam_function_s *func,
		unsigned int low, unsigned int hign, unsigned int gain)
{
	unsigned int mid = 0;
	unsigned int rate = 0;
...
static unsigned int aet_gain_pre = 0, format_gain_pre;

static unsigned int isp_ae_cal_new_para(struct isp_dev_s *devp)
{
	...
	if (aet_gain_new == 0) {
		pr_info("[isp] %s: cal ae error, aet_gain_pre:%d, ",
				__func__, aet_gain_pre);
		pr_info("format_gain_pre:%d ... ...\n", format_gain_pre);
		return 0;
	}
	ae_sens.new_step = isp_ae_find_step(func, 0,
			aepa->max_step, aet_gain_new);
	ae_sens.shutter = 1;
	ae_sens.gain = 1;
	pr_info("[isp] %s: format_gain_new:%d,", __func__,
			format_gain_new);
	pr_info(" aet_gain_new:%d new_step:%d... ...\n",
			aet_gain_new, ae_sens.new_step);

	if (func && func->set_aet_new_step) {
		func->set_aet_new_step(func->priv_data, ae_sens.new_step,
				ae_sens.shutter, ae_sens.gain);
...
unsigned int isp_awb_load_pre_para(struct isp_dev_s *devp)
{
	if ((awb_gain_pre.r_val == 0) &&
		(awb_gain_pre.r_val == 0) &&
		(awb_gain_pre.r_val == 0)) {
		pr_info("[isp] %s cal awb error, r:%d g:%d b:%d ... ...\n",
				__func__, awb_gain.r_val,
				awb_gain.g_val, awb_gain.b_val);
		return 0;
	}
...
void isp_sm_init(struct isp_dev_s *devp)
{
...
void isp_set_manual_exposure(struct isp_dev_s *devp)
{
...
void af_sm_init(struct isp_dev_s *devp)
{
...
int isp_ae_save_current_para(struct isp_dev_s *devp)
{
	struct cam_function_s *func = &devp->cam_param->cam_function;
	struct isp_ae_sm_s *aepa = &sm_state.isp_ae_parm;
	aepa->max_step = func->get_aet_max_step(func->priv_data);
	...
	if (func && func->get_aet_gain_by_step)
		aet_gain_pre = func->get_aet_gain_by_step(func->priv_data,
			ae_sens.new_step);
...
unsigned int isp_tune_exposure(struct isp_dev_s *devp)
{
	struct isp_ae_sm_s *aepa = &sm_state.isp_ae_parm;
	struct cam_function_s *func = &devp->cam_param->cam_function;
...
void isp_ae_sm(struct isp_dev_s *devp)
{
	...
	unsigned int targrate, targstep, avg, avgo, i,
		radium_delta, lpfcoef, sub_avg[16] = {0};
	...
	case AE_INIT:
		...
		isp_set_ae_win(aepa->win_l, aepa->win_r,
				aepa->win_t, aepa->win_b);
		aepa->pixel_sum = parm->h_active * parm->v_active;
		aepa->sub_pixel_sum = aepa->pixel_sum >> 4;
		aepa->max_lumasum1 = ((aepa->pixel_sum >> 4) *
				aep->ae_ratio_low) >> 7;
		aepa->max_lumasum2 = ((aepa->pixel_sum >> 4) *
				aep->ae_ratio_low2mid) >> 7;
		aepa->max_lumasum3 = ((aepa->pixel_sum >> 4) *
				aep->ae_ratio_mid2high) >> 7;
		aepa->max_lumasum4 = ((aepa->pixel_sum >> 4) *
				aep->ae_ratio_high) >> 7;
		if (devp->ae_info.manul_level == 0)
			aepa->targ = aep->targetmid;
		targ_ag_adjust_dir = 1;/*0:up; 2:down; 1:stable*/
		if ((func == NULL) ||
			(func->get_aet_max_gain == NULL) ||
			(func->get_aet_min_gain == NULL) ||
			(func->get_aet_max_step == NULL)) {
			pr_info("[tvin isp error][AE_INIT]%s:", __func__);
			pr_info("AE FUNC POINT TO NULL!\n");
			break;
		} else {
			aepa->max_gain =
				func->get_aet_max_gain(func->priv_data);
			aepa->min_gain =
				func->get_aet_min_gain(func->priv_data);
			aepa->max_step =
				func->get_aet_max_step(func->priv_data);
		}
		pr_info("ae,win_l=%d,win_r=%d,win_t=%d,win_b=%d\n",
				aepa->win_l, aepa->win_r,
				aepa->win_t, aepa->win_b);
		pr_info("aepa->max_lumasum1=%d,max_lumasum2=%d,=%d,=%d",
				aepa->max_lumasum1, aepa->max_lumasum2,
				aepa->max_lumasum3, aepa->max_lumasum4);
		pr_info("aepa->alert_r=%d,g=%d,b=%d\n",
				aepa->alert_r, aepa->alert_g,
				aepa->alert_b);
		pr_info("aepa->sub_pixel_sum=%d,aepa->max_gain=%d,",
				aepa->sub_pixel_sum, aepa->max_gain);
		pr_info("aepa->min_gain=%d,aepa->max_step=%d\n",
				aepa->min_gain, aepa->max_step);
		sm_state.isp_ae_parm.isp_ae_state = AE_SHUTTER_ADJUST;
		break;
	case AE_SHUTTER_ADJUST:
		if ((func == NULL) ||
			(func->get_aet_current_gain == NULL) ||
			(func->get_aet_current_step == NULL)) {
			pr_info("[tvin isp error][AE_SHUTTER_ADJUST]%s:",
					__func__);
			pr_info("AE FUNC POINT TO NULL!\n");
			break;
		} else {
			aepa->cur_gain =
				func->get_aet_current_gain(func->priv_data);
			aepa->pre_gain = aepa->cur_gain;
			aepa->cur_step =
				func->get_aet_current_step(func->priv_data);
		}
		...
		for (i = 0; i < 16; i++) {
			...
			if ((i == ae_step) &&
				(ae_sm_dg&AE_CALCULATE_LUMA_AVG_DG)) {
				pr_info("sub_avg[%d]=%d,ae->luma_win=%d,",
						i, sub_avg[i], ae->luma_win[i]);
				pr_info("aep->coef_cur=%d,aep->coef_env=%d,",
					aep->coef_cur[i], aep->coef_env[i]);
				pr_info("avg_sum=%d\n", avg_sum);
			}
		}
		avg = avg_sum >> 10;
		...
		/*calculate luma targ*/
		sum = (ph->hist.gamma[58]+
				ph->hist.gamma[57]) << ph->hist.hist_pow;
		if (ae_sm_dg&AE_CALCULATE_LUMA_TARG_DG) {
			pr_info("avg=%d,aepa->cur_gain=%d,",
					avg, aepa->cur_gain);
			pr_info("avg_sum=%d,luma_win[9]=%d,",
					avg_sum, ae->luma_win[9]);
			pr_info("sub[9]=%d\n", sub_avg[9]);
			pr_info("ph->hist.gamma[58]=%d,%d,%d\n",
				ph->hist.gamma[58], sum, ph->hist.hist_pow);
		}
		if ((devp->ae_info.manul_level == 0) &&
			((ae_sm_dg&AE_DYNAMIC_ADJUST_DG) == 0)) {
			if ((sum > aepa->max_lumasum4) ||
				((sum > aepa->max_lumasum3) &&
				(targ_ag_adjust_dir == 2))) {
				targ_ag_adjust_dir = 2;
				aepa->targ -= 2;
				if (aepa->targ
					< (aep->targetmid-aep->ae_min_diff))
					aepa->targ =
						aep->targetmid-aep->ae_min_diff;
			} else if ((sum < aepa->max_lumasum1) ||
				((sum < aepa->max_lumasum2) &&
					(targ_ag_adjust_dir == 0))) {
				targ_ag_adjust_dir = 0;
				aepa->targ++;
				if (aepa->targ
					> (aep->targetmid+aep->ae_max_diff))
					aepa->targ =
						aep->targetmid+aep->ae_max_diff;
			} else
				targ_ag_adjust_dir = 1;
		}
		radium_delta = (avg > aepa->targ) ?
				(avg - aepa->targ):(aepa->targ - avg);
		sm_state.env = ENV_LOW;
		/* isp_set_ae_thrlpf(aep->thr_r_low,
		 * aep->thr_g_low, aep->thr_b_low, aep->lpftype_low); */
		if (ae_sm_dg&AE_CALCULATE_LUMA_TARG_DG)
			pr_info("avg=%d,targ=%d,targ_ag_adjust_dir=%d\n",
					avg, aepa->targ, targ_ag_adjust_dir);
		/*luma avg check*/
		if (((sm_state.status == ISP_AE_STATUS_UNSTABLE) &&
			(radium_delta > aep->radium_inner_l)) ||
		...
		lpfcoef = (devp->flag & ISP_FLAG_CAPTURE) ?
				aep->fast_lpfcoef:aep->slow_lpfcoef;
		newstep = (aepa->cur_step*lpfcoef +
				targstep*(256-lpfcoef))>>8;
		if (ae_sm_dg&AE_EXPOSURE_ADJUST_DG) {
			pr_info("targstep = %d,targrate =%d,",
					targstep, targrate);
			pr_info("newstep =%d,lpf =%d,cur_step =%d\n",
		...
		if (aep->ae_skip[0] == 0x1) {
			...
			if (ae_sm_dg&AE_GAIN_ADJUST_SEL)
				sm_state.isp_ae_parm.isp_ae_state =
					AE_GAIN_ADJUST;
			else
				sm_state.isp_ae_parm.isp_ae_state =
					AE_SHUTTER_ADJUST;
...
/* -128	1.164  2.018  0		 0 */
static inline int matrix_yuv601_rgb_r(unsigned int y,
		unsigned int u, unsigned int v)
{
	return ((y-16)*1192+(u-128)*0+(v-128)*1634+0) >> 10;
}

static inline int matrix_yuv601_rgb_g(unsigned int y,
		unsigned int u, unsigned int v)
{
	return ((y-16)*1192+(u-128)*(-400)+(v-128)*(-833)+0) >> 10;
}

static inline int matrix_yuv601_rgb_b(unsigned int y,
		unsigned int u, unsigned int v)
{
...
/* -128	1.164  2.115  0		 0 */
static inline int matrix_yuv709_rgb_r(unsigned int y,
		unsigned int u, unsigned int v)
{
	return ((y-16)*1192+(u-128)*0+(v-128)*1836+0) >> 10;
}

static inline int matrix_yuv709_rgb_g(unsigned int y,
		unsigned int u, unsigned int v)
{
	return ((y-16)*1192+(u-128)*(-218)+(v-128)*(-547)+0) >> 10;
}

static inline int matrix_yuv709_rgb_b(unsigned int y,
		unsigned int u, unsigned int v)
{
...
int isp_awb_save_current_para(struct isp_dev_s *devp)
{
	isp_awb_get_gain(&awb_gain_pre);

	pr_info("[isp] %s save awb, r:%d g:%d b:%d ... ...\n",
		__func__, awb_gain_pre.r_val,
		awb_gain_pre.g_val, awb_gain_pre.b_val);

	return 0;
}
void isp_awb_sm(struct isp_dev_s *devp)
{
	...
	unsigned int rg, bg, target_r, target_b, i,
		yy, countlimity, cnt, u, v;
	struct awb_yuv_stat_s yuv_ex[4];
	...
	case AWB_INIT:
		awba->win_l = (parm->h_active *
				awbp->ratio_winl) >> 10;
		awba->win_r = ((parm->h_active *
				awbp->ratio_winr) >> 10) - 1;
		awba->win_t = (parm->v_active *
				awbp->ratio_wint) >> 10;
		awba->win_b = ((parm->v_active *
				awbp->ratio_winb) >> 10) - 1;
		pr_info("awb,win_l=%d,win_r=%d,win_t=%d,win_b=%d\n",
				awba->win_l, awba->win_r,
				awba->win_t, awba->win_b);
		isp_set_awb_win(awba->win_l, awba->win_r,
				awba->win_t, awba->win_b);
		if (sm_state.env == ENV_HIGH) {
			isp_set_awb_yuv_thr(awbp->thr_yh_h,
					awbp->thr_yl_h, awbp->thr_du_h,
					awbp->thr_dv_h);
			isp_set_awb_rgb_thr(awbp->thr_gb_h,
					awbp->thr_gr_h, awbp->thr_br_h);
		} else if (sm_state.env == ENV_MID) {
			isp_set_awb_yuv_thr(awbp->thr_yh_m,
					awbp->thr_yl_m, awbp->thr_du_m,
					awbp->thr_dv_m);
			isp_set_awb_rgb_thr(awbp->thr_gb_m,
					awbp->thr_gr_m, awbp->thr_br_m);
		} else {/*sm_state.env == ENV_LOW or ENV_NULL*/
			isp_set_awb_yuv_thr(awbp->thr_yh_l,
					awbp->thr_yl_l, awbp->thr_du_l,
					awbp->thr_dv_l);
			isp_set_awb_rgb_thr(awbp->thr_gb_l,
					awbp->thr_gr_l, awbp->thr_br_l);
		}
		awba->pixel_sum = parm->h_active * parm->v_active;
		awba->countlimitrgb = ((awba->pixel_sum >> 2) *
				awbp->ratio_rgb) >> 6;
		awba->countlimityh = ((awba->pixel_sum >> 2) *
				awbp->ratio_yh) >> 6;
		awba->countlimitym = ((awba->pixel_sum >> 2) *
				awbp->ratio_ym) >> 6;
		awba->countlimityl = ((awba->pixel_sum >> 2) *
				awbp->ratio_yl) >> 6;
	...
	case AWB_CHECK:
		...
		for (i = 0; i < 3; i++) {
			if (i == 0) {
				...
				memcpy(yuv_ex, awb->yuv_mid,
					4*sizeof(struct awb_yuv_stat_s));
			} else if (i == 1) {
				yy = awbp->yyh;
				countlimity = awba->countlimityh;
				memcpy(yuv_ex, awb->yuv_high,
					4*sizeof(struct awb_yuv_stat_s));
			} else if (i == 2) {
				yy = awbp->yyl;
				countlimity = awba->countlimityl;
				memcpy(yuv_ex, awb->yuv_low,
					4*sizeof(struct awb_yuv_stat_s));
			}
			...
			if (cnt >= countlimity) {
				...
				if (parm->v_active >= 720) {
					r[i+1] =
					matrix_yuv709_rgb_r(yy, u+128, v+128);
					g[i+1] =
					matrix_yuv709_rgb_g(yy, u+128, v+128);
					b[i+1] =
					matrix_yuv709_rgb_b(yy, u+128, v+128);
				} else {
					r[i+1] =
					matrix_yuv601_rgb_r(yy, u+128, v+128);
					g[i+1] =
					matrix_yuv601_rgb_g(yy, u+128, v+128);
					b[i+1] =
					matrix_yuv601_rgb_b(yy, u+128, v+128);
				}
			} else {
				r[i+1] = 0;
				g[i+1] = 0;
		...
		/*AWB_RGB_BLEND*/
		r[4] = (r[0]*awbp->coef_r[0]+r[1]*awbp->coef_r[1]+
				r[2]*awbp->coef_r[2]+r[3]*awbp->coef_r[3])>>8;
		g[4] = (g[0]*awbp->coef_g[0]+g[1]*awbp->coef_g[1]+
				g[2]*awbp->coef_g[2]+g[3]*awbp->coef_g[3])>>8;
		b[4] = (b[0]*awbp->coef_b[0]+b[1]*awbp->coef_b[1]+
				b[2]*awbp->coef_b[2]+b[3]*awbp->coef_b[3])>>8;
		if (awb_sm_dg&AWB_RGB_BLEND_DG) {
			pr_info("r=%d,%d,%d,%d,%d,g=%d,%d,%d,%d,%d,",
					 r[0], r[1], r[2], r[3], r[4],
					 g[0], g[1], g[2], g[3], g[4]);
			pr_info("b=%d,%d,%d,%d,%d\n",
					 b[0], b[1], b[2], b[3], b[4]);
		...
		if (awb_sm_dg&AWB_TEMP_CHECK_DG)
			pr_info("rg=%d,bg=%d\n", rg, bg);
		if (((awba->status == ISP_AWB_STATUS_UNSTABLE) &&
			((rg > 1024 + awbp->inner_rg) ||
			(rg < 1024 - awbp->inner_rg))) ||
			((rg > 1024 + awbp->outer_rg) ||
				(rg < 1024 - awbp->outer_rg)) ||
			((awba->status == ISP_AWB_STATUS_UNSTABLE) &&
				((bg > 1024 + awbp->inner_bg) ||
				(bg < 1024 - awbp->inner_bg))) ||
				((bg > 1024 + awbp->outer_bg) ||
				(bg < 1024 - awbp->outer_bg))
			) {
			awba->status = ISP_AWB_STATUS_UNSTABLE;
			...
			if (awb_sm_dg&AWB_TEMP_ADJUST_DG) {
				pr_info("cur_r_val=%d,cur_b_val=%d,",
						awb_gain.r_val, awb_gain.b_val);
				pr_info("target_r=%d,target_b=%d\n",
						target_r, target_b);
			}
			target_r = (target_r > awbp->r_max) ?
					awbp->r_max : target_r;
			target_r = (target_r < awbp->r_min) ?
					awbp->r_min : target_r;
			target_b = (target_b > awbp->b_max) ?
					awbp->b_max : target_b;
			target_b = (target_b < awbp->b_min) ?
					awbp->b_min : target_b;
			if (awb_sm_dg&AWB_GAIN_SET_DG)
				isp_awb_set_gain(target_r, awb_gain.g_val,
						target_b);
		} else {
			awba->status = ISP_AWB_STATUS_STABLE;
			break;
...
unsigned long long div64(unsigned long long n, unsigned long long d)
{
	unsigned int n_bits = 0, d_bits = 0, i = 0;
	/* q for quotient, t for temporary */
	unsigned long long q = 0, t = 0;
...
static unsigned long long isp_abs64(unsigned long long a,
		unsigned long long b)
{
	return a > b?(a-b):(b-a);
}
static unsigned long long get_fv_base_blnr(struct isp_blnr_stat_s *blnr)
{
	...
	mul_ac = (sum_ac > 0x00000000ffffffff) ?
			0xffffffffffffffff : sum_ac*sum_ac;
	sum_ac = (unsigned long long)blnr->af_ac[0];
	return sum_ac;
	/* return div64(mul_ac,sum_dc); */
}
static bool isp_af_is_lost_focus(struct isp_af_info_s *af_info,
		struct xml_algorithm_af_s *af_alg)
{
	unsigned long long *v_dc, sum_vdc = 0, ave_vdc = 0,
			delta_dc = 0, tmp_vdc = 0;
	...
	/* during hysteresis ,still last state*/
		...
		if (af_sm_dg&AF_DETECT) {
			pr_info("ave_vdc:%llu,af_info->last_move:%d ",
					ave_vdc, af_info->last_move);
			pr_info("keep last state.\n");
		}
		return false;
	}
	if (af_sm_dg&AF_DETECT)
...
void isp_af_detect(struct isp_dev_s *devp)
{
	...
	case AF_DETECT_INIT:
		isp_set_blenr_stat(af_info->x0, af_info->y0,
				af_info->x1, af_info->y1);
		af_info->cur_index = 0;
		sm_state.af_state = AF_GET_STEPS_INFO;
		break;
	case AF_GET_STEPS_INFO:
		memcpy(&af_info->af_detect[af_info->cur_index],
				&af_info->isr_af_data,
				sizeof(struct isp_blnr_stat_s));
...
static unsigned int check_hillside(struct isp_af_info_s *af_info,
		struct xml_algorithm_af_s *af_alg)
{
	unsigned int cur_ac, last_ac, delta_ac, delta_ac_ratio, ret;
	unsigned int valid_step_cnt = af_info->valid_step_cnt;

	if (valid_step_cnt < 2)
		return 0;/* avoid first step */
	cur_ac = af_info->af_fine_data[valid_step_cnt - 1].af_data.af_ac[0];
	last_ac = af_info->af_fine_data[valid_step_cnt - 2].af_data.af_ac[0];
	...
	if (af_sm_dg&AF_FINE_TUNE) {
		pr_info("[check hillside]delta_ac_ratio:%d,",
				 delta_ac_ratio);
		pr_info("cur_ac:%d,last_ac:%d .\n", cur_ac, last_ac);
	}
	if ((last_ac > cur_ac) &&
		(delta_ac_ratio > af_alg->hillside_fall)) {
		ret = 1;/* fall fillside */
	} else if ((last_ac < cur_ac) &&
		(delta_ac_ratio > af_alg->hillside_fall)) {
...
static unsigned int isp_af_get_fine_step(struct isp_af_info_s *af_info,
		struct xml_algorithm_af_s *af_alg)
{
	unsigned int i = 0, j = 0, cur_grid = 0, max_grid = 0, best_step = 0;
	unsigned long long delta_fv, fv[FOCUS_GRIDS], max_fv = 0,
			min_fv = 0xffffffffffffffff, sum_fv = 0, moment = 0;
	struct isp_af_fine_tune_s af_fine_data_ex;

	for (i = 0; i < af_info->valid_step_cnt; i++) {
		for (j = i+1; j < af_info->valid_step_cnt; j++) {
			if (af_info->af_fine_data[j].cur_step <
					af_info->af_fine_data[i].cur_step) {
				memcpy(&af_fine_data_ex,
					&af_info->af_fine_data[i],
					sizeof(struct isp_af_fine_tune_s));
				memcpy(&af_info->af_fine_data[i],
					&af_info->af_fine_data[j],
					sizeof(struct isp_af_fine_tune_s));
				memcpy(&af_info->af_fine_data[j],
					&af_fine_data_ex,
					sizeof(struct isp_af_fine_tune_s));
			}
		}
	...
	if (af_sm_dg&AF_BEST_STEP) {
		pr_info("%s ac[0] ac[1] ac[2] ac[3] ", __func__);
		pr_info("dc[0] dc[1] dc[2] dc[3] ");
		pr_info("af0_ac af1_ac\n");
	}
	for (i = 0; i < af_info->valid_step_cnt; i++) {
		...
		if (af_sm_dg&AF_BEST_STEP)
			pr_info("%s %u %u %u %u %u %u %u %u %u %u\n",
					__func__,
				af_info->af_fine_data[i].af_data.ac[0],
				af_info->af_fine_data[i].af_data.ac[1],
				af_info->af_fine_data[i].af_data.ac[2],
				af_info->af_fine_data[i].af_data.ac[3],
				af_info->af_fine_data[i].af_data.dc[0],
				af_info->af_fine_data[i].af_data.dc[1],
				af_info->af_fine_data[i].af_data.dc[2],
				af_info->af_fine_data[i].af_data.dc[3],
				af_info->af_fine_data[i].af_data.af_ac[0],
				af_info->af_fine_data[i].af_data.af_ac[1]);
		if (max_fv < fv[i]) {
			max_fv = fv[i];
			cur_grid = i;
		}
	...
	/* too much stroke */
		...
		moment += fv[cur_grid - 1]*
		(unsigned long long)af_info->af_fine_data[cur_grid -
		    1].cur_step;
		moment += fv[cur_grid]*
		(unsigned long long)af_info->af_fine_data[cur_grid].cur_step;
		moment += fv[cur_grid + 1]*
		(unsigned long long)af_info->af_fine_data[cur_grid +
		    1].cur_step;
		...
		moment += (unsigned long long)fv[cur_grid - 2]*
		(unsigned long long)af_info->af_fine_data[cur_grid -
		    2].cur_step;
		moment += (unsigned long long)fv[cur_grid - 1]*
		(unsigned long long)af_info->af_fine_data[cur_grid -
		    1].cur_step;
		moment += (unsigned long long)fv[cur_grid]*
		(unsigned long long)af_info->af_fine_data[cur_grid].cur_step;
		moment += (unsigned long long)fv[cur_grid + 1]*
		(unsigned long long)af_info->af_fine_data[cur_grid +
		    1].cur_step;
...
void isp_af_fine_tune(struct isp_dev_s *devp)
{
	static unsigned int af_delay;
	struct xml_algorithm_af_s *af_alg = devp->isp_af_parm;
	struct isp_af_info_s *af_info = &devp->af_info;
	...
	case AF_SCAN_INIT:
		...
		memset(af_info->af_fine_data, 0,
				FOCUS_GRIDS*sizeof(struct isp_af_fine_tune_s));
		if (af_info->cur_step <
			(af_alg->step[af_alg->valid_step_cnt - 1] +
					af_alg->step[0])/2) {
			af_info->cur_index = 0;
	...
	case AF_GET_COARSE_INFO_H:/* from H step --> L step */
		...
		if ((atomic_read(&af_info->writeable) <= 0) &&
			(af_info->cur_index ==
			    (af_alg->valid_step_cnt - 1)) &&
			(af_info->cur_step !=
			    af_alg->step[af_alg->valid_step_cnt - 1])) {
			if (af_info->cur_step <
				af_alg->step[af_alg->valid_step_cnt - 1]) {
				if ((af_alg->step[af_alg->valid_step_cnt - 1] -
					af_info->cur_step)
					> af_alg->jump_offset)
					af_info->cur_step =
					af_info->cur_step + af_alg->jump_offset;
				else
					af_info->cur_step =
					af_alg->step[af_info->cur_index];
			} else
				af_info->cur_step =
					af_alg->step[af_info->cur_index];
		...
		/*get isp af info*/
		if ((atomic_read(&af_info->writeable) <= 0) &&
			(af_delay >= af_alg->field_delay)) {
			af_info->valid_step_cnt++;
			af_info->af_fine_data[af_info->valid_step_cnt -
				1].cur_step = af_info->cur_step;
			memcpy(&af_info->af_fine_data[af_info->valid_step_cnt -
				1].af_data,
				&af_info->isr_af_data,
				sizeof(struct isp_blnr_stat_s));
			if ((af_info->cur_index == 0) ||
				(check_hillside(af_info, af_alg) == 1)) {
				sm_state.af_state = AF_GET_FINE_INFO;
				af_info->great_step =
					isp_af_get_fine_step(af_info, af_alg);
			} else {
				af_info->cur_index--;
				af_info->cur_step =
					af_alg->step[af_info->cur_index];
	...
	case AF_GET_COARSE_INFO_L:/* from L step --> H step */
		...
		if ((atomic_read(&af_info->writeable) <= 0) &&
			(af_info->cur_index == 0) &&
		(af_info->cur_step != af_alg->step[0])) {
	...
	case AF_CALC_GREAT:
		if (atomic_read(&af_info->writeable) <= 0) {
			af_info->great_step =
				isp_af_get_fine_step(af_info, af_alg);
			if ((af_info->cur_step - af_alg->jump_offset)
				> af_info->great_step) {
				af_info->cur_step =
					af_info->cur_step - af_alg->jump_offset;
				sm_state.af_state = AF_GET_FINE_INFO;
		...
		if (atomic_read(&af_info->writeable) <= 0) {
			if (af_sm_dg&AF_FINE_TUNE) {
				pr_info("[af_sm..]:af_info->cur_step:%d,",
							af_info->cur_step);
				pr_info("af_alg->jump_offset:%d,",
							af_alg->jump_offset);
				pr_info("af_info->great_step:%d.\n",
							af_info->great_step);
			}
			if ((af_info->cur_step - af_alg->jump_offset)
				> af_info->great_step) {
	...
	case AF_SUCCESS:
		if ((atomic_read(&af_info->writeable) <= 0) &&
			(af_delay >= af_alg->field_delay*2)) {
			/*get last blnr*/
			memcpy(&af_info->last_blnr, &af_info->isr_af_data,
				sizeof(struct isp_blnr_stat_s));
			if (af_sm_dg&AF_FINE_TUNE) {
				pr_info("[af] last blnr:ac0=%u ac1=%u ac2=%u ac3=%u ",
						af_info->last_blnr.ac[0],
						af_info->last_blnr.ac[1],
						af_info->last_blnr.ac[2],
						af_info->last_blnr.ac[3]);
				pr_info("dc0=%u dc1=%u dc2=%u dc3=%u ",
						af_info->last_blnr.dc[0],
						af_info->last_blnr.dc[1],
						af_info->last_blnr.dc[2],
						af_info->last_blnr.dc[3]);
				pr_info("af0_ac=%u af1_ac=%u.\n",
						af_info->last_blnr.af_ac[0],
						af_info->last_blnr.af_ac[1]);
			}
			...
			isp_set_blenr_stat(af_info->x0, af_info->y0,
					af_info->x1, af_info->y1);
			isp_set_af_scan_stat(af_info->x0, af_info->y0,
					af_info->x1, af_info->y1);
		}
		break;
	default:
...
void isp_af_save_current_para(struct isp_dev_s *devp)
{
	struct isp_af_info_s *af_info = &devp->af_info;
	struct xml_algorithm_af_s *af_alg = devp->isp_af_parm;
	af_info->last_move = false;
...
static void isp_set_flash(struct isp_dev_s *devp,
		unsigned flash_mode, unsigned level)
{
	if (!flash_mode)
		...
		flash_on(devp->flash.mode_pol_inv,
				devp->flash.led1_pol_inv,
				devp->flash.pin_mux_inv,
				devp->wave);
...
void isp_set_flash_mode(struct isp_dev_s *devp)
{
	if (!devp->flash.valid)
		/*no flash*/
...
void capture_sm_init(struct isp_dev_s *devp)
{
	struct isp_capture_sm_s *cap_sm = &sm_state.cap_sm;
	struct xml_capture_s *parm = devp->capture_parm;
...
int isp_capture_sm(struct isp_dev_s *devp)
{
	static unsigned int start_jf, multi_count;
	...
	case CAPTURE_INIT:
		isp_set_blenr_stat(0, 0, devp->info.h_active-1,
				devp->info.v_active-1);
		if (parm->pretime) {
	...
	case CAPTURE_PRE_WAIT:
		if (time_after((unsigned long)jiffies,
				(unsigned long)(start_jf+parm->pretime))) {
			cap_sm->capture_state = CAPTURE_FLASH_ON;
			start_jf = 0;
			if (capture_debug)
	...
	case CAPTURE_FLASH_ON:
		if ((sm_state.flash == ISP_FLASH_STATUS_NULL) &&
			(cap_sm->flash_mode == FLASH_MODE_AUTO)) {
			return ret;
		}
		if (((cap_sm->flash_mode == FLASH_MODE_AUTO) &&
			(sm_state.flash == ISP_FLASH_STATUS_ON)) ||
	...
	case CAPTURE_EYE_WAIT:
		if (time_after((unsigned long)jiffies,
			(unsigned long)(start_jf+parm->eyetime))) {
	...
	case CAPTURE_POS_WAIT:
		if (time_after((unsigned long)jiffies,
			(unsigned long)(start_jf+parm->postime))) {
	...
	case CAPTURE_FLASHW:
		if (time_after((unsigned long)jiffies,
			(unsigned long)(start_jf+cap_sm->fr_time))) {
			isp_set_flash(devp, FLASH_TORCH, 0);
			ret = TVIN_BUF_NULL;
			if (capture_debug)
...
void isp_sm_uninit(struct isp_dev_s *devp)
{
