...
static unsigned int af_pr;
static unsigned int ioctl_debug;
static unsigned int isr_debug;
static unsigned int ae_flag;
	...
	if (!strcmp(parm[0], "r")) {
		...
		for (i = 0; i < 4; i++)
			pr_info("awb->yuv_low[%d].sum=%d,count=%d\n", i,
				awb->yuv_low[i].sum, awb->yuv_low[i].count);
		for (i = 0; i < 4; i++)
			pr_info("awb->yuv_mid[%d].sum=%d,count=%d\n", i,
				awb->yuv_mid[i].sum, awb->yuv_mid[i].count);
		for (i = 0; i < 4; i++)
			pr_info("awb->yuv_high[%d].sum=%d,count=%d\n", i,
				awb->yuv_high[i].sum, awb->yuv_high[i].count);
		/* echo wb_test 1 >/sys/class/isp/isp0/debug
		 * --disable 3a,disable gamma correction,lensd */
	...
	/* echo reconfigure h w bayer_fmt >/sys/class/isp/isp0/debug */
		...
		if (parm[1] && parm[2] && parm[3]) {
			...
			if (kstrtol(parm[2], 10, &val) < 0)
				...
				height = val;
			/* bayer = simple_strtol(parm[3], NULL, 10); */
...
static ssize_t af_debug_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t len)
{
	...
	if (!strcmp(parm[0], "jump")) {
		...
		if (kstrtol(parm[1], 16, &val) < 0)
			...
			data[0] = val;
		pr_info("%s to 0x%x.\n", parm[0], data[0]);
		devp->cam_param->cam_function.set_af_new_step(
				devp->cam_param->cam_function.priv_data,
				data[0]);
	/* echo start control min max dir delay step >af_debug */
	} else if (!strcmp(parm[0], "start")) {
		af = kmalloc(sizeof(struct af_debug_s), GFP_KERNEL);
		if (IS_ERR_OR_NULL(af)) {
			pr_info("%s kmalloc error.\n", __func__);
			return len;
		}
		memset(af, 0, sizeof(struct af_debug_s));
		if (parm[1] && parm[2] && parm[3] && parm[4]) {
			/* data[0] = simple_strtol(parm[1],NULL,16);//control */
			/* data[1] = simple_strtol(parm[2], NULL, 10); */
			if (kstrtol(parm[2], 10, &val) < 0)
				return -EINVAL;
			else
				data[1] = val;
			/* data[2] = simple_strtol(parm[3], NULL, 10); */
			if (kstrtol(parm[3], 10, &val) < 0)
				return -EINVAL;
			else
				data[2] = val;
			/* data[3] = simple_strtol(parm[4], NULL, 10); */
			if (kstrtol(parm[4], 10, &val) < 0)
				return -EINVAL;
			else
				data[3] = val;
			/* data[4] = simple_strtol(parm[5], NULL, 10); */
			if (kstrtol(parm[5], 10, &val) < 0)
				return -EINVAL;
			else
				data[4] = val;
			/* data[5] = simple_strtol(parm[6], NULL, 10); */
			if (kstrtol(parm[6], 10, &val) < 0)
				return -EINVAL;
			else
				data[5] = val;
			/* data[6] = simple_strtol(parm[7], NULL, 10); */
			if (kstrtol(parm[7], 10, &val) < 0)
				return -EINVAL;
			else
				data[6] = val;
			/* data[7] = simple_strtol(parm[8], NULL, 10); */
			if (kstrtol(parm[8], 10, &val) < 0)
				return -EINVAL;
			else
				data[7] = val;
			/* data[8] = simple_strtol(parm[9], NULL, 10); */
			if (kstrtol(parm[9], 10, &val) < 0)
				return -EINVAL;
			else
				data[8] = val;
			/* data[9] = simple_strtol(parm[10], NULL, 10); */
			if (kstrtol(parm[10], 10, &val) < 0)
				return -EINVAL;
			else
				data[9] = val;
		}
		...
		pr_info("%s:full scan from %u-%u-%u-%u.\n",
				__func__, af->min_step, af->pre_threshold,
				af->post_threshold, af->max_step);
		...
		if (IS_ERR_OR_NULL(af))
			return len;
		devp->flag &= (~ISP_FLAG_AF_DBG);
		pr_info("ac[0]   ac[1]    ac[2]   ac[3]   dc[0]  ");
		pr_info(" dc[1]   dc[2]   dc[3]   af0_ac   af1_ac\n");
		if (af->dir) {
			for (i = af->min_step; i <= af->max_step;
				i += af->mid_step) {
				cursor = i;
				while ((af->data[cursor].ac[0] == 0) &&
						(cursor != 0)) {
					cursor--;
				}
				pr_info("step[%4u]:%u %u %u %u %u %u %u %u %u %u\n",
					i, af->data[cursor].ac[0],
					af->data[cursor].ac[1],
					af->data[cursor].ac[2],
					af->data[cursor].ac[3],
					af->data[cursor].dc[0],
					...
					af->data[cursor].dc[2],
					af->data[cursor].dc[3],
					af->data[cursor].af_ac[0],
					af->data[cursor].af_ac[1]);
				/* msleep(10); */
				usleep_range(90000, 100000);
			}
		} else {
			for (i = af->max_step; i <= af->min_step;
				i += af->mid_step) {
				cursor = i;
				while ((af->data[cursor].ac[0] == 0) &&
						(cursor != 0))
		...
		if (kstrtol(parm[1], 10, &val) < 0)
			...
			devp->af_test.max = val;
		/* if (devp->af_test.af_win) */
		kfree(devp->af_test.af_win);
		/* if (devp->af_test.af_bl) */
		kfree(devp->af_test.af_bl);
		devp->af_test.af_win = kmalloc(sizeof(struct isp_af_stat_s)*
				devp->af_test.max, GFP_KERNEL);
		devp->af_test.af_bl = kmalloc(sizeof(struct isp_blnr_stat_s)*
				devp->af_test.max, GFP_KERNEL);
		devp->af_test.cnt = 0;
		devp->flag |= ISP_TEST_FOR_AF_WIN;
	} else if (!strcmp(parm[0], "af_print")) {
		int i = 0;
		pr_info("af:f0_win0 f1_win0 f0_win1 f1_win1 f0_win2 f1_win2 ");
		pr_info("f0_win3 f1_win3 f0_win4 f1_win4 f0_win5 ");
		pr_info("f1_win5 f0_win6 f1_win6 f0_win7 f1_win7\n");
		for (i = 0; i < devp->af_test.cnt; i++) {
			pr_info("%u %u %u %u %u %u %u %u %u %u %u %u %u %u %u %u\n",
				devp->af_test.af_win[i].luma_win[0],
				devp->af_test.af_win[i].luma_win[1],
				devp->af_test.af_win[i].luma_win[2],
				devp->af_test.af_win[i].luma_win[3],
				devp->af_test.af_win[i].luma_win[4],
				devp->af_test.af_win[i].luma_win[5],
				devp->af_test.af_win[i].luma_win[6],
				devp->af_test.af_win[i].luma_win[7],
				devp->af_test.af_win[i].luma_win[8],
				devp->af_test.af_win[i].luma_win[9],
				devp->af_test.af_win[i].luma_win[10],
				devp->af_test.af_win[i].luma_win[11],
				devp->af_test.af_win[i].luma_win[12],
				devp->af_test.af_win[i].luma_win[13],
				devp->af_test.af_win[i].luma_win[14],
				devp->af_test.af_win[i].luma_win[15]);
			/* msleep(1); */
		...
		for (i = 0; i < devp->af_test.cnt; i++) {
			pr_info("%u %u %u %u %u %u %u %u\n",
				...
				devp->af_test.af_bl[i].ac[1],
				devp->af_test.af_bl[i].ac[2],
				devp->af_test.af_bl[i].ac[3],
				devp->af_test.af_bl[i].dc[0],
				devp->af_test.af_bl[i].dc[1],
				devp->af_test.af_bl[i].dc[2],
...
static ssize_t af_debug_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	size_t len = 0;

	struct isp_dev_s *devp = dev_get_drvdata(dev);
	len += sprintf(buf+len, "0x%x 0x%x 0x%x 0x%x\n",
			devp->blnr_stat.dc[0], devp->blnr_stat.dc[1],
			devp->blnr_stat.dc[2], devp->blnr_stat.dc[3]);

	return len;
}

static void af_stat(struct af_debug_s *af, struct cam_function_s *ops)
{
	if (af->state == 0) {
...
static ssize_t ae_param_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t len)
{
...
static ssize_t ae_param_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	size_t len = 0;
	struct isp_dev_s *devp;
...
static ssize_t awb_param_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t len)
{
	char *parm[21] = {NULL};
	struct isp_dev_s *devp;
...
static ssize_t awb_param_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	size_t len = 0;
	char *buff = "show";
...
static ssize_t af_param_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t len)
{
	char *parm[3] = {NULL};
	struct isp_dev_s *devp;
...
static ssize_t af_param_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	size_t len = 0;
	char *buff = "show";
...
static ssize_t capture_param_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t len)
{
	char *parm[3];
	struct isp_dev_s *devp;
...
static ssize_t capture_param_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	size_t len = 0;
	char *buff = "show";
...
static ssize_t wave_param_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t len)
{
	char *parm[3];
	struct isp_dev_s *devp;
	...
	if (!strcmp(parm[0], "torch")) {
		...
		torch_level(devp->flash.mode_pol_inv,
				devp->flash.led1_pol_inv,
				devp->flash.pin_mux_inv,
				devp->flash.torch_pol_inv,
				devp->wave, level);
	} else{
...
static ssize_t wave_param_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	size_t len = 0;
	char *buff = "show";
...
static ssize_t gamma_debug_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t len)
{
...
static ssize_t gamma_debug_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
...
#if 0/* for no use */
static ssize_t ls_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	ssize_t len = 0;

	len += sprintf(buf+len, " command format:\n");
	len += sprintf(buf+len, " echo psize_v2h ocenter_c2l ");
	len += sprintf(buf+len, "ocenter_c2t gain_0db curvature_gr ");
	len += sprintf(buf+len, "curvature_r curvature_b curvature_gb ");
	len += sprintf(buf+len, "force_enable > ...\n");

	len += sprintf(buf+len, " Example:\n");
	len += sprintf(buf+len, " echo enable 100 50 50 0 120 120 "
	len += sprintf(buf+len, "120 120 1 > /sys/class/isp/isp0/lens\n");

	return len;
}

static ssize_t ls_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t len)
{
	char *buf_orig, *parm[10] = {NULL};
	unsigned int psize_v2h = 0, ocenter_c2l = 0, ocenter_c2t = 0,
			gain_0db = 0, curvature_gr = 0, curvature_r = 0;
	unsigned int curvature_b = 0, curvature_gb = 0;
	struct isp_dev_s *devp;
	struct isp_info_s_info_s *info = &devp->info;
	...
	if (!strcmp(parm[0], "enable")) {
		...
		if (kstrtol(parm[9], 10, &val) < 0)
			...
			force_enable = val;
		pr_info("psize_v2h:%u hactive:%u vactive:%u ocenter_c2l:%u ",
				psize_v2h, info->h_active,
				info->v_active, ocenter_c2l);
		pr_info("ocenter_c2t:%u gain_0db:%u curvature_gr:%u ",
				ocenter_c2t, gain_0db, curvature_gr);
		pr_info("curvature_r:%u curvature_b:%u curvature_gb:%u ",
				curvature_r, curvature_b, curvature_gb);
		pr_info("force_enable:%u\n", force_enable);

		isp_ls_curve(psize_v2h, info->h_active, info->v_active,
			ocenter_c2l, ocenter_c2t, gain_0db, curvature_gr,
			curvature_r, curvature_b, curvature_gb, force_enable);
	}
	kfree(buf_orig);
...
*/
static ssize_t aet_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	ssize_t len = 0;
...
static ssize_t aet_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t len)
{
...
static int isp_thread(struct isp_dev_s *devp)
{
	unsigned newstep = 0;
	struct cam_function_s *func = &devp->cam_param->cam_function;
	pr_info("isp_thread is run!\n");
	while (1) {
		if (ae_flag&0x2) {
			ae_flag &= (~0x2);
			newstep = isp_tune_exposure(devp);
			pr_info("set new step2 %d\n", newstep);
			if (func && func->set_aet_new_step)
				func->set_aet_new_step(func->priv_data,
						newstep, true, true);
		}
		if (atomic_read(&devp->ae_info.writeable) &&
			func && func->set_aet_new_step) {
			if (isp_debug)
				pr_info("[isp] set new step:%d\n",
						ae_sens.new_step);
			if (ae_adjust_enable)
				func->set_aet_new_step(func->priv_data,
						ae_sens.new_step,
		...
		if (devp->flag & ISP_AF_SM_MASK) {
			if (atomic_read(&devp->af_info.writeable) &&
					func && func->set_af_new_step) {
				func->set_af_new_step(func->priv_data,
						devp->af_info.cur_step);
...
static int start_isp_thread(struct isp_dev_s *devp)
{
	if (!devp->kthread) {
...
static void stop_isp_thread(struct isp_dev_s *devp)
{
	if (devp->kthread) {
...
{
	struct isp_dev_s *devp = container_of(fe, struct isp_dev_s, frontend);
	struct vdin_parm_s *parm = (struct vdin_parm_s *)fe->private_data;
	struct isp_info_s *info = &devp->info;
	...
	devp->cam_param = (struct cam_parameter_s *)parm->reserved;
	if (IS_ERR_OR_NULL(devp->cam_param)) {
		...
		isp_hw_enable(false);
		isp_set_def_config(devp->cam_param->xml_regs_map,
				info->fe_port, info->bayer_fmt,
				info->h_active, info->v_active);
		...
		devp->af_info.radius =
				info->h_active/devp->isp_af_parm->radius_ratio;
		devp->af_info.af_detect =
				kmalloc(sizeof(struct isp_blnr_stat_s)*
				devp->isp_af_parm->detect_step_cnt, GFP_KERNEL);
		devp->af_info.v_dc =
				kmalloc(sizeof(unsigned long long)*
				devp->isp_af_parm->detect_step_cnt, GFP_KERNEL);
	}
	return 0;
}
...
static void isp_fe_close(struct tvin_frontend_s *fe)
{
	...
	memset(&devp->info, 0, sizeof(struct isp_info_s));
	/*power down isp hw*/
	isp_hw_enable(false);
...
static int isp_fe_ioctl(struct tvin_frontend_s *fe, void *arg)
{
	...
	struct isp_dev_s *devp = container_of(fe, struct isp_dev_s, frontend);
	struct cam_parameter_s *param = (struct cam_parameter_s *)arg;
	enum cam_command_e cmd;
	enum cam_cmd_state_e ret = CAM_STATE_SUCCESS;
	...
	case CAM_COMMAND_FULLSCAN:
		isp_set_af_scan_stat(devp->af_info.x0, devp->af_info.y0,
				devp->af_info.x1, devp->af_info.y1);
		devp->flag |= ISP_FLAG_TOUCH_AF;
	...
	case CAM_COMMAND_TOUCH_FOCUS:
		if (!(devp->flag & ISP_FLAG_CAPTURE)) {
			...
			x0 =
				devp->isp_af_parm->x > devp->af_info.radius ?
				devp->isp_af_parm->x-devp->af_info.radius:0;
			y0 =
				devp->isp_af_parm->y > devp->af_info.radius ?
				devp->isp_af_parm->y-devp->af_info.radius:0;
			...
			if (ioctl_debug) {
				pr_info("focus win: center(%u,%u) ",
					devp->isp_af_parm->x,
					devp->isp_af_parm->y);
				pr_info("left(%u %u) ", x0, y0);
				pr_info("right(%u,%u).\n", x1, y1);
			}
...
static int isp_fe_isr(struct tvin_frontend_s *fe, unsigned int hcnt64)
{
	...
	if (devp->flag & ISP_TEST_FOR_AF_WIN) {
		...
		/* isp_get_af_scan_stat(
		 * &devp->af_test.af_bl[devp->af_test.cnt]); */
		isp_get_af_stat(&devp->af_test.af_win[devp->af_test.cnt]);
		/* isp_get_ae_stat(
		 * &devp->af_test.ae_win[devp->af_test.cnt]); */
		devp->af_test.cnt += 1;
...
{
	struct isp_dev_s *devp = (struct isp_dev_s *)arg;
	if ((devp->flag & ISP_FLAG_AE) && (ae_enable))
		isp_ae_sm(devp);
...
static void  isp_do_work(struct work_struct *work)
{
	...
	struct isp_dev_s *devp = container_of(work, struct isp_dev_s, isp_wq);

	unsigned newstep = 0;
	struct cam_function_s *func = &devp->cam_param->cam_function;
	if (!(devp->flag & ISP_FLAG_START)) {
		pr_info("[isp]%s:isp has stop\n", __func__);
		return;
	}
	if (!(devp->flag & ISP_FLAG_TEST_WB)) {
		if ((devp->flag & ISP_FLAG_AE) && (ae_enable))
...
static void isp_sig_propery(struct tvin_frontend_s *fe,
...
static bool isp_frame_skip(struct tvin_frontend_s *fe)
{
	...
	if (devp->isp_fe && devp->isp_fe->sm_ops) {
		if (devp->isp_fe->sm_ops->check_frame_skip) {
			sm_ops = devp->isp_fe->sm_ops;
			return sm_ops->check_frame_skip(devp->isp_fe);
		}
	}
	return 0;
}
...
static int isp_open(struct inode *inode, struct file *file)
{
	...
	devp = container_of(inode->i_cdev, struct isp_dev_s, cdev);
...
static int isp_add_cdev(struct cdev *cdevp,
...
static int isp_probe(struct platform_device *pdev)
{
	...
	devp = kmalloc(sizeof(struct isp_dev_s), GFP_KERNEL);
	memset(devp, 0, sizeof(struct isp_dev_s));
	if (!devp) {
