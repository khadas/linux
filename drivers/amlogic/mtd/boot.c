...
/*provide a policy that calculate the bakup number of bootloader*/
static int get_boot_num(struct mtd_info *mtd, size_t rwsize)
{
	struct aml_nand_chip *aml_chip = mtd_to_nand_chip(mtd);
	size_t bad_blk_len_low = 0, bad_blk_len_up = 0, skip;
	size_t aviable_space;
	loff_t offset = 0;
	int ret = 1, error = 0; /*inital for only one copy*/
	...
	if (aml_chip->support_new_nand == 1) {
		if (en_slc) {
			...
			if (type == HYNIX_1YNM_8GB)
				read_page =
					pagelist_1ynm_hynix256[read_page+1]+
	...
	int oob_required, int page, int cached, int raw)
{
	...
	if (en_slc) {
		if (aml_chip->support_new_nand == 1) {
			if (type == HYNIX_1YNM_8GB)
				write_page = pagelist_1ynm_hynix256[page+1];
			else
...
WRITE_BAD_BLOCK:
	...
	if (aml_chip->support_new_nand == 1) {
		if (type == HYNIX_1YNM_8GB) {
			if ((page + 1) > 1)
				priv_slc_page = pagelist_1ynm_hynix256[page];
			else
				priv_slc_page = page;
	...
	loff_t *ppos)
{
	...
	if (*ppos >= (loff_t)(BOOT_TOTAL_PAGES * mtd->writesize)) {
		pr_info(KERN_ERR "boot read: data access violation!\n");
		return -EFAULT;
	}
	align_count =
	((((u32)count + mtd->writesize)-1)/mtd->writesize)*mtd->writesize;
	...
	for (rd_size = 0; rd_size < align_count; rd_size += mtd->writesize) {
			 ...
			 size_t count, loff_t *ppos)
{
	...
	if (*ppos >= (loff_t)(BOOT_TOTAL_PAGES * mtd->writesize)) {
		pr_info(KERN_ERR "boot write: data access violation!\n");
		return -EFAULT;
	}
	...
	if (!data_buf) {
		pr_err("malloc buf for rom_write failed\n");
		goto err_exit0;
	}
	...
	chipnr = (int)(addr >> chip->chip_shift);
	chip->select_chip(mtd, chipnr);
	for (wr_size = 0; wr_size < align_count; wr_size += mtd->writesize) {
		while ((addr%mtd->erasesize == 0)
			&& chip->block_bad(mtd, addr, 0))
			addr += mtd->erasesize;
		page = addr >> chip->page_shift;
		pr_info("%s %d wr_size=0x%zx page=0x%x\n",
			__func__, __LINE__, wr_size, page);
...
static int boot_ioctl(struct file *file, u_int cmd, u_long arg)
{
	...
	case BOOT_SET_INFO:
	{
		 ...
		 *because bad block, need call get_boot_num to
		 *get boot copies number
		 */
	...
	case BOOT_GET_INFO:
	{
		...
		int boot_num = aml_chip_uboot->boot_copy_num;
		if (put_user(boot_num, &(info->bootnum)))
			return -EFAULT;
		break;
	}
...
static long boot_compat_ioctl(struct file *file, unsigned int cmd,
	unsigned long arg)
{
	struct uboot_file_info *ufi = file->private_data;
	struct mtd_info *mtd = ufi->mtd;
...
int boot_device_register(struct aml_nand_chip *aml_chip)
{
	...
	if (ret < 0) {
		pr_err("class_register(&uboot_class) failed!\n");
		goto exit_error0;
	}
