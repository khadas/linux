	...
	unsigned char *buf, int len, unsigned bch_mode)
{
	...
	aml_chip->data_dma_addr =
		dma_map_single(aml_chip->device, aml_chip->aml_nand_data_buf,
...
#if (!AML_CFG_CONHERENT_BUFFER)
	dma_unmap_single(aml_chip->device, aml_chip->data_dma_addr,
		(mtd->writesize + mtd->oobsize),
		DMA_TO_DEVICE);
#endif
	return ret;
...
#endif
	info_buf =
(unsigned *)&(aml_chip->user_info_buf[(count-1)*info_times_int_len]);
	/*smp_wmb*/
	smp_wmb();
	/*wmb*/
	wmb();
...
static int m3_nand_probe(struct aml_nand_platform *plat,
	unsigned dev_num, struct device *dev)
#else
