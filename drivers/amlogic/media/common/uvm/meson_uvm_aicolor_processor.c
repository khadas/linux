...
static void dump_vf(struct vframe_s *vf, phys_addr_t addr, struct uvm_aicolor_info *info, int num)
{
	...
	snprintf(name_buf, sizeof(name_buf), "/data/aicolor_ge2dOut_%d.rgb", num);
	...
	vfs_fsync(fp, 0);
	aicolor_print(PRINT_NN_DUMP, "aicolor: write %u size to addr%p\n",
		write_size, data);
	codec_mm_unmap_phyaddr(data);
	filp_close(fp, NULL);
	set_fs(fs);

	snprintf(name_buf, sizeof(name_buf), "/data/aicolor_dec_%d.yuv", num);
	fp = filp_open(name_buf, O_CREAT | O_RDWR, 0644);
	if (IS_ERR(fp))
		return;
	...
	if (!data || !data_uv) {
		aicolor_print(PRINT_ERROR, "%s: vmap failed.\n", __func__);
		return;
	}
	fs = get_fs();
	set_fs(KERNEL_DS);
	...
	vfs_fsync(fp, 0);
	aicolor_print(PRINT_NN_DUMP, "aicolor: write y data %u size from addr%p\n",
		write_size, data);
	aicolor_print(PRINT_NN_DUMP, "aicolor: write uv data %u size from addr%p\n",
		write_size_uv, data_uv);
	aicolor_print(PRINT_NN_DUMP, "aicolor: yuv w %d, h %d.\n",
...
int aicolor_getinfo(void *arg, char *buf)
{
	...
	if (aicolor_info->get_info_type == AICOLOR_GET_RGB_DATA) {
		...
		if (uvm_aicolor_dump) {
			if (dump_count < uvm_aicolor_dump) {
				dump_vf(vf, addr, aicolor_info, dump_count + 1);
				dump_count++;
			} else {
				aicolor_print(PRINT_ERROR, "finish dump %d vframe.\n",
					uvm_aicolor_dump);
				uvm_aicolor_dump = 0;
				dump_count = 0;
			}
