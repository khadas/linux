 ...
 */
static enum VLOCK_PROC_MD vlock_work_mode = VLOCK_PROC_MODE_MANUAL_ENC;
unsigned int vlock_enable;
unsigned int vlock_hw_clk_ok;
...
static unsigned int vlock_input_type;
...
static signed int framelock_priority;

static signed int vlock_enc_maxtune_line = 12;
module_param(vlock_enc_maxtune_line, uint, 0664);
MODULE_PARM_DESC(vlock_enc_maxtune_line, "\n vlock_enc_maxtune_line\n");

static signed int vlock_enc_maxtune_pixels = 10;
module_param(vlock_enc_maxtune_pixels, uint, 0664);
MODULE_PARM_DESC(vlock_enc_maxtune_pixels, "\n vlock_enc_maxtune_pixels\n");
...
static u32 phase_en_after_frqlock;
static u32 vlock_ss_en = 1;

static struct vlock_proc_sig_sts vlock0;
static struct vlock_proc_sig_sts vlock1;
static struct vlock_proc_sig_sts vlock2;
static struct vlock_proc_sig_sts *vlock_tab[VLOCK_PROC_ENC_MAX];

/*static unsigned int hhi_pll_reg_vlock_ctl;*/
static unsigned int vlk_log_size = 60;/*size base: vlock_log_s*/
module_param(vlk_log_size, uint, 0664);
MODULE_PARM_DESC(vlk_log_size, "\n vlk_log_size\n");

static unsigned int vlk_log_cnt;/*cnt base: vlock_log_s*/
module_param(vlk_log_cnt, uint, 0664);
MODULE_PARM_DESC(vlk_log_cnt, "\n vlk_log_cnt\n");

static unsigned int vlk_log_delta_frac = 100;
module_param(vlk_log_delta_frac, uint, 0664);
MODULE_PARM_DESC(vlk_log_delta_frac, "\n vlk_log_delta_frac\n");

static unsigned int vlk_log_delta_m;
module_param(vlk_log_delta_m, uint, 0664);
MODULE_PARM_DESC(vlk_log_delta_m, "\n vlk_log_delta_m\n");

static unsigned int vlk_log_delta_en;
module_param(vlk_log_delta_en, uint, 0664);
MODULE_PARM_DESC(vlk_log_delta_en, "\n vlk_log_delta_en\n");

static unsigned int vlk_log_delta_ivcnt = 0xff;
module_param(vlk_log_delta_ivcnt, uint, 0664);
MODULE_PARM_DESC(vlk_log_delta_ivcnt, "\n vlk_log_delta_ivcnt\n");

static unsigned int vlk_log_delta_ovcnt = 0xff;
module_param(vlk_log_delta_ovcnt, uint, 0664);
MODULE_PARM_DESC(vlk_log_delta_ovcnt, "\n vlk_log_delta_ovcnt\n");

static unsigned int vlk_log_delta_vcnt = 0xff;
module_param(vlk_log_delta_vcnt, uint, 0664);
MODULE_PARM_DESC(vlk_log_delta_vcnt, "\n vlk_log_delta_vcnt\n");
...
static unsigned int vlk_latch_en_cnt = 20;
module_param(vlk_latch_en_cnt, uint, 0664);
MODULE_PARM_DESC(vlk_latch_en_cnt, "\n vlk_latch_en_cnt\n");

static u32 vlk_tune_sync_on = 1;
module_param(vlk_tune_sync_on, uint, 0664);
MODULE_PARM_DESC(vlk_tune_sync_on, "\n vlk_tune_sync_on\n");

static unsigned int vlock_latch_flag;
module_param(vlock_latch_flag, uint, 0664);
MODULE_PARM_DESC(vlock_latch_flag, "\n vlock_latch_flag\n");

static int freerun_type = VLOCK_PROC_GAME_MODE;/* 0:game mode;1:freerun mode */
module_param(freerun_type, int, 0664);
MODULE_PARM_DESC(freerun_type, "\n enable or disable freerun\n");

static unsigned int vlock_log_en;
static struct vlock_proc_log_s **vlock_log;
//static u16 frc_init_first_flag;

//static signed int err_accum;
//static unsigned int last_i_vsync;

static u32 loop0_err_lmt = 0xb800;
static u32 loop1_err_lmt = 0x4800;
static u32 loop_err_rs = 3;
static u32 loop_err_gain = 128;
static u32 loop0_en = 2;	/*0:off, 1:on 2:auto*/
static u32 loop1_en = 1;	/*0:off, 1:on 2:auto*/
static u32 speed_up_en = 1;
...
static struct vlock_proc_reg_map vlock_reg_maps[VLOCK_PROC_REG_MAP_END] = {0};

static struct vlock_proc_reg_map_tab regmap_tab_tm2[] = {
	{.base = 0xff000000,	.size = 0x4000},/*vpu*/
	{.base = 0xff63c000,	.size = 0x0100},/*hiu*/
	{.base = 0x00000000,	.size = 0x0000},/*anactrl*/
};

static struct vlock_proc_reg_map_tab regmap_tab_t7[] = {
	{.base = 0xff000000,	.size = 0x4000,},/*vpu*/
	{.base = 0x00000000,	.size = 0x0000,},/*hiu*/
	{.base = 0xfe008000,	.size = 0x0100,},/*anactrl*/
};

int vlock_proc_hiu_reg_read(unsigned int reg, unsigned int *val)
{
...
int vlock_proc_hiu_reg_write(unsigned int reg, unsigned int val)
{
				     ...
				     unsigned int start, unsigned int len)
{
	...
	vlock_proc_hiu_reg_read(reg, &rd_val);
	vlock_proc_hiu_reg_write(reg, ((rd_val &
	     ~(((1L << (len)) - 1) << (start))) |
	    (((value) & ((1L << (len)) - 1)) << (start))));
	return 0;
}

u32 vlock_proc_read_vpp_reg(u32 reg)
{
	return READ_VPP_REG(reg);
}

void vlock_proc_write_vpp_reg(u32 reg, const u32 value)
{
	WRITE_VPP_REG(reg, value);
}

u32 vlock_proc_read_vpp_reg_bits(u32 reg, const u32 start, const u32 len)
{
	return READ_VPP_REG_BITS(reg, start, len);
}

void vlock_proc_write_vpp_reg_bits(u32 reg, const u32 value, const u32 start, const u32 len)
{
	WRITE_VPP_REG_BITS(reg, value, start, len);
}

int vlock_proc_init_reg_map(struct device *dev, struct vlock_proc_sig_sts *pvlock)
{
	int i = 0;
	struct vlock_proc_reg_map_tab *regmap;

	if (!pvlock->dtdata)
		return -1;

	if (pvlock->dtdata->vlk_chip >= vlk_chip_t7)
		regmap = (struct vlock_proc_reg_map_tab *)&regmap_tab_t7;
	else
		regmap = (struct vlock_proc_reg_map_tab *)&regmap_tab_tm2;

	for (i = VLOCK_PROC_REG_MAP_VPU; i < VLOCK_PROC_REG_MAP_END; i++) {
...
void vlock_proc_write_reg(enum vlock_proc_regmap_e bus, u32 addr, u32 val)
{
	if (vlock_reg_maps[bus].p) {
		writel(val, vlock_reg_maps[bus].p + addr);
	} else {
		if (bus == VLOCK_PROC_REG_MAP_HIU)
			vlock_proc_hiu_reg_write(addr, val);
		else if (bus == VLOCK_PROC_REG_MAP_VPU)
			WRITE_VPP_REG(addr, val);
	}
}

int vlock_proc_read_reg(enum vlock_proc_regmap_e bus, u32 addr)
{
	u32 val;
	...
	if (bus == VLOCK_PROC_REG_MAP_HIU) {
		vlock_proc_hiu_reg_read(addr, &val);
		return val;
	} else if (bus == VLOCK_PROC_REG_MAP_VPU) {
...
static int vlock_proc_reg_wt_bits(enum vlock_proc_regmap_e bus, unsigned int reg,
			unsigned int value, unsigned int start, unsigned int len)
{
	unsigned int rd_val;

	rd_val = vlock_proc_read_reg(bus, reg);
	vlock_proc_write_reg(bus, reg, ((rd_val &
	     ~(((1L << (len)) - 1) << (start))) |
	    (((value) & ((1L << (len)) - 1)) << (start))));
	return 0;
}

static unsigned int vlock_proc_reg_rd_bits(enum vlock_proc_regmap_e bus, unsigned int _reg,
			  unsigned int _start, unsigned int _len)
{
	return (vlock_proc_read_reg(bus, _reg) >> (_start)) & ((1L << (_len)) - 1);
}

u32 vlock_proc_get_panel_pll_m(struct vlock_proc_sig_sts *pvlock)
{
	u32 val;
	...
	if (pvlock->dtdata->vlk_pll_sel == vlock_proc_pll_sel_hdmi) {
		vlock_proc_hiu_reg_read(HHI_HDMI_PLL_CNTL, &val);
	} else {
		if (pvlock->dtdata->vlk_chip >= vlk_chip_t7) {
			if (pvlock->idx == VLOCK_PROC_ENC0)
				val =
				vlock_proc_reg_rd_bits(VLOCK_PROC_REG_MAP_ANACTRL,
						ANACTRL_TCON_PLL0_CNTL0, 0, 8);
			else if (pvlock->idx == VLOCK_PROC_ENC1)
				val =
				vlock_proc_reg_rd_bits(VLOCK_PROC_REG_MAP_ANACTRL,
						ANACTRL_TCON_PLL1_CNTL0, 0, 8);
			else if (pvlock->idx == VLOCK_PROC_ENC2)
				val =
				vlock_proc_reg_rd_bits(VLOCK_PROC_REG_MAP_ANACTRL,
						ANACTRL_TCON_PLL2_CNTL0, 0, 8);
			else
				val =
				vlock_proc_reg_rd_bits(VLOCK_PROC_REG_MAP_ANACTRL,
						ANACTRL_TCON_PLL0_CNTL0, 0, 8);
		} else {
			vlock_proc_hiu_reg_read(pvlock->hhi_pll_reg_m, &val);
		}
	}
...
u32 vlock_proc_get_panel_pll_frac(struct vlock_proc_sig_sts *pvlock)
{
	u32 val;
	...
	if (pvlock->dtdata->vlk_pll_sel == vlock_proc_pll_sel_hdmi) {
		vlock_proc_hiu_reg_read(HHI_HDMI_PLL_CNTL2, &val);
	} else {
		if (pvlock->dtdata->vlk_chip >= vlk_chip_t7) {
			if (pvlock->idx == VLOCK_PROC_ENC0)
				val =
				vlock_proc_reg_rd_bits(VLOCK_PROC_REG_MAP_ANACTRL,
						ANACTRL_TCON_PLL0_CNTL1, 0, 19);
			else if (pvlock->idx == VLOCK_PROC_ENC1)
				val =
				vlock_proc_reg_rd_bits(VLOCK_PROC_REG_MAP_ANACTRL,
						ANACTRL_TCON_PLL1_CNTL1, 0, 19);
			else if (pvlock->idx == VLOCK_PROC_ENC2)
				val =
				vlock_proc_reg_rd_bits(VLOCK_PROC_REG_MAP_ANACTRL,
						ANACTRL_TCON_PLL2_CNTL1, 0, 19);
			else
				val =
				vlock_proc_reg_rd_bits(VLOCK_PROC_REG_MAP_ANACTRL,
						ANACTRL_TCON_PLL0_CNTL1, 0, 19);
		} else {
			vlock_proc_hiu_reg_read(pvlock->hhi_pll_reg_frac, &val);
		}
	}
...
void vlock_proc_set_panel_pll_m(struct vlock_proc_sig_sts *pvlock, u32 val)
{
#ifdef CONFIG_AMLOGIC_LCD
	u32 m = val;
#endif
	...
	if (pvlock->dtdata->vlk_pll_sel == vlock_proc_pll_sel_hdmi) {
		vlock_proc_hiu_reg_write(HHI_HDMI_PLL_CNTL, val);
	} else {
		if (pvlock->dtdata->vlk_chip >= vlk_chip_t7) {
			if (pvlock->idx == VLOCK_PROC_ENC0)
				vlock_proc_reg_wt_bits(VLOCK_PROC_REG_MAP_ANACTRL,
...
void vlock_proc_set_panel_pll_frac(struct vlock_proc_sig_sts *pvlock, u32 val)
{
#ifdef CONFIG_AMLOGIC_LCD
	u32 frac = val;
#endif
	...
	if (pvlock->dtdata->vlk_pll_sel == vlock_proc_pll_sel_hdmi) {
		vlock_proc_hiu_reg_write(HHI_HDMI_PLL_CNTL2, val);
	} else {
		if (pvlock->dtdata->vlk_chip >= vlk_chip_t7) {
			if (pvlock->idx == VLOCK_PROC_ENC0)
				vlock_proc_reg_wt_bits(VLOCK_PROC_REG_MAP_ANACTRL,
...
void vlock_proc_set_panel_ss(u32 onoff)
{
#ifdef CONFIG_AMLOGIC_LCD
	if (onoff)
		lcd_ss_enable(1);
...
enum vlock_proc_enc_num_e vlock_proc_get_cur_enc_mode(void)
{
	const struct vinfo_s *vinfo;
	unsigned int viu_mux;
	enum vlock_proc_enc_num_e enc_mux;
	...
	switch (viu_mux) {
	case 0:
		enc_mux = VLOCK_PROC_ENC0;
		break;
	case 1:
		enc_mux = VLOCK_PROC_ENC1;
		break;
	case 2:
		enc_mux = VLOCK_PROC_ENC2;
		break;
	default:
		enc_mux = VLOCK_PROC_ENC0;
		break;
...
int __attribute__((weak))vlock_proc_frc_is_on(void)
{
	return 0;
}

static void vlock_proc_tune_sync_frc(u32 frc_vporch_cal, unsigned char frc_s2l_en)
{
	u32 max_lncnt;
	u32 max_pxcnt;
	u32 frc_v_porch;
...
static unsigned int vlock_proc_set_frc_status(int frc_status)
{
...
bool vlock_proc_vsync_skip_for_frc(void)
{
	int ret = false;
	struct vlock_proc_sig_sts *pvlock;
	enum vlock_proc_enc_num_e enc_mux = VLOCK_PROC_ENC0;

	enc_mux = vlock_proc_get_cur_enc_mode();
...
int vlock_proc_sync_frc_vporch(struct vlock_proc_frc_param frc_param)
{
	...
	vlock_proc_set_frc_status(frc_param.s2l_en);
	...
	if (pvlock->dtdata->vlk_ctl_for_frc) {
		if (pvlock->fsm_sts == VLOCK_STATE_ENABLE_STEP1_DONE ||
			...
			vlock_proc_tune_sync_frc(frc_param.frc_v_porch, frc_param.s2l_en);
			ret = 0;
		}
	} else {
...
EXPORT_SYMBOL(vlock_proc_sync_frc_vporch);

static void vlock_proc_tune_sync(struct vlock_proc_sig_sts *pvlock)
{
	u32 offset_enc = pvlock->offset_encl;

	if (pvlock->dtdata->vlk_chip == vlk_chip_t3 && pvlock->idx == VLOCK_PROC_ENC0) {
		if (vlk_tune_sync_on) {
			...
			if (!vlock_proc_frc_is_on() || vlock_frc_is_on == VLOCK_V_FRONT_PORCH_REV_B)
				return;
...
static unsigned int vlock_proc_check_input_hz(struct vframe_s *vf)
{
...
static unsigned int vlock_proc_check_output_hz(unsigned int sync_duration_num,
 ...
 * param: vlock_pll_sel
 */
static void vlock_proc_pll_select(struct vlock_proc_sig_sts *pvlock,
					u32 vlock_work_mode, u32 workmd)
{
	if (vlock_debug & VLOCK_DEBUG_INFO)
		pr_info("%s md:%d\n", __func__, workmd);

	if (pvlock->dtdata->vlk_chip >= vlk_chip_t7) {
		if (workmd == vlock_proc_pll_sel_disable) {
			if (pvlock->idx == VLOCK_PROC_ENC0) {
				/* enc0: need disable vlock clk lock */
				vlock_proc_reg_wt_bits(VLOCK_PROC_REG_MAP_ANACTRL,
						ANACTRL_TCON_PLL_VLOCK, 0, 0, 1);
			} else if (pvlock->idx == VLOCK_PROC_ENC1) {
				/* enc1: need disable vlock clk lock */
				vlock_proc_reg_wt_bits(VLOCK_PROC_REG_MAP_ANACTRL,
						ANACTRL_TCON_PLL_VLOCK, 0, 1, 1);
			} else if (pvlock->idx == VLOCK_PROC_ENC2) {
				/* enc2: need disable vlock clk lock */
				vlock_proc_reg_wt_bits(VLOCK_PROC_REG_MAP_ANACTRL,
						ANACTRL_TCON_PLL_VLOCK, 0, 2, 1);
			}
		} else {
			if (IS_VLOCK_PROC_AUTO_PLL_MODE(vlock_work_mode)) {
				if (pvlock->idx == VLOCK_PROC_ENC0) {
					/*enc0*/
					vlock_proc_reg_wt_bits(VLOCK_PROC_REG_MAP_ANACTRL,
							ANACTRL_TCON_PLL_VLOCK, 1, 0, 1);
				} else if (pvlock->idx == VLOCK_PROC_ENC1) {
					/*enc1*/
					vlock_proc_reg_wt_bits(VLOCK_PROC_REG_MAP_ANACTRL,
							ANACTRL_TCON_PLL_VLOCK, 1, 1, 1);
				} else if (pvlock->idx == VLOCK_PROC_ENC2) {
					/*enc2*/
					vlock_proc_reg_wt_bits(VLOCK_PROC_REG_MAP_ANACTRL,
							ANACTRL_TCON_PLL_VLOCK, 1, 2, 1);
				}
			}
		}
		...
		if (workmd == vlock_proc_pll_sel_disable) {
			...
			if (IS_VLOCK_PROC_AUTO_PLL_MODE(vlock_work_mode)) {
				if (workmd == vlock_proc_pll_sel_hdmi) {
		...
		if (workmd == vlock_proc_pll_sel_disable) {
			if (pvlock->dtdata->vlk_hw_ver >= vlk_hw_ver2) {
				vlock_hiu_reg_wt_bits(HHI_HDMI_PLL_VLOCK_CNTL, 0x4, 0, 3);
				vlock_hiu_reg_wt_bits(HHI_HDMI_PLL_VLOCK_CNTL, 0x0, 0, 3);
			}
		} else {
			if (pvlock->dtdata->vlk_hw_ver >= vlk_hw_ver2) {
				if (IS_VLOCK_PROC_AUTO_MODE(vlock_work_mode)) {
					if (IS_VLOCK_PROC_AUTO_ENC_MODE(vlock_work_mode))
						vlock_hiu_reg_wt_bits(HHI_HDMI_PLL_VLOCK_CNTL,
								      0, 0, 3);
					else
						vlock_hiu_reg_wt_bits(HHI_HDMI_PLL_VLOCK_CNTL,
								      0x7, 0, 3);
				} else if (IS_VLOCK_PROC_MANUAL_MODE(vlock_work_mode)) {
 ...
 * is valid.
 */
void vlock_proc_set_phase_frq_lock_speed(struct vlock_proc_sig_sts *pvlock)
{
	u32 data = 0;
	u32 offset_vlck = pvlock->offset_vlck;

	if (pvlock->dtdata->vlk_hw_ver < vlk_hw_tm2verb)
		return;

	if (speed_up_en && IS_VLOCK_PROC_AUTO_MODE(vlock_work_mode)) {
		data = 0;
...
void vlock_proc_reset(struct vlock_proc_sig_sts *pvlock, u32 onoff)
{
	u32 offset_vlck = pvlock->offset_vlck;
 ...
 */
static void vlock_proc_enable(struct vlock_proc_sig_sts *pvlock, bool enable)
{
...
#ifndef CONFIG_AMLOGIC_REMOVE_OLD
		if (vlock_work_mode & VLOCK_MODE_MANUAL_PLL) {
			vlock_proc_hiu_reg_read(HHI_HDMI_PLL_CNTL6, &tmp_value);
...
static void vlock_proc_hw_reinit(struct vlock_proc_sig_sts *pvlock,
...
static void vlock_proc_setting(struct vframe_s *vf, struct vlock_proc_sig_sts *pvlock)
{
	...
	vlock_proc_set_phase_frq_lock_speed(pvlock);
	/*vlock_set_clk_src, after tm2 use clk tree*/
	if (pvlock->dtdata->vlk_chip <= vlk_chip_tm2)
		vlock_proc_hiu_reg_write(HHI_VID_LOCK_CLK_CNTL, 0x80);

	if (IS_VLOCK_PROC_ENC_MODE(vlock_work_mode)) {
		/*init default config for enc mode*/
		vlock_proc_hw_reinit(pvlock, vlock_enc_setting, VLOCK_DEFAULT_REG_SIZE);
		/*vlock line limit*/
		/*WRITE_VPP_REG(VPU_VLOCK_OUTPUT0_CAPT_LMT + offset_vlck,*/
		/*	vlock_capture_limit);*/

		/* VLOCK_CNTL_EN disable */
		vlock_proc_enable(pvlock, 0);
		/* disable to adjust pll */
		WRITE_VPP_REG_BITS(VPU_VLOCK_CTRL + offset_vlck, 0, 29, 1);
		/* CFG_VID_LOCK_ADJ_EN enable */
		if ((vlock_work_mode & (VLOCK_PROC_MODE_MANUAL_ENC |
		...
		if ((vf->type_original & VIDTYPE_TYPEMASK) &&
		    !(vlock_work_mode & VLOCK_PROC_MODE_MANUAL_SOFT_ENC)) {
			/*tl1 fix i problem*/
			if (get_cpu_type() < MESON_CPU_MAJOR_ID_TL1)
				input_hz = input_hz >> 1;
			else
		...
		WRITE_VPP_REG_BITS(VPU_VLOCK_ADJ_EN_SYNC_CTRL + offset_vlck,
				   vlk_latch_en_cnt, 8, 8);
		WRITE_VPP_REG_BITS(pvlock->enc_video_mode_addr + offset_enc, 1, 15, 1);
	}
	if ((vlock_work_mode & (VLOCK_PROC_MODE_AUTO_PLL |
		VLOCK_PROC_MODE_MANUAL_PLL))) {
		/* av pal in,1080p60 hdmi out as default */
		vlock_proc_hw_reinit(pvlock, vlock_pll_setting, VLOCK_DEFAULT_REG_SIZE);
		/*
		 *set input & output freq
		 *bit0~7:input freq
		 *bit8~15:output freq
		...
		if (vlock_adapt == 0)
			...
			WRITE_VPP_REG_BITS(VPU_VLOCK_MISC_CTRL + offset_vlck,
					   input_hz, 16, 8);
		hiu_reg_value = vlock_proc_get_panel_pll_m(pvlock);
		hiu_reg_value_2 = vlock_proc_get_panel_pll_frac(pvlock);
	...
	if (pvlock->phlock_en) {
		if (!phase_en_after_frqlock) {
			if (IS_VLOCK_PROC_PLL_MODE(vlock_work_mode))
				vlock_proc_hw_reinit(pvlock, vlock_pll_phase_setting,
						VLOCK_PHASE_REG_SIZE);
			else
				vlock_proc_hw_reinit(pvlock, vlock_encl_phase_setting,
						VLOCK_PHASE_REG_SIZE);
	...
	switch (VLOCK_PROC_OUT_ENCL/*READ_VPP_REG_BITS(VPU_VIU_VENC_MUX_CTRL, 0, 2)*/) { /* ?? */
	case VLOCK_PROC_OUT_ENCL:/* ENCL */
		WRITE_VPP_REG_BITS(VPU_VLOCK_CTRL + offset_vlck, VLOCK_PROC_OUT_ENCL, 26, 2);
		break;
	case VLOCK_PROC_OUT_ENCI:/* ENCI */
		WRITE_VPP_REG_BITS(VPU_VLOCK_CTRL + offset_vlck, VLOCK_PROC_OUT_ENCI, 26, 2);
		break;
	case VLOCK_PROC_OUT_ENCP: /* ENCP */
		WRITE_VPP_REG_BITS(VPU_VLOCK_CTRL + offset_vlck, VLOCK_PROC_OUT_ENCP, 26, 2);
		break;
	default:
		break;
	}
...
void vlock_proc_vmode_check(struct vlock_proc_sig_sts *pvlock)
{
	const struct vinfo_s *vinfo;
	u32 offset_vlck;
	u32 offset_enc;
...
static void vlock_proc_lock_status_check(struct vlock_proc_sig_sts *pvlock)
{
	if (vlock_proc_get_phlock_flag() && vlock_proc_get_vlock_flag()) {
...
static void vlock_proc_disable_step1(struct vlock_proc_sig_sts *pvlock)
{
	...
	if (IS_VLOCK_PROC_PLL_MODE(vlock_work_mode)) {
		if (pvlock->dtdata->vlk_hw_ver >= vlk_hw_ver2) {
			...
			vlock_proc_set_panel_pll_m(pvlock, pvlock->val_m);
			/*tmp_value = vlock_get_panel_pll_frac(pvlock);*/
			/*m_reg_value = tmp_value & 0x1ffff;*/
			/*if (m_reg_value != (pvlock->val_frac & 0xfff))*/
			vlock_proc_set_panel_pll_frac(pvlock, pvlock->val_frac);
				/*vlock_hiu_reg_write(hhi_pll_reg_frac,*/
				/*	vlock.val_frac);*/
			pr_info("vlock restore original m:0x%x f:0x%x\n",
				pvlock->val_m, pvlock->val_frac);
		} else {
			tmp_value = vlock_proc_get_panel_pll_frac(pvlock);
			...
			if (m_reg_value != pvlock->pre_hiu_reg_frac) {
				...
				vlock_proc_set_panel_pll_frac(pvlock, tmp_value);
			}
			tmp_value = vlock_proc_get_panel_pll_m(pvlock);
			m_reg_value = tmp_value & 0x1ff;
			if (m_reg_value != pvlock->pre_hiu_reg_m &&
			    pvlock->pre_hiu_reg_m != 0) {
				tmp_value = (tmp_value & 0xfffffe00) |
					(pvlock->pre_hiu_reg_m & 0x1ff);
				vlock_proc_set_panel_pll_m(pvlock, tmp_value);
			}
		}
	}
	if (IS_VLOCK_PROC_ENC_MODE(vlock_work_mode)) {
	...
	if (vlock_work_mode & VLOCK_PROC_MODE_MANUAL_MIX_PLL_ENC) {
		vlock_work_mode &= ~VLOCK_PROC_MODE_MANUAL_MIX_PLL_ENC;
		vlock_work_mode |= VLOCK_PROC_MODE_MANUAL_PLL;
	}
...
static bool vlock_proc_disable_step2(struct vlock_proc_sig_sts *pvlock)
{
#ifndef CONFIG_AMLOGIC_REMOVE_OLD
	...
	if (vlock_dis_cnt > 0) {
		...
		vlock_proc_pll_select(pvlock, vlock_work_mode, vlock_proc_pll_sel_disable);
...
void vlock_proc_log_start(void)
{
	...
	vlock_log = kmalloc_array(vlk_log_size, sizeof(struct vlock_proc_log_s *),
				  GFP_KERNEL);
	if (!vlock_log)
		return;
	for (i = 0; i < vlk_log_size; i++) {
...
void vlock_proc_reg_get(struct vlock_proc_sig_sts *pvlock)
{
	unsigned int i;
	u32 offset_vlck = pvlock->offset_vlck;

	for (i = 0; i < VLOCK_REG_NUM; i++)
		vlock_log[vlk_log_cnt]->vlock_regs[i] =
			READ_VPP_REG(VPU_VLOCK_CTRL + offset_vlck + i);
}

static void vlock_proc_enable_step3_enc(struct vlock_proc_sig_sts *pvlock)
{
	...
	if (!(vlock_debug & VLOCK_DEBUG_ENC_PIXEL_ADJ_DIS) &&
	    pvlock->dtdata->vlk_chip < vlk_chip_tm2) {/*only adj line*/
		polity_pixel_num =
		...
		if (polity_pixel_num) {
			...
			if (pixel_num > vlock_enc_maxtune_pixels)
				pixel_num = vlock_enc_maxtune_pixels;
	...
	if (!(vlock_debug & VLOCK_DEBUG_ENC_LINE_ADJ_DIS) && vlock_proc_vsync_skip_for_frc()) {
		...
		if (polity_line_num) {
			...
			if (line_num > vlock_enc_maxtune_line)
				line_num = vlock_enc_maxtune_line;
	...
	if (vlock_log_en && vlk_log_cnt < vlk_log_size) {
		vlock_log[vlk_log_cnt]->enc_line_max = enc_max_line;
		vlock_log[vlk_log_cnt]->line_num_adj = line_num;
		vlock_log[vlk_log_cnt]->enc_pixel_max = enc_max_pixel;
		vlock_log[vlk_log_cnt]->pixel_num_adj = pixel_num;
		vlock_proc_reg_get(pvlock);
		vlk_log_cnt++;
	}
	/*if (vlock_debug & VLOCK_DEBUG_INFO)*/
	/*	pr_info(">>>[%s]\n", __func__);*/
...
static void vlock_proc_enable_step3_soft_enc(struct vlock_proc_sig_sts *pvlock)
{
	...
	if (vlock_log_en && vlk_log_cnt < vlk_log_size) {
		vlock_log[vlk_log_cnt]->enc_line_max = pvlock->pre_enc_max_line;
		vlock_log[vlk_log_cnt]->line_num_adj = line_adj;
		vlock_log[vlk_log_cnt]->enc_pixel_max = pvlock->pre_enc_max_pixel;
		vlock_log[vlk_log_cnt]->pixel_num_adj = pixel_adj;
		vlock_log[vlk_log_cnt]->T0 = T0;
		vlock_log[vlk_log_cnt]->vdif_err = err;
		vlock_log[vlk_log_cnt]->err_sum = pvlock->err_accum;
		vlock_log[vlk_log_cnt]->margin = margin;
		vlock_proc_reg_get(pvlock);
		vlk_log_cnt++;
	}
	if (vlock_debug & VLOCK_DEBUG_INFO)
...
static void vlock_proc_pll_adj_limit_check(unsigned int *pll_val)
{
...
static void vlock_proc_update_pll(struct vlock_proc_sig_sts *pvlock)
{
	/* pll data update */
	if (pvlock->dtdata->vlk_chip >= vlk_chip_t7) {
		if (pvlock->idx == VLOCK_PROC_ENC0) {
			vlock_proc_reg_wt_bits(VLOCK_PROC_REG_MAP_ANACTRL,
				ANACTRL_TCON_PLL_VLOCK, 2, 3, 2);
			vlock_proc_reg_wt_bits(VLOCK_PROC_REG_MAP_ANACTRL,
				ANACTRL_TCON_PLL_VLOCK, 0, 3, 2);
		} else if (pvlock->idx == VLOCK_PROC_ENC1) {
			vlock_proc_reg_wt_bits(VLOCK_PROC_REG_MAP_ANACTRL,
				ANACTRL_TCON_PLL_VLOCK, 2, 5, 2);
			vlock_proc_reg_wt_bits(VLOCK_PROC_REG_MAP_ANACTRL,
				ANACTRL_TCON_PLL_VLOCK, 0, 5, 2);
		} else if (pvlock->idx == VLOCK_PROC_ENC1) {
			vlock_proc_reg_wt_bits(VLOCK_PROC_REG_MAP_ANACTRL,
				ANACTRL_TCON_PLL_VLOCK, 2, 7, 2);
			vlock_proc_reg_wt_bits(VLOCK_PROC_REG_MAP_ANACTRL,
				ANACTRL_TCON_PLL_VLOCK, 0, 7, 2);
		}
	}
}

static void vlock_proc_enable_step3_pll(struct vlock_proc_sig_sts *pvlock)
{
	...
	if (abs_val > vlk_log_delta_ivcnt && vlk_log_delta_en & (1 << 0))
		pr_info("%s: abs_ivcnt over 0x%x:0x%x(last:0x%x,cur:0x%x)\n",
			__func__, vlk_log_delta_ivcnt,
	...
	if (pvlock->dtdata->vlk_hw_ver < vlk_hw_ver2)
		vlock_proc_pll_adj_limit_check(&m_f_reg_value);
	 ...
	 */
	if (vlock_input_type &&
	    pvlock->dtdata->vlk_hw_ver < vlk_hw_ver2)
	...
	if (pvlock->dtdata->vlk_hw_ver < vlk_hw_ver2) {
		...
		if (abs_val > vlk_log_delta_m &&
		    (vlk_log_delta_en & (1 << 4)))
		...
		if (pvlock->m_update_cnt++ > VLOCK_UPDATE_M_CNT) {
			...
			if (org_m != new_m || pre_m != org_m) {
				...
				if (tar_m != pre_m) {
					vlock_proc_set_panel_pll_m(pvlock, m_reg_value);
					mchang = 1;
					if (vlock_debug & VLOCK_DEBUG_INFO)
	...
	if (pvlock->dtdata->vlk_hw_ver < vlk_hw_ver2) {
		abs_val = abs(((m_f_reg_value & 0xfff) >> 2) -
			(tmp_value & 0xfff));
		if (abs_val >= vlk_log_delta_frac &&
		    (vlk_log_delta_en & (1 << 3)))
		...
		if (abs_val >= vlock_delta_limit &&
			...
			/*vlock_hiu_reg_write(hhi_pll_reg_frac, tmp_value);*/
			vlock_proc_set_panel_pll_frac(pvlock, tmp_value);
		...
		/*	!mchang) */{
			if (pvlock->f_update_cnt++ > VLOCK_UPDATE_F_CNT) {
				...
				vlock_proc_set_panel_pll_frac(pvlock, tmp_value);/*16:0*/
			}
		}
	}

	vlock_proc_update_pll(pvlock);
...
void vlock_proc_clear_frame_counter(struct vlock_proc_sig_sts *pvlock)
{
...
void vlock_proc_set_sts_by_frame_lock(bool en)
{
	...
	if (!en) {
		vlock_proc_disable_step1(pvlock);
		vlock_proc_disable_step2(pvlock);
		vlock_proc_clear_frame_counter(pvlock);
	} else {
...
void vlock_proc_enable_step3_auto_enc(struct vlock_proc_sig_sts *pvlock)
{
...
void vlock_proc_dev_param_init(void)
{
	vlock_tab[VLOCK_PROC_ENC0] = &vlock0;
	vlock_tab[VLOCK_PROC_ENC1] = &vlock1;
	vlock_tab[VLOCK_PROC_ENC2] = &vlock2;
}
...
void vlock_proc_status_init(void)
{
	...
	if (vlk_chip_type_id == vlk_chip_id_s5 ||
		vlk_chip_cls_id == VLK_AD_CHIP)
		return;
	...
	vinfo = get_current_vinfo();
	vlock_proc_dev_param_init();
	pvlock = vlock_tab[VLOCK_PROC_ENC0];

	if (!pvlock->dtdata)
		return;

	if (pvlock->dtdata->vlk_chip == vlk_chip_t7)
		max_enc_num = VLOCK_PROC_ENC2;
	else
		max_enc_num = VLOCK_PROC_ENC0;

	for (i = VLOCK_PROC_ENC0; i <= max_enc_num; i++) {
		pvlock = vlock_tab[i];
		pvlock->idx = i;
		if (i == VLOCK_PROC_ENC0) {
			pvlock->offset_encl = 0x00;
			pvlock->offset_vlck = 0x00;
		} else if (i == VLOCK_PROC_ENC1) {
			pvlock->offset_encl = 0x600;
			pvlock->offset_vlck = 0x40;
		} else if (i == VLOCK_PROC_ENC2) {
		...
		/*READ_VPP_REG_BITS(VPU_VIU_VENC_MUX_CTRL, 0, 2)*/) {/* ?? */
		switch (VLOCK_PROC_OUT_ENCL
		case 0:
		...
		pvlock->val_m = vlock_proc_get_panel_pll_m(pvlock);
		pvlock->val_frac = vlock_proc_get_panel_pll_frac(pvlock);
		...
		pr_info("%s vlock_enable:%d\n", __func__, vlock_enable);
	}
	pr_info("%s vlock_enable:%d adj_type:%d mode:%d\n", __func__, vlock_enable,
...
void vlock_proc_dts_match_init(struct vlock_match_data_s *pdata)
{
	...
	vlock_proc_dev_param_init();
	for (i = VLOCK_PROC_ENC0; i <= VLOCK_PROC_ENC2; i++) {
		pvlock = vlock_tab[i];
		pvlock->dtdata = pdata;
	}
	/*vlock_enable = vlock.dtdata.vlk_support;*/
...
void vlock_proc_set_phase(struct vlock_proc_sig_sts *pvlock, u32 percent)
{
	u32 ia;
	u32 data = 0;
	u32 offset_vlck;
...
void vlock_proc_set_phase_en(struct vlock_proc_sig_sts *pvlock, u32 en)
{
	u32 offset_vlck = pvlock->offset_vlck;
	...
	if (en) {
		...
		if (IS_VLOCK_PROC_PLL_MODE(vlock_work_mode))
			vlock_proc_hw_reinit(pvlock,
					vlock_pll_phase_setting, VLOCK_PHASE_REG_SIZE);
		else
			vlock_proc_hw_reinit(pvlock,
					vlock_encl_phase_setting, VLOCK_PHASE_REG_SIZE);
		/*WRITE_VPP_REG_BITS(VPU_VLOCK_CTRL + offset_vlck, 3, 0, 2);*/
		if (IS_VLOCK_PROC_PLL_MODE(vlock_work_mode))
			WRITE_VPP_REG_BITS(VPU_VLOCK_CTRL + offset_vlck, 1, 0, 1);
		else
...
u32 vlock_proc_get_phase_en(u32 enc_idx)
{
	struct vlock_proc_sig_sts *pvlock;

	if (enc_idx < VLOCK_PROC_ENC_MAX)
...
void vlock_proc_phaselock_check(struct vlock_proc_sig_sts *pvlock,
			   struct vframe_s *vf)
{
	/*vs_i*/
	u32 ia;
...
bool vlock_proc_get_phlock_flag_ex(struct vlock_proc_sig_sts *pvlock)
{
	u32 flag;
	u32 sts;
	u32 offset_vlck;
...
bool vlock_proc_get_vlock_flag_ex(struct vlock_proc_sig_sts *pvlock)
{
	u32 flag;
	u32 sts;
	u32 offset_vlck;
...
bool vlock_proc_get_phlock_flag(void)
{
	...
	return vlock_proc_get_phlock_flag_ex(pvlock);
}
...
bool vlock_proc_get_vlock_flag(void)
{
	...
	return vlock_proc_get_vlock_flag_ex(pvlock);
}

u32 vlock_proc_get_vlock_sts(void)
{
	struct vlock_proc_sig_sts *pvlock;
	enum vlock_proc_enc_num_e enc_mux = VLOCK_PROC_ENC0;
...
void vlock_proc_enc_timing_monitor(struct vlock_proc_sig_sts *pvlock)
{
...
void vlock_proc_auto_pll_sts_check(struct vlock_proc_sig_sts *pvlock)
{
	u32 m_f_reg_value;
	u32 pll_m, pll_f;
	u32 offset_vlck = pvlock->offset_vlck;
...
u32 vlock_proc_fsm_check_support(struct vlock_proc_sig_sts *pvlock,
			    struct vframe_s *vf, struct vinfo_s *vinfo)
{
	...
	if (freerun_type == VLOCK_PROC_GAME_MODE &&
		...
		if (vlock_debug & VLOCK_DEBUG_INFO)
			pr_info("[%s]  FREERUN input 50 or 60hz, fix out frame rate,video_clk :%d!!!\n",
				__func__, video_clk);
		ret = true;
	} else if (freerun_type == VLOCK_PROC_FREERUN_MODE &&
			vinfo->fr_adj_type == VOUT_FR_ADJ_FREERUN){
		if (vlock_debug & VLOCK_DEBUG_INFO)
			pr_info("[%s] video_clk :%d VOUT_FR_ADJ_FREERUN and freerun_type == VLOCK_PROC_FREERUN_MODE!!!\n",
				__func__, video_clk);
		ret = false;
	}
...
void vlock_proc_vmd_input_check(struct vlock_proc_sig_sts *pvlock)
{
...
u32 vlock_proc_fsm_input_check(struct vlock_proc_sig_sts *pvlock, struct vframe_s *vf)
{
	u32 ret = 0;
	u32 vframe_sts;
	...
	vlock_proc_vmode_check(pvlock);
	vlock_proc_lock_status_check(pvlock);

	if (vf) {
		vinfo = get_current_vinfo();
		pvlock->input_hz = vlock_proc_check_input_hz(vf);
		pvlock->output_hz =
		vlock_proc_check_output_hz(vinfo->sync_duration_num,
	...
	if (pvlock->vf_sts)
		pvlock->md_support = vlock_proc_fsm_check_support(pvlock, vf, vinfo);

	return ret;
}

u32 vlock_proc_fsm_to_en_func(struct vlock_proc_sig_sts *pvlock,
			 struct vframe_s *vf)
{
	u32 ret = 0;
	struct vinfo_s *vinfo;
	...
	offset_enc = pvlock->offset_encl;
	vlock_proc_input_sel(pvlock, vf->type, vf->source_type);
	if (vf->source_type != pre_source_type ||
		...
		//}
		vinfo = get_current_vinfo();
		vlock_proc_enable_step1(vf, vinfo, pvlock);
		if (vlock_ss_en && IS_VLOCK_PROC_PLL_MODE(vlock_work_mode)) {
			vlock_proc_set_panel_ss(false);
			pvlock->ss_sts = false;
		}
		ret = 1;
	}
...
u32 vlock_proc_fsm_en_step1_func(struct vlock_proc_sig_sts *pvlock,
			    struct vframe_s *vf)
{
	u32 ret = 0;
	u32 input_vs_cnt;
	...
	if (pvlock->frame_cnt_in <= 3 &&
	    ((vlock_work_mode & (VLOCK_PROC_MODE_MANUAL_ENC |
	    VLOCK_PROC_MODE_MANUAL_PLL)))) {
		/*WRITE_VPP_REG_BITS(VPU_VLOCK_CTRL + offset_vlck, 1, 5, 1);*/
		/*WRITE_VPP_REG_BITS(VPU_VLOCK_CTRL + offset_vlck, 1, 2, 1);*/
		vlock_proc_reset(pvlock, 1);
		...
		vlock_proc_pll_select(pvlock, vlock_work_mode, pvlock->dtdata->vlk_pll_sel);
...
u32 vlock_proc_fsm_check_lock_sts(struct vlock_proc_sig_sts *pvlock,
			     struct vframe_s *vf)
{
	u32 frqlock_sts = vlock_proc_get_vlock_flag_ex(pvlock);
	u32 phlock_sts = vlock_proc_get_phlock_flag_ex(pvlock);
	u32 pherr;
	//static u32 rstflag;
	u32 ret = VLOCK_PROC_CHG_NONE;
	//static u32 cnt;
	...
	if ((vlock_debug & VLOCK_DEBUG_INFO) && pvlock->chk_lock_sts_cnt == 1 &&
	    IS_VLOCK_PROC_AUTO_MODE(vlock_work_mode))
	...
	/*check phase flag*/
	if (pvlock->dtdata->vlk_hw_ver >= vlk_hw_ver2 && pvlock->phlock_en) {
		if (pvlock->start_chk_ph) {
			if (!vlock_proc_get_phlock_flag_ex(pvlock)) {
				if (pvlock->all_lock_cnt++ > 2) {
					...
					if (vlock_debug & VLOCK_DEBUG_INFO_ERR)
						pr_info("ph unlock:ph err=0x%x\n",
							pherr);
					return VLOCK_PROC_CHG_NEED_RESET;
				}
			}
		} else {
...
u32 vlock_proc_fsm_en_step2_func(struct vlock_proc_sig_sts *pvlock,
			    struct vframe_s *vf)
{
	u32 ret = 0;
	...
	if (vlock_dynamic_adjust &&
		...
		(IS_VLOCK_PROC_MANUAL_MODE(vlock_work_mode))) {
		if (IS_VLOCK_PROC_MANUAL_ENC_MODE(vlock_work_mode))
			vlock_proc_enable_step3_enc(pvlock);
		else if (IS_VLOCK_PROC_MANUAL_PLL_MODE(vlock_work_mode) &&
			 (!pvlock->pll_mode_pause))
			vlock_proc_enable_step3_pll(pvlock);
		else if (IS_VLOCK_PROC_MANUAL_SOFTENC_MODE(vlock_work_mode))
			vlock_proc_enable_step3_soft_enc(pvlock);
	}

	if (IS_VLOCK_PROC_ENC_MODE(vlock_work_mode)) {
		vlock_proc_enable_step3_auto_enc(pvlock);
		vlock_proc_enc_timing_monitor(pvlock);
	}

	if (IS_VLOCK_PROC_AUTO_PLL_MODE(vlock_work_mode))
		vlock_proc_auto_pll_sts_check(pvlock);

	/*check phase*/
	vlock_proc_phaselock_check(pvlock, vf);

	ret = vlock_proc_fsm_check_lock_sts(pvlock, vf);

	if (ret == VLOCK_PROC_CHG_NEED_RESET)
		return 0;
	else
		return 1;
}

void vlock_proc_fsm_monitor(struct vframe_s *vf, struct vlock_proc_sig_sts *pvlock)
{
	u32 changed;
	u32 timeout = 0;
	//u32 percent;
	...
	changed = vlock_proc_fsm_input_check(pvlock, vf);
	vlock_proc_vmd_input_check(pvlock);
		...
		if (pvlock->vf_sts) {
			...
			if (pvlock->frame_cnt_in++ >= VLOCK_START_CNT) {
				...
				if (pvlock->md_support) {
					if (vlock_proc_fsm_to_en_func(pvlock, vf)) {
						vlock_proc_clear_frame_counter(pvlock);
						pvlock->fsm_sts =
	...
	case VLOCK_STATE_ENABLE_STEP1_DONE:
		...
		if (pvlock->vf_sts) {
			pvlock->frame_cnt_in++;
			pvlock->frame_cnt_no = 0;
			if (vlock_proc_fsm_en_step1_func(pvlock, vf))
				pvlock->fsm_sts = VLOCK_STATE_ENABLE_STEP2_DONE;
		} else if (pvlock->vmd_chg) {
			vlock_proc_clear_frame_counter(pvlock);
			pvlock->vmd_chg = false;
	...
	case VLOCK_STATE_ENABLE_STEP2_DONE:
		if (pvlock->vf_sts) {
			...
			if (!pvlock->md_support) {
				...
				vlock_proc_clear_frame_counter(pvlock);
				pvlock->fsm_sts = VLOCK_STATE_DISABLE_STEP1_DONE;
			} else if (vlock_latch_flag & FLAG_VLOCK_DISABLE) {
				/*disable vlock by vecm cmd*/
				vlock_proc_disable_step1(pvlock);
				timeout = 0;
				while (vlock_proc_disable_step2(pvlock) == 0) {
					if (timeout++ > 4)
						break;
				}
				//vlock_enable = 0;
				//vlock_latch_flag &= ~FLAG_VLOCK_DISABLE;
				if (vlock_debug & VLOCK_DEBUG_INFO)
					pr_info("[%s] vlock dis\n", __func__);
				vlock_proc_clear_frame_counter(pvlock);
				pvlock->fsm_sts = VLOCK_STATE_DISABLE_STEP1_DONE;
				...
				/*normal mode*/
				pvlock->frame_cnt_no = 0;
				if (vlock_proc_fsm_en_step2_func(pvlock, vf) <= 0 ||
					...
					vlock_proc_get_phlock_flag_ex(pvlock) &&
					vlock_proc_get_vlock_flag_ex(pvlock) && vlock_is_ph_opt) {
					if (vlock_ph_opt_percent > 0) {
						vlock_adjust = vlock_adjust - 1;
						vlock_proc_set_phase(pvlock, vlock_adjust);
					...
					if (vlock_debug & VLOCK_DEBUG_PHASE_OPTIMIZE)
						...
						vlock_proc_get_phlock_flag(),
						vlock_proc_get_vlock_flag(),
						vlock_proc_get_phlock_flag_ex(pvlock),
						vlock_proc_get_vlock_flag_ex(pvlock));
				}
			}
		} else if (pvlock->vmd_chg) {
			vlock_proc_clear_frame_counter(pvlock);
	...
	case VLOCK_STATE_DISABLE_STEP1_DONE:
		vlock_proc_disable_step1(pvlock);
		if (vlock_proc_disable_step2(pvlock))
			pvlock->fsm_sts = VLOCK_STATE_NULL;
		else
			pvlock->fsm_sts = VLOCK_STATE_DISABLE_STEP2_DONE;
		break;
	...
	case VLOCK_STATE_DISABLE_STEP2_DONE:
		if (vlock_proc_disable_step2(pvlock)) {
			pvlock->fsm_sts = VLOCK_STATE_NULL;
			if (vlock_latch_flag & FLAG_VLOCK_DISABLE) {
				vlock_enable = 0;
				vlock_latch_flag &= ~FLAG_VLOCK_DISABLE;
			}
		}
	...
	if (((vlock_work_mode & (VLOCK_PROC_MODE_AUTO_PLL |
	     VLOCK_PROC_MODE_AUTO_ENC))) &&
	    vlock_log_en && vlk_log_cnt < vlk_log_size &&
 ...
 */
u32 vlock_proc_chk_is_small_win(struct vpp_frame_par_s *cur_video_sts)
{
		   ...
		   struct vpp_frame_par_s *cur_video_sts)
{
	...
	if (probe_ok == 0 || !vlock_enable || !cur_video_sts) {
		if (vlock_debug & VLOCK_DEBUG_INFO) {
			pr_info("%s probe_ok:%d vlock_enable:%d\n",
				__func__, probe_ok, vlock_enable);
			if (!cur_video_sts)
	...
	if (!vlock_hw_clk_ok && (vlock_debug & VLOCK_DEBUG_INFO))
		pr_info("hw clk not config\n");

	if (!(vlock_debug & VLOCK_DEBUG_FORCE_ON)) {
		if (vlock_proc_chk_is_small_win(cur_video_sts)) {
			if (pvlock->dtdata->vlk_ctl_for_frc)
				pvlock->fsm_sts = VLOCK_STATE_NULL;
			if (vlock_debug & VLOCK_DEBUG_INFO)
	...
	if (pvlock->dtdata->vlk_new_fsm)
		vlock_proc_fsm_monitor(vf, pvlock);
}
EXPORT_SYMBOL(vlock_proc_process);
...
void vlock_proc_param_set(unsigned int val, enum vlock_proc_param_e sel)
{
	...
	case VLOCK_PROC_EN:
		vlock_enable = val;
		break;
	case VLOCK_PROC_ADAPT:
		vlock_adapt = val;
		break;
	case VLOCK_PROC_MODE:
		vlock_work_mode = val;
		break;
	case VLOCK_PROC_DIS_CNT_LIMIT:
		vlock_dis_cnt_limit = val;
		break;
	case VLOCK_PROC_DELTA_LIMIT:
		vlock_delta_limit = val;
		break;
	case VLOCK_PROC_PLL_M_LIMIT:
		vlock_pll_m_limit = val;
		break;
	case VLOCK_PROC_DELTA_CNT_LIMIT:
		vlock_delta_cnt_limit = val;
		break;
	case VLOCK_PROC_DEBUG:
		vlock_debug = val;
		break;
	case VLOCK_PROC_DYNAMIC_ADJUST:
		vlock_dynamic_adjust = val;
		break;
	case VLOCK_PROC_DIS_CNT_NO_VF_LIMIT:
		vlock_dis_cnt_no_vf_limit = val;
		break;
	case VLOCK_PROC_LINE_LIMIT:
		vlock_line_limit = val;
		break;
	case VLOCK_PROC_SUPPORT:
		vlock_support = val;
		break;
	default:
...
void vlock_proc_status(struct vlock_proc_sig_sts *pvlock)
{
	...
	pr_info("\nvlock_proc Idx:%d parameters status\n", pvlock->idx);
	pr_info("vlock_proc driver version: %s\n", VLOCK_VER);
	pr_info("vlock_mode:%d\n", vlock_work_mode);
	pr_info("vlock_enable:%d\n", vlock_enable);
	...
	pr_info("vlk_hw_ver:%d\n", pvlock->dtdata->vlk_hw_ver);
...
void vlock_proc_reg_dump(struct vlock_proc_sig_sts *pvlock)
{
	...
	val = vlock_proc_get_panel_pll_m(pvlock);
	pr_info("pll m=0x%08x\n", val);
	val = vlock_proc_get_panel_pll_frac(pvlock);
			  ...
			  enum vframe_source_type_e source_type)
{
	...
	vlock_input_type = type & VIDTYPE_TYPEMASK;
	//if (vlock_input_type == VIDTYPE_PROGRESSIVE ||
	//    (vlock_mode & VLOCK_MODE_MANUAL_SOFT_ENC))
	//	return;
	if (vlock_input_type == VIDTYPE_INTERLACE_TOP) {
		/*
		 *1:fromhdmi rx ,
		 *2:from tv-decoder,
		 *3:from dvin,
		...
		if (source_type == VFRAME_SOURCE_TYPE_TUNER ||
			...
			/* Input Vsync source select from tv-decoder */
			WRITE_VPP_REG_BITS(VPU_VLOCK_CTRL + offset_vlck,
				VLOCK_PROC_SRC_TV_DEC, 16, 3);
		else if (source_type == VFRAME_SOURCE_TYPE_HDMI)
			/* Input Vsync source select from hdmi-rx */
			WRITE_VPP_REG_BITS(VPU_VLOCK_CTRL + offset_vlck,
				VLOCK_PROC_SRC_HDMI, 16, 3);
	} else {
...
void vlock_proc_lcd_param_work(struct work_struct *p_work)
{
	...
	while (i++ < VLOCK_LCD_RETRY_MAX) {
		...
		if (param[0] & LCD_VLOCK_PARAM_BIT_UPDATE) {
			if (param[0] & LCD_VLOCK_PARAM_BIT_VALID) {
				vlock_enable = param[1];
				vlock_work_mode = param[2];
				vlock_pll_m_limit = param[3];
				vlock_line_limit = param[4];

				if (vlock_work_mode &
					VLOCK_PROC_MODE_MANUAL_MIX_PLL_ENC) {
					vlock_work_mode &=
						~VLOCK_PROC_MODE_MANUAL_MIX_PLL_ENC;
					vlock_work_mode |= VLOCK_PROC_MODE_MANUAL_PLL;
				}
	...
	pr_info("lcd vlock_enable=%d, vlock_work_mode=0x%x i=%d\n",
		vlock_enable, vlock_work_mode, i);
}
#endif
...
void vlock_proc_dts_config(struct device_node *node)
{
	...
	ret = of_property_read_u32(node, "vlock_enable", &val);
	if (!ret)
		vlock_enable = val;
	...
	ret = of_property_read_u32(node, "framelock_priority", &val);
	if (!ret)
		framelock_priority = val;

	pr_info("%s vlk_en=%d, vlk_md=0x%x vlk_pll_m_limit=%d vlk_line_limit=%d flk_priority=%d\n",
		__func__, vlock_enable, vlock_work_mode, vlock_pll_m_limit,
		vlock_line_limit, framelock_priority);
...
int vlock_proc_notify_callback(struct notifier_block *block, unsigned long cmd,
			  void *para)
{
	const struct vinfo_s *vinfo;
	u32 cnt = 0;
	...
	case VOUT_EVENT_MODE_CHANGE_PRE:
		...
		framelock_mode_chg(cmd);
		if (pvlock->dtdata->vlk_new_fsm &&
		    pvlock->fsm_sts >= VLOCK_STATE_ENABLE_STEP1_DONE &&
		    pvlock->fsm_sts <= VLOCK_STATE_DISABLE_STEP1_DONE) {
			/*stop vlock*/
			vlock_proc_disable_step1(pvlock);
			while (!vlock_proc_disable_step2(pvlock)) {
				if (cnt++ > 10)
					break;
			}
		}
...
int vlock_proc_phase_config(char *str)
{
...
__setup("video_reverse=", vlock_proc_phase_config);
...
void vlock_proc_hiu_reg_config(struct device *dev)
{
	...
	vlock_proc_init_reg_map(dev, pvlock);
...
void vlock_proc_parse_param(char *buf_orig, char **parm)
{
	char *ps, *token;
	unsigned int n = 0;
	char delim1[3] = " ";
			 ...
			 struct class_attribute *attr, char *buf)
{
	...
	len += sprintf(buf + len,
		"echo vlock_work_mode val(0/1/2) > /sys/class/amvecm/vlock\n");
	len += sprintf(buf + len,
		"echo vlock_enable val(0/1) > /sys/class/amvecm/vlock\n");
	len += sprintf(buf + len,
		...
		const char *buf, size_t count)
{
	...
	long val;
	unsigned int temp_val;
	enum vlock_proc_param_e sel = VLOCK_PROC_PARAM_MAX;
	struct vlock_proc_sig_sts *pvlock;
	enum vlock_proc_enc_num_e enc_mux = VLOCK_PROC_ENC0;
	...
	vlock_proc_parse_param(buf_orig, (char **)&parm);
	if (!strncmp(parm[0], "vlock_mode", 10)) {
		if (kstrtol(parm[1], 10, &val) < 0)
			return -EINVAL;
		temp_val = val;
		sel = VLOCK_PROC_MODE;
	} else if (!strncmp(parm[0], "vlock_enable", 8)) {
		if (kstrtol(parm[1], 10, &val) < 0)
			return -EINVAL;
		temp_val = val;
		sel = VLOCK_PROC_EN;
		...
		if (kstrtol(parm[1], 10, &val) < 0)
			return -EINVAL;
		temp_val = val;
		sel = VLOCK_PROC_SUPPORT;
	} else if (!strncmp(parm[0], "enable", 6)) {
		vlock_latch_flag |= FLAG_VLOCK_ENABLE;
		vlock_proc_set_en(true);
	} else if (!strncmp(parm[0], "disable", 7)) {
		vlock_latch_flag |= FLAG_VLOCK_DISABLE;
		if (pvlock->fsm_sts <= VLOCK_STATE_ENABLE_STEP1_DONE)
			vlock_proc_set_en(false);
	} else if (!strncmp(parm[0], "status", 6)) {
		if (!parm[1]) {
			temp_val = VLOCK_PROC_ENC0;
		} else {
			if (kstrtol(parm[1], 10, &val) < 0)
				return -EINVAL;
			if (val <= VLOCK_PROC_ENC2)
				temp_val = val;
			else
				temp_val = VLOCK_PROC_ENC0;
		}
		pvlock = vlock_tab[temp_val];
		vlock_proc_status(pvlock);
		...
		vlock_proc_log_start();
	} else if (!strncmp(parm[0], "log_stop", 8)) {
		vlock_proc_log_stop();
	} else if (!strncmp(parm[0], "log_print", 9)) {
		vlock_proc_log_print();
	} else if (!strncmp(parm[0], "phase", 5)) {
		if (kstrtol(parm[1], 10, &val) < 0)
			return -EINVAL;
		vlock_proc_set_phase(pvlock, val);
	} else if (!strncmp(parm[0], "phlock_en", 9)) {
		if (kstrtol(parm[1], 10, &val) < 0)
			return -EINVAL;
		pvlock = vlock_tab[VLOCK_PROC_ENC0];
		vlock_proc_set_phase_en(pvlock, val);
		pvlock = vlock_tab[VLOCK_PROC_ENC1];
		vlock_proc_set_phase_en(pvlock, val);
		pvlock = vlock_tab[VLOCK_PROC_ENC2];
		vlock_proc_set_phase_en(pvlock, val);
	...
	if (sel < VLOCK_PROC_PARAM_MAX)
		vlock_proc_param_set(temp_val, sel);
	kfree(buf_orig);
	return count;
}
