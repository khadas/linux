...
static int meson_gdc_open(struct inode *inode, struct file *file)
{
	...
	gdc_log(LOG_DEBUG, "Success open\n");
...
static int gdc_process_input_ion_info(struct gdc_context_s *context,
				      struct gdc_settings_ex *gs_ex)
{
	int ret = -1;
	unsigned long addr;
	...
	for (i = 0; i < plane_number; i++) {
		...
		ret = meson_ion_share_fd_to_phys(fd,
						 (phys_addr_t *)&addr,
						 &len);
		if (ret < 0) {
...
static int gdc_process_output_ion_info(struct gdc_context_s *context,
				       struct gdc_settings_ex *gs_ex)
{
	int ret = -1;
	unsigned long addr;
			       ...
			       struct gdc_settings_ex *gs_ex)
{
	...
	if (gs_ex->config_buffer.mem_alloc_type == AML_GDC_MEM_ION) {
		/* ion alloc */
		ret = meson_ion_share_fd_to_phys(gs_ex->config_buffer.shared_fd,
						 (phys_addr_t *)&addr,
						 &len);
		if (ret < 0) {
			...
			struct gdc_settings_with_fw *gs_with_fw)
{
	...
	if (gs_with_fw->output_buffer.mem_alloc_type == AML_GDC_MEM_ION) {
		/* ion alloc */
		ret = gdc_process_output_ion_info
			(context,
	...
	if (gs_with_fw->input_buffer.mem_alloc_type == AML_GDC_MEM_ION) {
		/* ion alloc */
		ret = gdc_process_input_ion_info
			(context,
			 (struct gdc_settings_ex *)gs_with_fw);
		if (ret < 0) {
			ret = -EINVAL;
...
static DEVICE_ATTR_RW(firmware1);
