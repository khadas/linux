...
#define HDR_MAX_STATUS       7
char *hdmitx21_hdr_status[HDR_MAX_STATUS] = {
	"HDR10Plus-VSIF",
	"DolbyVision-Std",
...
static void hdmitx_early_suspend(struct early_suspend *h)
{
	...
	bool need_rst_ratio = hdmitx21_find_vendor_ratio(hdev);
...
static int set_disp_mode_auto(void)
{
	...
	if (!hdmitx21_edid_check_valid_mode(hdev, para)) {
...
static void hdmitx_set_drm_pkt(struct master_display_info_s *data)
{
	...
	enum hdmi_tf_type hdmi_hdr_status = hdmitx21_get_cur_hdr_st();
...
/* if using VSIF/DOVI, then only clear DV_VS10_SIG, else disable VSIF */
	...
	if (hdev->hdr_transfer_feature == T_BT709 &&
		...
		if (hdmi_hdr_status == HDMI_HDR_SMPTE_2084 || hdmi_hdr_status == HDMI_HDR_HLG) {
			pr_info("%s: HDR->SDR, hdmi_hdr_status=%d\n",
			    ...
			    const char *buf, size_t count)
{
	...
	if (strncmp(buf, "unplug_powerdown", 16) == 0) {
		 ...
		 * emulate SDR->HDR->DV switch
		 */
		/* step1: SDR-->HDR */
		data.features = 0x00091000;
		hdmitx_set_drm_pkt(&data);
		/* in issue log, there's about 20ms period between
		 * hdr->dov_ll switch, here we can add delay of mute_frames
		 * to emulate. The shorter the delay, the easier to reproduce
		 */
 ...
 */
static bool hdmi21_sink_disp_mode_sup(enum hdmi_vic vic)
{
	const struct hdmi_timing *timing;
	char *mode;
	...
	if (_is_y420_vic(vic)) {
		if (drm_hdmitx_chk_mode_attr_sup(mode, "420,8bit"))
			return true;
		if (drm_hdmitx_chk_mode_attr_sup(mode, "rgb,8bit"))
			return true;
		if (drm_hdmitx_chk_mode_attr_sup(mode, "444,8bit"))
			return true;
		if (drm_hdmitx_chk_mode_attr_sup(mode, "422,12bit"))
			return true;
	} else {
		if (drm_hdmitx_chk_mode_attr_sup(mode, "rgb,8bit"))
			return true;
	...
	for (i = 0; i < prxcap->VIC_count; i++) {
		...
		if (hdev->data->efuse_dis_output_4k) {
			if (timing->v_active >= 2160)
				continue;
		} else if (hdev->data->efuse_dis_hdmi_4k60) {
			if (timing->v_active >= 2160 && timing->v_freq >= 50000)
				continue;
		}
				    ...
				    struct device_attribute *attr, char *buf)
{
	...
	pos += snprintf(buf + pos, PAGE_SIZE, (hdr_status < HDR_MAX_STATUS) ?
			hdmitx21_hdr_status[hdr_status] : "invalid");
...
static int amhdmitx_probe(struct platform_device *pdev)
{
	...
	hdmitx21_audio_mute_op(1, 0); /* default audio clock is ON */
	if (!hdev->pxp_mode)
...
static int hdmitx21_boot_edid_check(char *str)
{
	unsigned int val = 0;
	struct hdmitx_dev *hdev = get_hdmitx21_device();
...
__setup("edid_check=", hdmitx21_boot_edid_check);
