...
void lcd_vbyone_interrupt_enable(struct aml_lcd_drv_s *pdrv, int flag)
{
	...
	if (flag) {
		...
		if (vx1_conf->intr_en) {
			vx1_fsm_acq_st = 0;
			/* clear interrupt */
			lcd_vcbus_setb(VBO_INTR_STATE_CTRL + offset, 0x01ff, 0, 9);
			lcd_vcbus_setb(VBO_INTR_STATE_CTRL + offset, 0, 0, 9);

			/* set hold in FSM_ACQ */
			if (vx1_conf->vsync_intr_en == 3 ||
			    vx1_conf->vsync_intr_en == 4)
				lcd_vcbus_setb(VBO_FSM_HOLDER_L + offset, 0, 0, 16);
			else
				lcd_vcbus_setb(VBO_FSM_HOLDER_L + offset, 0xffff, 0, 16);
			/* enable interrupt */
			lcd_vcbus_setb(VBO_INTR_UNMASK + offset,
				       VBYONE_INTR_UNMASK, 0, 15);
		} else {
			/* mask interrupt */
			lcd_vcbus_write(VBO_INTR_UNMASK + offset, 0x0);
			if (vx1_conf->vsync_intr_en) {
				/* keep holder for vsync monitor enabled */
				/* set hold in FSM_ACQ */
				if (vx1_conf->vsync_intr_en == 3 ||
				    vx1_conf->vsync_intr_en == 4)
					lcd_vcbus_setb(VBO_FSM_HOLDER_L + offset,
						       0, 0, 16);
				else
					lcd_vcbus_setb(VBO_FSM_HOLDER_L + offset,
						       0xffff, 0, 16);
...
static void lcd_vx1_hold_reset(struct aml_lcd_drv_s *pdrv)
{
	...
	vx1_fsm_acq_st = 0;
	lcd_vcbus_write(VBO_INTR_UNMASK + offset, 0x0); /* mask interrupt */
	/* clear interrupt */
	lcd_vcbus_setb(VBO_INTR_STATE_CTRL + offset, 0x01ff, 0, 9);
	lcd_vcbus_setb(VBO_INTR_STATE_CTRL + offset, 0, 0, 9);

	/* clear FSM_continue */
	lcd_vcbus_setb(VBO_INTR_STATE_CTRL + offset, 0, 15, 1);

	lcd_vbyone_sw_reset(pdrv);
	/* clear lockn raising flag */
	lcd_vcbus_setb(VBO_INTR_STATE_CTRL + offset, 1, 7, 1);
	/* clear lockn raising flag */
	lcd_vcbus_setb(VBO_INTR_STATE_CTRL + offset, 0, 7, 1);

	/* enable interrupt */
	lcd_vcbus_setb(VBO_INTR_UNMASK + offset, VBYONE_INTR_UNMASK, 0, 15);
}
...
int lcd_vbyone_vsync_handler(struct aml_lcd_drv_s *pdrv)
{
	...
	if (lcd_vx1_isr_flag == 0)
		return 0;
	if (lcd_vcbus_read(VBO_STATUS_L + offset) & 0x40) /* hpd detect */
		return 0;

	lcd_vcbus_setb(VBO_INTR_STATE_CTRL + offset, 1, 0, 1);
	lcd_vcbus_setb(VBO_INTR_STATE_CTRL + offset, 0, 0, 1);
	...
	if (vx1_conf->vsync_intr_en == 4) {
		if (vsync_cnt == 3) {
			lcd_vcbus_setb(VBO_INTR_STATE_CTRL + offset, 0x3ff, 0, 10);
			lcd_vcbus_setb(VBO_INTR_STATE_CTRL + offset, 0, 0, 10);
			vsync_cnt++;
		} else if (vsync_cnt >= 5) {
			vsync_cnt = 0;
			if ((lcd_vcbus_read(VBO_INTR_STATE + offset) & 0x40)) {
				lcd_vbyone_hw_filter(pdrv, 0);
				lcd_vbyone_sw_reset(pdrv);
				LCDPR("[%d]: vx1 sw_reset 4\n", pdrv->index);
				while (lcd_vcbus_read(VBO_STATUS_L + offset) & 0x4)
					break;
				lcd_vcbus_setb(VBO_INTR_STATE_CTRL + offset,
					       0x3ff, 0, 10);
				lcd_vcbus_setb(VBO_INTR_STATE_CTRL + offset, 0, 0, 10);
				lcd_vbyone_hw_filter(pdrv, 1);
			}
		...
		if (vsync_cnt < VSYNC_CNT_VX1_RESET) {
			...
			if (lcd_vcbus_read(VBO_STATUS_L + offset) & 0x20) {
				vsync_cnt = VSYNC_CNT_VX1_STABLE;
				lcd_vbyone_hw_filter(pdrv, 1);
			} else {
				vsync_cnt++;
		...
		if (vsync_cnt >= 5) {
			...
			if (!(lcd_vcbus_read(VBO_STATUS_L + offset) & 0x20)) {
				...
				lcd_vcbus_setb(VBO_INTR_STATE_CTRL + offset, 0, 15, 1);
				lcd_vcbus_setb(VBO_INTR_STATE_CTRL + offset, 1, 15, 1);
			} else {
				lcd_vbyone_hw_filter(pdrv, 1);
			}
		} else {
		...
		if (vx1_training_wait_cnt >= VX1_TRAINING_TIMEOUT) {
			if ((lcd_vcbus_read(VBO_STATUS_L + offset) & 0x3f) != 0x20) {
				if (vx1_timeout_reset_flag == 0) {
...
static irqreturn_t lcd_vbyone_interrupt_handler(int irq, void *data)
{
	...
	encl_clk = lcd_encl_clk_msr(pdrv);
	data32 = (lcd_vcbus_read(VBO_INTR_STATE + offset) & 0x7fff);
	/* clear the interrupt */
	data32_1 = ((data32 >> 9) << 3);
	if (data32 & 0x1c0)
		data32_1 |= (1 << 2);
	...
	if (data32 & 0x7)
		data32_1 |= (1 << 0);
	lcd_vcbus_setb(VBO_INTR_STATE_CTRL + offset, data32_1, 0, 9);
	lcd_vcbus_setb(VBO_INTR_STATE_CTRL + offset, 0, 0, 9);
...
static void lcd_vbyone_interrupt_init(struct aml_lcd_drv_s *pdrv)
{
	...
	if (vx1_conf->vsync_intr_en == 3 ||
		...
		lcd_vcbus_setb(VBO_FSM_HOLDER_L + offset, 0xffff, 0, 16);
	/* set hold in FSM_CDR */
	lcd_vcbus_setb(VBO_FSM_HOLDER_H + offset, 0, 0, 16);
	/* not wait lockn to 1 in FSM_ACQ */
	lcd_vcbus_setb(VBO_CTRL_L + offset, 1, 10, 1);
	...
	lcd_vcbus_setb(VBO_INTR_STATE_CTRL + offset, 1, 12, 1);    /* intr pulse width */
	lcd_vcbus_setb(VBO_INTR_STATE_CTRL + offset, 0x01ff, 0, 9); /* clear interrupt */
	lcd_vcbus_setb(VBO_INTR_STATE_CTRL + offset, 0, 0, 9);
