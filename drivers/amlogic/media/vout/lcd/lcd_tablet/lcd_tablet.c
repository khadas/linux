				    ...
				    struct device *dev)
{
	...
	case LCD_VBYONE:
		...
		if (ret) {
			...
			if (lcd_debug_print_flag) {
				LCDPR
				("phy vswing=0x%x, preemphasis=0x%x\n",
	...
	case LCD_MIPI:
		...
		if (ret) {
			...
			pconf->lcd_control.mipi_config->bit_rate_max = para[1];
			pconf->lcd_control.mipi_config->factor_numerator =
						para[2];
		...
		pconf->lcd_control.mipi_config->operation_mode_display =
			...
			pconf->lcd_control.mipi_config->clk_always_hs =
						para[6];
			pconf->lcd_control.mipi_config->phy_switch = para[7];
		}
...
static void lcd_set_vinfo(unsigned int sync_duration)
{
	...
	lcd_tablet_config_update(lcd_drv->lcd_config);
#ifdef CONFIG_AMLOGIC_VPU
	vpu_dev_clk_request(lcd_drv->lcd_vpu_dev,
			    lcd_drv->lcd_config->lcd_timing.lcd_clk);
#endif
