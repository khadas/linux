...
static void lcd_lvds_phy_set(struct aml_lcd_drv_s *pdrv, int status)
{
	...
	unsigned short lvds_flag_6lane_map1[2][3] = {
		{0x000f, 0x001f, 0x041f}, {0x01e0, 0x03e0, 0x0be0}};
	...
	if (pdrv->data->chip_type == LCD_CHIP_T5W) {
		...
		if (pdrv->config.control.lvds_cfg.dual_port) {
			flag = lvds_flag_6lane_map1[0][bit_idx] | lvds_flag_6lane_map1[1][bit_idx];
		} else {
			if (pdrv->config.control.lvds_cfg.port_swap)
				flag = lvds_flag_6lane_map1[1][bit_idx];
			else
				flag = lvds_flag_6lane_map1[0][bit_idx];
		}
	}
...
static void lcd_mlvds_phy_set(struct aml_lcd_drv_s *pdrv, int status)
{
	...
	struct phy_config_s *phy = &pdrv->config.phy_cfg;
	unsigned int bypass, cntl14 = 0, flag = 0;
	unsigned char i;
	...
	if (status) {
		...
		lcd_ana_write(HHI_DIF_CSI_PHY_CNTL14, cntl14);
		lcd_phy_cntl_set(pdrv, phy, status, flag, bypass, 1, mlvds_conf->pi_clk_sel);
	} else {
		lcd_phy_cntl_set(pdrv, phy, status, flag, 1, 0, 0);
	}
}
...
static unsigned int lcd_phy_amp_dft_t5w(struct aml_lcd_drv_s *pdrv)
{
...
struct lcd_phy_ctrl_s lcd_phy_ctrl_t5 = {
	...
	.phy_amp_dft_val = lcd_phy_amp_dft_t5w,
