...
static void lcd_lvds_control_set(struct lcd_config_s *pconf)
{
	...
	lcd_vcbus_write(LVDS_GEN_CNTL,
			(lcd_vcbus_read(LVDS_GEN_CNTL) | (1 << 4) |
			(fifo_mode << 0)));
...
static void lcd_vbyone_power_on_wait_stable(struct lcd_config_s *pconf)
{
	...
	if (pconf->lcd_control.vbyone_config->ctrl_flag & 0x4) {
		lcd_vbyone_cdr_training_hold(pconf->lcd_control.vbyone_config,
					     0);
	}
	while (((lcd_vcbus_read(VBO_STATUS_L) & 0x3f) != 0x20) && (i > 0)) {
		udelay(50);
...
static void lcd_vx1_hpll_timer_handler(struct timer_list *timer)
{
	struct aml_lcd_drv_s *lcd_drv = aml_lcd_get_driver();
	int encl_clk;
...
int lcd_tv_driver_init(void)
{
	...
	case LCD_VBYONE:
		...
		delay_ms = LCD_VX1_WAIT_STABLE_DELAY;
		if (lcd_drv->workqueue) {
...
int lcd_vbyone_interrupt_up(void)
{
	...
	if (request_irq(viu_vsync_irq, lcd_vbyone_vsync_isr, IRQF_SHARED,
	...
	timer_setup(&vx1_hpll_timer, &lcd_vx1_hpll_timer_handler, 0);
