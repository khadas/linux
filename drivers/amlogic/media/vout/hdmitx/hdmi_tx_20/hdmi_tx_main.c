				...
				char *buf)
{
	int i, pos = 0;
	int j = 0;
	int frame_packing, top_and_bottom, side_by_side;
	enum hdmi_vic vic;
	struct rx_cap *prxcap = &hdmitx_device.RXCap;
			    ...
			    struct device_attribute *attr, char *buf)
{
	...
	for (i = 0; i < prxcap->AUD_count; i++) {
		...
		aud_coding_type[prxcap->RxAudioCap[i].audio_format_code],
...
static int amhdmitx_probe(struct platform_device *pdev)
{
	...
	amhdmitx_vpu_dev_regiter(&hdmitx_device);
...
/* When init hdmi, clear the hdmitx module edid ram and edid buffer. */
	...
	hdmitx_device.hdmi_wq =
	alloc_workqueue(DEVICE_NAME, WQ_HIGHPRI | WQ_CPU_INTENSIVE, 0);
	...
	hdmitx_device.rxsense_wq =
	alloc_workqueue(hdmitx_extcon_rxsense->name,
