...
/* reset HDMITX APB & TX */
void hdmitx_sys_reset(struct hdmitx_dev *hdev)
{
	switch (hdev->data->chip_type) {
	case MESON_CPU_ID_GXBB:
	case MESON_CPU_ID_GXTVBB:
...
static void hdmi_tvenc_vesa_set(struct hdmitx_vidpara *param)
{
	...
	if (INTERLACE_MODE) {
		de_v_begin_odd =
			to_signed((hd_read_reg(P_ENCP_VIDEO_OFLD_VOAV_OFST) &
			  ...
			  struct hdmitx_audpara *audio_param)
{
	...
	hdmitx_set_reg_bits(HDMITX_DWC_FC_AUDSCHNLS7,
			    aud_csb_sampfreq[audio_param->sample_rate],
			    0, 4); /*CSB 27:24*/
			     ...
			     struct hdmitx_audpara *audio_param)
{
	...
	if (GET_OUTCHN_NO(hdev->aud_output_ch))
		hdmitx_set_reg_bits(HDMITX_DWC_FC_AUDICONF0,
				    GET_OUTCHN_NO(hdev->aud_output_ch) - 1,
				    4, 3);
...
static void hdmitx_setupirq(struct hdmitx_dev *phdev)
{
	...
	r = request_irq(phdev->irq_hpd, &intr_handler,
			...
			(void *)phdev);
	if (r != 0)
		pr_info(HW "register irq %d failed\n", phdev->irq_hpd);
}
...
static unsigned int CONNECT2REG(unsigned int reg)
{
	return hdmitx_rd_reg(reg) + (hdmitx_rd_reg(reg + 1) << 8);
}
...
void DUMP_HDMITXREG_SECTION(unsigned int start, unsigned int end)
{
...
static void hdmitx_read_edid(unsigned char *rx_edid)
{
	...
	while (byte_num < 128 * blk_no) {
		...
		/* Wait until I2C done */
		timeout = 0;
		while ((!(hdmitx_rd_reg(HDMITX_DWC_IH_I2CM_STAT0) &
		       (1 << 1))) && (timeout < 3)) {
			mdelay(2);
			timeout++;
		}
...
static void hdcp_updateksv(void)
{
	size_t list = 0;
	size_t size = 0;
	size_t i = 0;
	...
	temp = kmalloc(MAX_KSV_LISTS * 5, GFP_ATOMIC);
	if (!temp)
		pr_info("%s: temp not valid\n", __func__);
 ...
 * /hdmi_tx_pixel_clk;
 */
void set_crt_video_enc(u32 vidx, u32 insel, u32 divn)
{
	if (vidx == 0) {
