...
unsigned int tmo_dbg;
module_param(tmo_dbg, uint, 0664);
MODULE_PARM_DESC(tmo_dbg, "HDR10 tone mapping dbg\n");
...
unsigned int panel_luma = 400;
module_param(panel_luma, uint, 0664);
MODULE_PARM_DESC(panel_luma, "display panel luminance\n");
...
u32 sc_thrd = 682;
module_param(sc_thrd, uint, 0664);
MODULE_PARM_DESC(sc_thrd, "scene change th\n");

u32 tm_iir = 1;
module_param(tm_iir, uint, 0664);
MODULE_PARM_DESC(tm_iir, "HDR_TM_IIR\n");
...
static int is_hdr_tmo_support(void)
{
	return 1;
}

static int time_iir(u32 *maxl,
	...
	int latest_hist_sum)
{
	...
	/*normalize to 10bit*/
	hist_diff[2] >>= 6;
	if (hist_diff[2] > sc_thrd || (*maxl <= panel_luma)) {
		ret = 1;
	} else {
...
static int gen_oe_x_val(u64 *oe_x_val)
{
...
static void decasteliau_alg(u64 *ebzcur,
	u64 step, u64 range, u64 *p, u32 order)
{
...
static u64 curvex[OE_X];
static u64 curvey[OE_X];
static u32 oo_gain[OE_X];
static u32 bld_gain[OE_X];

static int gen_knee_anchor(u32 maxl, u32 panell,
	u64 *sx, u64 *sy, u64 *anchor)
{
	int i;
	...
	hdr10p_basisootf_params_init(&basisootf_param);

	for (i = 0; i < MAX_BEIZER_ORDER - 1; i++)
		anchor_linear[i] = (i + 1) *
			(1 << MAX12_BIT) / MAX_BEIZER_ORDER;

	pr_hdr_tm("knee anchor p1: %lld, %lld, %lld, %lld, %lld, %lld,",
		sx1, sy1,
		anchor_linear[0], anchor_linear[1],
		anchor_linear[2], anchor_linear[3]);
	pr_hdr_tm(" %lld, %lld, %lld, %lld, %lld\n",
	...
	if (maxl <= panell) {
		...
		for (i = 0; i < MAX_BEIZER_ORDER - 1; i++) {
			anchor[i] = blendcoef * anchor_linear[i] +
		...
		anchor0 = hdr10p_calcp1((u32)sx1, (u32)sy1,
	...
	*sy = sy1;
	...
	pr_hdr_tm("knee anchor p2: %lld, %lld, %lld, %lld, %lld, %lld,",
		sx1, sy1,
		anchor[0], anchor[1],
		anchor[2], anchor[3]);
	pr_hdr_tm(" %lld, %lld, %lld, %lld, %lld\n",
...
static int dynamic_hdr_sdr_ootf(u32 maxl, u32 panell,
	u64 sx, u64 sy, u64 *anchor)
{
	int i;
	u32 tgtl = 10000;
	...
	pr_hdr_tm("d1: %d, %d, %lld, %lld, %lld, %lld, %lld, %lld,",
		maxl, panell,
		sx, sy,
		anchor[0], anchor[1],
		anchor[2], anchor[3]);
	...
	u32 *luminance)
{
	...
	if (tm_iir)
		scn_chang_flag = time_iir(&maxl,
			latest_hist_diff, latest_hist_sum);
	...
	if (hdr10_tm_sel == 1) {
		dynamic_hdr_sdr_ootf(maxl, panel_l, sx, sy, anchor);

		if (scn_chang_flag == 0 || scn_chang_flag == 1) {
			memcpy(data_oo_y_lut, oo_gain, sizeof(u32) * OE_X);
			memcpy(bld_gain, oo_gain, sizeof(u32) * OE_X);
		} else {
			for (i = 0; i < OE_X; i++)
				data_oo_y_lut[i] = (bld_gain[i] * bld_ratio +
					((1 << 8) - bld_ratio) * oo_gain[i]) >> 8;

			memcpy(bld_gain, data_oo_y_lut, sizeof(u32) * OE_X);
		}
	} else if (hdr10_tm_sel == 2) {
		hdr10_tmo_gen_data(oo_gain, latest_hdr_hist);
		memcpy(data_oo_y_lut, oo_gain, sizeof(u32) * OE_X);
	} else {
		memcpy(data_oo_y_lut, data_oo_y_lut_def, sizeof(u32) * OE_X);
	}
