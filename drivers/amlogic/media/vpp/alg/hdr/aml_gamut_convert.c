...
#include "aml_gamut_convert.h"

unsigned int gamut_print;
module_param(gamut_print, uint, 0664);
MODULE_PARM_DESC(gamut_print, "print gamut");
...
#define pr_gmt(fmt, args...)\
	do {\
		if (gamut_print)\
			pr_info("gamut_convert: " fmt, ## args);\
	} while (0)\
...
uint force_primary_ctrl;
module_param(force_primary_ctrl, uint, 0664);
MODULE_PARM_DESC(force_primary_ctrl, "\n force_primary_ctrl\n");

uint force_primary_num = 8;
u32 force_primary_src[8] = {
	0.708 * NORM + 0.5, 0.292 * NORM + 0.5, /* R */
	0.17 * NORM + 0.5, 0.797 * NORM + 0.5, /* G */
	0.131 * NORM + 0.5, 0.046 * NORM + 0.5, /* B */
	0.3127 * NORM + 0.5, 0.3290 * NORM + 0.5
};
module_param_array(force_primary_src, uint, &force_primary_num, 0664);
MODULE_PARM_DESC(force_primary_src, "\n force_src_primary\n");

u32 force_primary_dst[8] = {
	0.64 * NORM + 0.5, 0.33 * NORM + 0.5, /* R */
	0.30 * NORM + 0.5, 0.60 * NORM + 0.5, /* G */
	0.15 * NORM + 0.5, 0.06 * NORM + 0.5, /* B */
	0.3127 * NORM + 0.5, 0.3290 * NORM + 0.5
};
module_param_array(force_primary_dst, uint, &force_primary_num, 0664);
MODULE_PARM_DESC(force_primary_dst, "\n force_primary_dst\n");
	...
	unsigned int primary_policy)
{
	...
	if (hdr_type == EN_GAMUT_HDRTYPE_SDR) {
		for (i = 0; i < 3; i++) {
			for (j = 0; j < 2; j++) {
	...
	} else if (hdr_type == EN_GAMUT_HDRTYPE_HDR10 ||
		hdr_type == EN_GAMUT_HDRTYPE_HLG ||
		hdr_type == EN_GAMUT_HDRTYPE_PRIMESL ||
		hdr_type == EN_GAMUT_HDRTYPE_HDR10PLUS) {
		if (primary_policy == 1) {
			for (i = 0; i < 3; i++) {
				for (j = 0; j < 2; j++) {
					src_prmy[i][j] =
	...
	if (force_primary_ctrl) {
		/* because of blue x,y too close to zero,
		 * and usually have difference
		 * force blue dest primary >= src primary to avoid blue clip
		 */
		if (hdr_type == EN_GAMUT_HDRTYPE_SDR) {
			if (force_primary_dst[5] > force_primary_src[5])
				force_primary_dst[5] = force_primary_src[5];

			if (force_primary_dst[4] > force_primary_src[4])
				force_primary_dst[4] = force_primary_src[4];
		}

		for (i = 0; i < 4; i++) {
			for (j = 0; j < 2; j++) {
				src_prmy[i][j] =
					force_primary_src[i * 2 + j];
				dest_prmy[i][j] =
					force_primary_dst[i * 2 + j];
			}
		}
	}
