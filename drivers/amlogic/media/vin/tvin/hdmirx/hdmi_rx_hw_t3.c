...
/* for t3 */
static const u32 phy_misci_t3[][4] = {
		 /* 0x05	 0x06	 0x07	 0x08 */
	{	 /* 24~45M */
...
static const u32 phy_dcha_t3[][4] = {
		/*  0x09	0x0a	 0x0b	0x0c */
	{	 /* 24~45M */
...
static const u32 phy_dchd_t3[][5] = {
...
unsigned int decimaltogray_t3(unsigned int x)
{
	return x ^ (x >> 1);
}

/* Gray code to Decimal */
unsigned int graytodecimal_t3(unsigned int x)
{
	unsigned int y = x;
...
void aml_pll_bw_cfg_t3(void)
{
	...
	od2_div = apll_tab_t3[bw].od2_div;
	od2 = apll_tab_t3[bw].od2;
	...
	if (rx.aml_phy.os_rate == 0) {
		...
		od -= 1;
		/* 0x0c[13:0] + 0x0b[9:0] change to 3G setting ,t3*/
	...
	rx.phy.aud_div = apll_tab_t3[bw].aud_div;
	...
	if (rx.aml_phy.vga_gain <= 0xfff) {
		hdmirx_wr_bits_amlphy(HHI_RX_PHY_DCHA_CNTL0, MSK(12, 0),
				      (decimaltogray_t3(rx.aml_phy.vga_gain & 0xf) |
				      (decimaltogray_t3(rx.aml_phy.vga_gain >> 4 & 0xf) << 4) |
				      (decimaltogray_t3(rx.aml_phy.vga_gain >> 8 & 0xf) << 8)));
	}
...
void aml_phy_long_cable_det_t3(void)
{
	...
	usleep_range(100, 110);
	data32 = hdmirx_rd_amlphy(HHI_RX_PHY_DCHD_STAT);
	tap2_0 = get_tap2_t3(data32 & 0x1f);
	tap2_1 = get_tap2_t3(((data32 >> 8) & 0x1f));
	tap2_2 = get_tap2_t3(((data32 >> 16) & 0x1f));
...
void aml_hyper_gain_tuning_t3(void)
{
	u32 data32;
	u32 tap0, tap1, tap2;
...
void aml_phy_offset_cal_t3(void)
{
	...
	data32 = phy_misci_t3[idx][0];
	hdmirx_wr_amlphy(HHI_RX_PHY_MISC_CNTL0, data32);
	usleep_range(5, 10);

	data32 = phy_misci_t3[idx][1];
	/* todo */
	/* step2-0xd8 */
	hdmirx_wr_amlphy(HHI_RX_PHY_MISC_CNTL1, data32);
	/*step2-0xe0*/
	hdmirx_wr_amlphy(HHI_RX_PHY_MISC_CNTL2, phy_misci_t3[idx][2]);
	/*step2-0xe1*/
	hdmirx_wr_amlphy(HHI_RX_PHY_MISC_CNTL3, phy_misci_t3[idx][3]);
	/*step2-0xe2*/
...
void aml_phy_cfg_t3(void)
{
	...
	if (rx.aml_phy.pre_int) {
		...
		hdmirx_wr_amlphy(HHI_RX_PHY_DCHA_CNTL0, phy_dcha_t3[idx][0]);
		/*step2-0xe3*/
		hdmirx_wr_amlphy(HHI_RX_PHY_DCHA_CNTL1, phy_dcha_t3[idx][1]);
		/*step2-0xe4*/
		hdmirx_wr_amlphy(HHI_RX_PHY_DCHA_CNTL2, phy_dcha_t3[idx][2]);
		/*step2-0xe5*/
		data32 = phy_dchd_t3[idx][0];
		hdmirx_wr_amlphy(HHI_RX_PHY_DCHD_CNTL0, data32);
		/*step2-0xe6*/
		hdmirx_wr_amlphy(HHI_RX_PHY_DCHD_CNTL1, phy_dchd_t3[idx][1]);
		/*step2-0xe7*/
		hdmirx_wr_amlphy(HHI_RX_PHY_DCHD_CNTL2, phy_dchd_t3[idx][2]);
		/*step2-0xc5*/
		hdmirx_wr_amlphy(HHI_RX_PHY_DCHA_CNTL3, phy_dcha_t3[idx][3]);
		/*step2-0xc6*/
		hdmirx_wr_amlphy(HHI_RX_PHY_DCHD_CNTL3, phy_dchd_t3[idx][3]);
		/*0xc7*/
		hdmirx_wr_amlphy(HHI_RX_PHY_DCHD_CNTL4, phy_dchd_t3[idx][4]);
 ...
 * rx phy v2 debug
 */
int count_one_bits_t3(u32 value)
{
	int count = 0;
...
void get_val_t3(char *temp, unsigned int val, int len)
{
	if ((val >> (len - 1)) == 0)
...
void comb_val_t3(char *type, unsigned int val_0, unsigned int val_1,
		 unsigned int val_2, int len)
{
	char out[32], v0_buf[16], v1_buf[16], v2_buf[16];
	int pos = 0;

	get_val_t3(v0_buf, val_0, len);
	get_val_t3(v1_buf, val_1, len);
	get_val_t3(v2_buf, val_2, len);
...
void dump_aml_phy_sts_t3(void)
{
	...
	//terminal = count_one_bits_t3(data32);
	usleep_range(100, 110);

	/* status-2 t3:todo*/
	data32 = (hdmirx_rd_amlphy(HHI_RX_PHY_DCHA_CNTL0) & 0xfff);
	ch0_vga = graytodecimal_t3(data32 & 0xf);
	ch1_vga = graytodecimal_t3((data32 >> 4) & 0xf);
	ch2_vga = graytodecimal_t3((data32 >> 8) & 0xf);
	usleep_range(100, 110);
	...
	comb_val_t3("	 dfe_tap0", dfe0_tap0, dfe1_tap0, dfe2_tap0, 8);
	comb_val_t3("	 dfe_tap1", dfe0_tap1, dfe1_tap1, dfe2_tap1, 6);
	comb_val_t3("	 dfe_tap2", dfe0_tap2, dfe1_tap2, dfe2_tap2, 5);
	comb_val_t3("	 dfe_tap3", dfe0_tap3, dfe1_tap3, dfe2_tap3, 4);
	comb_val_t3("	 dfe_tap4", dfe0_tap4, dfe1_tap4, dfe2_tap4, 4);
	comb_val_t3("	 dfe_tap5", dfe0_tap5, dfe1_tap5, dfe2_tap5, 4);
	comb_val_t3("	 dfe_tap6", dfe0_tap6, dfe1_tap6, dfe2_tap6, 4);
	comb_val_t3("	 dfe_tap7", dfe0_tap7, dfe1_tap7, dfe2_tap7, 4);
	comb_val_t3("	 dfe_tap8", dfe0_tap8, dfe1_tap8, dfe2_tap8, 4);

	comb_val_t3("slicer_ofst0", sli0_ofst0, sli1_ofst0, sli2_ofst0, 6);
	comb_val_t3("slicer_ofst1", sli0_ofst1, sli1_ofst1, sli2_ofst1, 6);
	comb_val_t3("slicer_ofst2", sli0_ofst2, sli1_ofst2, sli2_ofst2, 6);
	comb_val_t3("slicer_ofst3", sli0_ofst3, sli1_ofst3, sli2_ofst3, 6);
	comb_val_t3("slicer_ofst4", sli0_ofst4, sli1_ofst4, sli2_ofst4, 6);
	comb_val_t3("slicer_ofst5", sli0_ofst5, sli1_ofst5, sli2_ofst5, 6);

	rx_pr("cdr_code=[%d,%d,%d]\n", cdr0_code, cdr1_code, cdr2_code);
	rx_pr("cdr_lock=[%d,%d,%d]\n", cdr0_lock, cdr1_lock, cdr2_lock);
	comb_val_t3("cdr_int", cdr0_init, cdr1_init, cdr2_init, 7);
}
...
void aml_phy_short_bist_t3(void)
{
	...
	if (rx.aml_phy.long_bist_en)
		rx_pr("t3 long bist start\n");
	else
		rx_pr("t3 short bist start\n");
...
void aml_phy_get_cdr_int_avr_t3(u32 cdr_ed_mode, u32 *ch0, u32 *ch1, u32 *ch2)
{
...
int aml_phy_get_iq_skew_val_t3(u32 val_0, u32 val_1)
{
	int val = val_0 - val_1;
...
void aml_phy_iq_skew_monitor_t3(void)
{
	...
	aml_phy_get_cdr_int_avr_t3(0x40000000, &cdr0_code_0, &cdr0_code_1, &cdr0_code_2);
	aml_phy_get_cdr_int_avr_t3(0x50000000, &cdr1_code_0, &cdr1_code_1, &cdr1_code_2);
	aml_phy_get_cdr_int_avr_t3(0x60000000, &cdr2_code_0, &cdr2_code_1, &cdr2_code_2);
	aml_phy_get_cdr_int_avr_t3(0x70000000, &cdr3_code_0, &cdr3_code_1, &cdr3_code_2);
	//iq_skew0 = value(clk0)-value(clk90)-32
	//iq_skew1 = value(clk180)-value(clk270)-32
	//iq_skew = average(iq_skew0,iq_skew1)
	//rx_pr("iq_skew=[%d,%d,%d]\n",

	iq0_skew_ch0 = aml_phy_get_iq_skew_val_t3(cdr0_code_0, cdr1_code_0);
	iq0_skew_ch1 = aml_phy_get_iq_skew_val_t3(cdr0_code_1, cdr1_code_1);
	iq0_skew_ch2 = aml_phy_get_iq_skew_val_t3(cdr0_code_2, cdr1_code_2);
	iq1_skew_ch0 = aml_phy_get_iq_skew_val_t3(cdr0_code_0, cdr1_code_0);
	iq1_skew_ch1 = aml_phy_get_iq_skew_val_t3(cdr0_code_1, cdr1_code_1);
	iq1_skew_ch2 = aml_phy_get_iq_skew_val_t3(cdr0_code_2, cdr1_code_2);
...
bool aml_get_tmds_valid_t3(void)
{
	...
	/* it is not necessary and t3 has not it */
	...
	/* modified in t3, 0x2b bit'0 tmds_align status */
