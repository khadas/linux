...
unsigned int rx_get_scdc_clkrate_sts(void)
{
	...
	if (rx.chip_id == CHIP_ID_TXHD ||
		...
		clk_rate = (hdmirx_rd_cor(COR_SCDC_TMDS_CFG) >> 1) & 1;
	else
...
static int TOP_init(void)
{
	...
	if (rx.chip_id == CHIP_ID_T7) {
		hdmirx_wr_top(TOP_SECURE_MODE,  1);
		/* Filter 100ns glitch */
...
void cor_init(void)
{
	...
	hdmirx_wr_cor(RX_ACR_CTRL1_AUD_IVCRX, 0x7a);//register address: 0x1401 (0x7a)
...
void rx_get_avi_params(void)
{
	if (rx.chip_id == CHIP_ID_T7) {
		...
		rx.cur.scan_info = 0;
		rx.cur.n_uniform_scale = 0;
		rx.cur.ext_colorimetry = 0;
		rx.cur.active_ratio = 0;
		rx.cur.active_valid = 0;
		rx.cur.bar_valid = 0;
		if (rx.cur.bar_valid == 3) {
			rx.cur.bar_end_top = 0;
			rx.cur.bar_start_bottom = 0;
			rx.cur.bar_end_left = 0;
			rx.cur.bar_start_right = 0;
		}
		rx.cur.colorimetry = 0;
		rx.cur.picture_ratio = 0;
	} else {
...
void rx_get_framerate(void)
{
	...
	if (rx.chip_id == CHIP_ID_T7) {
		...
		if (tmp == 0)
			...
			rx.cur.frame_rate = (tmp * 100000) / tmp;
	} else {
...
void rx_get_de_sts(void)
{
	...
	if (rx.chip_id == CHIP_ID_T7) {
		...
		rx.cur.hactive = tmp * 100 / rx.cur.colordepth * 8;
...
void aml_phy_switch_port_tl1_tm2(void)
{
	u32 data32;
