...
int adc_set_filter_ctrl(bool on, enum filter_sel module_sel, void *data)
{
	...
	case FILTER_ATV_DEMOD:
		if (devp->filter_flg & (FILTER_TVAFE | FILTER_DTV_DEMOD)) {
			...
			pr_err("%s: DEMOD fail!!!, filter_flg: %d\n",
	...
	case FILTER_TVAFE:
		if (devp->filter_flg & (FILTER_ATV_DEMOD | FILTER_DTV_DEMOD)) {
			...
			pr_err("%s: AFE fail!!!, filter_flg: %d\n",
					__func__, devp->filter_flg);
			break;
		}
	...
	case FILTER_DTV_DEMOD:
		if (devp->filter_flg & (FILTER_ATV_DEMOD | FILTER_TVAFE)) {
			...
			pr_err("%s: DDEMOD fail!!!, filter_flg: %d\n",
					__func__, devp->filter_flg);
			break;
		}
	...
	case FILTER_DTV_DEMODT2:
		if (devp->filter_flg & (FILTER_ATV_DEMOD | FILTER_TVAFE)) {
			...
			pr_err("%s:DMODPLL fail!!!, filter_flg: %d\n",
					__func__, devp->filter_flg);
			break;
		}
	...
	case ADC_CLK_24M:
		if (chip == ADC_CHIP_S4 || chip == ADC_CHIP_S4D) {
			...
			if (p_dtv_para->delsys == SYS_DVBC_ANNEX_A &&
				...
				adc_wr_hiu(pll_addr->adc_pll_cntl_0 + reg_offset, 0x3105047d);
...
int adc_get_status(enum adc_sel module_sel)
{
	...
	if (module_sel == ADC_ATV_DEMOD) {
		if ((adc_devp->pll_flg & ADC_ATV_DEMOD) &&
		    !mutex_is_locked(&adc_devp->pll_mutex))
			ret = 1;
	}
	if (adc_devp->print_en & ADC_DBG_GET_STATUS)
		pr_info("%s:module_sel:%#x,pll_flag:%#x mutex:%#x ret:%#x\n",
			__func__, module_sel, adc_devp->pll_flg,
			!mutex_is_locked(&adc_devp->pll_mutex), ret);
	return ret;
}
EXPORT_SYMBOL(adc_get_status);
...
static void adc_dump_regs(void)
{
	...
	pr_info("pll_flag=0x%x filter_flg=0x%x delsys:%d print_en=0x%x\n",
		adc_devp->pll_flg, adc_devp->filter_flg,
		adc_devp->delsys, adc_devp->print_en);
	...
	if (chip_id == ADC_CHIP_S4 || chip_id == ADC_CHIP_S4D) {
		pr_info("HHI_S2_DADC_CNTL(0xd0):0x%x\n", adc_rd_hiu(0xd0));
		pr_info("HHI_S2_DADC_CNTL2(0xd1):0x%x\n", adc_rd_hiu(0xd1));
	} else {
	...
	if (chip_id == ADC_CHIP_S4 || chip_id == ADC_CHIP_S4D) {
		pr_info("HHI_VDAC_CNTL0(0xb0):0x%x\n", adc_rd_hiu(0xb0));
		pr_info("HHI_VDAC_CNTL1(0xb1):0x%x\n", adc_rd_hiu(0xb1));
	} else {
	...
	if (parm[0] && !strcmp(parm[0], "state")) {
		...
		if (val) {
			if (tmp_val) {
				...
				adc_set_ddemod_default(&p_dtv_para);
