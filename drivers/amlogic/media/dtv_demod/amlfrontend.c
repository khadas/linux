...
static int dtvdemod_dvbs_read_signal_strength(struct dvb_frontend *fe,
		s16 *strength);
static int gxtv_demod_dvbc_read_signal_strength(struct dvb_frontend *fe,
		s16 *strength);
...
static int gxtv_demod_dvbc_read_status_timer(struct dvb_frontend *fe,
...
static int dvbt_isdbt_read_status(struct dvb_frontend *fe,
		enum fe_status *status, bool re_tune)
{
	struct aml_dtvdemod *demod = (struct aml_dtvdemod *)fe->demodulator_priv;
	unsigned char s = 0;
	unsigned int fsm = 0;
...
static int dvbc_read_status(struct dvb_frontend *fe, enum fe_status *status, bool re_tune)
{
	...
	if (tuner_find_by_name(fe, "r842") && c->symbol_rate == 0 && is_blind >= 3 &&
		...
		PR_DVBC("%s: r29=0x%x, time_passed=%u\n", __func__, r29, demod->time_passed);
		if (((r29 >> 16) & 0xf) == 7)
			agc = 1;
		...
		if (agc == 0) {
			if (demod->time_passed < check_agc_time) {
				...
				*status = FE_TIMEDOUT;
				...
				last_agc_time = curTime;
			}

			goto finish;
		} else if (agc == 1 && curTime - last_agc_time < 3000 &&
