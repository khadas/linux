...
{
	int offset = 0x0;
	unsigned int mask, addr, val, tmp;
	...
	mask = sr0_dej_setting[level].mask;
	addr = sr0_dej_setting[level].addr - offset;
	val = sr0_dej_setting[level].val;
	tmp = aml_read_vcbus(addr);
	WRITE_VPP_REG(addr, (tmp & (~mask)) | (val & mask));
}
