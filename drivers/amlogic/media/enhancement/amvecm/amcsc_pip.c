		       ...
		       enum vd_path_e vd_path)
{
	...
	bool hdr10_plus_support =
		...
		== 1);
	uint *h10p_process_mode, *cur_h10p_process_mode;

	h10p_process_mode = hdr10_plus_process_mode;
	cur_h10p_process_mode = cur_hdr10_plus_process_mode;
	...
	if (get_hdr_module_status(vd_path) != HDR_MODULE_ON) {
		...
		h10p_process_mode[vd_path] = PROC_BYPASS;
		...
		if (source_format[vd_path] == HDRTYPE_MVC) {
			...
			h10p_process_mode[oth_path] = PROC_BYPASS;
			target_format[vd_path] = BT709;
			...
			h10p_process_mode[vd_path] = PROC_MATCH;
	...
	if (is_dolby_vision_on() &&
		...
		if (cur_h10p_process_mode[vd_path]
		    != h10p_process_mode[vd_path])
	...
	if (change_flag & SIG_OUTPUT_MODE_CHG) {
		...
		case HDRTYPE_HDR10PLUS:
			cur_h10p_process_mode[oth_path] =
				PROC_OFF;
			break;
		case HDRTYPE_HDR10:
		...
		case HDRTYPE_MVC:
			...
			cur_h10p_process_mode[oth_path] = PROC_BYPASS;
			break;
		default:
			break;
		}
	...
	case HDRTYPE_HDR10:
		if (vd_path == VD2_PATH && is_dolby_vision_on()) {
			...
			if (vd_path == VD1_PATH) {
				...
				temp = content_max_lumin[vd_path];
				hdr10_plus_process_update(temp);
			} else {
