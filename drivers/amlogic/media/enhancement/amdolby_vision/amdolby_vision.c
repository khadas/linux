...
static inline void WRITE_VPP_REG_BITS
		(u32 reg,
		 const u32 value,
		 const u32 start,
		 const u32 len)
...
static inline u32 READ_VPP_REG_BITS
		(u32 reg,
		 const u32 start,
		 const u32 len)
{
...
{
	if (is_meson_tm2_tvmode() || is_meson_txlx_tvmode())
		return true;
	else
		return false;
}
...
{
	if (vinfo && vinfo->hdr_info.hdr10plus_info.ieeeoui == 0x90848B &&
	...
	while (p < req->aux_buf + req->aux_size - 8) {
		...
		if (type == 0x01000000) {
			...
			*src_format = FORMAT_DOVI;
			...
			if (rpu_ret < 0) {
				pr_dolby_error
				("meta(%d), pts(%lld)->metadata parser fail\n",
				 size, vf->pts_us64);
				ret = 3;
			} else {
			 ...
			 struct hdr10_param_s *p_hdr10_param)
{
	...
	if (flag) {
		pr_dolby_dbg
			("HDR10: primaries %d, maxcontent %d, flag %d\n",
...
#ifdef V2_4
		if (vinfo)
			prepare_vsif_pkt(&vsif, &dovi_setting, vinfo);
#endif
#ifdef HDMI_SEND_ALL_PKT
		hdr10_data.features =
			  (1 << 29)	/* video available */
...
int dolby_vision_wait_metadata(struct vframe_s *vf)
{
	...
	if (dolby_vision_flags & FLAG_CERTIFICAION) {
		...
		if (is_meson_tvmode() && !force_stb_mode)
			ott_mode = tv_dovi_setting->input_mode !=
				INPUT_MODE_HDMI;
		if (setting_update_count > crc_count && ott_mode)
			return 1;
	}
	...
	if (dolby_vision_wait_init && dolby_vision_wait_count) {
		dolby_vision_wait_count--;
		pr_dolby_dbg("delay wait %d\n", dolby_vision_wait_count);
		ret = 1;
	} else if (dolby_vision_core1_on && (dolby_vision_on_count <=
		dolby_vision_run_mode_delay))
		ret = 1;
...
int unregister_dv_functions(void)
{
	...
	if (p_funcs_stb || p_funcs_tv) {
		pr_info("*** unregister dv functions ***\n");
		if (pq_config_fake) {
...
void tv_dolby_vision_crc_clear(int flag)
{
	...
	pr_info("crc_clear, crc_output_buf %p\n", crc_output_buf);
	if (crc_output_buf)
	 ...
	 size_t count, loff_t *ppos)
{
	...
	if (out && data_size > 0) {
		...
		pr_info("read crc size %d, res: %d, ret: %d\n",
			data_size, res, ret_val);
		tv_dolby_vision_crc_clear(0);
	}
	return ret_val;
}

static int amdolby_vision_release
...
static long amdolby_vision_ioctl
...
#ifdef CONFIG_COMPAT
static long amdolby_vision_compat_ioctl
	(struct file *file, unsigned int cmd,
	 unsigned long arg)
{
	unsigned long ret;
...
static ssize_t amdolby_vision_dv_mode_show
		(struct class *cla,
		 struct class_attribute *attr, char *buf)
{
