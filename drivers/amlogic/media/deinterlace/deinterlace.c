...
void di_vpu_dev_register(struct di_dev_s *vdevp)
{
	vdevp->di_vpu_clk_gate_dev = vpu_dev_register(VPU_VPU_CLKB,
						      DEVICE_NAME);
	vdevp->di_vpu_pd_dec = vpu_dev_register(VPU_AFBC_DEC,
						      DEVICE_NAME);
	vdevp->di_vpu_pd_dec1 = vpu_dev_register(VPU_AFBC_DEC1,
						      DEVICE_NAME);
	vdevp->di_vpu_pd_vd1 = vpu_dev_register(VPU_VIU_VD1,
						      DEVICE_NAME);
	vdevp->di_vpu_pd_post = vpu_dev_register(VPU_DI_POST,
						      DEVICE_NAME);
}
/*
 * di task
...
/* nr/blend0/ei0/mtn0 clock gate */
	if (mirror_disable) {
		...
		if (!is_meson_gxl_cpu() && !is_meson_gxm_cpu() &&
			...
			vpu_dev_clk_gate_off(de_devp->di_vpu_clk_gate_dev);
...
static void di_reg_process_irq(void)
{
	...
	if (vframe) {
		...
		vpu_dev_clk_gate_on(de_devp->di_vpu_clk_gate_dev);
...
static void di_shutdown(struct platform_device *pdev)
{
	...
	if (!is_meson_txlx_cpu())
		vpu_dev_clk_gate_off(di_devp->di_vpu_clk_gate_dev);
	kfree(di_devp);
	di_devp = NULL;
