/*
 * drivers/amlogic/camera/sp2518.c
 *
...
#define SP2518_CAMERA_VERSION	\
		KERNEL_VERSION(SP2518_CAMERA_MAJOR_VERSION,	\
		SP2518_CAMERA_MINOR_VERSION,	\
		SP2518_CAMERA_RELEASE)
...
static int sp2518_night_or_normal;  /* add by sp_yjp,20120905 */
...
struct sp2518_qmenu_set_s {
	__u32   id;
	int     num;
	struct v4l2_querymenu *sp2518_qmenu;
};

struct sp2518_qmenu_set_s sp2518_qmenu_set[] = {
	{
		.id             = V4L2_CID_DO_WHITE_BALANCE,
...
void SP2518_init_regs(struct sp2518_device *dev)
{
	...
	while (1) {
		if (SP2518_script[i].val == 0xff &&
			SP2518_script[i].addr == 0xff) {
			pr_info("SP2518_write_regs success in initial.\n");
			break;
		}
		if ((i2c_put_byte_add8_new(client, SP2518_script[i].addr,
					   SP2518_script[i].val)) < 0) {
			pr_err("fail in initial SP2518.\n");
			return;
		}
		i++;
	}
	...
	aml_plat_cam_data_t* plat_dat=
		(aml_plat_cam_data_t*)client->dev.platform_data;
	if  (plat_dat && plat_dat->custom_init_script)  {
	    i=0;
	    aml_camera_i2c_fig_t*  custom_script =
			(aml_camera_i2c_fig_t*)plat_dat->custom_init_script;
	    while (1)
	    {
		if (custom_script[i].val == 0xff &&
			custom_script[i].addr == 0xff)
		{
		    pr_info("SP2518_write_custom_regs success in initial\n");
		    break;
		}
		if  ((i2c_put_byte_add8_new(client,custom_script[i].addr,
				custom_script[i].val)) < 0)
		{
		    pr_err("fail in initial SP2518 custom_regs.\n");
		    return;
		}
		i++;
	    }
...
*
*************************************************************************/
void SP2518_set_param_wb(struct sp2518_device *dev,
...
*
*************************************************************************/
void SP2518_set_param_exposure(struct sp2518_device *dev,
...
*
*************************************************************************/
void SP2518_set_param_effect(struct sp2518_device *dev,
...
*
*************************************************************************/
void SP2518_set_night_mode(struct sp2518_device *dev,
			   enum  camera_night_mode_flip_e enable)
{
	...
	if (enable) {
		sp2518_night_or_normal =
			1; /* =1,night mode; =0,normal mode*/
		...
		if (Antiflicker == DCAMERA_FLICKER_50HZ) {
			/* disable AE,add by sp_yjp,20120905 */
			i2c_put_byte_add8_new(client, 0xfd, 0x00);
			i2c_put_byte_add8_new(client, 0x32, 0x05);
			pr_info("night mode 50hz\n");
#ifdef CLK24M_48M

			/* capture preview night 48M 50hz fix 6FPS maxgain */
			i2c_put_byte_add8_new(client, 0xfd, 0x00);
			/* 0x01  modify by sp_yjp,20120905 */
			i2c_put_byte_add8_new(client, 0x03, 0x00);
			/* 0xd4  modify by sp_yjp,20120905 */
			i2c_put_byte_add8_new(client, 0x04, 0x09);
			...
			/* 0x01  modify by sp_yjp,20120905 */
			i2c_put_byte_add8_new(client, 0x03, 0x00);
			/* 0x86  modify by sp_yjp,20120905 */
			i2c_put_byte_add8_new(client, 0x04, 0x06);
			i2c_put_byte_add8_new(client, 0x09, 0x02);
			i2c_put_byte_add8_new(client, 0x0a, 0x64);
		...
		/* Camera Enable night mode  1/5 Frame rate //zyy test */
		/* i2c_put_byte_add8_new(client,);*/
		sp2518_night_or_normal =
			0; /* =1,night mode; =0,normal mode */
		...
		if (Antiflicker == DCAMERA_FLICKER_50HZ) {
			...
			i2c_put_byte_add8_new(client, 0xfd, 0x00);
			i2c_put_byte_add8_new(client, 0x32, 0x05);
			pr_info("normal mode 50hz\n");
#ifdef CLK24M_48M
			/* capture preview daylight 48M 50hz fix 9FPS maxgain */
			i2c_put_byte_add8_new(client, 0xfd, 0x00);
			/* 0x02  modify by sp_yjp,20120905 */
			i2c_put_byte_add8_new(client, 0x03, 0x00);
			/* 0xbe  modify by sp_yjp,20120905 */
			i2c_put_byte_add8_new(client, 0x04, 0x09);
			...
			i2c_put_byte_add8_new(client, 0xd1, 0x05);
			/* exp_nr_outd_8lsb */
			i2c_put_byte_add8_new(client, 0xd7, 0x71);
			i2c_put_byte_add8_new(client, 0xd8, 0x00);
			/* exp_outd_nr_8lsb */
			i2c_put_byte_add8_new(client, 0xd9, 0x75);
			i2c_put_byte_add8_new(client, 0xda, 0x00);
			i2c_put_byte_add8_new(client, 0xfd, 0x00);
			...
			/* /ae setting */
			i2c_put_byte_add8_new(client, 0xfd, 0x00);
			/* 0x03  modify by sp_yjp,20120905 */
			i2c_put_byte_add8_new(client, 0x03, 0x00);
			/* 0xf6  modify by sp_yjp,20120905 */
			i2c_put_byte_add8_new(client, 0x04, 0x09);
			i2c_put_byte_add8_new(client, 0x09, 0x00);
			...
			i2c_put_byte_add8_new(client, 0xfd, 0x00);
			i2c_put_byte_add8_new(client, 0x32, 0x05);
			pr_info("normal mode 60hz\n");
#ifdef CLK24M_48M
			/* capture preview daylight 48M 60Hz fix 9FPS maxgain */
			i2c_put_byte_add8_new(client, 0xfd, 0x00);
			/* 0x02  modify by sp_yjp,20120905 */
			i2c_put_byte_add8_new(client, 0x03, 0x00);
			/* 0x4c  modify by sp_yjp,20120905 */
			i2c_put_byte_add8_new(client, 0x04, 0x09);
			i2c_put_byte_add8_new(client, 0x09, 0x00);
			...
			i2c_put_byte_add8_new(client, 0xd1, 0x04);
			/* exp_nr_outd_8lsb */
			i2c_put_byte_add8_new(client, 0xd7, 0x5e);
			i2c_put_byte_add8_new(client, 0xd8, 0x00);
			/* exp_outd_nr_8lsb */
			i2c_put_byte_add8_new(client, 0xd9, 0x62);
			i2c_put_byte_add8_new(client, 0xda, 0x00);
			i2c_put_byte_add8_new(client, 0xfd, 0x00);
#elif defined(CLK24M_72M)
			...
			/* 0x03  modify by sp_yjp,20120905 */
			i2c_put_byte_add8_new(client, 0x03, 0x00);
			/* 0x4e  modify by sp_yjp,20120905 */
			i2c_put_byte_add8_new(client, 0x04, 0x09);
			i2c_put_byte_add8_new(client, 0x09, 0x00);
		...
		/* Disable night mode  1/2 Frame rate */
		/* i2c_put_byte_add8_new(client,);*/
	}
	/* enable AE,add by sp_yjp,20120905 */
	i2c_put_byte_add8_new(client, 0xfd, 0x00);
	i2c_put_byte_add8_new(client, 0x32, 0x0d);
}    /* SP2518_NightMode */
void SP2518_set_param_banding(struct sp2518_device *dev,
			      enum  camera_banding_flip_e banding)
{
	...
	case CAM_BANDING_50HZ:
		...
		pr_info(" set_SP2518_anti_flicker  50hz\n");
		break;
	case CAM_BANDING_60HZ:
		Antiflicker = DCAMERA_FLICKER_60HZ;
		pr_info(" set_SP2518_anti_flicker  60hz\n");
		break;
	default:
		break;
	}
...
void SP2518_set_resolution(struct sp2518_device *dev, int height, int width)
{
	...
	if ((width < 1600) && (height < 1198)) {
		...
		/*****************************88
			...
			//AGain_shutter = ((AGain_shutterH<<8)|
				(AGain_shutterL&0xff));
			DGain_shutter = (DGain_shutterH<<8|
				(DGain_shutterL&0xff));
			...
			ret = i2c_put_byte_add8_new(client,
				0x0305, shutter&0xff);

			ret = i2c_put_byte_add8_new(client,
				0x0307, AGain_shutterL);
			ret = i2c_put_byte_add8_new(client,
				0x0306, AGain_shutterH);
			ret = i2c_put_byte_add8_new(client,
				0x0308, DGain_shutter);
			*******************/
		/* mdelay(100); */
...
static int sp2518_setting(struct sp2518_device *dev, int PROP_ID, int value)
{
	...
	case V4L2_CID_DO_WHITE_BALANCE:
		if (sp2518_qctrl[0].default_value != value) {
			...
			pr_info(KERN_INFO " set camera  white_balance=%d.\n ",
				value);
		}
		break;
	case V4L2_CID_EXPOSURE:
		if (sp2518_qctrl[1].default_value != value) {
			...
			pr_info(KERN_INFO "set camera  exposure=%d.\n",
				value);
		}
		break;
	case V4L2_CID_COLORFX:
		if (sp2518_qctrl[2].default_value != value) {
			...
			pr_info(KERN_INFO "set camera  effect=%d.\n",
				value);
		}
		break;
	case V4L2_CID_WHITENESS:
		if (sp2518_qctrl[3].default_value != value) {
			...
			pr_info("@@@SP_000:SP2518_set_param_banding,value=%d\n",
				value);
			SP2518_set_param_banding(dev, value);
			pr_info("@@@SP_111:sp2518_night_or_normal = %d",
				sp2518_night_or_normal);
			SP2518_set_night_mode(dev, sp2518_night_or_normal);
			pr_info(KERN_INFO "set camera  banding=%d.\n",
				value);
		}
		break;
	case V4L2_CID_BLUE_BALANCE:
		if (sp2518_qctrl[4].default_value != value) {
			...
			pr_info("@@@SP_222:SP2518_set_night_mode,night mode=%d\n",
				value);
			pr_info("@@@SP_333:sp2518_night_or_normal = %d",
				sp2518_night_or_normal);
			SP2518_set_night_mode(dev, value);
			pr_info(KERN_INFO "set camera  scene mode=%d.\n",
				value);
		}
		break;
	case V4L2_CID_HFLIP:    /* set flip on H. */
...
{
	struct sp2518_device *dev =
		container_of(dma_q, struct sp2518_device, vidq);
			       ...
			       struct videobuf_buffer *vb)
{
	...
	struct sp2518_device *dev;
	struct sp2518_buffer   *buf  =
		container_of(vb, struct sp2518_buffer, vb);
	struct videobuf_res_privdata *res = vq->priv_data;
	fh = container_of(res, struct sp2518_fh, res);
...
static int sp2518_open(struct file *file)
{
	...
	if (dev->vminfo.mem_alloc_succeed) {
		...
		videobuf_queue_res_init(&fh->vb_vidq,
			&sp2518_video_res_qops, NULL,
			&dev->slock, fh->type,
			V4L2_FIELD_INTERLACED,
			sizeof(struct sp2518_buffer),
			(void *)&fh->res, NULL);
	} else {
		videobuf_queue_vmalloc_init(&fh->vb_vidq,
				&sp2518_video_vmall_qops, NULL,
				&dev->slock, fh->type,
				V4L2_FIELD_INTERLACED,
...
#if 0
static int sp2518_g_chip_ident(struct v4l2_subdev *sd,
...
static ssize_t sp2518_show(struct device *dev, struct device_attribute *attr,
			   char *_buf)
{
...
static struct device *sp2518_dev;
static struct class   *sp2518_class;
static DEVICE_ATTR(sp2518, S_IRUGO, sp2518_show, sp2518_store);/*0666*/
...
static ssize_t sp2518_show_mine(struct kobject *kobj,
...
static ssize_t sp2518_store_mine(struct kobject *kobj,
				 struct kobj_attribute *attr,
...
static struct kobj_attribute sp2518_attribute = __ATTR(sp2518, S_IRUGO,
		sp2518_show_mine, sp2518_store_mine);
			...
			const struct i2c_device_id *id)
{
	...
	if (plat_dat) {
		...
		pr_err("camera sp2518: have no platform data\n");
		kfree(t);
		return -1;
	}
	...
	if (IS_ERR(sp2518_class)) {
		pr_err("Create class sp2518 fail.\n");
		return -ENOMEM;
	}
	sp2518_dev = device_create(sp2518_class, NULL,
		MKDEV(0, 1), NULL, "dev");
