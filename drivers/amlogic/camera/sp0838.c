/*
 * drivers/amlogic/camera/sp0838.c
 *
...
#define SP0838_CAMERA_VERSION	\
		KERNEL_VERSION(SP0838_CAMERA_MAJOR_VERSION,	\
		SP0838_CAMERA_MINOR_VERSION,	\
		SP0838_CAMERA_RELEASE)
...
struct sp0838_qmenu_set {
	__u32   id;
	int     num;
	struct v4l2_querymenu *sp0838_qmenu;
};

struct sp0838_qmenu_set sp0838_qmenu_set[] = {
	{
		.id             = V4L2_CID_DO_WHITE_BALANCE,
	...
	for (i = 0; i < ARRAY_SIZE(sp0838_qmenu_set); i++)
		if (a->id && a->id == sp0838_qmenu_set[i].id) {
			for (j = 0; j < sp0838_qmenu_set[i].num; j++) {
				condition = a->index ==
				sp0838_qmenu_set[i].sp0838_qmenu[j].index;
				if (condition) {
					memcpy(a,
					&(sp0838_qmenu_set[i].sp0838_qmenu[j]),
					sizeof(*a));
					return 0;
				}
			}
...
/* should include 320x240 and 640x480,
		those two size are used for recording */
static struct v4l2_frmsize_discrete sp0838_prev_resolution[] = {
	{176, 144},
	{320, 240},
	{352, 288},
	{640, 480},
...
struct aml_camera_i2c_fig1_s SP0838_script[] = {
	...
	{0x6f, 0x7f},
	{0x70, 0x01},
	{0x71, SP0838_P0_0x71}, /* »ÒªØ„–÷ */
	{0x72, SP0838_P0_0x72}, /* »ı¬÷¿™„–÷µ */
	{0x73, SP0838_P0_0x73}, /* ±ﬂ‘µ’˝œÚ‘ˆ“Ê÷ */
	{0x74, SP0838_P0_0x74}, /* ±ﬂ‘µ∑¥œÚ‘ˆ“Ê÷ */
	{0x75, 0x7f},
	{0x76, 0x01},
	{0xcb, 0x07},
	{0xcc, 0x04},
...
void SP0838_init_regs(struct sp0838_device *dev)
{
	...
	while (1) {
		...
		if (SP0838_script[i].val == 0xff &&
				SP0838_script[i].addr == 0xff)
			break;
		if ((i2c_put_byte_add8(client, buf, 2)) < 0) {
			pr_err("fail in initial SP0838.\n");
			return;
		}
		i++;
	}
...
static void sp0838_set_resolution(struct sp0838_device *dev, int height,
				  int width)
{
	int i = 0;
	unsigned char buf[2];
	...
	pr_info("set resolution 640X480\n");
	...
	while (1) {
		...
		if (resolution_script[i].val == 0xff &&
				resolution_script[i].addr == 0xff)
			break;
		if ((i2c_put_byte_add8(client, buf, 2)) < 0) {
			pr_err("fail in setting resolution\n");
			return;
		}
		i++;
	}
...
void set_SP0838_param_wb(struct sp0838_device *dev, enum  camera_wb_flip_e para)
{
	...
	pr_info(" camera set_SP0A19_param_wb=%d.\n ", para);
	switch (para) {
	case CAM_WB_AUTO:
		buf[0] = 0xfd;
		buf[1] = 0x01;
		...
		buf[0] = 0x32;
		/* temp_reg|0x10;
			SP0A19 AWB enable bit[1] ie. 0x02; */
		buf[1] = 0x15;
		i2c_put_byte_add8(client, buf, 2);
		break;
...
*
*************************************************************************/
void SP0838_night_mode(struct sp0838_device *dev,
...
void SP0838_set_param_banding(struct sp0838_device *dev,
			      enum  camera_night_mode_flip_e banding)
{
...
*   None
*
* GLOBALS AFFECTED  ¡¡∂»µ»º∂ µ˜Ω⁄≤Œ ˝
*
*************************************************************************/
void set_SP0838_param_exposure(struct sp0838_device *dev,
			       enum camera_exposure_e para)/* ∆ÿπ‚µ˜Ω⁄ */
{
	struct i2c_client *client = v4l2_get_subdevdata(&dev->sd);
...
*   None
*
* GLOBALS AFFECTED  Ãÿ–ß≤Œ ˝
*
*************************************************************************/
void set_SP0838_param_effect(struct sp0838_device *dev,
			     enum camera_effect_flip_e para)/* Ãÿ–ß…Ë÷√ */
{
...
{
	struct sp0838_device *dev = container_of(dma_q,
		struct sp0838_device, vidq);
...
static void res_free_buffer(struct videobuf_queue *vq,
			    struct sp0838_buffer *buf)
{
	struct sp0838_fh *fh;
	struct sp0838_device *dev;
...
static void vmall_free_buffer(struct videobuf_queue *vq,
			      struct sp0838_buffer *buf)
{
	struct sp0838_fh *fh;
	struct sp0838_device *dev;
			       ...
			       struct videobuf_buffer *vb)
{
	struct sp0838_fh *fh;
	struct sp0838_device *dev;
	struct sp0838_buffer *buf  = container_of(vb,
		struct sp0838_buffer, vb);
	struct videobuf_res_privdata *res = vq->priv_data;
	fh = container_of(res, struct sp0838_fh, res);
				      ...
				      struct v4l2_frmivalenum *fival)
{
	...
	for (k = 0; k < ARRAY_SIZE(sp0838_frmivalenum); k++) {
		condition = ((fival->index == sp0838_frmivalenum[k].index) &&
		...
		if (condition) {
			memcpy(fival, &sp0838_frmivalenum[k],
	...
	if (get_cpu_type() >= MESON_CPU_MAJOR_ID_M8) {
		if (ret == 0)
			p->reserved  = convert_canvas_index(fh->fmt->fourcc,
			    CAMERA_USER_CANVAS_INDEX + p->index * 3);
		else
			p->reserved = 0;
	}
	return ret;
...
static int vidioc_streamon(struct file *file, void *priv, enum v4l2_buf_type i)
{
	...
	pr_info("h=%d, v=%d, frame_rate=%d, bt_path = %d, vdin_path = %d .\n",
			sp0838_h_active, sp0838_v_active,
			sp0838_frmintervals_active.denominator,
...
static int vidioc_streamoff(struct file *file, void *priv, enum v4l2_buf_type i)
{
	...
	int ret = 0;
	vdin_path = fh->dev->cam_info.vdin_path;
	pr_info("sp0838 streamoff vdin_path = %d\n", vdin_path);
	if (fh->type != V4L2_BUF_TYPE_VIDEO_CAPTURE)
		return -EINVAL;
	if (i != fh->type)
		return -EINVAL;
...
static int sp0838_close(struct file *file)
{
	...
	unsigned vdin_path = fh->dev->cam_info.vdin_path;
	sp0838_have_open = 0;
			...
			const struct i2c_device_id *id)
{
	...
	if (plat_dat) {
		...
		pr_err("camera sp0838: have no platform data\n");
		kfree(t);
		kfree(client);
		return -1;
	}
