/*
 * drivers/amlogic/camera/common/config_parser.c
 *
...
int parse_body_head(char *buffer, int *no, int check, char *name)
{
	...
	if (iter == NULL)
		return -WRONG_FORMAT;
	ret = sscanf(iter, "%d", no);
	iter = strstr(iter, "name");
	iter += 5;
	if (iter == NULL)
		return -WRONG_FORMAT;
	ret = sscanf(iter, "%s", name);
	return 0;
}
int parse_aet_element_info(char **iter, struct sensor_aet_info_s *info)
{
	...
	*iter += 7;
	ret = sscanf(*iter, "%x", &(info->fmt_main_fr));
	*iter = strstr(*iter, ",");
	*iter += 1;
	ret = sscanf(*iter, "%x", &(info->fmt_capture));
	*iter = strstr(*iter, ",");
	*iter += 1;
	ret = sscanf(*iter, "%x", &(info->fmt_hactive));
	*iter = strstr(*iter, ",");
	*iter += 1;
	ret = sscanf(*iter, "%x", &(info->fmt_vactive));
	*iter = strstr(*iter, ",");
	*iter += 1;
	ret = sscanf(*iter, "%x", &(info->fmt_rated_fr));
	*iter = strstr(*iter, ",");
	*iter += 1;
	ret = sscanf(*iter, "%x", &(info->fmt_min_fr));
	*iter = strstr(*iter, ",");
	*iter += 1;
	ret = sscanf(*iter, "%x", &(info->tbl_max_step));
	*iter = strstr(*iter, ",");
	*iter += 1;
	ret = sscanf(*iter, "%x", &(info->tbl_rated_step));
	*iter = strstr(*iter, ",");
	*iter += 1;
	ret = sscanf(*iter, "%x", &(info->tbl_max_gain));
	*iter = strstr(*iter, ",");
	*iter += 1;
	ret = sscanf(*iter, "%x", &(info->tbl_min_gain));
	*iter = strstr(*iter, ",");
	*iter += 1;
	ret = sscanf(*iter, "%x", &(info->format_transfer_parameter));
	*iter = strstr(*iter, ",");
	*iter += 1;
	return 0;
...
{
	int ret;
	ret = sscanf(*iter, "%x", &(tbl->exp));
	*iter = strstr(*iter, ",");
	*iter += 1;
	ret = sscanf(*iter, "%x", &(tbl->ag));
	*iter = strstr(*iter, ",");
	*iter += 1;
	ret = sscanf(*iter, "%x", &(tbl->vts));
	*iter = strstr(*iter, ",");
	*iter += 1;
	ret = sscanf(*iter, "%x", &(tbl->gain));
	*iter = strstr(*iter, ",");
	*iter += 1;
	ret = sscanf(*iter, "%x", &(tbl->fr));
	*iter = strstr(*iter, ",");
	*iter += 1;
	return 0;
}
		 ...
		 int *remained, int *offset)
{
	...
	while (check < sum) {
		...
		if (iter == NULL)
			return -WRONG_FORMAT;
		ret = parse_body_head(iter, &(cf->eff.eff[check].num), check,
	     ...
	     int *remained, int *offset)
{
	...
	while (check < sum) {
		...
		if (iter == NULL)
			return -WRONG_FORMAT;
		ret = parse_body_head(iter, &(cf->hw.hw[check].num), check,
				      cf->hw.hw[check].name);
		if (ret != 0)
			return -BODY_HEAD_FAILED;
		iter = strstr(iter, "export");
		...
		while (iter != NULL && iter < eter) {
			ret = sscanf(iter, "%x", &(cf->hw.hw[check].export[i]));
			i++;
			iter = strstr(iter, ",");
			if (iter == NULL)
				break;
	     ...
	     int *remained, int *offset)
{
	...
	while (check < sum) {
		...
		if (iter == NULL)
			return -WRONG_FORMAT;
		ret = parse_body_head(iter, &(cf->wb.wb[check].num), check,
				      cf->wb.wb[check].name);
		if (ret != 0)
			return -BODY_HEAD_FAILED;
		iter = strstr(iter, "export");
	       ...
	       int *remained, int *offset)
{
	...
	i = 0;
	while (iter != NULL && iter < eter) {
		ret = sscanf(iter, "%x", &(cf->wave.export[i]));
		/* pr_err("wave:%x\n",cf->wave.export[i]); */
		iter = strstr(iter, ",");
		i++;
		if (iter == NULL)
		...
		int *remained, int *offset)
{
	...
	while (check < sum) {
		...
		ret = parse_body_head(iter, &((cf->scene.scene[check]).num),
		...
		int *remained, int *offset)
{
	...
	for (i = 0; i < GAMMA_MAX && iter != NULL;) {
		ret = sscanf(iter, "%x",
			(unsigned int *) &(cf->gamma.gamma_r[i]));
		i++;
		iter = strstr(iter, ",");
		if (iter == NULL)
			break;
	...
	for (i = 0; i < GAMMA_MAX && iter != NULL;) {
		ret = sscanf(iter, "%x",
				(unsigned int *) &(cf->gamma.gamma_g[i]));
		i++;
		iter = strstr(iter, ",");
		if (iter == NULL)
			break;
	...
	for (i = 0; i < GAMMA_MAX && iter != NULL;) {
		ret = sscanf(iter, "%x",
			(unsigned int *) &(cf->gamma.gamma_b[i]));
		i++;
		iter = strstr(iter, ",");
		if (iter == NULL)
			break;
		    ...
		    int *remained, int *offset)
{
	...
	while (i < WB_SENSOR_MAX && iter != NULL) {
		ret = sscanf(iter, "%x", &(cf->wb_sensor_data.export[i]));
	     ...
	     int *remained, int *offset)
{
	...
	while (i < CM_MAX && iter != NULL) {
		ret = sscanf(iter, "%x", &(cf->cm.export[i]));
		/* pr_err("cm:%x\n",cf->cm.export[i]); */
		i++;
		iter = strstr(iter, ",");
		if (iter == NULL)
	     ...
	     int *remained, int *offset)
{
	...
	while (check < sum) {
		...
		ret = parse_body_head(iter, &(cf->nr.nr[check].num), check,
				      cf->nr.nr[check].name);
		if (ret != 0)
			return -BODY_HEAD_FAILED;
		iter = strstr(iter, "export");
		...
		while (i < NR_MAX && iter != NULL) {
			ret = sscanf(iter, "%x", &(cf->nr.nr[check].export[i]));
