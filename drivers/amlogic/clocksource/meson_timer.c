...
/* /    aml_write_reg32(P_ISA_TIMERE, 0); */
     ...
     * clocks_calc_mult_shift(m, s, 1MHz, NSEC_PER_SEC, 0).
...
void   local_timer_set_affinity(void *hcpu)
{
	int cpuidx = (long)hcpu;
	struct meson_clock *mclk = &per_cpu(percpu_mesonclock, cpuidx);
	if (cpuidx)
		irq_set_affinity(mclk->irq.irq, cpumask_of(cpuidx));
...
static ssize_t timere_show(struct class *cla,
		struct class_attribute *attr, char *buf)
{
	return sprintf(buf, "%u\n", readl_relaxed(clocksrc_base));
}
