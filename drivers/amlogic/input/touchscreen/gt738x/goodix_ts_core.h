...
struct goodix_ext_attribute {
	...
	ssize_t (*show)(struct goodix_ext_module *mod, char *buf);
	ssize_t (*store)(struct goodix_ext_module *mod, const char *but, size_t size);
};
 ...
 */
static inline struct goodix_ts_device *ts_device(struct goodix_ts_core
						 *core)
{
	if (!core)
		return NULL;
...
#endif
#else /* MODULE */
#ifndef CONFIG_TOUCHSCREEN_GOODIX_GT738X_UPDATE_MODULE
static inline int goodix_do_fw_update(int mode)
{
	return -1111;
}
