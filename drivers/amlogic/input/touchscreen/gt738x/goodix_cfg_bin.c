...
static int goodix_parse_cfg_bin(struct goodix_cfg_bin *cfg_bin)
{
	...
	for (i = 0; i < cfg_bin->head.pkg_num; i++) {
		...
		if (i == cfg_bin->head.pkg_num - 1) {
			...
			if (offset2 <= offset1) {
				ts_err("ofs er,pkg:%d, ofs1:%d,ofs2:%d",
				       i, offset1, offset2);
				r = -EINVAL;
				goto exit;
			}
		...
		cfg_bin->cfg_pkgs[i].cfg = &cfg_bin->bin_data[offset1
					+ TS_PKG_HEAD_LEN];
	}
	...
	for (i = 0; i < cfg_bin->head.pkg_num; i++) {
		...
		ts_debug("coor:%02x",
			 cfg_bin->cfg_pkgs[i].reg_info.coor.addr);
...
static int goodix_cfg_bin_proc(struct goodix_ts_core *core_data)
{
	...
	cfg_bin = kzalloc(sizeof(*cfg_bin), GFP_KERNEL);
