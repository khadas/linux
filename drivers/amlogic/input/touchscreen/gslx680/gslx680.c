...
static void report_data(struct gsl_ts *ts, u16 x, u16 y, u8 pressure, u8 id)
{
	...
	print_info("report id=%d,x=%d,y=%d######\n",id,x,y);	
...
static int gsl_ts_early_suspend(struct device *tp_d)
{
...
static int gsl_ts_late_resume(struct device *tp_d)
{
	struct gsl_ts *ts = container_of(tp_d, struct gsl_ts, tp);
	printk("[GSLX680] Enter %s\n", __func__);
...
*/	if (!i2c_check_functionality(client->adapter, I2C_FUNC_I2C)) {
	...
	ts->wake_pin=of_get_named_gpio(np, "reset-gpio", 0);
	
	if (gpio_is_valid(ts->wake_pin)) {
		rc = gpio_request(ts->wake_pin, "gslX680 wake pin");
		if (rc != 0) {
	...
	if (gpio_is_valid(ts->irq_pin)) {
		rc = gpio_request(ts->irq_pin, "gslX680 irq pin");
		if (rc != 0) {
