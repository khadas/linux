...
static int gsensor_sysfs_init(void)
{
	...
	if (android_gsensor_kobj == NULL) {
		PRINT_INFO(KERN_ERR
			"gsensor_sysfs_init: subsystem_register failed\n");
		ret = -ENOMEM;
		goto err;
	}
	...
	if (ret) {
		PRINT_INFO(KERN_ERR
			"gsensor_sysfs_init:  sysfs_create_group failed\n");
		goto err4;
	}
...
static int dmard06_tx_data(struct i2c_client *client, char *txData, int length)
{
	...
	if (ret < 0) {
		pr_err("err in write dmard06, %d byte(s) should be written,.\n",
			length);
		return -EIO;
	} else
		return 0;
...
static int dmard06_set_rate(struct i2c_client *client, char rate)
{
	...
	/* 因为增加了滤波功能,即每RawDataLength次才上报一次,
	   所以提升gsensor两个档级 */
...
#else
	schedule_delayed_work(&dmard06->delaywork,
			msecs_to_jiffies(dmard06->delay));
#endif
	PRINT_INFO("\n---------dmard06_start-----------\n");
...
static void dmard06_report_value(struct i2c_client *client,
					struct dmard06_axis *axis)
{
	...
	aml_sensor_report_acc(client, dmard06->input_dev, axis->x, axis->y,
				axis->z);
}
...
static int dmard06_get_data(struct i2c_client *client)
{
	...
	/*(buffer[0] & 0x40) || (buffer[1] & 0x40) || (buffer[2] & 0x40)*/0);
	x = buffer[0]>>1;
	y = buffer[1]>>1;
	z = buffer[2]>>1;
...
static long dmard06_ioctl(struct file *file, unsigned int cmd,
						unsigned long arg)
{
	...
	int ret = -1;
	char rate;
	struct i2c_client *client = container_of(dmard06_device.parent,
...
{
	struct dmard06_data *dmard06 =
		container_of(work, struct dmard06_data, work);
...
static void  dmard06_delaywork_func(struct work_struct *work)
{
	...
	struct dmard06_data *dmard06 =
		container_of(delaywork, struct dmard06_data, delaywork);
	struct i2c_client *client = dmard06->client;
...
static const struct file_operations dmard06_fops = {
	.owner = THIS_MODULE,
	.open = dmard06_open,
...
static int dmard06_suspend(android_early_suspend_t *h)
{
	struct i2c_client *client =
		container_of(dmard06_device.parent, struct i2c_client, dev);
...
static int dmard06_init_client(struct i2c_client *client)
{
	...
	ret = request_irq(client->irq, dmard06_interrupt, IRQF_TRIGGER_LOW,
					client->dev.driver->name, dmard06);
	PRINT_INFO("request irq is %d,ret is  0x%x\n", client->irq, ret);
	if (ret) {
		PRINT_INFO(KERN_ERR
			"dmard06_init_client: request irq failed, ret is %d\n",
			ret);
		return ret;
	}
