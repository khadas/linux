...
{
	return STK_ALS_DATA(stk_readb(STK_ALS_DT1_REG),
...
#endif
	/*CONFIG_STK_ALS_TRANSMITTANCE_TUNING*/
}
...
static ssize_t lux_range_show(struct kobject *kobj,
	struct kobj_attribute *attr, char *buf)
{
...
static ssize_t lux_show(struct kobject *kobj,
	struct kobj_attribute *attr, char *buf)
{
	int32_t lux;
	STK_LOCK(1);
...
static ssize_t lux_res_show(struct kobject *kobj,
	struct kobj_attribute *attr, char *buf)
{
...
ssize_t stk_bin_sysfs_read(als_lux_range)
{
	...
	if (count != sizeof(uint32_t)) {
		WARNING("STK ALS Driver:Err -->Read Lux Range(bin) size !=4\n");
		return 0;
	}
#endif
...
#if __DEBUG_SYSFS_BIN
	if (count != sizeof(uint32_t)) {
		WARNING("STK ALS Driver:Err-->Read Distance Range size !=4\n");
		return 0;
	}
#endif
...
ssize_t stk_bin_sysfs_read(als_enable)
{
	...
	if (count != sizeof(uint8_t)) {
		WARNING("STK ALS Driver:Err -->Read als_enable_bin size !=1\n");
		return 0;
	}
#endif
	STK_LOCK(1);
...
#if __DEBUG_SYSFS_BIN
	if (count != sizeof(uint32_t)) {
		WARNING("STK ALS Driver : Err-->Read als_min_delay size !=4\n");
		return 0;
	}
#endif
...
static struct bin_attribute als_enable_bin_attribute =
	...
	sizeof(uint8_t));
/* <---DEPRECATED */
static struct bin_attribute als_delay_bin_attribute =
	__ATTR_BIN_RW(als_delay_bin, als_delay_read, als_delay_write,
	sizeof(uint32_t));
static struct bin_attribute als_min_delay_bin_attribute =
	__ATTR_BIN_RO(als_min_delay_bin, als_min_delay_read, sizeof(uint32_t));
/* DEPRECATED---> */

#ifdef CONFIG_STK_SYSFS_DBG

#ifdef CONFIG_STK_ALS_TRANSMITTANCE_TUNING
static ssize_t als_transmittance_show(struct kobject *kobj,
	struct kobj_attribute *attr, char *buf)
{
...
static struct kobj_attribute als_transmittance_attribute =
	(struct kobj_attribute)__ATTR_RW(als_transmittance);
#endif /* CONFIG_STK_ALS_TRANSMITTANCE_TUNING */
...
static struct platform_device *stk_oss_dev; /* Device structure */

static int stk_sysfs_create_files(struct kobject *kobj,
...
/*
static void stk_sysfs_remove_bin_files(struct kobject *kobj,
	struct bin_attribute** bin_attrs)
{
	while(*bin_attrs!=NULL) {
		...
		const struct i2c_device_id *id)
{
	...
	input_set_abs_params(pStkAlsData->input_dev, ABS_MISC, 0,
		alscode2lux((1<<12)-1), 0, 0);
	...
	err = stk_sysfs_create_bin_files(&(pStkAlsData->input_dev->dev.kobj),
		sensetek_optical_sensors_bin_attrs);
	if (err)
		return -ENOMEM;
	err = stk_sysfs_create_files(&(pStkAlsData->input_dev->dev.kobj),
		sensetek_optical_sensors_attrs);
	if (err)
		return -ENOMEM;
#ifdef CONFIG_STK_SYSFS_DBG
	err = sysfs_create_group(&(pStkAlsData->input_dev->dev.kobj),
		&sensetek_optics_sensors_attrs_group);
	if (err)
		return -ENOMEM;
#endif
