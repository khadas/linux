...
static void bma222_work_func(struct work_struct *work)
{
	...
	struct gsen_platform_data *pdata =
	(struct gsen_platform_data *)bma222->bma222_client->dev.platform_data;

	bma222_read_accel_xyz(bma222->bma222_client, &acc);
	if (!pdata) {
		aml_sensor_report_acc(bma222->bma222_client, bma222->input,
		acc.x, acc.y, acc.z);
	} else{
		input_report_abs(bma222->input, ABS_X, acc.x * pdata->rotator[0]
			+acc.y * pdata->rotator[1]+acc.z * pdata->rotator[2]);
		input_report_abs(bma222->input, ABS_Y, acc.x * pdata->rotator[3]
			+acc.y * pdata->rotator[4]+acc.z * pdata->rotator[5]);
		input_report_abs(bma222->input, ABS_Z, acc.x * pdata->rotator[6]
			+acc.y * pdata->rotator[7]+acc.z * pdata->rotator[8]);
...
static ssize_t bma222_enable_int_store(struct device *dev,
		...
		const char *buf, size_t count)
{
	int type = 0, value = 0, ret = 0;
	struct i2c_client *client = to_i2c_client(dev);
	struct bma222_data *bma222 = i2c_get_clientdata(client);

	ret = sscanf(buf, "%d%d", &type, &value);
	if (ret == -1)
		return -EINVAL;
...
static void bma222_irq_work_func(struct work_struct *work)
{
	...
	case 0x02:
		for (i = 0; i < 3; i++) {
			...
			pr_info("High G interrupt happened,exis is %d,", i);
			pr_info("first is %d,sign is %d\n",
	...
	case 0x04:
		for (i = 0; i < 3; i++) {
			...
			pr_info("Slop interrupt happened,exis is %d,", i);
			pr_info("first is %d,sign is %d\n",
					first_value, sign_value);
		}
		break;
	...
	case 0x40:
		bma222_get_orient_status(bma222->bma222_client,
				    &first_value);
		pr_info("orient interrupt happened,%s\n",
				orient[first_value]);
		if (first_value == 0)
	...
	case 0x80:
		bma222_get_orient_flat_status(bma222->bma222_client,
				    &sign_value);
		pr_info("flat interrupt happened,flat status is %d\n",
				    sign_value);
		if (sign_value == 1) {
...
/* gpio_enable_edge_int(gpio_to_idx(INT_PORT), 1, client->irq - INT_GPIO_0); */
	...
	if (tempvalue == BMA222_CHIP_ID || tempvalue == BMA222E_CHIP_ID) {
		...
		pr_info("BMA222 registered I2C driver!\n");
	} else{
		pr_info("Bosch Sensortec Device not found,");
		pr_info("i2c error %d\n", tempvalue);
		err = -ENODEV;
		goto kfree_exit;
	}
