...
/*1 means define device in this driver*/
#define DEVICE_CREATE_BYSELF            1
/*2 means define device in system file*/
#define DEVICE_CREATE_BYPLATFORM        2
 /*DEVICE_CREATE_BYSELF //DEVICE_CREATE_BYPLATFORM*/
#define DEVICE_CREATE_MODE              DEVICE_CREATE_BYPLATFORM
#define MIR3DA_INPUT_DEV_NAME           "mir3da"
...
static void				*mir_handle;
static int    Log_level = DEBUG_ERR;

#define MI_DATA(format, ...)  \
do { \
	if (DEBUG_DATA&Log_level) { \
		dprintk(KERN_ERR MI_TAG format "\n", ## __VA_ARGS__); } \
} while (0)
...
do { \
	if (DEBUG_FUNC&Log_level) { \
		dprintk(KERN_ERR MI_TAG "%s is called, line: %d\n", \
		__func__, __LINE__); } \
} while (0)
...
static struct i2c_board_info         mir3da_i2c_boardinfo = {
	I2C_BOARD_INFO(MIR3DA_DRV_NAME, MIR3DA_I2C_ADDR) };
#endif

#if MIR3DA_OFFSET_TEMP_SOLUTION
static const char OffsetFileName[] = "/data/misc/miraGSensorOffset.txt";
...
static void sensor_write_work(struct work_struct *work)
{
	...
	if (IS_ERR(filep)) {
		...
		filep->f_op->write(filep, pWorkInfo->buffer,
			pWorkInfo->len, &filep->f_pos);
...
static int sensor_sync_read(u8 *offset)
{
	...
	err = sscanf(m_work_info.buffer, "%x,%x,%x,%x,%x,%x,%x,%x,%x",
...
static ssize_t mir3da_odr_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	int ret;
	int odr;
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	rst = sscanf(buf, "%d,%d,%d,%d,%d,%d,%d,%d,%d\n",
		&off[0], &off[1], &off[2], &off[3], &off[4],
		&off[5], &off[6], &off[7], &off[8]);
	if (rst < 0)
		return -EINVAL;
...
static DEVICE_ATTR(enable, 0666,
	mir3da_enable_show, mir3da_enable_store);
static DEVICE_ATTR(delay, 0666,
	mir3da_delay_show, mir3da_delay_store);
static DEVICE_ATTR(axis_data, 0666, mir3da_axis_data_show, NULL);
static DEVICE_ATTR(reg_data, 0666,
	mir3da_reg_data_show, mir3da_reg_data_store);
static DEVICE_ATTR(log_level, 0666,  mir3da_log_level_show,
	mir3da_log_level_store);
#if MIR3DA_OFFSET_TEMP_SOLUTION
static DEVICE_ATTR(offset,  0666,  mir3da_offset_show,
	mir3da_offset_store);
static DEVICE_ATTR(calibrate_miraGSensor, 0666,
	mir3da_calibrate_show,          mir3da_calibrate_store);
#endif
#if FILTER_AVERAGE_ENHANCE
static DEVICE_ATTR(average_enhance, 0666,
...
int i2c_smbus_read(void *handle, u8 addr, u8 *data)
{
	int                 res = 0;
...
int i2c_smbus_read_block(void *handle, u8 addr, u8 count, u8 *data)
{
	int                 res = 0;
	struct i2c_client   *client = (struct i2c_client *)handle;
...
int i2c_smbus_write(void *handle, u8 addr, u8 data)
{
	int                 res = 0;
	struct i2c_client   *client = (struct i2c_client *)handle;
...
MIR_GENERAL_OPS_DECLARE(ops_handle, i2c_smbus_read, i2c_smbus_read_block,
	i2c_smbus_write, sensor_sync_write,
	sensor_sync_read, msdelay, printk, sprintf);
#else
MIR_GENERAL_OPS_DECLARE(ops_handle, i2c_smbus_read, i2c_smbus_read_block,
	...
	const struct i2c_device_id *id)
{
	...
	mir_handle = mir3da_core_init((void *)client);
	if (NULL == mir_handle) {
