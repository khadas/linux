...
/* acceleration samples 5ms */
#define MMA8452_CTRL_PWRON_1_5MS		0x10
/* acceleration samples 10ms */
#define MMA8452_CTRL_PWRON_1_10MS		0x18
...
static void mma8452_work_func(struct work_struct *work)
{
	...
	if (mma8452_read_accel_xyz(mma8452->mma8452_client, &acc) == 0) {
		...
		aml_sensor_report_acc(mma8452->mma8452_client, mma8452->input,
			acc.x, acc.y, acc.z);
...
static void mma8452_poweron(void)
{
	...
	if (mma8452_i2c_tx_data(buf, 2) < 0)
		dprintk("mma8452 power on fail...1\n");
	buf[0] = MMA8452_XYZ_DATA_CFG;
	buf[1] = MMA8452_CTRL_MODE_2G;
	if (mma8452_i2c_tx_data(buf, 2) < 0)
		dprintk("mma8452 power on fail...2\n");
	buf[0] = MMA8452_REG_CTRL;
	buf[1] = MMA8452_CTRL_PWRON_1_5MS;
	buf[1] |= MMA8452_CTRL_ACTIVE;
	if (mma8452_i2c_tx_data(buf, 2) < 0)
		dprintk("mma8452 power on fail...3\n");
	/* wait PWRON done */
...
static void enable_change_mma8452_reg(int enable)
{
	...
	buf[0] = MMA8452_REG_CTRL;
	if (mma8452_i2c_rx_data(buf, 1) < 0) {
		dprintk("read reg[0x%x] fail", buf[0]);
		return;
	}
	if (enable) {
		...
		buf[0] = MMA8452_REG_CTRL;
		if (mma8452_i2c_tx_data(buf, 2) < 0)
			dprintk("mma8452 suspend fail\n");
	} else {
		buf[1] = buf[0] | MMA8452_CTRL_ACTIVE;
		buf[0] = MMA8452_REG_CTRL;
		if (mma8452_i2c_tx_data(buf, 2) < 0)
			dprintk("mma8452 active fail\n");
	}
}
...
static void reg_help(void)
{
	dprintk("Usage:\n");
	dprintk("w\n");
	dprintk(" write register\n");
	dprintk("r\n");
	dprintk(" read register\n");
	dprintk("Example:\n");
	dprintk(" echo \"r 00\" > /sys/devices/virtual/input/input4/reg\n");
	dprintk(" echo \"w 01 08\" > /sys/devices/virtual/input/input4/reg\n");
}
		...
		const char *buf, size_t count)
{
	...
	if (flag) {
		...
		while (*start == ' ')
			start++;
		error = kstrtoul(start, 16, &value);
		if (error)
			return error;
		data[1] = value;
		dprintk("Write reg[0x%lx]=0x%x\n", reg, data[1]);
		enable_change_mma8452_reg(1);
		if (mma8452_i2c_tx_data(data, 2) < 0) {
			dprintk("write reg[%lx] fail", reg);
			return -EINVAL;
		}
		...
		if (mma8452_i2c_rx_data(data, 1) < 0) {
			dprintk("read reg[0x%lx] fail", reg);
			return -EINVAL;
		}
		dprintk("Read reg[%lx]=0x%x\n", reg, data[0]);
	} else {
		if (mma8452_i2c_rx_data(data, 1) < 0) {
			dprintk("read reg[0x%lx] fail", reg);
			return -EINVAL;
		}
		dprintk("Read reg[0x%lx]=0x%x\n", reg, data[0]);
	}
		...
		const struct i2c_device_id *id)
{
	...
	if (!i2c_check_functionality(client->adapter, I2C_FUNC_I2C)) {
		dprintk(KERN_INFO "i2c_check_functionality error\n");
		goto exit;
	}
