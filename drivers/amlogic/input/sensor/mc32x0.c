...
static ssize_t	show_axis_force(struct device *dev,
	struct device_attribute *attr, char *buf);
static ssize_t	show_xyz_force(struct device *dev,
	struct device_attribute *attr, char *buf);
...
static void mc32x0_read_tilt(u8 *pt)
{
	u32 result;
	assert(mc32x0_i2c_client);
	result = i2c_smbus_read_byte_data(mc32x0_i2c_client,
		MC32X0_Tilt_Status_REG);
	*pt = result;
}

ssize_t	show_orientation(struct device *dev, struct device_attribute *attr,
	char *buf)
{
	int result;
...
ssize_t show_xyz_force(struct device *dev, struct device_attribute *attr,
	char *buf)
{
	int i;
	s8 xyz[3];
...
ssize_t	show_axis_force(struct device *dev, struct device_attribute *attr,
	char *buf)
{
	s8 force;
		...
		const char *buf, size_t count)
{
	...
	mc32x0_idev->poll_interval = poll_interval =
		(delay > MAX_POLL_INTERVAL) ? MAX_POLL_INTERVAL : delay;
	return count;
}
...
int mc32x0_set_image(struct i2c_client *client)
{
	...
	if (data == 0x19 || data == 0x29)
		...
		dprintk("mc32x0: unrecognized product id: %x\n", data);
		return -1;
	}
	...
	if (McubeID & MCUBE_8G_14BIT) {
		...
		comres += i2c_smbus_write_byte_data(client,
			MC32X0_Mode_Feature_REG, data);

		data = 0x00;
		comres += i2c_smbus_write_byte_data(client,
			MC32X0_Sleep_Count_REG, data);

		data = 0x00;
		comres += i2c_smbus_write_byte_data(client,
			MC32X0_Sample_Rate_REG, data);

		data = 0x00;
		comres += i2c_smbus_write_byte_data(client,
			MC32X0_Tap_Detection_Enable_REG, data);

		data = 0x3F;
		comres += i2c_smbus_write_byte_data(client,
			MC32X0_RANGE_Control_REG, data);

		data = 0x00;
		comres += i2c_smbus_write_byte_data(client,
			MC32X0_Interrupt_Enable_REG, data);
		/* #endif */
...
#if DEBUG
	dprintk("mcube %s reg0x07 = 0x%x  ,comres = %d\n",
...
static int mc32x0_init_client(struct i2c_client *client)
{
	...
	/* plat_data = (struct mxc_mc32x0_platform_data *)client
		->dev.platform_data; */
	/* assert(plat_data); */
...
static void report_abs(void)
{
	...
	/* mc3210 */
		...
		for (i = 0; i < 6; i++)
			raw_buf[i] = i2c_smbus_read_byte_data(mc32x0_i2c_client,
				i+MC32X0_XOUT_EX_L_REG);
		...
		const struct i2c_device_id *id)
{
	...
	input_set_abs_params(idev, ABS_Z, -MAX_ABS_Z, MAX_ABS_Z,
		INPUT_FUZZ, INPUT_FLAT);
#if DEBUG
	dprintk("***** Sensor MC32X0 param: max_abs_x = %d,max_abs_y = %d,",
		MAX_ABS_X, MAX_ABS_Y);
	dprintk("max_abs_z = %d INPUT_FUZZ = %d,INPUT_FLAT = %d\n",
		 MAX_ABS_Y, INPUT_FUZZ, INPUT_FLAT);
#endif
	result = input_register_polled_device(mc32x0_idev);


	result = sysfs_create_group(&mc32x0_idev->input->dev.kobj,
		&mc32x0_attribute_group);
	assert(result == 0);
	if (result) {
...
static int mc32x0_remove(struct i2c_client *client)
{
	...
	mutex_lock(&sensor_lock);
	result = i2c_smbus_write_byte_data(client, MC32X0_Mode_Feature_REG,
		MC32X0_MODE_SLEEP);
	assert(result == 0);
