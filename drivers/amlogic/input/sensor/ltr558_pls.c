...
static int LTR558_DEVICE_IC;
static int ltr558_probed;

static int calibrate_high_ltr558;	/* add for calibrate */
static int calibrate_low_ltr558;
static int calibrate_status_ltr558;	/* add for calibrate */
...
static int final_lux_val_ltr558;

static int ltr558_irq;
static unsigned char suspend_flag_ltr558;
...
static ssize_t ltr558_store_suspend(struct device *cd,
...
static int pls_rx_data(char *buf, int len)
{
	...
	if (ret < 0)
		dprintk(KERN_ERR "%s i2c read error: %d\n", __func__, ret);
...
static int ltr558_pls_tx_data(char *buf, int len)
{
	...
	if (ret < 0)
		dprintk(KERN_ERR "%s i2c write error: %d\n", __func__, ret);
...
static int ltr558_ps_enable(int gainrange)
{
	...
	if (error < 0)
		...
		if (0 == calibrate_status_ltr558) {
			...
			/* upper threshold lower byte */
			ltr558_i2c_write_reg(0x90, calibrate_high_ltr558);
			/* upper threshold upper byte */
			ltr558_i2c_write_reg(0x91, 0x00);
			/* lower threshold lower byte */
			ltr558_i2c_write_reg(0x92, 0x00);
			/* lower  threshold upper byte */
	...
	LTR558_DEBUG("%s: ltr558_ps_enable over, error = %d ", __func__, error);
	LTR558_DEBUG("avarage =%d calibrate_status_ltr558=%d\n",
		avarage, calibrate_status_ltr558);
	return error;
}
...
/*----------------------------------------------------------------------------*/
static ssize_t ltr558_show_ps(struct device *cd,
	struct device_attribute *attr, char *buf)
{
	/* ssize_t res; */
	unsigned int  dat = 0;
	...
	return snprintf(buf, PAGE_SIZE,
		"ps_data %d     0x%x    high=0x%x low=0x%x  0x8c=0x%x\n",
		dat, dat, calibrate_high_ltr558, calibrate_low_ltr558, dat1);
}
static ssize_t ltr558_show_als(struct device *cd,
	struct device_attribute *attr, char *buf)
{
	/* ssize_t res; */
	unsigned int  dat = 0;
...
/*
static ssize_t ltr558_ps_status(struct device* cd,
	struct device_attribute *attr, char* buf)
{
	unsigned char dat=0;
	if(!the_data_ltr558)
...
static ssize_t ltr558_store_reg(struct device *cd,
...
static ssize_t ltr558_als_status(struct device *cd,
	struct device_attribute *attr, char *buf)
{
	unsigned char dat = 0;
	if (!the_data_ltr558)
...
static DEVICE_ATTR(als_status_ltr558,
	S_IRUGO | S_IWUSR, ltr558_als_status, NULL);
static DEVICE_ATTR(ps_status_ltr558,
	S_IRUGO | S_IWUSR, ltr558_ps_status, ltr558_store_reg);
static DEVICE_ATTR(als_data_ltr558,
	S_IRUGO | S_IWUSR, ltr558_show_als,	NULL);
static DEVICE_ATTR(ps_data_ltr558,
	S_IRUGO | S_IWUSR, ltr558_show_ps,	NULL);
/* gionee liudj add for debug end */
static DEVICE_ATTR(suspend_ltr558,
	S_IRUGO | S_IWUSR, ltr558_show_suspend, ltr558_store_suspend);
static DEVICE_ATTR(version_ltr558,
	S_IRUGO | S_IWUSR, ltr558_show_version, NULL);
...
#if 0
static long ltr558_ps_ioctl(struct file *file, unsigned int cmd,
	unsigned long arg)
{
	int ret = 0;
	int buffer = 0;
...
#if 0
static const struct file_operations ltr558_ps_fops = {
	.owner  = THIS_MODULE,
	.open  = ltr558_ps_open,
...
static int ltr558_als_read(int gainrange)
{
	...
	if (0 == alsval_ch1 + alsval_ch0) {
		ratio = 1000;
		/* dprintk("LTR558 als read data error!!\n"); */
	} else
	...
	if (ratio < 450) {
		...
		ch0_coeff = 37725;
		ch1_coeff = 13663;
		luxdata_int  = ((alsval_ch0*ch0_coeff)
			- (alsval_ch1*ch1_coeff))/10000;
		luxdata_int = luxdata_int + 1;
...
static long ltr558_als_ioctl(struct file *file, unsigned int cmd,
	unsigned long arg)
{
	int ret = 0;
	int buffer = 1;
 ...
 */
#if 0
static const struct file_operations ltr558_als_fops = {
	.owner  = THIS_MODULE,
...
static void ltr558_report_dps(int data, struct input_dev *input)
{
	...
	dprintk("%s: proximity = %d", __func__, dps_data);
...
static void ltr558_pls_work(struct work_struct *work)
{
	...
	if (0x03 == (als_ps_status & 0x03)) {
		...
		if (final_prox_val_ltr558 >= calibrate_high_ltr558) {
			/* <3cm */
			/* upper threshold lower byte */
			...
			/* lower threshold lower byte */
			ltr558_i2c_write_reg(0x92, calibrate_low_ltr558);
			/* lower  threshold upper byte */
			ltr558_i2c_write_reg(0x93, 0x00);

		} else if (final_prox_val_ltr558 <=  calibrate_low_ltr558) {
			/* >5cm */
			/* upper threshold lower byte */
			ltr558_i2c_write_reg(0x90, calibrate_high_ltr558);
			/* upper threshold upper byte */
	...
	if ((0x0c != (als_ps_status & 0x0c)) &&
		(0x03 != (als_ps_status & 0x03))) {
		dprintk("LTR558 inturrept error !!!\n");
		LTR558_DEBUG("interrupt= %d  newdata= %d als_ps_status= 0x%x\n",
		 interrupt, newdata, als_ps_status);
	}
...
reset:
	...
	/* upper threshold lower byte 0x00 modify for poll */
	ltr558_i2c_write_reg(0x97, 0x01);
	/* upper threshold upper byte 0x01 modify for poll */
	...
	const struct i2c_device_id *id)
{
	...
	if (LTR558_DEVICE_IC) {
		...
		struct ltr558_pls_platform_data *pdata =
			client->dev.platform_data;

		LTR558_DEBUG("%s:start probe", __func__);
		/* Return 1 if adapter supports everything we need, 0 if not.*/
		if (!i2c_check_functionality(adapter, I2C_FUNC_I2C)) {
			LTR558_DEBUG("%s: LTR-558ALS function check failed.\n",
				__func__);
			ret = -EIO;
			goto out;
		} else
			LTR558_DEBUG("%s:LTR-558ALS function check success.\n",
				__func__);
		...
		the_data_ltr558->ltr_work_queue =
			create_singlethread_workqueue(LTR558_DEVICE);
		...
		if (ret < 0) {
			LTR558_DEBUG(KERN_ERR
				"%s: input device regist failed\n", __func__);
		...
		LTR558_DEBUG("I2C name=%s, addr=0x%x, gpio=%d, irq=%d",
			client->name, client->addr, pdata->irq_gpio_number,
			client->irq);
		...
		ret = ltr558_devinit();
		if (ret) {
			LTR558_DEBUG(KERN_ALERT "%s: LTR-558ALS init failed.\n",
				__func__);
			goto exit_devinit_failed;
		} else
			LTR558_DEBUG(KERN_ALERT
				"%s: LTR-558ALS device init success.\n",
				__func__);

		the_data_ltr558->ltr_early_suspend.level =
			EARLY_SUSPEND_LEVEL_DISABLE_FB + 25;
		the_data_ltr558->ltr_early_suspend.suspend =
			ltr558_early_suspend;
...
static void __exit ltr558_driverexit(void)
{
	...
	LTR558_DEBUG(KERN_ALERT
		">>> %s: LTR-558ALS Driver Module REMOVED <<<\n", __func__);
}
