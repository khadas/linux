...
/* ambient light mode dynamic integration time ADC low threshold */
#define ALS_MIN_COUNT		15
#define ALS_MAX_LUX		60000		/* ambient light mode MAX lux */
#define ALS_MIN_LUX		0

/*24 bits integer, 8 bits fraction*/
/* incandescent threshold (1.55859375) */
#define ALS_I_THRESHOLD		0x018F
/* fluorescent threshold (2.08984375) */
#define ALS_F_THRESHOLD		0x0217
/* incandescent and fluorescent mix threshold (1.59765625) */
#define ALS_M_THRESHOLD		0x0199
...
//txbyte: I2C bus transmit byte(s). Single byte(0X01) transmit only
	slave address.
//data: setting value.
//
// Example: If you want to write single byte to 0x1D register address,
	show below elan_sensor_I2C_Write(client,0x1D,0x01,0X02,0xff);
//
 */
...
static int sensor_power_on(struct i2c_client *client)
{
	...
	pr_info("power_on Enter\n");

	reg_l9 = regulator_get(&client->dev, "vcc_l9");

	if (IS_ERR(reg_l9)) {
		pr_err("on:could not get regulator_get, rc = %ld\n",
				PTR_ERR(reg_l9));
		return -ENODEV;
	}
	...
	if (rc) {
		pr_info("power_on:regulator_set_voltage\n");
		return -EINVAL;
	}
	...
	if (rc) {
		dev_err(&client->dev,
			"power_on:unable to enable regulator: %d\n", rc);
		return -EINVAL;
	}
	...
	if (IS_ERR(reg_lvs4)) {
		pr_err("power_on:could not get regulator_get, rc = %ld\n",
				PTR_ERR(reg_lvs4));
		return -ENODEV;
	}
...
static int elan_sensor_lsensor_enable(struct elan_epl_data *epld)
{
	...
	/* pr_info("--- ALS sensor Enable ---\n"); */
	mutex_lock(&als_enable_mutex);

	/* set register 0 */
	/* average cycle = 32 times, sensing mode = continuous,
		sensor mode = als, gain = auto */
	...
	regdata = epl_data->epl_average_item | epl_data->epl_sc_mode |
			epl_data->epl_op_mode | epl_data->epl_sensor_gain_item;
	ret = elan_sensor_I2C_Write(client, REG_0, W_SINGLE_BYTE,
					0X02, regdata);
	...
	epld->epl_integrationtime_item =
		intT_selection[epld->epl_integrationtime_index];
	regdata = epld->epl_integrationtime_item << 4 |
			epl_data->epl_intpersistency | epl_data->epl_adc_item;
	epld->epl_integrationtime = intT_table[epld->epl_integrationtime_item];
	ret = elan_sensor_I2C_Write(client, REG_1, W_SINGLE_BYTE, 0X02,
					regdata);
	...
	ret = elan_sensor_I2C_Write(client, REG_7, W_SINGLE_BYTE, 0x02,
						EPL_C_START_RUN);
	/* pr_info("--- ALS sensor setting finish ---\n"); */
	msleep(20);

	if (ret != 0x02) {
		epld->enable_lflag = 0;
		pr_err("ALS-sensor i2c err\n");
	}
...
static void incandescent_lux(void)
{
	...
	gRawData.lux =
		((gRawData.als_ch1_raw * c) / epld->epl_integrationtime) >> 8;
}
...
static void fluorescent_lux(void)
{
	...
	/* lux = (channel1 * c) / integration time */
	uint32_t c = ((epld->als_ax2 * gRawData.ratio) >> 8) -
			((epld->als_bx * ratio_square) >> 8) - epld->als_c;
	gRawData.lux =
		((gRawData.als_ch1_raw * c) / epld->epl_integrationtime) >> 8;
}
...
static void elan_epl_als_equation(void)
{
	...
	if (gRawData.ratio <= epld->als_i_threshold) {
		...
		incandescent_lux();
	} else if (gRawData.ratio > epld->als_i_threshold &&
			gRawData.ratio < epld->als_m_threshold) {
		incandescent_lux();
	} else if (gRawData.ratio >= epld->als_m_threshold &&
...
static void elan_epl_als_rawdata(void)
{
	...
	maxvalue = (gRawData.als_ch0_raw > gRawData.als_ch1_raw) ?
			gRawData.als_ch0_raw : gRawData.als_ch1_raw;
	minvalue = (gRawData.als_ch0_raw > gRawData.als_ch1_raw) ?
	...
	if (maxvalue <= ALS_MAX_COUNT && minvalue >= ALS_MIN_COUNT) {
		...
		if (maxvalue > ALS_MAX_COUNT) {
			if (epld->epl_integrationtime_index == 0) {
				epld->epl_integrationtime_item =
					intT_selection[epld->
					epl_integrationtime_index];
				epld->epl_integrationtime =
				intT_table[epld->epl_integrationtime_item];
				pr_info("lgiht sensor lux max %d\n",
					ALS_MAX_LUX);
				input_report_abs(epld->als_input_dev, ABS_MISC,
					ALS_MAX_LUX);
				input_sync(epld->als_input_dev);
			} else {
				...
				epld->epl_integrationtime =
					intT_table[epld->
					epl_integrationtime_item];
				pr_info("sensor integrate time down(%d us)\n",
					epld->epl_integrationtime);
				regdata = epld->epl_integrationtime_item << 4
					| epld->epl_intpersistency
					| epld->epl_adc_item;
				elan_sensor_I2C_Write(client, REG_1,
					W_SINGLE_BYTE, 0X02, regdata);
				/* restart sensor */
				elan_sensor_I2C_Write(client, REG_7,
					W_SINGLE_BYTE, 0X02, EPL_C_RESET);
				elan_sensor_I2C_Write(client,
					REG_7, W_SINGLE_BYTE, 0x02,
					EPL_C_START_RUN);
			}
		}

		if (minvalue < ALS_MIN_COUNT) {
			pr_info("ADJ_Min value=%d, intT_index=%d, intT=%d\n",
				minvalue, epld->epl_integrationtime_index,
				epld->epl_integrationtime);
			if (epld->epl_integrationtime_index == maxIndex) {
				...
				epld->epl_integrationtime =
					intT_table[epld->
					epl_integrationtime_item];
				pr_info("ambient light sensor min lux ,");
				pr_info("ch1_raw=%x, ch0_raw=%x ,IntTime=%d\n",
					gRawData.als_ch1_raw ,
					gRawData.als_ch0_raw,
					epld->epl_integrationtime);
				pr_info("lgith sensor lux min = %d\n",
					ALS_MIN_LUX);
				input_report_abs(epld->als_input_dev, ABS_MISC,
					ALS_MIN_LUX);
				input_sync(epld->als_input_dev);
				...
				epld->epl_integrationtime =
					intT_table[epld->
					epl_integrationtime_item];
				pr_info("sensor integration time up (%d us)\n",
					epld->epl_integrationtime);
				regdata = epld->epl_integrationtime_item
					<<4 | epld->epl_intpersistency
					| epld->epl_adc_item;
				elan_sensor_I2C_Write(client, REG_1,
					W_SINGLE_BYTE, 0X02, regdata);
				/* restart sensor */
				elan_sensor_I2C_Write(client, REG_7,
					W_SINGLE_BYTE, 0X02, EPL_C_RESET);
				elan_sensor_I2C_Write(client, REG_7,
					W_SINGLE_BYTE, 0x02, EPL_C_START_RUN);
			}
		}
	}
}
...
static void polling_do_work(struct work_struct *work)
{
	...
	if (epld->enable_lflag == 0) {
		...
		if (epld->read_flag == 0) {
			...
			elan_sensor_I2C_Write(client, REG_14, R_TWO_BYTE,
				0x01, 0x00);
			elan_sensor_I2C_Read(client);
			gRawData.als_ch0_raw = (gRawData.raw_bytes[1]<<8)
				| gRawData.raw_bytes[0];

			elan_sensor_I2C_Write(client, REG_16, R_TWO_BYTE,
				0x01, 0x00);
			elan_sensor_I2C_Read(client);
			gRawData.als_ch1_raw = (gRawData.raw_bytes[1]<<8)
				| gRawData.raw_bytes[0];
...
static ssize_t elan_ls_sensor_als_ch0_rawdata_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	long *tmp = (long *)buf;
...
static ssize_t elan_ls_sensor_als_ch1_rawdata_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	long *tmp = (long *)buf;
...
static ssize_t elan_ls_incandescent_threshold_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	uint32_t value;
	struct elan_epl_data *epld = epl_data;
...
static ssize_t elan_ls_incandescent_threshold_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	long *tmp = (long *)buf;
	struct elan_epl_data *epld = epl_data;
...
static ssize_t elan_ls_fluorescent_threshold_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	int value;
	struct elan_epl_data *epld = epl_data;
...
static ssize_t elan_ls_fluorescent_threshold_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	long *tmp = (long *)buf;
	struct elan_epl_data *epld = epl_data;
...
static ssize_t elan_ls_mix_threshold_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	int value;
	struct elan_epl_data *epld = epl_data;
...
static ssize_t elan_ls_mix_threshold_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	long *tmp = (long *)buf;
	struct elan_epl_data *epld = epl_data;
...
static ssize_t elan_ls_als_ax2_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	int value;
	struct elan_epl_data *epld = epl_data;
...
static ssize_t elan_ls_als_ax2_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct elan_epl_data *epld = epl_data;
	long *tmp = (long *)buf;
...
static ssize_t elan_ls_als_bx_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	int value;
	struct elan_epl_data *epld = epl_data;
...
static ssize_t elan_ls_als_bx_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct elan_epl_data *epld = epl_data;
	long *tmp = (long *)buf;
...
static ssize_t elan_ls_als_c_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	int value;
	struct elan_epl_data *epld = epl_data;
...
static ssize_t elan_ls_als_c_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct elan_epl_data *epld = epl_data;
	long *tmp = (long *)buf;
...
static ssize_t elan_ls_als_ax_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	int value;
	struct elan_epl_data *epld = epl_data;
...
static ssize_t elan_ls_als_ax_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct elan_epl_data *epld = epl_data;
	long *tmp = (long *)buf;
...
static ssize_t elan_ls_als_b_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	int value;
	struct elan_epl_data *epld = epl_data;
...
static ssize_t elan_ls_als_b_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct elan_epl_data *epld = epl_data;
	long *tmp = (long *)buf;
...
static ssize_t elan_ls_als_lux_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	long *tmp = (long *)buf;
	tmp[0] = gRawData.lux;
...
static DEVICE_ATTR(elan_ls_operationmode, S_IRUGO|S_IWUSR|S_IWGRP,
	elan_ls_operationmode_show, elan_ls_operationmode_store);

static DEVICE_ATTR(elan_ls_sensor_als_ch0_rawdata, S_IRUGO,
	elan_ls_sensor_als_ch0_rawdata_show, NULL);
static DEVICE_ATTR(elan_ls_sensor_als_ch1_rawdata, S_IRUGO,
	elan_ls_sensor_als_ch1_rawdata_show, NULL);
static DEVICE_ATTR(elan_ls_als_lux, S_IRUGO, elan_ls_als_lux_show, NULL);

static DEVICE_ATTR(elan_ls_incandescent_threshold, S_IRUGO|S_IWUSR|S_IWGRP,
	elan_ls_incandescent_threshold_show,
	elan_ls_incandescent_threshold_store);
static DEVICE_ATTR(elan_ls_fluorescent_threshold, S_IRUGO|S_IWUSR|S_IWGRP,
	elan_ls_fluorescent_threshold_show,
	elan_ls_fluorescent_threshold_store);
static DEVICE_ATTR(elan_ls_mix_threshold, S_IRUGO|S_IWUSR|S_IWGRP,
	elan_ls_mix_threshold_show, elan_ls_mix_threshold_store);

static DEVICE_ATTR(elan_ls_als_ax2, S_IRUGO|S_IWUSR|S_IWGRP,
	elan_ls_als_ax2_show, elan_ls_als_ax2_store);
static DEVICE_ATTR(elan_ls_als_bx, S_IRUGO|S_IWUSR|S_IWGRP,
	elan_ls_als_bx_show, elan_ls_als_bx_store);
static DEVICE_ATTR(elan_ls_als_c, S_IRUGO|S_IWUSR|S_IWGRP,
	elan_ls_als_c_show, elan_ls_als_c_store);
static DEVICE_ATTR(elan_ls_als_ax, S_IRUGO|S_IWUSR|S_IWGRP,
	elan_ls_als_ax_show, elan_ls_als_ax_store);
static DEVICE_ATTR(elan_ls_als_b, S_IRUGO|S_IWUSR|S_IWGRP,
	elan_ls_als_b_show, elan_ls_als_b_store);
				...
				unsigned long arg)
{
	...
	pr_info("als ioctl cmd %d\n", _IOC_NR(cmd));
	pr_info("elan_als_ioctl cmd num =%d\n", _IOC_NR(cmd));
	switch (cmd) {
	case ELAN_EPL6800_IOCTL_GET_LFLAG:
		pr_info("elan ambient-light IOCTL Sensor get lflag\n");
		flag = epld->enable_lflag;
		if (copy_to_user(argp, &flag, sizeof(flag)))
			return -EFAULT;
		pr_info("elan ambient-light Sensor get lflag %d\n", flag);
	break;

	case ELAN_EPL6800_IOCTL_ENABLE_LFLAG:

		pr_info("elan ambient-light IOCTL Sensor set lflag\n");
		...
		pr_info("elan ambient-light Sensor set lflag %d\n", flag);
			break;
...
{
	int err = 0;
	pr_info("lightsensor_setup enter.\n");

	epld->als_input_dev = input_allocate_device();
	if (!epld->als_input_dev) {
		pr_err("[epl6800 error]%s: could not alloc ls input device\n",
			__func__);
		return -ENOMEM;
	}
					...
					const struct i2c_device_id *id)
{
	...
	pr_info("chip id REG 0x00 value = %8x\n",
			i2c_smbus_read_byte_data(client, 0x00));
	pr_info("chip id REG 0x01 value = %8x\n",
			i2c_smbus_read_byte_data(client, 0x08));
	pr_info("chip id REG 0x02 value = %8x\n",
			i2c_smbus_read_byte_data(client, 0x10));
	pr_info("chip id REG 0x03 value = %8x\n",
			i2c_smbus_read_byte_data(client, 0x18));
	pr_info("chip id REG 0x04 value = %8x\n",
			i2c_smbus_read_byte_data(client, 0x20));
	pr_info("chip id REG 0x05 value = %8x\n",
			i2c_smbus_read_byte_data(client, 0x28));
	pr_info("chip id REG 0x06 value = %8x\n",
			i2c_smbus_read_byte_data(client, 0x30));
	pr_info("chip id REG 0x07 value = %8x\n",
			i2c_smbus_read_byte_data(client, 0x38));
	pr_info("chip id REG 0x09 value = %8x\n",
			i2c_smbus_read_byte_data(client, 0x48));
	pr_info("chip id REG 0x0D value = %8x\n",
			i2c_smbus_read_byte_data(client, 0x68));
	pr_info("chip id REG 0x0E value = %8x\n",
			i2c_smbus_read_byte_data(client, 0x70));
	pr_info("chip id REG 0x0F value = %8x\n",
			i2c_smbus_read_byte_data(client, 0x71));
	pr_info("chip id REG 0x10 value = %8x\n",
			i2c_smbus_read_byte_data(client, 0x80));
	pr_info("chip id REG 0x11 value = %8x\n",
			i2c_smbus_read_byte_data(client, 0x88));
	pr_info("chip id REG 0x13 value = %8x\n",
	...
	pr_info("elan sensor probe success.\n");
