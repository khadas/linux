 ...
 * INDIRECT OR CONSEQUENTIAL DAMAGES WITH RESPECT ANY CLAIMS ARISING FROM THE
...
static int mxc622x_acc_device_power_on(struct mxc622x_acc_data *acc)
{
	...
	if (acc->pdata->power_on) {
		...
		dprintk("%s acc->pdata->power_on err=%d\n", __func__, err);
		if (err < 0) {
			dev_err(&acc->client->dev,
				"power_on failed: %d\n", err);
			return err;
	...
	*/
	...
	case MXC622X_ACC_IOCTL_GET_COOR_XYZ:
		...
		if (err < 0)
			return err;
#ifdef DEBUG
		/*dprintk(KERN_ALERT "%s Get coordinate xyz:[%d, %d, %d]\n",
	...
	case MXC622X_ACC_IOCTL_GET_CHIP_ID:
		{
			...
			if (copy_to_user(argp, devinfo, sizeof(devinfo))) {
				dprintk("%s error in ", __func__);
				dprintk("copy_to_user(IOCTL_GET_CHIP_ID)\n");
				return -EINVAL;
			}
		}
		break;
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	acc->pdata->poll_interval =
		(delay > acc->pdata->max_interval) ?
		acc->pdata->max_interval : delay;
	acc->pdata->poll_interval =
		max(acc->pdata->poll_interval,
		acc->pdata->min_interval);
		...
		const struct i2c_device_id *id)
{
	...
	if ((tempvalue & 0x000F) == WHOAMI_MXC622X_ACC) {
		...
		acc->client = NULL;
		dprintk("I2C driver not registered! Device unknown 0x%x\n",
			tempvalue);
