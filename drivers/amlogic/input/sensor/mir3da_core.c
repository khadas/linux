...
#define MIR3DA_DATA_LEN         6
	struct reg_obj_t                   data_sect[MIR3DA_DATA_LEN];
...
struct gsensor_obj_s {
	...
	struct reg_obj_t                   chip_id;
	struct reg_obj_t                   mod_id;
	struct reg_obj_t                   soft_reset;
	struct reg_obj_t                   power;

#define MIR3DA_INIT_SECT_LEN    11
#define MIR3DA_OFF_SECT_LEN     MIR3DA_OFFSET_LEN
#define MIR3DA_ODR_SECT_LEN	3

	struct reg_obj_t                   init_sect[MIR3DA_INIT_SECT_LEN];
	struct reg_obj_t                   offset_sect[MIR3DA_OFF_SECT_LEN];
	struct reg_obj_t                   odr_sect[MIR3DA_ODR_SECT_LEN];

	struct gsensor_data_obj_s   data;

	int (*calibrate)(void *handle, int z_dir);
	int (*auto_calibrate)(void *handle, int xyz[3]);
	int (*int_ops)(void *handle, struct mir_int_ops_t *ops);
	int (*get_reg_data)(void *handle, char *buf);
};
...
*/
static int SOCLE_calibrate(void *handle, int z_dir);
static int SOCLE_auto_calibrate(void *handle, int xyz[3]);
static int NSA_MLM_calibrate(void *handle, int z_dir);
static int NSA_NTO_calibrate(void *handle, int z_dir);
static int NSA_MLM_auto_calibrate(void *handle, int xyz[3]);
static int NSA_NTO_auto_calibrate(void *handle, int xyz[3]);
#if MIR3DA_AUTO_CALIBRATE
static int mir3da_auto_calibrate(void *handle, int x, int y, int z);
#endif /* !MIR3DA_AUTO_CALIBRATE */
static int SOCLE_interrupt_ops(void *handle, struct mir_int_ops_t *ops);
static int NSA_interrupt_ops(void *handle, struct mir_int_ops_t *ops);
static int SOCLE_get_reg_data(void *handle, char *buf);
static int NSA_get_reg_data(void *handle, char *buf);
...
/*
static struct gsensor_obj_s	mir3da_gsensor[] = {
	MIR_SOCLE, MIR_NSA_NTO, MIR_NSA_MLM };
*/
static struct gsensor_obj_s	mir3da_gsensor[] = {
	{	"SOCLE",
		{ SOCLE_REG_WHO_AM_I, 0xFF, 0x13 },
	...
	{	"NSA_NTO",
		{ NSA_REG_WHO_AM_I,	0xFF, 0x13 },
		{ NSA_REG_FIFO_CTRL,	0xFF, 0x00 },
		{ NSA_REG_SPI_I2C,	0x24, 0x24 },
		{ NSA_REG_POWERMODE_BW, 0xC0, 0xC0 },

		{
			{ NSA_REG_G_RANGE,		0x03,	0x00	},
			{ NSA_REG_POWERMODE_BW,         0xFF,   0x1e    },
			{ NSA_REG_ODR_AXIS_DISABLE,     0xFF,   0x07    },
			{ NSA_REG_INTERRUPT_SETTINGS2,  0xFF,   0x00    },
			{ NSA_REG_INTERRUPT_MAPPING2,   0xFF,   0x00    },
...
do { \
	if (DEBUG_FUNC&Log_level) { \
		mir3da_gsensor_drv.method->myprintf(MI_TAG \
			"%s is called, line: %d\n", __func__, __LINE__); } \
} while (0)
...
struct mir3da_core_ctx {
	struct mir3da_filter_param_s    filter_param;
	struct FilterAverageContext            tFac[3];
};

static struct mir3da_core_ctx       core_ctx;
#endif
	...
	short sample)
{
	...
	if (fac->filter_param_l == fac->filter_param_h) {
		...
		if (abs(fac->sample_l - fac->sample_h) > fac->filter_threhold) {
			MI_DATA("fac->sample_l = %d, fac->sample_h = %d\n",
...
int mir3da_register_read(void *handle, short addr, unsigned char *data)
{
	...
	if (MIR3DA_REG_PAGE(addr) > 0) {
		res = mir3da_gsensor_drv.method->smi.read(handle,
			0x0, &cur_page);
		if (res != 0)
			return res;
...
int mir3da_register_read_continuously(void *handle,
	short addr, unsigned char count, unsigned char *data)
{
	unsigned char      cur_page;
	int     res = 0;
...
int mir3da_register_write(void *handle, short addr, unsigned char data)
{
	unsigned char      cur_page;
	int     res = 0;
...
int mir3da_register_mask_write(void *handle, short addr,
...
static int mir3da_read_raw_data(void *handle, short *x, short *y, short *z)
{
	unsigned char    tmp_data[6] = {0};

	if (mir3da_register_read_continuously(handle,
		mir3da_gsensor_drv.obj[gsensor_mod].data.data_sect[0].addr,
		6, tmp_data) != 0) {
		MI_ERR("i2c block read failed\n");
		return -1;
	}

	*x = ((short)(tmp_data[1] << mir3da_gsensor_drv.obj[
		gsensor_mod].data.data_fmt.msbw | tmp_data[0]))>>
		(8-mir3da_gsensor_drv.obj[gsensor_mod].data.data_fmt.lsbw);
	*y = ((short)(tmp_data[3] << mir3da_gsensor_drv.obj[
		gsensor_mod].data.data_fmt.msbw | tmp_data[2]))>>
		(8-mir3da_gsensor_drv.obj[gsensor_mod].data.data_fmt.lsbw);
	*z = ((short)(tmp_data[5] << mir3da_gsensor_drv.obj[
		gsensor_mod].data.data_fmt.msbw | tmp_data[4]))>>
		(8-mir3da_gsensor_drv.obj[gsensor_mod].data.data_fmt.lsbw);
...
static int mir3da_read_offset_from_file(unsigned char *offset)
{
	...
	MI_MSG("sensor_sync_read, offset = 0x%x,0x%x,0x%x,0x%x,0x%x,",
		offset[0], offset[1], offset[2], offset[3], offset[4]);
	MI_MSG("0x%x,0x%x,0x%x,0x%x",
...
void manual_load_cali_file(void *handle)
{
	...
	if (!bLoad) {
		...
		if (readOffsetCnt%10 == 0) {
			...
			MI_DATA("%s,bLoad = %d, readOffsetCnt=%d.\n",
				__func__, bLoad, readOffsetCnt);
...
#if MIR3DA_OFFSET_TEMP_SOLUTION
int mir3da_read_offset(void *handle, unsigned char *offset)
{
	int     i, res = 0;
...
static int mir3da_write_offset_to_file(unsigned char *offset)
{
	...
	MI_MSG("sensor_sync_write, offset = 0x%x,0x%x,0x%x,0x%x,0x%x,",
		offset[0], offset[1], offset[2], offset[3], offset[4]);
	MI_MSG("0x%x,0x%x,0x%x,0x%x",
		offset[5], offset[6], offset[7], offset[8]);
...
int check_linearity_offset(void *handle, int *step)
{
	...
	struct LinearityData       xdata[2] = {{0} };
	struct LinearityData       ydata[2] = {{0} };
	struct LinearityData       zdata[2] = {{0} };

	for (i = 10; i <= 0x3ff; i += 50) {
		result |= mir3da_register_write(handle,
			SOCLE_REG_OTP_XOFF_L, i & 0xFF);
		result |= mir3da_register_write(handle,
			SOCLE_REG_OTP_XOFF_H, (i & 0xFF00) >> 8);
		result |= mir3da_register_write(handle,
			SOCLE_REG_OTP_YOFF_L, i & 0xFF);
		result |= mir3da_register_write(handle,
			SOCLE_REG_OTP_YOFF_H, (i & 0xFF00) >> 8);
		result |= mir3da_register_write(handle,
			SOCLE_REG_OTP_ZOFF_L, i & 0xFF);
		result |= mir3da_register_write(handle,
			SOCLE_REG_OTP_ZOFF_H, (i & 0xFF00) >> 8);
		...
		if (result) {
			MI_MSG("chip op failed , result = %d\n", result,
				"detect_linearity_ratio");
			return result;
		}

		if (abs(x) < 1800 && xdata_count < 2) {
			MI_MSG("xdata_count = %d, x = %d i = %d\n",
				xdata_count, x, i,
				"detect linearity ratio");
		...
		if (xdata_count == 2 && ydata_count == 2 && zdata_count == 2) {
			...
			step[0] = (xdata[1].val - xdata[0].val)/(xdata[1].off
				- xdata[0].off);
			step[1] = (ydata[1].val - ydata[0].val)/(ydata[1].off
				- ydata[0].off);
			step[2] = (zdata[1].val - zdata[0].val)/(zdata[1].off
				- zdata[0].off);
	...
	int *fine, int fine_step)
{
	...
	*fine = 100*(off-(*coarse)*coarse_step)/fine_step;
	...
	MI_MSG("off = %d; delta_coarse = %d; delta_fine = %d",
		off, *coarse, *fine);
}

#if MIR3DA_AUTO_CALIBRATE
static int NSA_once_calibrate(void *handle, int coarse_step[3],
	int fine_step[3], int xyz[3])
{
	...
	MI_MSG("Old coarse_x = %d; coarse_y = %d; coarse_z = %d;",
		coarse[0], coarse[1], coarse[2]);
	MI_MSG("fine_x = %d; fine_y = %d; fine_z = %d;",
	...
	for (i = 0; i < 3; i++) {
		...
		mir3da_cali_off_to_lsb((xyz[i]-target[i]), &coarse_delta[i],
			coarse_step[i], &fine_delta[i], fine_step[i]);
	...
	if (cycle_read_xyz(handle, &xyz[0], &xyz[1], &xyz[2], 10))
		return -1;
	MI_MSG("---calibrate_Done, x = %d, y = %d, z = %d,",
		xyz[0], xyz[1], xyz[2]);
	MI_MSG("coarse_x = %d, coarse_y = %d, coarse_z = %d,",
		coarse[0], coarse[1], coarse[2]);
	MI_MSG("fine_x = %d, fine_y = %d, fine_z = %d",
		fine[0], fine[1], fine[2]);
...
static int NSA_calibrate(void *handle, int coarse_step[3],
	int fine_step[3], int fine_max, int target[3])
{
	...
	if ((abs(target[0]) + abs(target[1]) + abs(target[2]))
		!= 0 && (abs(target[0]) + abs(target[1]) +
		abs(target[2])) != 1024) {
	...
	for (i = 0; i < 3; i++) {
		...
		mir3da_register_mask_write(handle,
			NSA_REG_COARSE_OFFSET_TRIM_X+i, 0xc0,
...
EXIT_8G_MOD:
	...
	if (cycle_read_xyz(handle, &xyz[0], &xyz[1], &xyz[2], ncycle))
		return -1;
	MI_MSG("---Stage1, coarse tune done: x = %d, y = %d, z = %d,",
		xyz[0], xyz[1], xyz[2]);
	MI_MSG("coarse_x = %d, coarse_y = %d, coarse_z = %d,",
		coarse[0], coarse[1], coarse[2]);
	MI_MSG("fine_x = %d, fine_y = %d, fine_z = %d",
	...
	for (i = 0; i < nLoop; i++) {
		...
		for (j = 0; j < 3; j++) {
			MI_MSG("xyz[%d]=%d, caorse[%d]=0x%x, fine[%d]=0x%x",
			...
			mir3da_register_mask_write(handle,
				NSA_REG_COARSE_OFFSET_TRIM_X+j,
				0xc0, (unsigned char)(0xc0&(fine[j]>>2)));
		}
	}
	MI_MSG("---Stage2, Fine tune done: x = %d, y = %d, z = %d,",
		xyz[0], xyz[1], xyz[2]);
	MI_MSG("coarse_x = %d, coarse_y = %d, coarse_z = %d,",
		coarse[0], coarse[1], coarse[2]);
	MI_MSG("fine_x = %d, fine_y = %d, fine_z = %d",
...
static int NSA_NTO_cali_step_calc(void *handle, int coarse[3],
	int x100_fine[3], int x100_cust[3])
{
	...
	unsigned int       const coarse_gain_map[] = {
		1000, 1125, 1250, 1375, 500, 625, 750, 875};   /* *1000  */
	unsigned char      const fine_dig_gain_map[] = {1, 2, 4, 8};

	if (mir3da_register_read_continuously(handle,
		NSA_REG_SENSITIVITY_TRIM_X, 3, fine_gain) != 0) {
		MI_ERR("i2c block read failed\n");
		return -1;
	}
...
static int NSA_MLM_cali_step_calc(void *handle, int coarse[3],
	int x100_fine[3], int x100_cust[3])
{
	int                i;
	unsigned int       total_gain[3] = {0};
	...
	if (mir3da_register_read_continuously(handle,
		NSA_REG_SENSITIVITY_TRIM_X, 3, gain) != 0){
		MI_ERR("i2c block read failed\n");
		return -1;
	}
...
static int NSA_NTO_calibrate(void *handle, int z_dir)
{
	...
	/* compute step */
	if (NSA_NTO_cali_step_calc(handle, coarse_step,
		fine_step, custom_step)) {
...
static int NSA_NTO_auto_calibrate(void *handle, int xyz[3])
{
	...
	if ((swap_plarity_old & (1<<0))) {
		...
		xyz[1] =
			((swap_plarity_old & (1<<3)) != 0) ? (-temp) : temp;
	} else{
		MI_ERR("==xy no swap==\n");
		xyz[0] =
...
static int NSA_MLM_auto_calibrate(void *handle, int xyz[3])
{
	...
	/* compute step */
	if (NSA_MLM_cali_step_calc(handle, coarse_step,
		fine_step, custom_step)) {
		MI_ERR("Compute step failed !");
		return -1;
	}
...
static int SOCLE_auto_calibrate(void *handle, int xyz[3])
{
	...
	for (i = 0; i < 3; i++) {
		...
		mir3da_cali_off_to_lsb((xyz[i]-target[i]),
			&coarse_delta[i], coarse_step[i],
	...
	MI_MSG("---calibrate_Done, x = %d, y = %d, z = %d,",
		xyz[0], xyz[1], xyz[2]);
	MI_MSG("off_x = %d, off_y = %d, off_z = %d",
		coarse[0], coarse[1], coarse[2]);
...
static int SOCLE_calibrate(void *handle, int z_dir)
{
	...
	if (mir3da_register_read_continuously(handle,
		(SOCLE_REG_OTP_XOFF_L|0x80), 6, tmp_data)) {
		MI_ERR("i2c block read failed\n");
		goto fail_exit;
	}
	...
	result = mir3da_register_write(handle,
		SOCLE_REG_OTP_XOFF_L, x_off & 0xFF);
	MI_ASSERT(result == 0);
	result = mir3da_register_write(handle,
		SOCLE_REG_OTP_XOFF_H, (x_off & 0xFF00) >> 8);
	MI_ASSERT(result == 0);
	result = mir3da_register_write(handle,
		SOCLE_REG_OTP_YOFF_L, y_off & 0xFF);
	MI_ASSERT(result == 0);
	result = mir3da_register_write(handle,
		SOCLE_REG_OTP_YOFF_H, (y_off & 0xFF00) >> 8);
	MI_ASSERT(result == 0);
	result = mir3da_register_write(handle,
		SOCLE_REG_OTP_ZOFF_L, z_off & 0xFF);
	MI_ASSERT(result == 0);
	result = mir3da_register_write(handle,
		SOCLE_REG_OTP_ZOFF_H, (z_off & 0xFF00) >> 8);
	MI_ASSERT(result == 0);
...
static int mir3da_auto_cali_condition_confirm(int x, int y,
	int z, int ave_xyz[3])
{
	int    max = 0, min = 0;
	int    i;
	...
	MI_MSG("ave_x = %d, ave_y = %d, ave_z = %d",
		ave_xyz[0], ave_xyz[1], ave_xyz[2]);

	if ((abs(max-min) > STABLE_CHECK_THRESHOLD) ||
		abs(ave_xyz[0]) > AUTO_CALI_THRESHOLD_XY ||
		abs(ave_xyz[1]) > AUTO_CALI_THRESHOLD_XY ||
		abs(abs(ave_xyz[2])-1024) > AUTO_CALI_THRESHOLD_Z)
		return -1;
	return 0;
}
	...
	case INTERRUPT_OP_CONFIG:
		...
		case INTERRUPT_ACTIVITY:
			if (ops->data.cfg.pin == INTERRUPT_PIN1) {
				...
				mir3da_register_mask_write(handle,
					SOCLE_REG_CTRL_REG3, (1<<6), (1<<6));

				mir3da_register_mask_write(handle,
					SOCLE_REG_INT1_THS, 0x7f,
					ops->data.cfg.int_cfg.act.threshold);
				mir3da_register_mask_write(handle,
					SOCLE_REG_INT1_DURATION, 0x7f,
					ops->data.cfg.int_cfg.act.duration);
			} else if (ops->data.cfg.pin == INTERRUPT_PIN2) {
				/* enable this int on INT2 */
				mir3da_register_mask_write(handle,
					SOCLE_REG_CTRL_REG6, (1<<5), (1<<5));

				mir3da_register_mask_write(handle,
					SOCLE_REG_INT2_THS, 0x7f,
					ops->data.cfg.int_cfg.act.threshold);
				mir3da_register_mask_write(handle,
					SOCLE_REG_INT2_DURATION, 0x7f,
					ops->data.cfg.int_cfg.act.duration);
			}
			break;
		...
		case INTERRUPT_CLICK:
			...
			mir3da_register_mask_write(handle, SOCLE_REG_CLICK_THS,
				0x7f, ops->data.cfg.int_cfg.clk.threshold);
			mir3da_register_mask_write(handle, SOCLE_REG_TIME_LIMIT,
				0x7f, ops->data.cfg.int_cfg.clk.click_time);
			...
			if (ops->data.cfg.pin == INTERRUPT_PIN1)
				mir3da_register_mask_write(handle,
					SOCLE_REG_CTRL_REG3, (1<<7), (1<<7));

			else if (ops->data.cfg.pin == INTERRUPT_PIN2)
				mir3da_register_mask_write(handle,
					SOCLE_REG_CTRL_REG6, (1<<7), (1<<7));
...
static int NSA_interrupt_ops(void *handle, struct mir_int_ops_t *ops)
{
	...
	case INTERRUPT_OP_ENABLE:
		...
		case INTERRUPT_ACTIVITY:
			...
			mir3da_register_mask_write(handle,
				NSA_REG_INTERRUPT_SETTINGS1, 0x07, 0x07);
			break;
		case INTERRUPT_CLICK:
			/* Enable single and double tap detect */
			mir3da_register_mask_write(handle,
				NSA_REG_INTERRUPT_SETTINGS1, 0x30, 0x30);
			break;
		}
		break;
	...
	case INTERRUPT_OP_CONFIG:
		...
		case INTERRUPT_CLICK:
			mir3da_register_mask_write(handle,
				NSA_REG_TAP_THRESHOLD, 0x1f,
				ops->data.cfg.int_cfg.clk.threshold);
			mir3da_register_mask_write(handle,
				NSA_REG_TAP_DURATION, (0x03<<5)|(0x07),
			...
			if (ops->data.cfg.pin == INTERRUPT_PIN1)
				mir3da_register_mask_write(handle,
					NSA_REG_INTERRUPT_MAPPING1, 0x30, 0x30);
			else if (ops->data.cfg.pin == INTERRUPT_PIN2)
				mir3da_register_mask_write(handle,
					NSA_REG_INTERRUPT_MAPPING3, 0x30, 0x30);
			break;
		}
		break;
	...
	case INTERRUPT_OP_DISABLE:
		...
		case INTERRUPT_ACTIVITY:
			/* Enable active interrupt */
			mir3da_register_mask_write(handle,
				NSA_REG_INTERRUPT_SETTINGS1, 0x07, 0x00);
			break;

		case INTERRUPT_CLICK:
			/* Enable single and double tap detect */
			mir3da_register_mask_write(handle,
				NSA_REG_INTERRUPT_SETTINGS1, 0x30, 0x00);
			break;
		}
		break;
...
int mir3da_get_filter_param(struct mir3da_filter_param_s *param)
{
	...
	MI_MSG("filter_param_h = %d, filter_param_l = %d, filter_threhold = %d",
...
int mir3da_get_enable(void *handle, char *enable)
{
	...
	*enable = (reg_data|mir3da_gsensor_drv.obj[gsensor_mod].power.mask) ?
		1 : 0;

	return res;
}

int mir3da_set_enable(void *handle, char enable)
{
	int             res = 0;
...
static int mir3da_soft_reset(void *handle)
{
	int             res = 0;
	unsigned char   reg_data;
...
int mir3da_module_detect(void *handle)
{
	...
	for (i = 0; i < sizeof(mir3da_gsensor)/sizeof(mir3da_gsensor[0]); i++) {
		res = mir3da_register_read(handle,
			mir3da_gsensor[i].chip_id.addr, &cid);
		if (res != 0)
			return res;
		...
		if (mir3da_gsensor[i].chip_id.value == cid) {
			res = mir3da_register_read(handle,
				mir3da_gsensor[i].mod_id.addr, &mid);
			if (res != 0)
				return res;
...
int mir3da_chip_resume(void *handle)
{
	...
	for (i = 0; i < MIR3DA_INIT_SECT_LEN; i++) {
		...
		reg_data = mir3da_gsensor_drv.obj[
			gsensor_mod].init_sect[i].value;
		res = mir3da_register_mask_write(handle,
