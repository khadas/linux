...
static const unsigned int OTPReg[6] = {
	0x67, 0x68, 0x69, 0x6A, 0x6C, 0x6E};
static const unsigned int EngReg[6] = {
	0x29, 0x2D, 0x31, 0x2A, 0x2E, 0x32};
...
static int STK_i2c_Rx(char *rxData, int length)
{
	...
	if (retry > 3) {
		dprintk(KERN_ERR "%s: i2c error, retry over 3\n", __func__);
		return -EIO;
	} else
		return 0;
}
...
static int STK831X_SetVD(struct stk831x_data *stk)
{
	...
	if (result < 0) {
		dprintk(KERN_ERR "%s: read back 0x66 error, result=%d\n",
			__func__, result);
		return result;
	}

	dprintk(KERN_INFO "%s:Read 0x66 = 0x%x\n",  __func__, reg24);
	if (reg24 != 0) {
		buffer[0] = 0x24;
		buffer[1] = reg24;
		dprintk(KERN_INFO "%s:write 0x%x to 0x24\n",
			__func__, buffer[1]);
		result = STK_i2c_Tx(buffer, 2);
		if (result < 0) {
			dprintk(KERN_ERR "%s:write 0x24 failed\n", __func__);
			return result;
		}
		...
		for (i = 0; i < 6; i++) {
			...
			if (result < 0) {
				dprintk(KERN_ERR
					"%s: read back 0x%x error, result=%d\n",
			...
			buffer[0] = EngReg[i];
			buffer[1] = readvalue;
			dprintk(KERN_INFO "%s:write 0x%x to 0x%x\n",
				__func__, buffer[1], buffer[0]);
			result = STK_i2c_Tx(buffer, 2);
			if (result < 0) {
				dprintk(KERN_ERR "%s:write 0x%x failed\n",
					__func__, buffer[0]);
				return result;
			}
		}
		...
		if (result < 0) {
			dprintk(KERN_ERR
				"%s: read back 0x70 error, result=%d\n",
				__func__, result);
			return result;
		}

		dprintk(KERN_INFO "%s:Read 0x70 = 0x%x\n",  __func__, reg24);
		if (reg24 != 0) {
			buffer[0] = 0x24;
			buffer[1] = reg24;
			dprintk(KERN_INFO "%s:write 0x%x to 0x24\n",
			...
			dprintk(KERN_INFO "%s: reg24=0, do nothing\n",
				__func__);
			return 0;
		}
	}
	...
	if (result < 0) {
		dprintk(KERN_ERR "%s:Read 0x24 failed\n", __func__);
		return result;
	}
	if (buffer[0] != reg24) {
		dprintk(KERN_ERR "%s: error, reg24=0x%x, read=0x%x\n",
			__func__, reg24, buffer[0]);
		return -1;
	}
	dprintk(KERN_INFO "%s: read 0x24 = 0x%x\n", __func__, buffer[0]);
	dprintk(KERN_INFO "%s: successfully\n", __func__);
		...
		if (event_since_en >= STK_TUNE_DELAY) {
			...
			if (stk_tune_index == STK_TUNE_NUM) {
				...
				atomic_set(&stk->cali_status,
					STK_K_SUCCESS_TUNE);
				event_since_en = 0;
				dprintk(KERN_INFO "%s:TUNE done, %d,%d,%d\n",
					__func__, offset[0],
					offset[1], offset[2]);
			}
		}
...
static int STK831x_CheckReading(int acc[], bool clear)
{
	...
	if (acc[0] == 2047 || acc[0] == -2048 || acc[1] == 2047 ||
		acc[1] == -2048 || acc[2] == 2047 || acc[2] == -2048) {
		dprintk(KERN_INFO "%s: acc:%o,%o,%o\n", __func__,
			acc[0], acc[1], acc[2]);
		check_result++;
	}
	if (clear) {
		if (check_result == 3) {
			...
			dprintk(KERN_INFO "%s: incorrect reading\n", __func__);
			check_result = 0;
			return 1;
		}
		check_result = 0;
...
static int STK831x_ReadSensorData(struct stk831x_data *stk)
{
	...
	if (result < 0) {
		dprintk(KERN_ERR "%s:i2c transfer error\n", __func__);
		return result;
	}
	...
	dprintk(KERN_INFO "%s:RAW  %4d,%4d,%4d\n", __func__, acc_xyz[0],
			acc_xyz[1], acc_xyz[2]);
#endif
	...
	if (((enable == 1) && (event_since_en_limit == 20)) ||
		(k_status == STK_K_RUNNING)) {
...
#ifdef STK_DEBUG_RAWDATA
	dprintk(KERN_INFO "%s:After FIR  %4d,%4d,%4d\n", __func__, acc_xyz[0],
		acc_xyz[1], acc_xyz[2]);
#endif
...
static int STK831x_ReportValue(struct stk831x_data *stk)
{
	...
	dprintk(KERN_INFO "%s:%4d,%4d,%4d\n", __func__, stk->raw_data[0],
		stk->raw_data[1], stk->raw_data[2]);
#endif
	aml_sensor_report_acc(this_client, stk->input_dev,
...
#ifdef STK_DEBUG_PRINT
#ifdef STK831X_HOLD_ODR
	dprintk(KERN_INFO "%s:sdelay_us=%d, Hold delay = %d\n",
		__func__, sdelay_us, STK831X_SAMPLE_TIME[STK831X_INIT_ODR]);
#else
	dprintk(KERN_INFO "%s:sdelay_us=%d\n", __func__, sdelay_us);
#endif
#endif
	mutex_lock(&stk->write_lock);
	if (stk->delay == sr_no) {
...
static int STK831x_SetRange(char srange)
{
	...
	char buffer[2] = "";
#ifdef STK_DEBUG_PRINT
	dprintk(KERN_INFO "%s:range=0x%x\n", __func__, srange);
#endif

	if (srange >= 3) {
		dprintk(KERN_ERR "%s:parameter out of range\n", __func__);
		return -1;
	}
...
static int STK831x_ReadByteOTP(char rReg, char *value)
{
	...
	*value = 0;
	...
	if (redo == 10) {
		dprintk(KERN_ERR "%s:OTP read repeat read 10 times! Failed!\n",
			__func__);
	...
	*value = buffer[0];
	...
	dprintk(KERN_INFO "%s: read 0x%x=0x%x\n", __func__, rReg, *value);
#endif
	return 0;
...
static int STK831x_WriteByteOTP(char wReg, char value)
{
	...
	do {
		...
		result = STK_i2c_Tx(buffer, 2);
		if (result < 0) {
			dprintk(KERN_ERR "%s:failed, err=0x%x\n", __func__,
				result);
			goto eng_i2c_w_err;
		}
		buffer[0] = 0x3E;
	...
	char offsetData[])
{
	...
	if (reg_comp[0] == regR[0] && reg_comp[1] == regR[1] &&
		reg_comp[2] == regR[2]) {
		dprintk(KERN_INFO "%s: ft pre-trimmed\n", __func__);
		ft_pre_trim = 1;
	}
...
#ifdef STK_DEBUG_CALI
	dprintk(KERN_INFO "%s:OTP step1 Success!\n", __func__);
#endif
	buffer[0] = 0x2A;
	result = STK_i2c_Rx(buffer, 1);
	if (result < 0)
...
#ifdef STK_DEBUG_CALI
	dprintk(KERN_INFO "%s:OTP step2 Success!\n", __func__);
#endif
	result = STK831x_ReadByteOTP(0x7F, &regR[0]);
	if (result < 0)
		goto eng_i2c_err;
	...
	if (result < 0)
		goto eng_i2c_err;
#ifdef STK_DEBUG_CALI
	dprintk(KERN_INFO "%s:OTP step3 Success!\n", __func__);
#endif
	return 0;

eng_i2c_err:
	dprintk(KERN_ERR "%s: read/write eng i2c error, result=0x%x\n",
		__func__, result);
	return result;

common_i2c_error:
	dprintk(KERN_ERR "%s: read/write common i2c error, result=0x%x\n",
		__func__, result);
	return result;
}
	...
	uint32_t delay_ms)
{
	...
	if (en_dis) {
		...
		result = STK_i2c_Rx(buffer, 1);
		if (result < 0) {
			dprintk(KERN_ERR "%s:failed, result=0x%x\n",
				__func__, result);
	...
	case NEGATIVE_Z_UP:
		...
		break;
	default:
		dprintk("%s: invalid stk831x_placement=%d\n",
	...
	if (abs(acc_ave[0]) > verify_diff || abs(acc_ave[1]) > verify_diff ||
		...
		dprintk(KERN_INFO "%s:Check data x:%d, y:%d, z:%d\n",
			__func__, acc_ave[0], acc_ave[1], acc_ave[2]);
		dprintk(KERN_ERR "%s:Check Fail, Calibration Fail\n", __func__);
		ret = -STK_K_FAIL_LRG_DIFF;
	}
#ifdef STK_DEBUG_CALI
	else
		dprintk(KERN_INFO "%s:Check data pass\n", __func__);
#endif
	if (en_dis) {
		buffer[0] = STK831X_MODE;
		result = STK_i2c_Rx(buffer, 1);
...
static int STK831x_SetCali(struct stk831x_data *stk, char sstate)
{
	...
	dprintk(KERN_INFO "%s: real_delay_ms =%d ms\n",
...
#ifdef STK_DEBUG_CALI
		dprintk(KERN_INFO "%s:before cali, acc=%d,%d,%d\n", __func__,
			stk->raw_data[0], stk->raw_data[1], stk->raw_data[2]);
#endif
	...
	if (acc_ave[2] <= -1)
		...
		dprintk(KERN_INFO
			"%s:acc_ave[2]=0, use default stk831x_placement\n",
			__func__);
#ifdef STK_DEBUG_CALI
	dprintk(KERN_INFO
		"%s:stk831x_placement=%d\n", __func__, stk831x_placement);
#endif
	acc_ave[2] = 0;
	if (store_location >= 2) {
		buffer[0] = 0x2B;
	...
	for (axis = 0; axis < 3; axis++) {
		...
		if (stk_driver_offset[axis] < -STK8313_MAX_DRIVER_OFFSET ||
			stk_driver_offset[axis] > STK8313_MAX_DRIVER_OFFSET)
			atomic_set(&stk->cali_status, STK_K_FAIL_OUT_RG);

	}

	dprintk(KERN_INFO "%s: stk_driver_offset:%d,%d,%d\n",
		__func__, stk_driver_offset[0],
		stk_driver_offset[1], stk_driver_offset[2]);
	if (atomic_read(&stk->cali_status) == STK_K_FAIL_OUT_RG)	{
		dprintk(KERN_ERR "%s: offset is too large\n", __func__);
		stk_driver_offset[0] = stk_driver_offset[1] =
			stk_driver_offset[2] = 0;
		if (org_enable)
	...
	if (store_location == 1) {
		...
		for (axis = 0; axis < 3; axis++) {
			...
			if ((char_offset[axis]>>7) == 0) {
				if (char_offset[axis] >= 0x20) {
					dprintk(KERN_ERR
						"offset[%d]=0x%x is too large,",
						axis, char_offset[axis]);
					dprintk(KERN_ERR "limit to 0x1f\n");
					char_offset[axis] = 0x1F;
				}
			} else {
				if (char_offset[axis] <= 0xDF) {
					dprintk(KERN_ERR "offset[%d]=0x%x",
						axis, char_offset[axis]);
					dprintk("is too large,limit to 0x20\n");
		...
		dprintk(KERN_INFO "%s: OTP offset:0x%x,0x%x,0x%x\n", __func__,
			char_offset[0], char_offset[1], char_offset[2]);
		if (store_location == 2) {
			result = stk_store_in_ic(stk, char_offset, 1,
				real_delay_ms);
			if (result == 0) {
				dprintk(KERN_INFO "%s successfully\n",
					__func__);
				...
				dprintk(KERN_ERR "%s fail, result=%d\n",
					__func__, result);
...
err_i2c_rw:
	...
	if (org_enable)
		STK831x_SetEnable(stk, 1);
	dprintk(KERN_ERR "%s: i2c read/write error, err=0x%x\n",
		__func__, result);
...
static int STK831x_GetCali(struct stk831x_data *stk)
{
	...
	dprintk(KERN_INFO "%s: driver offset:%d,%d,%d\n",
		__func__, stk_driver_offset[0],
		stk_driver_offset[1],
		stk_driver_offset[2]);
#ifdef STK_TUNE
	dprintk(KERN_INFO
		"stk_tune_done=%d,stk_tune_index=%d,stk_tune_offset=%d,%d,%d\n",
		stk_tune_done, stk_tune_index, stk_tune_offset_record[0],
		stk_tune_offset_record[1],
	...
	if ((stk_get_file_content(r_buf, STK_ACC_CALI_FILE_SIZE)) == 0) {
		if (r_buf[0] == STK_ACC_CALI_VER0 && r_buf[1] ==
			STK_ACC_CALI_VER1) {
			if (r_buf[2] & 0x80)
				i_offset[0] = ((int)r_buf[2]<<4) +
					(r_buf[3]>>4) - 4096;
			else
				i_offset[0] = ((int)r_buf[2]<<4) +
					(r_buf[3]>>4);
			if (r_buf[4] & 0x80)
				i_offset[1] = ((int)r_buf[4]<<4) +
					(r_buf[5]>>4) - 4096;
			else
				i_offset[1] = ((int)r_buf[4]<<4) +
					(r_buf[5]>>4);
			if (r_buf[6] & 0x80)
				i_offset[2] = ((int)r_buf[6]<<4) +
					(r_buf[7]>>4) - 4096;
			else
				i_offset[2] = ((int)r_buf[6]<<4) +
					(r_buf[7]>>4);
			...
			dprintk(KERN_ERR "r_buf=0x%x,0x%x,0x%x,0x%x,0x%x,",
				r_buf[0], r_buf[1], r_buf[2],
				r_buf[3], r_buf[4]);

			dprintk(KERN_ERR "0x%x,0x%x,0x%x,0x%x\n",
				r_buf[5], r_buf[6], r_buf[7], r_buf[8]);
		}
	} else
		dprintk(KERN_INFO "%s: No file offset\n", __func__);
...
static int STK831x_Init(struct stk831x_data *stk, struct i2c_client *client)
{
	...
	dprintk(KERN_INFO "%s: Initialize stk8313\n", __func__);
	...
	result = STK_i2c_Tx(buffer, 2);
	if (result < 0) {
		dprintk(KERN_ERR "%s:interrupt init failed\n", __func__);
		return result;
	}
#endif
	...
	result = STK_i2c_Tx(buffer, 2);
	if (result < 0) {
		dprintk(KERN_ERR "%s:set range failed\n", __func__);
		return result;
	}
...
static void stk_handle_first_en(struct stk831x_data *stk)
{
	...
	if ((stk_get_file_content(r_buf, STK_ACC_CALI_FILE_SIZE)) == 0) {
		if (r_buf[0] == STK_ACC_CALI_VER0 && r_buf[1] ==
			...
			if (r_buf[2] & 0x80)
				...
				stk_driver_offset[0] =
					((int)r_buf[2]<<4) + (r_buf[3]>>4);
			if (r_buf[4] & 0x80)
				stk_driver_offset[1] = ((int)r_buf[4]<<4) +
					(r_buf[5]>>4) - 4096;
			else
				stk_driver_offset[1] = ((int)r_buf[4]<<4) +
					(r_buf[5]>>4);
			if (r_buf[6] & 0x80)
				stk_driver_offset[2] = ((int)r_buf[6]<<4) +
					(r_buf[7]>>4) - 4096;
			else
				stk_driver_offset[2] = ((int)r_buf[6]<<4) +
					(r_buf[7]>>4);
		...
		dprintk(KERN_INFO "%s: set offset:%d,%d,%d\n", __func__,
			stk_tune_offset_record[0],
	...
	dprintk(KERN_INFO "%s: finish, cali_status = 0x%x\n",
...
static int32_t stk_get_ic_content(struct stk831x_data *stk)
{
	...
	if (regR&0x20) {
		...
		dprintk(KERN_INFO "%s: OTP 2 used\n", __func__);
		return 2;
	}
	if (regR&0x10) {
		if (regR == 0x1b) {
			dprintk(KERN_INFO "%s: b2 chip\n", __func__);
			return 0;
		}
		atomic_set(&stk->cali_status, STK_K_SUCCESS_FT1);
		dprintk(KERN_INFO "%s: OTP 1 used\n", __func__);
		return 1;
	}
	return 0;
}
	...
	char FT_index, uint32_t delay_ms)
{
...
#ifdef STK_DEBUG_CALI
	/*dprintk(KERN_INFO "%s:Check All OTP Data after write
		0x2B 0x2F 0x33\n", __func__);*/
	/*STK831x_ReadAllOTP();*/
#endif
	...
	if (result < 0) {
		dprintk(KERN_INFO "%s: write OTP%d fail\n",
			__func__, FT_index);
		goto ic_err_misc;
	}
	...
	if (result) {
		dprintk(KERN_ERR "%s: calibration check2 fail\n", __func__);
		goto ic_err_misc;
	}
	return 0;
...
static int32_t stk_get_file_content(char *r_buf, int8_t buf_size)
{
	...
	if (IS_ERR(cali_file)) {
		dprintk(KERN_ERR "%s: filp_open error, no offset file!\n",
			__func__);
		return -ENOENT;
	} else {
		fs = get_fs();
...
static int stk_permission_thread(void *data)
{
	...
	do {
		...
		/*if(ret < 0)
			dprintk("fail to execute sys_fchmodat,
				ret = %d\n", ret);
		*/
		if (retry++ > 10)
			break;
	...
	set_fs(fs);
	dprintk(KERN_INFO "%s exit, retry=%d\n", __func__, retry);
	return 0;
}
...
static int stk_store_in_file(char offset[], char mode)
{
	...
	if (IS_ERR(cali_file)) {
		dprintk(KERN_ERR "%s: filp_open error!\n", __func__);
		...
		if (ret != STK_ACC_CALI_FILE_SIZE) {
			dprintk(KERN_ERR "%s: write error!\n", __func__);
			filp_close(cali_file, NULL);
		...
		ret = cali_file->f_op->read(cali_file, r_buf,
			STK_ACC_CALI_FILE_SIZE, &cali_file->f_pos);
		if (ret < 0) {
			dprintk(KERN_ERR "%s: read error!\n", __func__);
			filp_close(cali_file, NULL);
			return -STK_K_FAIL_R_BACK;
		}
		set_fs(fs);

		/*dprintk(KERN_INFO "%s: read ret=%d!\n", __func__, ret);*/
	...
	struct stk831x_data *stk_int)
{
	...
	if (gpio_request(STK_INT_PIN, "EINT")) {
		dprintk(KERN_ERR "%s:gpio_request() failed\n", __func__);
		return -1;
	}
	...
	if (irq < 0) {
		dprintk(KERN_ERR "%s:gpio_to_irq() failed\n", __func__);
		return -1;
	}
	client->irq = irq;
	stk_int->irq = irq;
#endif
	dprintk(KERN_INFO "%s: irq # = %d\n", __func__, irq);
	if (irq < 0)
		dprintk(KERN_ERR "%s: irq number was not specified!\n",
			__func__);
	error = request_irq(client->irq, stk_mems_irq_handler,
		IRQF_TRIGGER_RISING , "stk-mems", stk_int);
	if (error < 0) {
		dprintk(KERN_ERR "%s: request_irq(%d) failed for (%d)\n",
		...
		const char *buf, size_t count)
{
	...
	if ((data == 0) || (data == 1))
		...
		dprintk(KERN_ERR "%s: invalud argument, data=%ld\n",
			__func__, data);
	return count;
}
		...
		struct device_attribute *attr, char *buf)
{
	...
	dprintk(KERN_INFO "driver version:%s\n", STK_ACC_DRIVER_VERSION);
	STK831x_ReadSensorData(stk);
		...
		const char *buf, size_t count)
{
	...
	dprintk(KERN_INFO "%s: reg[0x%x]=0x%x\n", __func__, w_reg[0], w_reg[1]);
	buffer[0] = w_reg[0];
	buffer[1] = w_reg[1];
		...
		struct device_attribute *attr, char *buf)
{
	...
	for (bb = 0; bb < 4; bb++) {
		...
		for (aa = 0; aa < 16; aa++) {
			no = bb*0x10+aa;
			dprintk(KERN_INFO "stk reg[0x%x]=0x%x\n",
				no, buffer[aa]);
			switch (no) {
			case 0x0:
			case 0x1:
	...
	num += scnprintf(buf+num, PAGE_SIZE,
		"INTSU=%02x,MODE=%02x,SR=%02x,OFSX=%02x,OFSY=%02x,OFSZ=%02x,",
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (atomic_read(&stk->firlength)) {
		dprintk(KERN_INFO "len = %2d, idx = %2d\n",
			stk->fir.num, stk->fir.idx);
		dprintk(KERN_INFO "sum = [%5d %5d %5d]\n",
			stk->fir.sum[0], stk->fir.sum[1], stk->fir.sum[2]);
		dprintk(KERN_INFO "avg = [%5d %5d %5d]\n",
		...
		const char *buf, size_t count)
{
	...
	error = kstrtoul(buf, 10, &data);
	if (error) {
		dprintk(KERN_ERR "%s: kstrtoul failed, error=%d\n",
			__func__, error);
		return error;
	}

	if (data > MAX_FIR_LEN)
		dprintk(KERN_ERR "%s: firlen exceed maximum filter length\n",
			__func__);
	else if (data < 1) {
...
static struct attribute_group stk831x_attribute_group = {
	/*
	#if (!defined(STK_ALLWINNER_PLATFORM) &&
		!defined(STK_INFOTMIC_PLATFORM))
		.name = "driver",
	#endif
	*/
	...
	const struct i2c_device_id *id)
{
	...
	dprintk(KERN_INFO "stk831x_probe: driver version:%s\n",
		STK_ACC_DRIVER_VERSION);
	if (!i2c_check_functionality(client->adapter, I2C_FUNC_I2C)) {
		dprintk(KERN_ERR "%s:i2c_check_functionality error\n",
			__func__);
		error = -ENODEV;
	...
	if (!stk) {
		dprintk(KERN_ERR "%s:memory allocation error\n", __func__);
		error = -ENOMEM;
	...
	stk->acc_poll_delay =
		ns_to_ktime(STK831X_SAMPLE_TIME[STK831X_INIT_ODR] *
		USEC_PER_MSEC);
	...
	if (stk_mems_work_queue)
		...
		dprintk(KERN_ERR "%s:create_workqueue error\n", __func__);
		error = -EPERM;
	...
	if (error) {
		dprintk(KERN_ERR "%s:stk831x initialization failed\n",
			__func__);
	...
	if (!stk->input_dev) {
		error = -ENOMEM;
		dprintk(KERN_ERR "%s:input_allocate_device failed\n",
			__func__);
	...
	if (error) {
		dprintk(KERN_ERR "%s:Unable to register input device: %s\n",
	...
	if (error) {
		dprintk(KERN_ERR "%s: misc_register failed\n", __func__);
