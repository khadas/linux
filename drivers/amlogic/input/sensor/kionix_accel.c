...
/* Timeout (miliseconds) */
#define KIONIX_ACCEL_EARLYSUSPEND_TIMEOUT	500
...
static int kionix_i2c_read(struct i2c_client *client, u8 addr,
		u8 *data, int len)
{
	struct i2c_msg msgs[] = {
		{
...
static int kionix_strtok(const char *buf, size_t count,
				char **token, const int token_nr)
{
	...
	/* We need to breakup the string into separate chunks
	 *in order for kstrtoint
	 * or strict_strtol to parse them without returning an error.
	...
	while ((start < end) && (i < token_nr)) {
		...
		if (*(buf2 + start) == '-') {
			/* Previous char(s) are numeric, so we store their
			*value first before proceed */
			if (num_nr > 0) {
	...
	if (atomic_read(&acceld->accel_enabled) > 0) {
		if (atomic_read(&acceld->accel_enable_resume) > 0) {
			...
			if (err < 0) {
				...
				x = ((s16) le16_to_cpu(((s16)(accel_data[acceld
					->axis_map_x] >> 2)) - 32)) << 6;
				y = ((s16) le16_to_cpu(((s16)(accel_data[acceld
					->axis_map_y] >> 2)) - 32)) << 6;
				z = ((s16) le16_to_cpu(((s16)(accel_data[acceld
					->axis_map_z] >> 2)) - 32)) << 6;
				...
				if (atomic_read(&acceld->accel_input_event)
					> 0) {
					aml_sensor_report_acc(acceld->client,
						acceld->input_dev, x, y, z);
#if 0
					input_report_abs(acceld->input_dev,
						ABS_X, acceld->accel_data[acceld
						->axis_map_x]);
					input_report_abs(acceld->input_dev,
						ABS_Y, acceld->accel_data[acceld
						->axis_map_y]);
					input_report_abs(acceld->input_dev,
						ABS_Z, acceld->accel_data[acceld
						->axis_map_z]);
						...
						unsigned int poll_interval)
{
	...
	/* Do not need to update CTRL_REG1 register if the sensor
		is not currently turn on */
	if (atomic_read(&acceld->accel_enabled) > 0) {
		err = i2c_smbus_write_byte_data(acceld->client,
			ACCEL_GRP1_CTRL_REG1,
			acceld->accel_registers[accel_grp1_ctrl_reg1]
	...
	if (atomic_read(&acceld->accel_enabled) > 0) {
		if (atomic_read(&acceld->accel_enable_resume) > 0) {
			...
			while (loop) {
				...
				err = kionix_i2c_read(acceld->client,
					ACCEL_GRP2_XOUT_L,
					(u8 *)accel_data.accel_data_s16, 6);
				/* mutex_unlock(&input_dev->mutex); */
				if (err < 0) {
					loop--;
					mdelay(KIONIX_I2C_RETRY_TIMEOUT);
			...
			if (err < 0) {
				...
				x = ((s16) le16_to_cpu(accel_data.accel_data_s16
					[acceld->axis_map_x])) >> acceld->shift;
				y = ((s16) le16_to_cpu(accel_data.accel_data_s16
					[acceld->axis_map_y])) >> acceld->shift;
				z = ((s16) le16_to_cpu(accel_data.accel_data_s16
						...
						unsigned int poll_interval)
{
	...
	/* Do not need to update DATA_CTRL_REG register if the ODR
		is not changed */
	...
	/* Do not need to update DATA_CTRL_REG register if the
		sensor is not currently turn on */
	if (atomic_read(&acceld->accel_enabled) > 0) {
		err = i2c_smbus_write_byte_data(acceld->client,
			ACCEL_GRP2_CTRL_REG1, 0);
		if (err < 0)
						...
						unsigned int poll_interval)
{
	...
	/* Do not need to update DATA_CTRL_REG register if the sensor
		is not currently turn on */
	if (atomic_read(&acceld->accel_enabled) > 0) {
		err = i2c_smbus_write_byte_data(acceld->client,
			ACCEL_GRP4_CTRL_REG1, 0);
...
static void kionix_accel_work(struct work_struct *work)
{
	...
	if (acceld->accel_drdy == 0)
		queue_delayed_work(acceld->accel_workqueue,
			&acceld->accel_work, acceld->poll_delay);
...
static int kionix_accel_disable(struct kionix_accel_driver *acceld)
{
	...
	if (atomic_read(&acceld->accel_enabled) > 0) {
		if (atomic_dec_and_test(&acceld->accel_enabled)) {
			...
			if (err < 0) {
				KMSGERR(&acceld->client->dev,
					"%s: accel_standby returned err = %d\n",
					__func__, err);
				goto exit;
			}
		...
		const char *buf, size_t count)
{
	...
	if (kionix_strtok(buf, count, &buf2, enable_count) < 0) {
		KMSGERR(&acceld->client->dev,
			"%s:Noenable data read. No enable data will updated.\n",
			__func__);
	} else {
		/* Removes any leading negative sign */
		while (*buf2 == '-')
			buf2++;
		err = kstrtouint((const char *)buf2, 10,
			(unsigned int *)&enable);
		if (err < 0) {
			KMSGERR(&acceld->client->dev,
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (kionix_strtok(buf, count, &buf2, delay_count) < 0) {
		KMSGERR(&acceld->client->dev,
			"No delay data be read. No delay data will updated.\n");
	}
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (kionix_strtok(buf, count, &buf2, direct_count) < 0) {
		KMSGERR(&acceld->client->dev,
			"No direct data read. No direct data will updated.\n");
	}
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (kionix_strtok(buf, count, buf2, cali_count) < 0) {
		KMSGERR(&acceld->client->dev,
			"No enough cali data read. No cali data will update\n");
	} else {
...
static DEVICE_ATTR(enable, S_IRUGO|S_IWUSR|S_IWGRP, kionix_accel_get_enable,
	kionix_accel_set_enable);
static DEVICE_ATTR(delay, S_IRUGO|S_IWUSR|S_IWGRP, kionix_accel_get_delay,
	kionix_accel_set_delay);
/* static DEVICE_ATTR(direct, S_IRUGO|S_IWUSR|S_IWGRP, kionix_accel_get_direct,
	kionix_accel_set_direct); */
static DEVICE_ATTR(data, S_IRUGO, kionix_accel_get_data, NULL);
static DEVICE_ATTR(cali, S_IRUGO|S_IWUSR|S_IWGRP, kionix_accel_get_cali,
	...
	if (atomic_read(&acceld->accel_suspend_continue) > 0) {
		...
		if (atomic_read(&acceld->accel_enabled) > 0) {
			...
			remaining = wait_event_interruptible_timeout(
				acceld->wqh_suspend, atomic_read(&acceld
				->accel_enabled) < 1, msecs_to_jiffies(
...
{
	struct kionix_accel_driver *acceld = container_of(h,
		struct kionix_accel_driver, early_suspend);
	int err;
	...
	if (atomic_read(&acceld->accel_suspended) == 1) {
		...
		if (err) {
			KMSGERR(&acceld->client->dev,
				"kionix_accel_power_on_init return err = %d\n",
				 err);
			goto exit;
		}
		...
		const struct i2c_device_id *id)
{
	...
	case KIONIX_ACCEL_WHO_AM_I_KXTE9:
		...
		if (acceld->accel_registers == NULL) {
			KMSGERR(&client->dev,
				"fail alloc memory for accel_registers\n");
			goto err_accel_pdata_exit;
		}
		acceld->accel_drdy = 0;
	...
	case KIONIX_ACCEL_WHO_AM_I_KXTJ9_1005:
		...
		if (acceld->accel_registers == NULL) {
			KMSGERR(&client->dev,
				"failed to alloc memory for accel_registers\n");
			goto err_accel_pdata_exit;
		}
	...
	case KIONIX_ACCEL_WHO_AM_I_KXCJK_1013:
		...
		if (acceld->accel_registers == NULL) {
			KMSGERR(&client->dev,
				"fail to alloc memory for accel_registers\n");
			goto err_accel_pdata_exit;
		}
		switch (acceld->accel_pdata.accel_res) {
		case KIONIX_ACCEL_RES_6BIT:
	...
	err = sysfs_create_group(&acceld->input_dev->dev.kobj,
	...
	if (err < 0)
		KMSGERR(&acceld->client->dev,
			"kionix_accel enable failed in driver !\n");
	else
		KMSGERR(&acceld->client->dev,
			"kionix_accel enable successful in driver!\n");
