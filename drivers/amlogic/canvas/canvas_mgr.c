...
static int canvas_pool_map_alloc_canvas_in(struct canvas_pool *pool, const char *owner)
{
    int i;
    int index = -1;
    ...
    do {
        i = find_next_zero_bit(pool->canvas_map, sizeof(pool->canvas_map), start_index);
        if (!VALID_CANVAS(i)) {
            if (!looped) {
                start_index = 1;
                looped = 1;
...
*canvas = 0x04030201;
*/
static int canvas_pool_map_alloc_canvas_continue_set(struct canvas_pool *pool, const char *owner, u32 *canvas, canvas_map_type_t type)
{
    int found_retry = 0;
    int found  = 0;
    ...
    do {
        ...
        switch (type) {
        case CANVAS_MAP_TYPE_4:
            found = (canvas1 > 0) && (canvas2 == canvas1 + 1) && (canvas3 == canvas2 + 1) && (canvas4 == canvas3 + 1);
            canvasval = canvas1 | (canvas2 << 8) | (canvas3 << 16) | (canvas4 << 24);
            break;
        case CANVAS_MAP_TYPE_YUV:
            found = (canvas2 > 0) && (canvas3 == canvas2 + 1) && (canvas4 == canvas3 + 1);
...
static int canvas_pool_map_free_canvas_in(struct canvas_pool *pool, int index)
{
    ...
    if (!test_and_clear_bit(index, pool->canvas_map)) {
        printk(KERN_WARNING "canvas is free and freed again! index =%d\n", index);
    }
...
static void canvas_pool_init(void)
{
    ...
    if (get_meson_cpu_version(MESON_CPU_VERSION_LVL_MAJOR) < MESON_CPU_MAJOR_ID_M6TV) {
        pool->canvas_max = 192;
    }
    pool->next_alloced_index = pool->canvas_max / 2; /* /start at end part index. */
    return;
}


int canvas_pool_register_const_canvas(int start_index, int end, const char *owner)
{
    int i;
    struct canvas_pool *pool = get_canvas_pool();
    for (i = 0; i < end - start_index + 1; i++) {
        ...
        if (test_and_set_bit(index, pool->canvas_map)) {
            printk(KERN_WARNING "canvas have be used inddex=%d,old owner =%s,new owner =%s\n", index, pool->info[index].owner, owner);
        }
        pool->info[index].owner = owner;
...
void canvas_pool_dump_canvas_info(void)
{
    ...
    for (i = 0; i < pool->canvas_max; i++) {
        ...
        if (ret == 0 && test_bit(i, pool->canvas_map)) {
            ...
            printk("canvas[%x] used=%d owner =%s, oldowner =%s,time=%ld,fixed=%d\n", (unsigned int)i, test_bit(i, pool->canvas_map), o1, o2, info.alloc_time, info.fixed_onwer);
        }
    }
}

u32 canvas_pool_alloc_canvas_table(const char *owner, u32 *tab, int size, canvas_map_type_t type)
{
    int i;
    struct canvas_pool *pool = get_canvas_pool();
    int err = 0;
    ...
    for (i = 0; i < size; i++) {
        u32 canvas = 0;
        err = canvas_pool_map_alloc_canvas_continue_set(pool, owner, &canvas, type); /*type equal num*/
        if (err) {
            break;
        }
        tab[i] = canvas;
    ...
    if (err) {
        printk(KERN_WARNING "%s: alloc canvas failed,on i=%d,size=%d\n", owner, i, size);
        if (i > 0) {
...
u32 canvas_pool_free_canvas_table(u32 *tab, int size)
{
    ...
    for (i = 0; i < size; i++) {
        ...
        if (canvas4 >= 0 && canvas4 != canvas3 && canvas4 != canvas2 && canvas4 != canvas1) {
            canvas_pool_map_free_canvas(canvas4);
        }
    }
...
u32 canvas_pool_dump_canvas_table(u32 *tab, int size)
{
    ...
    for (i = 0; i < size; i++) {
        ...
        canvas2 = (tab[i] >> 8) & 0xff;
        canvas3 = (tab[i] >> 16) & 0xff;
        printk("0x%08x:Y=%x,U=%x,V=%x\n", tab[i], (u32)canvas1, (u32)canvas2, (u32)canvas3);
    }
    return 0;
}
...
static ssize_t canvas_pool_map_show(struct class *class, struct class_attribute *attr, char *buf)
{
    struct canvas_pool *pool = get_canvas_pool();
    ...
    if (jiffies - pool->last_cat_map > 5 * HZ) {
        pool->next_dump_index = 0; /*timeout :not continue dump,dump from first.*/
    }

    for (i = pool->next_dump_index ; i < max && size < PAGE_SIZE - 256; i++) {
        const char *o1, *o2;
        ret = canvas_pool_get_canvas_info(i, &info);
        canvas_read(i, &canvas);
        o1 = info.owner ? info.owner : "none";
        o2 = info.oldowner ? info.oldowner : "none";
        size += snprintf(buf + size, PAGE_SIZE - size, "Canvas[%02x],used=%d,fixed=%d\towner=%s,\toldowner=%s,\ttime=%ld,\taddr=%08x,\tsize =%d X %d,\twrap=%d,\tblkmod=%d\n",
                         (unsigned int)i,
                         test_bit(i, pool->canvas_map),
                         info.fixed_onwer,
                         o1, o2, info.alloc_time,
...
static ssize_t canvas_pool_states_show(struct class *class, struct class_attribute *attr, char *buf)
{
    ssize_t size = 0;
    int i;
    struct canvas_pool *pool = get_canvas_pool();
    size += snprintf(buf + size, PAGE_SIZE - size, "canvas tatol num: %d\n", pool->canvas_max);
    size += snprintf(buf + size, PAGE_SIZE - size, "canvas next_dump_index: %d\n", pool->next_dump_index);
    size += snprintf(buf + size, PAGE_SIZE - size, "canvas next_alloced_index: %d\n", pool->next_alloced_index);
    size += snprintf(buf + size, PAGE_SIZE - size, "canvas map:\n");
    for (i = 0; i < sizeof(pool->canvas_map) / (8 * sizeof(u32)); i++) {
        size += snprintf(buf + size, PAGE_SIZE - size, "% 3d - % 3d : %08x\n", i * 32, (i + 1) * 32 - 1, (u32)pool->canvas_map[i]);
    }
    return size;
}

static ssize_t canvas_pool_debug_show(struct class *class, struct class_attribute *attr, char *buf)
{
    ssize_t size = 0;
    size += snprintf(buf + size, PAGE_SIZE - size, "help: echo n > canvas_pool_debug\n");
    size += snprintf(buf + size, PAGE_SIZE - size, "1:next_dump_index=0\n");
    size += snprintf(buf + size, PAGE_SIZE - size, "2:next_alloced_index=0\n");
    return size;
}
static ssize_t canvas_pool_debug_store(struct class *class, struct class_attribute *attr, const char *buf, size_t size)
{
    unsigned val;
    ssize_t ret = 0;
    struct canvas_pool *pool = get_canvas_pool();
...
static struct class_attribute canvas_class_attrs[] = {
    ...
    __ATTR(canvas_pool_debug, S_IRUGO | S_IWUSR | S_IWGRP, canvas_pool_debug_show, canvas_pool_debug_store),
    __ATTR_NULL
};
...
int canvas_pool_get_static_canvas_by_name(const char *owner, u8 *tab, int size)
{
    ...
    for (i = 0; i < pool->canvas_max && j < size; i++) {
        info = &pool->info[i];
        if (test_bit(i, pool->canvas_map) && info->fixed_onwer && !strcmp(info->owner, owner)) {
            tab[j] = i;
            j++;
        }
    }
    if (j > 0) {
        ...
        printk(KERN_INFO "not found register static canvas for %s\n", owner);
        return 0;
    }
}
