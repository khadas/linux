...
#define pr_dbg(fmt, args...) pr_info(KERN_DEBUG "Canvas: " fmt, ## args)
#define pr_error(fmt, args...) pr_info(KERN_ERR "Canvas: " fmt, ## args)
...
void
canvas_config(u32 index, ulong addr, u32 width, u32 height, u32 wrap,
	      u32 blkmode)
{
	ulong flags;
	ulong fiq_flag;
	struct canvas_device_info *info = &canvas_info;
	struct canvas_s *canvasP = &canvas_info.canvasPool[index];

	if (!CANVAS_VALID(index))
		return;
	if (!canvas_pool_canvas_alloced(index)) {
		pr_info(KERN_ERR "!!canvas_config without alloc,index=%d\n",
			index);
		dump_stack();
		return;
	}
	...
	if (is_meson_m8m2_cpu()) {
		...
		canvas_io_write(info->reg_base + DC_CAV_LUT_DATAH_M8M2,
				((((width + 7) >> 3) >>
				CANVAS_WIDTH_LWID) <<
				CANVAS_WIDTH_HBIT) | ((height &
				CANVAS_HEIGHT_MASK) <<
				CANVAS_HEIGHT_BIT) |
				((wrap & CANVAS_XWRAP) ? CANVAS_XWRAP : 0) |
				((wrap & CANVAS_YWRAP) ? CANVAS_YWRAP : 0) |
				((blkmode & CANVAS_BLKMODE_MASK) <<
				 CANVAS_BLKMODE_BIT));

		canvas_io_write(info->reg_base + DC_CAV_LUT_ADDR_M8M2,
		...
		canvas_io_write(info->reg_base + DC_CAV_LUT_ADDR,
				CANVAS_LUT_WR_EN | index);
...
void canvas_copy(u32 src, u32 dst)
{
	...
	if (!canvas_pool_canvas_alloced(src)
		...
		pr_info(KERN_ERR "!!canvas_copy  without alloc,src=%d,dst=%d\n",
			src, dst);
		dump_stack();
		return;
	}
	...
	if (is_meson_m8m2_cpu()) {
		...
		canvas_io_write(info->reg_base + DC_CAV_LUT_DATAL,
				...
				((((width + 7) >> 3) & CANVAS_WIDTH_LMASK) <<
				 CANVAS_WIDTH_LBIT));

		canvas_io_write(info->reg_base + DC_CAV_LUT_DATAH,
				((((width + 7) >> 3) >> CANVAS_WIDTH_LWID) <<
				 CANVAS_WIDTH_HBIT) |
				((height & CANVAS_HEIGHT_MASK) <<
				 CANVAS_HEIGHT_BIT) |
				((wrap & CANVAS_XWRAP) ? CANVAS_XWRAP : 0) |
				((wrap & CANVAS_YWRAP) ? CANVAS_YWRAP : 0) |
				((blkmode & CANVAS_BLKMODE_MASK) <<
...
void canvas_update_addr(u32 index, u32 addr)
{
	...
	if (!canvas_pool_canvas_alloced(index)) {
		pr_info(KERN_ERR
			"!!canvas_update_addr  without alloc,index=%d\n",
			index);
		dump_stack();
		return;
	}
	...
	if (is_meson_m8m2_cpu()) {
		...
		canvas_io_write(info->reg_base + DC_CAV_LUT_DATAL_M8M2,
				(((canvas->addr + 7) >> 3) &
				 CANVAS_ADDR_LMASK) |
				((((canvas->width + 7) >> 3) &
				  CANVAS_WIDTH_LMASK) << CANVAS_WIDTH_LBIT));

		canvas_io_write(info->reg_base + DC_CAV_LUT_DATAH_M8M2,
				((((canvas->width +
				    7) >> 3) >> CANVAS_WIDTH_LWID) <<
				 CANVAS_WIDTH_HBIT) | ((canvas->
							height &
							CANVAS_HEIGHT_MASK) <<
						       CANVAS_HEIGHT_BIT) |
				((canvas->
		...
		canvas_io_write(info->reg_base + DC_CAV_LUT_DATAH,
				((((canvas->width + 7) >> 3) >>
				  CANVAS_WIDTH_LWID) <<
				 CANVAS_WIDTH_HBIT) |
				((canvas->height & CANVAS_HEIGHT_MASK) <<
				 CANVAS_HEIGHT_BIT) |
				((canvas->wrap & CANVAS_XWRAP) ?
				 CANVAS_XWRAP : 0) |
				((canvas->wrap & CANVAS_YWRAP) ?
				 CANVAS_YWRAP : 0) | ((canvas->blkmode &
						       CANVAS_BLKMODE_MASK) <<
						      CANVAS_BLKMODE_BIT));
...
static ssize_t
canvas_type_show(struct kobject *kobj, struct attribute *attr, char *buf)
{
...
static int canvas_probe(struct platform_device *pdev)
{
	...
	if (info->reg_base == NULL) {
		dev_err(&pdev->dev,
			"devm_ioremap_nocache for canvas failed!\n");
		goto err1;
	}
	...
	for (i = 0; i < info->max_canvas_num; i++) {
		r = kobject_init_and_add(&info->canvasPool[i].kobj,
					 &canvas_attr_type, &pdev->dev.kobj,
					 "%d", i);
		if (r) {
...
static int canvas_remove(struct platform_device *pdev)
{
	...
	amcanvas_manager_exit();
	devm_iounmap(&pdev->dev, info->reg_base);
	devm_release_mem_region(&pdev->dev, info->res.start,
