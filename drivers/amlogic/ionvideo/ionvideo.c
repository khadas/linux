				...
				struct ionvideo_buffer *buf)
{
	...
	if (dev->freerun_mode == 0) {
		...
		buf->vb.v4l2_buf.timecode.frames = 0;
...
static const struct vb2_ops ionvideo_video_qops = {
	.queue_setup = queue_setup, .buf_prepare = buffer_prepare, .buf_queue =
		buffer_queue, .start_streaming = start_streaming,
	.stop_streaming = stop_streaming, .wait_prepare = ionvideo_unlock,
