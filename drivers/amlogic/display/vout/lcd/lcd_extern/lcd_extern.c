...
int get_lcd_extern_dt_data(struct device_node *of_node,
		struct lcd_extern_config_s *pdata)
{
	...
	err = of_property_read_string(of_node,
		"dev_name", (const char **)&pdata->name);
	if (err) {
	...
	case LCD_EXTERN_SPI:
		...
		if (err) {
			...
			if (val > 0) {
				err = lcd_extern_gpio_request(val);
				if (err) {
					pr_info("faild to alloc spi_clk ");
					pr_info("gpio (%s)!\n", str);
				}
				pdata->spi_clk = val;
		...
		if (err) {
			...
			if (val > 0) {
				err = lcd_extern_gpio_request(val);
				if (err) {
					pr_info("faild to alloc spi_data ");
					pr_info("gpio (%s)!\n", str);
				}
				pdata->spi_data = val;
