...
/* ***********************************************
 * eDP Host Controller register offset address define
 * VCBUS_BASE = 0x0(0x0000 - 0xffff)
 * *********************************************** */
...
/* ***********************************************
 * eDP Sink device DPCD address define
 * AUX offset address
 * *********************************************** */
/* DPCD information */
...
/* v1.2? */
#define EDP_DPCD_I2C_SPEED_CAP				0x000C
#define EDP_DPCD_CONFIGURATION_CAP			0x000D
#define EDP_DPCD_TRAINING_AUX_RD_INTERVAL		0x000E
#define EDP_DPCD_MUTI_STREAM_TRANSPORT_CAP		0x0021
#define EDP_DPCD_PSR_SUPPORT				0x0070
#define EDP_DPCD_PSR_CAPS				0x0071
/* ***** */
/* Downstream Port 0 - 15 Capabilities */
#define EDP_DPCD_DOWNSTREAM_PORT_CAPS			0x0080
...
/* Source Device-Specific Field 0x0303 - 0x003FF :
 *     Reserved for vendor-specific usage */
/* Source IEEE OUI 7:0 */
#define EDP_DPCD_SOURCE_IEEE_OUI_0			0x0300
/* Source IEEE OUI 15:8 */
#define EDP_DPCD_SOURCE_IEEE_OUI_1			0x0301
/* Source IEEE OUI 23:16 */
#define EDP_DPCD_SOURCE_IEEE_OUI_2			0x0302

/* Sink Device-Specific Field    0x0403 - 0x004FF :
 *     Reserved for vendor-specific usage */
/* Sink IEEE OUI 7:0 */
#define EDP_DPCD_SINK_IEEE_OUT_0			0x0400
/* Sink IEEE OUI 15:8 */
#define EDP_DPCD_SINK_IEEE_OUT_1			0x0401
/* Sink IEEE OUI 23:16 */
#define EDP_DPCD_SINK_IEEE_OUT_2			0x0402

/* Branch Device-Specific Field  0x0503 - 0x005FF :
 *     Reserved for vendor-specific usage */
/* Branch Device IEEE OUI 7:0 */
#define EDP_DPCD_BRANCH_DEVICE_IEEE_OUI_0		0x0500
/* Branch Device IEEE OUI 15:8 */
#define EDP_DPCD_BRANCH_DEVICE_IEEE_OUI_1		0x0501
/* Branch Device IEEE OUI 23:16 */
#define EDP_DPCD_BRANCH_DEVICE_IEEE_OUI_2		0x0502
/* Set Power */
#define EDP_DPCD_SET_POWER				0x0600
 ...
 * register access api
 * *********************************************** */
#define EDP_REG_BASE    0x160000
static inline unsigned int dptx_reg_read(unsigned int reg)
{
	int ret, val;
	ret = aml_reg_read(IO_APB_BUS_BASE, (EDP_REG_BASE+reg), &val);
	if (ret) {
		pr_err("read edp_tx reg %x error %d\n", reg, ret);
		return -1;
	} else {
		return val;
	}
...
static inline void dptx_reg_write(unsigned int reg, unsigned int val)
{
	int ret;
	ret = aml_reg_write(IO_APB_BUS_BASE, (EDP_REG_BASE+reg), val);
	if (ret) {
		pr_err("write edp_tx reg %x error %d\n", reg, ret);
		return;
	} else {
		return;
	}
...
static inline void dptx_reg_setb(unsigned int reg, unsigned int value,
...
static inline unsigned int dptx_reg_getb(unsigned int reg,
		unsigned int _start, unsigned int _len)
{
	return (dptx_reg_read(reg) >> (_start)) & ((1L << (_len))-1);
}

static inline void dptx_reg_set_mask(unsigned int reg, unsigned int _mask)
{
	dptx_reg_write(reg, (dptx_reg_read(reg) | (_mask)));
}

static inline void dptx_reg_clr_mask(unsigned int reg, unsigned int _mask)
{
	dptx_reg_write(reg, (dptx_reg_read(reg) & (~(_mask))));
}
