/*
 * drivers/amlogic/display/vout/lcd/lcd_reg.h
 *
...
#define VID_REG_START                              0x1400
#define VID_REG_END                                0x14ff
static inline enum lcd_reg_bus_e check_lcd_reg(unsigned int reg)
{
	if ((reg >= VID_REG_START) && (reg <= VID_REG_END)) {
		if (lcd_chip_type == LCD_CHIP_M6)
			return LCD_REG_CBUS;
		else
			return LCD_REG_VCBUS;
	} else {
		return LCD_REG_CBUS;
	}
}

static inline unsigned int lcd_reg_read(unsigned int reg)
{
	if (check_lcd_reg(reg) == LCD_REG_VCBUS)
		return aml_read_vcbus(reg);
	else
		return aml_read_cbus(reg);
};

static inline void lcd_reg_write(unsigned int reg, unsigned int value)
{
	if (check_lcd_reg(reg) == LCD_REG_VCBUS)
...
static inline void lcd_reg_setb(unsigned int reg, unsigned int value,
...
static inline unsigned int lcd_reg_getb(unsigned int reg,
		unsigned int _start, unsigned int _len)
{
	if (check_lcd_reg(reg) == LCD_REG_VCBUS)
		return (aml_read_vcbus(reg) >> _start) & ((1L << _len)-1);
	else
		return (aml_read_cbus(reg) >> _start) & ((1L << _len)-1);
}

static inline void lcd_reg_set_mask(unsigned int reg, unsigned int _mask)
{
	if (check_lcd_reg(reg) == LCD_REG_VCBUS)
		aml_write_vcbus(reg, (aml_read_vcbus(reg) | (_mask)));
	else
		aml_write_cbus(reg, (aml_read_cbus(reg) | (_mask)));
}

static inline void lcd_reg_clr_mask(unsigned int reg, unsigned int _mask)
{
	if (check_lcd_reg(reg) == LCD_REG_VCBUS)
		aml_write_vcbus(reg, (aml_read_vcbus(reg) & (~(_mask))));
	else
