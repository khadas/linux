/*
 * drivers/amlogic/display/osd_ext/osd_hw.c
 *
			 ...
			 s32 in_fence_fd)
{
	...
	if (!fence_map) {
		osd_log_err("could not allocate osd_ext_fence_map\n");
		return -ENOMEM;
	}
		...
		if (enable) {
			...
			if ((osd_ext_hw.free_scale_data[OSD1].x_end > 0)
				...
				vf.width =
				    ...
				    osd_ext_hw.free_scale_data[index].x_start +
				    1;
				vf.height =
				    osd_ext_hw.free_scale_data[index].y_end -
				    osd_ext_hw.free_scale_data[index].y_start +
				    1;
			} else {
...
static void osd_ext_set_free_scale_enable_mode1(u32 index, u32 enable)
{
	...
	if (index == OSD1) {
		if (osd_ext_hw.free_scale_enable[index]) {
			if ((osd_ext_hw.free_scale_data[OSD1].x_end > 0)
					&& h_enable) {
				osd_ext_hw.free_scale_width[OSD1] =
				    osd_ext_hw.free_scale_data[OSD1].x_end -
				    osd_ext_hw.free_scale_data[OSD1].x_start;
			}

			if ((osd_ext_hw.free_scale_data[OSD1].y_end > 0)
					&& v_enable) {
				osd_ext_hw.free_scale_height[OSD1] =
				    osd_ext_hw.free_scale_data[OSD1].y_end -
...
void osd_ext_set_free_scale_width_hw(u32 index, u32 width)
{
...
void osd_ext_set_free_scale_height_hw(u32 index, u32 height)
{
...
static  void  osd1_update_disp_freescale_enable(void)
{
	...
	vf_phase_step = ((osd_ext_hw.free_scale_height[OSD1] + 1) << 20) /
		dst_h;
	...
	if (get_cpu_type() >= MESON_CPU_MAJOR_ID_M8) {
		...
		if (osd_ext_hw.free_scale_enable[OSD1]) {
			osd_reg_set_bits(VPP2_OSD_SCI_WH_M1,
				osd_ext_hw.free_scale_width[OSD1], 16, 13);
			osd_reg_set_bits(VPP2_OSD_SCI_WH_M1,
				osd_ext_hw.free_scale_height[OSD1], 0, 13);
...
static  void  osd2_update_disp_freescale_enable(void)
{
	...
	hf_phase_step =
		((osd_ext_hw.free_scale_width[OSD2] + 1) << 18) / dst_w;
	hf_phase_step = (hf_phase_step << 6);

	dst_h = osd_ext_hw.free_dst_data[OSD2].y_end -
		osd_ext_hw.free_dst_data[OSD2].y_start + 1;
	vf_phase_step =
		((osd_ext_hw.free_scale_height[OSD2] + 1) << 20) / dst_h;
	vf_phase_step = (vf_phase_step << 4);
	...
	if (get_cpu_type() >= MESON_CPU_MAJOR_ID_M8) {
		...
		if (osd_ext_hw.free_scale_enable[OSD2]) {
			osd_reg_set_bits(VPP2_OSD_SCI_WH_M1,
				osd_ext_hw.free_scale_width[OSD2], 16, 13);
			osd_reg_set_bits(VPP2_OSD_SCI_WH_M1,
				osd_ext_hw.free_scale_height[OSD2], 0, 13);
...
static void osd1_update_disp_geometry(void)
{
	...
	if (osd_ext_hw.block_mode[OSD1]) {
		...
		if (osd_ext_hw.scale[OSD1].h_enable ||
			...
			data32 = (osd_ext_hw.free_scale_data[OSD1].x_start
					& 0x1fff)
				| (osd_ext_hw.free_scale_data[OSD1].x_end
					& 0x1fff) << 16;
			osd_reg_write(VIU2_OSD1_BLK0_CFG_W1, data32);
			data32 = ((osd_ext_hw.free_scale_data[OSD1].y_start +
				   osd_ext_hw.pandata[OSD1].y_start) & 0x1fff)
				| ((osd_ext_hw.free_scale_data[OSD1].y_end +
