/*
 * drivers/amlogic/display/osd_ext/osd_fb.c
 *
...
static int osddev_ext_setcolreg(unsigned regno, u16 red, u16 green, u16 blue,
...
static int osd_ext_setcolreg(unsigned regno, unsigned red, unsigned green,
		...
		unsigned long arg)
{
	...
	case FBIOGET_OSD_ORDER:
		...
		break;
	case FBIOPUT_OSD_FREE_SCALE_WIDTH:
		osd_ext_set_free_scale_width_hw(info->node - 2, arg);
		break;
	case FBIOPUT_OSD_FREE_SCALE_HEIGHT:
		osd_ext_set_free_scale_height_hw(info->node - 2, arg);
		break;
	case FBIOPUT_OSD_FREE_SCALE_ENABLE:
				 ...
				 const char *buf, size_t count)
{
	...
	osd_ext_set_free_scale_width_hw(fb_info->node - 2, free_scale_width);
				  ...
				  const char *buf, size_t count)
{
	...
	osd_ext_set_free_scale_height_hw(fb_info->node - 2, free_scale_height);
