/*
 * drivers/amlogic/display/backlight/aml_ldim/iw7019_lpf.c
 *
 ...
 * decided by software functions,
 * for example, enviromental brightness, and so on
 */
...
void print_luma_hist(void)
{
	...
	LDIMPR("luma histgram:\n");
	for (i = 0; i < 2; i++) {
		for (j = 0; j < 8; j++)
			/*printk("%8d ", luma_hist[8 * i + j]);
		printk("\n");*/
			LDIMPR("%8d ", luma_hist[8 * i + j]);
		LDIMPR("\n");
	}
}
...
void calculate_current(void)
{
	...
	for (i = 0; i < 16; i++) {
		x = bri_current[i];
		y = lpf_speed * bri_target[i];/* 256 */
		z = (lpf_speed * x + 128) >> 8;
		bri_current[i] = x + y - z;
		/* 20bit: 12bit inter and 8 bit fraction */
	}
}
...
/* 相当于直接把BL_matrix传递到luma_hist[16] */
