...
static const struct capture_fmt mp_fmts[] = {
	...
	/* yuv420 */
	{
		...
		.uv_swap = 0,
		.write_format = 0,
		.output_format = ISP32_MI_OUTPUT_YUV422 | ISP35_MI_MP_TILE,
	}, {
		...
		.uv_swap = 0,
		.write_format = 0,
		.output_format = ISP32_MI_OUTPUT_YUV420 | ISP35_MI_MP_TILE,
	},
};
...
{
	u32 val, mask = ISP3X_MPSELF_UPD | ISP3X_SPSELF_UPD;
...
static int mi_frame_end(struct rkisp_stream *stream, u32 state)
{
	...
	if (stream->id == RKISP_STREAM_VIR ||
	    (stream->id == RKISP_STREAM_MP && dev->cap_dev.wrap_line &&
	     !stream->dummy_buf.mem_priv && stream->dummy_buf.dbuf))
		return 0;
...
rkisp_start_streaming(struct vb2_queue *queue, unsigned int count)
{
	...
	if (count == 0 && !stream->dummy_buf.dbuf &&
