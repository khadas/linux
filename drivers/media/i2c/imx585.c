 ...
 *
 * V0.0X01.0X00 first version from imx415.
 */
 ...
 */
static __maybe_unused const struct regval imx585_linear_10bit_3840x2160_2376M_regs_2lane[] = {
	{0x3008, 0x7F},
	{0x300A, 0x5B},
	{0x3028, 0x4C},
	{0x3029, 0x04},
 ...
 */
static __maybe_unused const struct regval imx585_linear_12bit_3840x2160_1782M_regs_2lane[] = {
	{0x3000,0x01},  // STANDBY
	{0x3001,0x00},  // REGHOLD
	{0x3002,0x01},  // XMSTA
...
static const struct imx585_mode supported_modes_2lane[] = {
	{
		...
		.exp_def = 0x08ca - 0x08,
		.hts_def = 0x044C * IMX585_2LANES * 2,
		.vts_def = 0x08ca,
		.global_reg_list = NULL,
		.reg_list = imx585_linear_12bit_3840x2160_1782M_regs_2lane,
		.hdr_mode = NO_HDR,
		.mipi_freq_idx = 3,
		.bpp = 12,
		.vc[PAD0] = 0,
...
int __imx585_power_on(struct imx585 *imx585)
{
	...
	cam_sw_regulator_bulk_init(imx585->cam_sw_inf, IMX585_NUM_SUPPLIES, imx585->supplies);
...
static int __maybe_unused imx585_resume(struct device *dev)
{
	...
	cam_sw_prepare_wakeup(imx585->cam_sw_inf, dev);

	usleep_range(4000, 5000);
	cam_sw_write_array(imx585->cam_sw_inf);

	if (__v4l2_ctrl_handler_setup(&imx585->ctrl_handler))
		dev_err(dev, "__v4l2_ctrl_handler_setup fail!");

	if (imx585->has_init_exp && imx585->cur_mode != NO_HDR) {	// hdr mode
		ret = imx585_ioctl(&imx585->subdev, PREISP_CMD_SET_HDRAE_EXP,
				    &imx585->cam_sw_inf->hdr_ae);
		if (ret) {
			dev_err(&imx585->client->dev, "set exp fail in hdr mode\n");
			return ret;
		}
	}
	return 0;
...
static int __maybe_unused imx585_suspend(struct device *dev)
{
	...
	cam_sw_write_array_cb_init(imx585->cam_sw_inf, client,
				   (void *)imx585->cur_mode->reg_list,
				   (sensor_write_array)imx585_write_array);
	cam_sw_prepare_sleep(imx585->cam_sw_inf);

	return 0;
}
#else
#define imx585_resume NULL
#define imx585_suspend NULL
#endif
...
static const struct dev_pm_ops imx585_pm_ops = {
	...
	SET_LATE_SYSTEM_SLEEP_PM_OPS(imx585_suspend, imx585_resume)
};
...
static int imx585_initialize_controls(struct imx585 *imx585)
{
	...
	exposure_max = mode->vts_def - 8;	//vts_def=08cA=2250
	imx585->exposure = v4l2_ctrl_new_std(handler, &imx585_ctrl_ops,
				V4L2_CID_EXPOSURE, IMX585_EXPOSURE_MIN,	//MIN=4
				exposure_max, IMX585_EXPOSURE_STEP,	//MAX=2242, STEP=1
				mode->exp_def);		//	exp_def=2250
	...
	//return v4l2_ctrl_new(hdl, ops, NULL, id, name, type, min, max, step, def, NULL, 0,flags, NULL, NULL, ptr_null, NULL);
	if (handler->error) {
		printk("=====%d=====\n", __LINE__);
		return handler->error;
	}
				  ...
				  struct i2c_client *client)
{
	...
	dev_info(dev, "khadas camera detected imx585 id %06x\n", reg);
			...
			const struct i2c_device_id *id)
{
	...
	if (!imx585->cam_sw_inf) {
		imx585->cam_sw_inf = cam_sw_init();
		cam_sw_clk_init(imx585->cam_sw_inf, imx585->xvclk, imx585->cur_mode->xvclk);
		cam_sw_reset_pin_init(imx585->cam_sw_inf, imx585->reset_gpio, 1);
		if (!IS_ERR(imx585->power_gpio))
			cam_sw_pwdn_pin_init(imx585->cam_sw_inf, imx585->power_gpio, 0);
	}
...
static void imx585_remove(struct i2c_client *client)
{
	...
	cam_sw_deinit(imx585->cam_sw_inf);
