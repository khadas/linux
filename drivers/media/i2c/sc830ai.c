...
static const struct sc830ai_mode *sc830ai_find_mode(struct sc830ai *sc830ai, int fps)
{
	const struct sc830ai_mode *mode = NULL;
	const struct sc830ai_mode *match = NULL;
	int cur_fps = 0;
	int i = 0;

	for (i = 0; i < ARRAY_SIZE(supported_modes); i++) {
		mode = &supported_modes[i];
		if (mode->width == sc830ai->cur_mode->width &&
		    mode->height == sc830ai->cur_mode->height &&
		    mode->hdr_mode == sc830ai->cur_mode->hdr_mode &&
		    mode->bus_fmt == sc830ai->cur_mode->bus_fmt) {
...
static int sc830ai_s_frame_interval(struct v4l2_subdev *sd,
				   struct v4l2_subdev_frame_interval *fi)
{
	struct sc830ai *sc830ai = to_sc830ai(sd);
	const struct sc830ai_mode *mode = NULL;
	...
	mode = sc830ai_find_mode(sc830ai, fps);
	if (mode == NULL) {
...
static const struct v4l2_subdev_video_ops sc830ai_video_ops = {
	...
	.s_frame_interval = sc830ai_s_frame_interval,
};
