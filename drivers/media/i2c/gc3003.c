...
static const struct gc3003_mode *gc3003_find_mode(struct gc3003 *gc3003, int fps)
{
	const struct gc3003_mode *mode = NULL;
	const struct gc3003_mode *match = NULL;
	int cur_fps = 0;
	int i = 0;

	for (i = 0; i < ARRAY_SIZE(supported_modes); i++) {
		mode = &supported_modes[i];
		if (mode->width == gc3003->cur_mode->width &&
		    mode->height == gc3003->cur_mode->height &&
		    mode->hdr_mode == gc3003->cur_mode->hdr_mode &&
		    mode->bus_fmt == gc3003->cur_mode->bus_fmt) {
...
static int gc3003_s_frame_interval(struct v4l2_subdev *sd,
				   struct v4l2_subdev_frame_interval *fi)
{
	struct gc3003 *gc3003 = to_gc3003(sd);
	const struct gc3003_mode *mode = NULL;
	...
	mode = gc3003_find_mode(gc3003, fps);
	if (mode == NULL) {
	...
	case RKMODULE_SET_HDR_CFG:
		...
		for (i = 0; i < gc3003->cfg_num; i++) {
			if (w == supported_modes[i].width &&
			    ...
			    supported_modes[i].bus_fmt == gc3003->cur_mode->bus_fmt) {
		...
		if (i == gc3003->cfg_num) {
			...
			gc3003->cur_fps = gc3003->cur_mode->max_fps;
		}
		break;
...
static const struct v4l2_subdev_video_ops gc3003_video_ops = {
	...
	.s_frame_interval = gc3003_s_frame_interval,
};
...
static void gc3003_modify_fps_info(struct gc3003 *gc3003)
{
	const struct gc3003_mode *mode = gc3003->cur_mode;

	gc3003->cur_fps.denominator = mode->max_fps.denominator * mode->vts_def /
				      gc3003->cur_vts;
}
