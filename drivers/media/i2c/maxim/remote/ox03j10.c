...
/*
 * Maxim Remote Sensor OmniVision OX03J10 driver
 *
...
#define OX03J10_MIPI_FREQ_324M		324000000

#define OX03J10_I2C_ADDR_DEF		0x36
...
struct ox03j10 {
	...
	const struct ox03j10_mode *supported_modes;
...
static int __maybe_unused ox03j10_i2c_write_reg(struct i2c_client *client,
...
static int __maybe_unused ox03j10_i2c_read_reg(struct i2c_client *client,
					u16 reg_addr, u32 val_len, u32 *reg_val)
{
...
static int __maybe_unused ox03j10_i2c_write_array(struct i2c_client *client,
						const struct i2c_regval *regs)
{
	...
	for (i = 0; (ret == 0) && (regs[i].reg_addr != REG_NULL); i++) {
		ret = ox03j10_i2c_write_reg(client, regs[i].reg_addr,
				OX03J10_REG_VALUE_08BIT, regs[i].reg_val);
	}
...
static int __ox03j10_power_on(struct ox03j10 *ox03j10)
{
	...
	dev_info(dev, "ox03j10 device power on\n");

	ret = regulator_enable(ox03j10->poc_regulator);
	if (ret < 0) {
...
static void __ox03j10_power_off(struct ox03j10 *ox03j10)
{
	...
	dev_info(dev, "ox03j10 device power off\n");

	ret = regulator_disable(ox03j10->poc_regulator);
	if (ret < 0)
...
static int ox03j10_runtime_resume(struct device *dev)
{
	...
	struct ox03j10 *ox03j10 = v4l2_get_subdevdata(sd);
	int ret = 0;
...
static const struct dev_pm_ops ox03j10_pm_ops = {
	SET_RUNTIME_PM_OPS(
		ox03j10_runtime_suspend, ox03j10_runtime_resume, NULL)
};
...
#endif
	const struct ox03j10_mode *def_mode = &ox03j10->supported_modes[0];
...
static void ox03j10_get_vicap_rst_inf(struct ox03j10 *ox03j10,
			struct rkmodule_vicap_reset_info *rst_info)
{
	struct i2c_client *client = ox03j10->client;

	rst_info->is_reset = ox03j10->hot_plug;
	ox03j10->hot_plug = false;
...
static void ox03j10_set_vicap_rst_inf(struct ox03j10 *ox03j10,
				struct rkmodule_vicap_reset_info rst_info)
{
	ox03j10->is_reset = rst_info.is_reset;
}
...
static long ox03j10_ioctl(struct v4l2_subdev *sd, unsigned int cmd, void *arg)
{
	...
	dev_dbg(&ox03j10->client->dev, "ioctl cmd = 0x%08x\n", cmd);
...
#ifdef CONFIG_COMPAT
static long ox03j10_compat_ioctl32(struct v4l2_subdev *sd, unsigned int cmd,
					unsigned long arg)
{
	...
	case RKMODULE_GET_VICAP_RST_INFO:
		...
		ret = ox03j10_ioctl(sd, cmd, vicap_rst_inf);
		if (!ret) {
...
{
	maxim_remote_ser_t *remote_ser = ox03j10->remote_ser;
	struct i2c_client *client = ox03j10->client;
	...
	ret = ox03j10_i2c_write_array(client, ox03j10->cur_mode->reg_list);
	if (ret) {
		dev_err(dev, "%s: ox03j10 i2c write array error\n", __func__);
		return ret;
	}

	/* In case these controls are set before streaming */
	mutex_unlock(&ox03j10->mutex);
	ret = v4l2_ctrl_handler_setup(&ox03j10->ctrl_handler);
	mutex_lock(&ox03j10->mutex);
	if (ret)
		return ret;
...
#endif
{
	...
	if (fse->index >= ox03j10->cfg_modes_num)
		return -EINVAL;

	if (fse->code != ox03j10->supported_modes[fse->index].bus_fmt)
		return -EINVAL;

	fse->min_width  = ox03j10->supported_modes[fse->index].width;
	fse->max_width  = ox03j10->supported_modes[fse->index].width;
	fse->max_height = ox03j10->supported_modes[fse->index].height;
	fse->min_height = ox03j10->supported_modes[fse->index].height;
...
#endif
{
	...
	if (fie->index >= ox03j10->cfg_modes_num)
		return -EINVAL;

	fie->code = ox03j10->supported_modes[fie->index].bus_fmt;
	fie->width = ox03j10->supported_modes[fie->index].width;
	fie->height = ox03j10->supported_modes[fie->index].height;
	fie->interval = ox03j10->supported_modes[fie->index].max_fps;
...
ox03j10_find_best_fit(struct ox03j10 *ox03j10, struct v4l2_subdev_format *fmt)
{
	...
	for (i = 0; i < ox03j10->cfg_modes_num; i++) {
		dist = ox03j10_get_reso_dist(&ox03j10->supported_modes[i], framefmt);
		if ((cur_best_fit_dist == -1 || dist < cur_best_fit_dist) &&
			(ox03j10->supported_modes[i].bus_fmt == framefmt->code)) {
			cur_best_fit_dist = dist;
			cur_best_fit = i;
		}
	}

	return &ox03j10->supported_modes[cur_best_fit];
}
...
#endif
{
	...
	u8 data_lanes = ox03j10->bus_cfg.bus.mipi_csi2.num_data_lanes;

	mutex_lock(&ox03j10->mutex);

	mode = ox03j10_find_best_fit(ox03j10, fmt);
	...
	if (fmt->which == V4L2_SUBDEV_FORMAT_TRY) {
		...
		*v4l2_subdev_get_try_format(sd, cfg, fmt->pad) = fmt->format;
		...
		__v4l2_ctrl_s_ctrl_int64(ox03j10->pixel_rate, pixel_rate);
...
#if KERNEL_VERSION(6, 1, 0) <= LINUX_VERSION_CODE
static int ox03j10_get_selection(struct v4l2_subdev *sd,
				struct v4l2_subdev_state *sd_state,
...
#endif
{
	...
	if (sel->target == V4L2_SEL_TGT_CROP_BOUNDS) {
		sel->r.left = 0;
		sel->r.width = ox03j10->cur_mode->width;
		sel->r.top = 0;
		sel->r.height = ox03j10->cur_mode->height;
		return 0;
	}
...
#if KERNEL_VERSION(6, 1, 0) <= LINUX_VERSION_CODE
static int ox03j10_g_mbus_config(struct v4l2_subdev *sd, unsigned int pad,
				struct v4l2_mbus_config *config)
{
	struct ox03j10 *ox03j10 = v4l2_get_subdevdata(sd);

	config->type = V4L2_MBUS_CSI2_DPHY;
	config->bus.mipi_csi2 = ox03j10->bus_cfg.bus.mipi_csi2;
...
static const struct v4l2_subdev_video_ops ox03j10_video_ops = {
	...
	.g_mbus_config = ox03j10_g_mbus_config,
#endif
};
...
static const struct v4l2_subdev_pad_ops ox03j10_pad_ops = {
	...
	.get_selection = ox03j10_get_selection,
#if KERNEL_VERSION(5, 10, 0) <= LINUX_VERSION_CODE
...
static int ox03j10_initialize_controls(struct ox03j10 *ox03j10)
{
	...
	ox03j10->link_freq = v4l2_ctrl_new_int_menu(handler, NULL,
				V4L2_CID_LINK_FREQ,
	...
	/* pixel rate = link frequency * 2 * lanes / BITS_PER_SAMPLE */
	data_lanes = ox03j10->bus_cfg.bus.mipi_csi2.num_data_lanes;
	pixel_rate = (u32)link_freq / mode->bpp * 2 * data_lanes;

	ox03j10->pixel_rate = v4l2_ctrl_new_std(handler, NULL,
					V4L2_CID_PIXEL_RATE,
...
static int ox03j10_parse_dt(struct ox03j10 *ox03j10)
{
	struct device *dev = &ox03j10->client->dev;
	...
	dev_info(dev, "=== ox03j10 parse dt ===\n");

	ret = of_property_read_u32(of_node, "cam-i2c-addr-def", &value);
	if (ret == 0) {
		dev_info(dev, "cam-i2c-addr-def property: 0x%x", value);
		ox03j10->cam_i2c_addr_def = value;
	} else {
		ox03j10->cam_i2c_addr_def = OX03J10_I2C_ADDR_DEF;
	}

	return 0;
}

static int ox03j10_mipi_data_lanes_parse(struct ox03j10 *ox03j10)
{
	struct device *dev = &ox03j10->client->dev;
	...
	mipi_data_lanes = ox03j10->bus_cfg.bus.mipi_csi2.num_data_lanes;
			...
			const struct i2c_device_id *id)
{
	...
	if (!ox03j10) {
		dev_err(dev, "ox03j10 probe no memory error\n");
		return -ENOMEM;
	}

	ox03j10->client = client;
	ox03j10->cam_i2c_addr_map = client->addr;
	...
	// poc regulator
	ox03j10->poc_regulator = devm_regulator_get(dev, "poc");
	if (IS_ERR(ox03j10->poc_regulator)) {
		if (PTR_ERR(ox03j10->poc_regulator) != -EPROBE_DEFER)
			dev_err(dev, "Unable to get PoC regulator (%ld)\n",
				PTR_ERR(ox03j10->poc_regulator));
		else
			dev_err(dev, "Get PoC regulator deferred\n");

		ret = PTR_ERR(ox03j10->poc_regulator);

		return ret;
	}

	ox03j10_mipi_data_lanes_parse(ox03j10);
	ox03j10->supported_modes = supported_modes;
	ox03j10->cfg_modes_num = ARRAY_SIZE(supported_modes);
	ox03j10->cur_mode = &ox03j10->supported_modes[0];
	...
	v4l2_set_subdevdata(sd, ox03j10);
	...
	snprintf(sd->name, sizeof(sd->name), "m%02d_%s_%s %s",
		 ox03j10->module_index, facing, OX03J10_NAME,
		 dev_name(sd->dev));
	...
	ox03j10_parse_dt(ox03j10);

	/* remote serializer bind */
	ox03j10->remote_ser = NULL;
	remote_ser = maxim_remote_cam_bind_ser(dev);
	if (remote_ser != NULL) {
		dev_info(dev, "remote serializer bind success\n");

		remote_ser->cam_i2c_addr_def = ox03j10->cam_i2c_addr_def;
		remote_ser->cam_i2c_addr_map = ox03j10->cam_i2c_addr_map;

		ox03j10->remote_ser = remote_ser;
	} else {
...
MODULE_DESCRIPTION("Maxim Remote Sensor OmniVision OX03J10 Driver");
MODULE_LICENSE("GPL");
