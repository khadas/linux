...
static int ov2735_g_frame_interval(struct v4l2_subdev *sd,
				    struct v4l2_subdev_frame_interval *fi)
{
	...
	if (ov2735->streaming)
		fi->interval = ov2735->cur_fps;
	else
		fi->interval = mode->max_fps;
	return 0;
}

static const struct ov2735_mode *ov2735_find_mode(struct ov2735 *ov2735, int fps)
{
	const struct ov2735_mode *mode = NULL;
	const struct ov2735_mode *match = NULL;
	int cur_fps = 0;
	int i = 0;

	for (i = 0; i < ARRAY_SIZE(supported_modes); i++) {
		mode = &supported_modes[i];
		if (mode->width == ov2735->cur_mode->width &&
		    mode->height == ov2735->cur_mode->height &&
		    mode->bus_fmt == ov2735->cur_mode->bus_fmt) {
...
static int ov2735_s_frame_interval(struct v4l2_subdev *sd,
				   struct v4l2_subdev_frame_interval *fi)
{
	struct ov2735 *ov2735 = to_ov2735(sd);
	const struct ov2735_mode *mode = NULL;
	...
	mode = ov2735_find_mode(ov2735, fps);
	if (mode == NULL) {
...
static const struct v4l2_subdev_video_ops ov2735_video_ops = {
	...
	.g_frame_interval = ov2735_g_frame_interval,
	.s_frame_interval = ov2735_s_frame_interval,
};
...
static void ov2735_modify_fps_info(struct ov2735 *ov2735)
{
	const struct ov2735_mode *mode = ov2735->cur_mode;

	ov2735->cur_fps.denominator = mode->max_fps.denominator * mode->vts_def /
				      ov2735->cur_vts;
}
...
static int ov2735_set_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_CID_VBLANK:
		...
		ov2735->cur_vts = ctrl->val + ov2735->cur_mode->height;
		ov2735_modify_fps_info(ov2735);
		break;
