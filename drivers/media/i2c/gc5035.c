				   ...
				   struct v4l2_subdev_frame_size_enum *fse)
{
	...
	if (fse->code != gc5035->cur_mode->bus_fmt)
		return -EINVAL;
...
static const struct gc5035_mode *gc5035_find_mode(struct gc5035 *gc5035, int fps)
{
	const struct gc5035_mode *mode = NULL;
	const struct gc5035_mode *match = NULL;
	int cur_fps = 0;
	int i = 0;

	for (i = 0; i < gc5035->cfg_num; i++) {
		mode = &supported_modes[i];
		if (mode->width == gc5035->cur_mode->width &&
		    mode->height == gc5035->cur_mode->height &&
		    mode->bus_fmt == gc5035->cur_mode->bus_fmt) {
...
static int gc5035_s_frame_interval(struct v4l2_subdev *sd,
				   struct v4l2_subdev_frame_interval *fi)
{
	struct gc5035 *gc5035 = to_gc5035(sd);
	const struct gc5035_mode *mode = NULL;
	...
	mode = gc5035_find_mode(gc5035, fps);
	if (mode == NULL) {
...
static const struct v4l2_subdev_video_ops gc5035_video_ops = {
	...
	.s_frame_interval = gc5035_s_frame_interval,
};
...
static void gc5035_modify_fps_info(struct gc5035 *gc5035)
{
	const struct gc5035_mode *mode = gc5035->cur_mode;

	gc5035->cur_fps.denominator = mode->max_fps.denominator * mode->vts_def /
				      gc5035->cur_vts;
}
...
static int gc5035_set_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_CID_VBLANK:
		...
		gc5035->cur_vts = ctrl->val + gc5035->cur_mode->height;
		gc5035_modify_fps_info(gc5035);
		break;
