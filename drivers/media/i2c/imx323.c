				   ...
				   struct v4l2_subdev_frame_size_enum *fse)
{
	...
	if (fse->code != imx323->cur_mode->bus_fmt)
		return -EINVAL;
				   ...
				   struct v4l2_subdev_frame_interval *fi)
{
	...
	if (imx323->streaming)
		fi->interval = imx323->cur_fps;
	else
		fi->interval = mode->max_fps;
	return 0;
}

static const struct imx323_mode *imx323_find_mode(struct imx323 *imx323, int fps)
{
	const struct imx323_mode *mode = NULL;
	const struct imx323_mode *match = NULL;
	int cur_fps = 0;
	int i = 0;

	for (i = 0; i < ARRAY_SIZE(supported_modes); i++) {
		mode = &supported_modes[i];
		if (mode->width == imx323->cur_mode->width &&
		    mode->height == imx323->cur_mode->height &&
		    mode->bus_fmt == imx323->cur_mode->bus_fmt) {
...
static int imx323_s_frame_interval(struct v4l2_subdev *sd,
				   struct v4l2_subdev_frame_interval *fi)
{
	struct imx323 *imx323 = to_imx323(sd);
	const struct imx323_mode *mode = NULL;
	...
	mode = imx323_find_mode(imx323, fps);
	if (mode == NULL) {
...
static const struct v4l2_subdev_video_ops imx323_video_ops = {
	...
	.s_frame_interval = imx323_s_frame_interval,
};
...
static void imx323_modify_fps_info(struct imx323 *imx323)
{
	const struct imx323_mode *mode = imx323->cur_mode;

	imx323->cur_fps.denominator = mode->max_fps.denominator * mode->vts_def /
				      imx323->cur_vts;
}
...
static int imx323_set_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_CID_VBLANK:
		...
		imx323->cur_vts = ctrl->val + imx323->cur_mode->height;
		imx323_modify_fps_info(imx323);
		break;
