...
static uint32_t RKX12x_HAL_CRU_ClkGetFreq(struct hwclk *hw, uint32_t clockName)
{
    ...
    case RKX120_CPS_PLL_TXPLL:
        freq = RKX12x_HAL_CRU_CORE_GetPllFreq(hw, &TXPLL_SETUP);
        hw->pllRate[RKX120_TXPLL] = freq;

        return freq;

    case RKX120_CPS_PLL_CPLL:
        freq = RKX12x_HAL_CRU_CORE_GetPllFreq(hw, &CPLL_SETUP);
    ...
    case RKX120_CPS_PCLKOUT_DVPTX:
        freq = RKX12x_HAL_CRU_CORE_MuxGetFreq3(hw, clkMux,
    ...
    case RKX121_CPS_CLK_LVDS1:
        ...
        freq = RKX12x_HAL_CRU_CORE_MuxGetFreq2(hw, clkMux, pRate0, pRate1);
        break;

    /* pre-bus: 100M */
    case RKX120_CPS_BUSCLK_TX_PRE0:
        freq = RKX12x_HAL_CRU_CORE_MuxGetFreq2(hw, clkMux,
                                               hw->pllRate[RKX120_TXPLL],
    ...
    case RKX120_CPS_BUSCLK_TX_PRE:
        ...
        freq = RKX12x_HAL_CRU_CORE_MuxGetFreq2(hw, clkMux, OSC_24M, pRate);
        break;
    ...
    if (clkDiv) {
        freq /= (RKX12x_HAL_CRU_CORE_ClkGetDiv(hw, clkDiv));
    }
...
static HAL_Status RKX12x_HAL_CRU_ClkSetFreq(struct hwclk *hw, uint32_t clockName, uint32_t rate)
{
    ...
    switch (clockName) {
    case RKX120_CPS_PLL_TXPLL:
        ret = RKX12x_HAL_CRU_CORE_SetPllFreq(hw, &TXPLL_SETUP, rate);
    ...
    case RKX120_CPS_PLL_CPLL:
        ret = RKX12x_HAL_CRU_CORE_SetPllFreq(hw, &CPLL_SETUP, rate);
    ...
    case RKX120_CPS_CLK_PWM_TX:
    case RKX120_CPS_PCLKOUT_DVPTX:
        mux = RKX12x_HAL_CRU_CORE_RoundFreqGetMux3(hw, rate,
                                                   hw->pllRate[RKX120_TXPLL],
    ...
    case RKX121_CPS_CLK_LVDS1:
        if (rate == _MHZ(150)) {
            return RKX12x_HAL_CRU_CORE_ClkSetMux(hw, clkMux, 0); /* input clock: clk_lvds1_cm */
        } else {
            RKX12x_HAL_CRU_ClkSetFreq(hw, RKX121_CPS_CLK_LVDS_C_LVDS_TX, rate);

            return RKX12x_HAL_CRU_CORE_ClkSetMux(hw, clkMux, 1);
        }
        break;

    /* pre-bus */
    case RKX120_CPS_BUSCLK_TX_PRE0:
        mux = RKX12x_HAL_CRU_CORE_RoundFreqGetMux2(hw, rate,
                                                   hw->pllRate[RKX120_TXPLL],
    ...
    case RKX120_CPS_BUSCLK_TX_PRE:
        if (rate == OSC_24M) {
            return RKX12x_HAL_CRU_CORE_ClkSetMux(hw, clkMux, 0);
        } else {
    ...
    case RKX120_CPS_CLK_PMA2PCS2LINK_CM:
        ...
        RKX12x_HAL_CRU_CORE_ClkSetMux(hw, clkMux, 0);
    ...
    if (clkDiv) {
        ...
        RKX12x_HAL_CRU_CORE_ClkSetDiv(hw, clkDiv, div);
    }

    if (clkMux) {
        RKX12x_HAL_CRU_CORE_ClkSetMux(hw, clkMux, mux);
    }
...
static void RKX12x_HAL_CRU_Init_SSCG(struct hwclk *hw)
{
    ...
    RKX12x_HAL_CRU_CORE_Write(hw, hw->cru_base + 0x0c, 0x1f000000 | ((amplitude & 0x1f) << 8));
    RKX12x_HAL_CRU_CORE_Write(hw, hw->cru_base + 0x0c, 0x00f00050);
    RKX12x_HAL_CRU_CORE_Write(hw, hw->cru_base + 0x0c, 0x00080000 | ((down_spread & 0x1) << 3));
    RKX12x_HAL_CRU_CORE_Write(hw, hw->cru_base + 0x04, 0x10000000);
    RKX12x_HAL_CRU_CORE_Write(hw, hw->cru_base + 0x0c, 0x00070000);
    CRU_MSG("%s: CPLL enable SSCG\n", hw->name);
#endif
#if RKX120_SSCG_TXPLL_EN
    /* down-spread, 0.8%, 37.5khz */
    RKX12x_HAL_CRU_CORE_Write(hw, hw->cru_base + 0x2c, 0x1f000000 | ((amplitude & 0x1f) << 8));
    RKX12x_HAL_CRU_CORE_Write(hw, hw->cru_base + 0x2c, 0x00f00050);
    RKX12x_HAL_CRU_CORE_Write(hw, hw->cru_base + 0x2c, 0x00080000 | ((down_spread & 0x1) << 3));
    RKX12x_HAL_CRU_CORE_Write(hw, hw->cru_base + 0x24, 0x10000000);
    RKX12x_HAL_CRU_CORE_Write(hw, hw->cru_base + 0x2c, 0x00070000);
                                             ...
                                             uint32_t muxValue, uint32_t divValue)
{
    ...
    RKX12x_HAL_CRU_CORE_Write(hw, RKX120_GRF_GPIO0B_IOMUX_H, GPIO0B7_TEST_CLKOUT);

    /* Enable clock */
    RKX12x_HAL_CRU_CORE_ClkEnable(hw, RKX120_CLK_TESTOUT_TOP_GATE);

    /* Mux, div */
    RKX12x_HAL_CRU_CORE_ClkSetDiv(hw, clkDiv, divValue);
    RKX12x_HAL_CRU_CORE_ClkSetMux(hw, clkMux, muxValue);
...
static uint32_t RKX12x_HAL_CRU_ClkGetExtFreq(struct hwclk *hw, uint32_t clk)
{
    ...
    RKX12x_HAL_CRU_CORE_ClkSetDiv(hw, clkDiv, div);
    RKX12x_HAL_CRU_CORE_ClkSetMux(hw, clkMux, clk);
    RKX12x_HAL_CRU_CORE_Write(hw, CAL_FREQ_EN_REG, 1); /* NOTE: 1: enable, 0: disable */
    HAL_SleepMs(2);
    freq = RKX12x_HAL_CRU_CORE_Read(hw, CAL_FREQ_REG);
    RKX12x_HAL_CRU_CORE_Write(hw, CAL_FREQ_EN_REG, 0x0);
...
struct clkOps rkx121_des_clk_ops =
{
