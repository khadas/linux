...
#ifndef __RKX12X_CRU_CORE_H__
#define __RKX12X_CRU_CORE_H__

#include "hal_def.h"
#include "hal_os_def.h"

#define RKX12X_DEBUG_CRU_INIT 0 /* 1: debug cru clock tree when cru init */
#define RKX12X_CRU_LOGLEVEL   3

#define __cru_print(level, fmt, ...)                                        \
({                                                                          \
    level < RKX12X_CRU_LOGLEVEL ? HAL_SYSLOG("[HAL CRU] " fmt, ##__VA_ARGS__) : 0; \
})
...
#define HAL_ENABLE_SET_RATE_VERIFY 0 /* 1: enable set rate verify */
...
#define CLK_FLG_CRU_INITED      (1 << 1) /* cru has been inited */

struct hwclk;

struct xferclk {
    HAL_ClockType type;
    u32 version; /* same as serdes version */
    char name[16]; /* slave addr is expected */
    void *client; /* i2c client */
    HAL_RegRead_t *read;
...
/***************************** Function Declare ******************************/
uint32_t RKX12x_HAL_CRU_CORE_Read(struct hwclk *hw, uint32_t reg);
uint32_t RKX12x_HAL_CRU_CORE_Write(struct hwclk *hw, uint32_t reg, uint32_t val);
uint32_t RKX12x_HAL_CRU_CORE_WriteMask(struct hwclk *hw, uint32_t reg,
                                       uint32_t msk, uint32_t val);

int RKX12x_HAL_CRU_CORE_FreqGetMux4(struct hwclk *hw,
                                    uint32_t freq, uint32_t freq0,
                                    uint32_t freq1, uint32_t freq2, uint32_t freq3);
int RKX12x_HAL_CRU_CORE_FreqGetMux3(struct hwclk *hw,
                                    uint32_t freq, uint32_t freq0,
                                    uint32_t freq1, uint32_t freq2);
int RKX12x_HAL_CRU_CORE_FreqGetMux2(struct hwclk *hw,
                                    uint32_t freq, uint32_t freq0, uint32_t freq1);

uint32_t RKX12x_HAL_CRU_CORE_MuxGetFreq4(struct hwclk *hw, uint32_t muxName,
                                         uint32_t freq0, uint32_t freq1,
                                         uint32_t freq2, uint32_t freq3);
uint32_t RKX12x_HAL_CRU_CORE_MuxGetFreq3(struct hwclk *hw, uint32_t muxName,
                                         uint32_t freq0, uint32_t freq1, uint32_t freq2);
uint32_t RKX12x_HAL_CRU_CORE_MuxGetFreq2(struct hwclk *hw, uint32_t muxName,
                                         uint32_t freq0, uint32_t freq1);

int RKX12x_HAL_CRU_CORE_RoundFreqGetMux4(struct hwclk *hw, uint32_t freq,
                                         uint32_t pFreq0, uint32_t pFreq1,
                                         uint32_t pFreq2, uint32_t pFreq3, uint32_t *pFreqOut);
int RKX12x_HAL_CRU_CORE_RoundFreqGetMux3(struct hwclk *hw, uint32_t freq,
                                         uint32_t pFreq0, uint32_t pFreq1,
                                         uint32_t pFreq2, uint32_t *pFreqOut);
int RKX12x_HAL_CRU_CORE_RoundFreqGetMux2(struct hwclk *hw, uint32_t freq,
 ...
 * @return pll rate.
 */
uint32_t RKX12x_HAL_CRU_CORE_GetPllFreq(struct hwclk *hw, struct PLL_SETUP *pSetup);
 ...
 * @return HAL_Status.
 */
HAL_Status RKX12x_HAL_CRU_CORE_SetPllFreq(struct hwclk *hw, struct PLL_SETUP *pSetup, uint32_t rate);
 ...
 * @return HAL_Status.
 */
HAL_Status RKX12x_HAL_CRU_CORE_SetPllPowerUp(struct hwclk *hw, struct PLL_SETUP *pSetup);
 ...
 * @return HAL_Status.
 */
HAL_Status RKX12x_HAL_CRU_CORE_SetPllPowerDown(struct hwclk *hw, struct PLL_SETUP *pSetup);
 ...
 * @return HAL_Check.
 */
HAL_Check RKX12x_HAL_CRU_CORE_ClkIsEnabled(struct hwclk *hw, uint32_t clk);
 ...
 * @return HAL_Status.
 */
HAL_Status RKX12x_HAL_CRU_CORE_ClkEnable(struct hwclk *hw, uint32_t clk);
 ...
 * @return HAL_Status.
 */
HAL_Status RKX12x_HAL_CRU_CORE_ClkDisable(struct hwclk *hw, uint32_t clk);
 ...
 * @return HAL_Check.
 */
HAL_Check RKX12x_HAL_CRU_CORE_ClkIsReset(struct hwclk *hw, uint32_t clk);
 ...
 * @return HAL_Status.
 */
HAL_Status RKX12x_HAL_CRU_CORE_ClkResetAssert(struct hwclk *hw, uint32_t clk);
 ...
 * @return HAL_Status.
 */
HAL_Status RKX12x_HAL_CRU_CORE_ClkResetDeassert(struct hwclk *hw, uint32_t clk);
 ...
 * @return NONE
 */
HAL_Status RKX12x_HAL_CRU_CORE_ClkSetDiv(struct hwclk *hw, uint32_t divName, uint32_t divValue);
 ...
 * @return div value
 */
uint32_t RKX12x_HAL_CRU_CORE_ClkGetDiv(struct hwclk *hw, uint32_t divName);
 ...
 * @return NONE
 */
HAL_Status RKX12x_HAL_CRU_CORE_ClkSetMux(struct hwclk *hw, uint32_t muxName, uint32_t muxValue);
 ...
 * @return mux value
 */
uint32_t RKX12x_HAL_CRU_CORE_ClkGetMux(struct hwclk *hw, uint32_t muxName);
 ...
 */
uint32_t RKX12x_HAL_CRU_CORE_ClkGetFreq(struct hwclk *hw, uint32_t clockName);
 ...
 * @attention these APIs allow direct use in the HAL layer.
 */
HAL_Status RKX12x_HAL_CRU_CORE_ClkSetFreq(struct hwclk *hw, uint32_t clockName, uint32_t rate);
 ...
 */
HAL_Status RKX12x_HAL_CRU_CORE_SetGlbSrst(struct hwclk *hw, eCRU_GlbSrstType type);
 ...
 * @return HAL_INVAL if the SoC does not support.
 */
HAL_Status RKX12x_HAL_CRU_CORE_ClkSetTestout(struct hwclk *hw, uint32_t clockName,
                                             uint32_t muxValue, uint32_t divValue);
/**
 * @brief  Dump all clock tree
 */
void RKX12x_HAL_CRU_CORE_ClkDumpTree(struct hwclk *hw);

/**
 * @brief  Register CRU
 * @param  xfer: the data to register
 * @param  hwclk: hwclk descriptor.
 * @return HAL_Status.
 */
HAL_Status RKX12x_HAL_CRU_CORE_Register(struct hwclk *hw, struct xferclk *xfer);

/**
 * @brief  CRU Clock Init
 * @param  hwclk: hwclk descriptor.
 * @return HAL_Status.
 */
HAL_Status RKX12x_HAL_CRU_CORE_ClkInit(struct hwclk *hw);

/************************** chip ops *************************************/
extern struct clkOps rkx120_des_clk_ops;
extern struct clkOps rkx121_des_clk_ops;

#endif /* __RKX12X_CRU_CORE_H__ */
