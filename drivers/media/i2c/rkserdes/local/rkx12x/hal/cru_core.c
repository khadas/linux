...
{
    return RKX12x_HAL_CRU_CORE_Write(hw, reg, VAL_MASK_WE(msk, val));
}
...
{
    return RKX12x_HAL_CRU_CORE_FreqGetMux4(hw, freq, freq0, freq1, freq2, freq2);
}

int RKX12x_HAL_CRU_CORE_FreqGetMux2(struct hwclk *hw,
                                    uint32_t freq, uint32_t freq0, uint32_t freq1)
{
    return RKX12x_HAL_CRU_CORE_FreqGetMux4(hw, freq, freq0, freq1, freq1, freq1);
}
...
{
    switch (RKX12x_HAL_CRU_CORE_ClkGetMux(hw, muxName)) {
    case 0:
        return freq0;
...
{
    return RKX12x_HAL_CRU_CORE_MuxGetFreq4(hw, muxName, freq0, freq1, freq2, freq2);
}

uint32_t RKX12x_HAL_CRU_CORE_MuxGetFreq2(struct hwclk *hw, uint32_t muxName,
                                         uint32_t freq0, uint32_t freq1)
{
    return RKX12x_HAL_CRU_CORE_MuxGetFreq4(hw, muxName, freq0, freq1, freq1, freq1);
}
...
{
    return RKX12x_HAL_CRU_CORE_RoundFreqGetMux4(hw, freq, pFreq0, pFreq1, pFreq2, 0, pFreqOut);
}

int RKX12x_HAL_CRU_CORE_RoundFreqGetMux2(struct hwclk *hw, uint32_t freq,
                                         uint32_t pFreq0, uint32_t pFreq1, uint32_t *pFreqOut)
{
    return RKX12x_HAL_CRU_CORE_RoundFreqGetMux4(hw, freq, pFreq0, pFreq1, 0, 0, pFreqOut);
}
...
uint32_t RKX12x_HAL_CRU_CORE_GetPllFreq(struct hwclk *hw, struct PLL_SETUP *pSetup)
{
    ...
    mode = PLL_GET_PLLMODE(RKX12x_HAL_CRU_CORE_Read(hw, pSetup->modeOffset),
                           pSetup->modeShift,
                           pSetup->modeMask);
    ...
    case RK_PLL_MODE_NORMAL:
        postdDv1 = PLL_GET_POSTDIV1(RKX12x_HAL_CRU_CORE_Read(hw, pSetup->conOffset0));
        fbDiv = PLL_GET_FBDIV(RKX12x_HAL_CRU_CORE_Read(hw, pSetup->conOffset0));
        postDiv2 = PLL_GET_POSTDIV2(RKX12x_HAL_CRU_CORE_Read(hw, pSetup->conOffset1));
        refDiv = PLL_GET_REFDIV(RKX12x_HAL_CRU_CORE_Read(hw, pSetup->conOffset1));
        dsmpd = PLL_GET_DSMPD(RKX12x_HAL_CRU_CORE_Read(hw, pSetup->conOffset1));
        frac = PLL_GET_FRAC(RKX12x_HAL_CRU_CORE_Read(hw, pSetup->conOffset2));
...
HAL_Status RKX12x_HAL_CRU_CORE_SetPllFreq(struct hwclk *hw, struct PLL_SETUP *pSetup, uint32_t rate)
{
    ...
    if (rate == RKX12x_HAL_CRU_CORE_GetPllFreq(hw, pSetup)) {
        return HAL_OK;
    }
    ...
    RKX12x_HAL_CRU_CORE_WriteMask(hw, pSetup->modeOffset, pSetup->modeMask, RK_PLL_MODE_SLOW << pSetup->modeShift);

    /* Pll Power down */
    RKX12x_HAL_CRU_CORE_WriteMask(hw, pSetup->conOffset1, PWRDOWN_MASK, 1 << PWRDOWN_SHIFT);

    /* Pll Config */
    RKX12x_HAL_CRU_CORE_WriteMask(hw, pSetup->conOffset1, PLL_POSTDIV2_MASK, pConfig->postDiv2 << PLL_POSTDIV2_SHIFT);
    RKX12x_HAL_CRU_CORE_WriteMask(hw, pSetup->conOffset1, PLL_REFDIV_MASK, pConfig->refDiv << PLL_REFDIV_SHIFT);
    RKX12x_HAL_CRU_CORE_WriteMask(hw, pSetup->conOffset0, PLL_POSTDIV1_MASK, pConfig->postDiv1 << PLL_POSTDIV1_SHIFT);
    RKX12x_HAL_CRU_CORE_WriteMask(hw, pSetup->conOffset0, PLL_FBDIV_MASK, pConfig->fbDiv << PLL_FBDIV_SHIFT);
    if (pSetup->sscgEn) {
        RKX12x_HAL_CRU_CORE_WriteMask(hw, pSetup->conOffset1, PLL_DSMPD_MASK, 0 << PLL_DSMPD_SHIFT);
    } else {
        RKX12x_HAL_CRU_CORE_WriteMask(hw, pSetup->conOffset1, PLL_DSMPD_MASK, pConfig->dsmpd << PLL_DSMPD_SHIFT);
    }

    if (pConfig->frac) {
        RKX12x_HAL_CRU_CORE_Write(hw, pSetup->conOffset2, (RKX12x_HAL_CRU_CORE_Read(hw, pSetup->conOffset2) & 0xff000000) | pConfig->frac);
    }

    /* Pll Power up */
    RKX12x_HAL_CRU_CORE_WriteMask(hw, pSetup->conOffset1, PWRDOWN_MASK, 0 << PWRDOWN_SHIFT);

    /* Waiting for pll lock */
    while (delay > 0) {
        if (pSetup->stat0) {
            if (RKX12x_HAL_CRU_CORE_Read(hw, pSetup->stat0) & (1 << pSetup->lockShift)) {
                break;
            }
        } else {
            if (RKX12x_HAL_CRU_CORE_Read(hw, pSetup->conOffset1) & (1 << pSetup->lockShift)) {
                break;
            }
        }
        HAL_DelayUs(2);
    ...
    /* Force PLL into normal mode */
    RKX12x_HAL_CRU_CORE_WriteMask(hw, pSetup->modeOffset, pSetup->modeMask, RK_PLL_MODE_NORMAL << pSetup->modeShift);
...
{
    RKX12x_HAL_CRU_CORE_Write(hw, pSetup->conOffset1, VAL_MASK_WE(PWRDOWN_MASK, 1 << PWRDOWN_SHIFT));
...
HAL_Check RKX12x_HAL_CRU_CORE_ClkIsEnabled(struct hwclk *hw, uint32_t clk)
{
    ...
    return (HAL_Check)((RKX12x_HAL_CRU_CORE_Read(hw, hw->gate_con0 + REG_X4(index)) & (1 << shift)) >> shift);
}
...
HAL_Status RKX12x_HAL_CRU_CORE_ClkEnable(struct hwclk *hw, uint32_t clk)
{
    ...
    if (hw->gate[gid].enable_count == 0) {
        ...
        RKX12x_HAL_CRU_CORE_Write(hw, hw->gate_con0 + REG_X4(index), VAL_MASK_WE(1U << shift, 0U << shift));
    }
...
HAL_Status RKX12x_HAL_CRU_CORE_ClkDisable(struct hwclk *hw, uint32_t clk)
{
    ...
    if (hw->gate[gid].enable_count == 0) {
        CRU_DBGF("%s: %s: clock(#%08x), gid=%d\n", __func__, hw->name, clk, gid);
        RKX12x_HAL_CRU_CORE_Write(hw, hw->gate_con0 + REG_X4(index), VAL_MASK_WE(1U << shift, 1U << shift));
    }
...
HAL_Check RKX12x_HAL_CRU_CORE_ClkIsReset(struct hwclk *hw, uint32_t clk)
{
    ...
    return (HAL_Check)((RKX12x_HAL_CRU_CORE_Read(hw, hw->softrst_con0 + REG_X4(index)) & (1 << shift)) >> shift);
}
...
HAL_Status RKX12x_HAL_CRU_CORE_ClkResetAssert(struct hwclk *hw, uint32_t clk)
{
    ...
    RKX12x_HAL_CRU_CORE_Write(hw, hw->softrst_con0 + REG_X4(index), VAL_MASK_WE(1U << shift, 1U << shift));
...
HAL_Status RKX12x_HAL_CRU_CORE_ClkResetDeassert(struct hwclk *hw, uint32_t clk)
{
    ...
    RKX12x_HAL_CRU_CORE_Write(hw, hw->softrst_con0 + REG_X4(index), VAL_MASK_WE(1U << shift, 0U << shift));
...
HAL_Status RKX12x_HAL_CRU_CORE_ClkSetDiv(struct hwclk *hw, uint32_t divName, uint32_t divValue)
{
    ...
    RKX12x_HAL_CRU_CORE_Write(hw, hw->sel_con0 + REG_X4(index), VAL_MASK_WE(mask, (divValue - 1U) << shift));
...
uint32_t RKX12x_HAL_CRU_CORE_ClkGetDiv(struct hwclk *hw, uint32_t divName)
{
    ...
    divValue = ((RKX12x_HAL_CRU_CORE_Read(hw, hw->sel_con0 + REG_X4(index)) & mask) >> shift) + 1;
...
HAL_Status RKX12x_HAL_CRU_CORE_ClkSetMux(struct hwclk *hw, uint32_t muxName, uint32_t muxValue)
{
    ...
    RKX12x_HAL_CRU_CORE_Write(hw, hw->sel_con0 + REG_X4(index), VAL_MASK_WE(mask, muxValue << shift));
...
uint32_t RKX12x_HAL_CRU_CORE_ClkGetMux(struct hwclk *hw, uint32_t muxName)
{
    ...
    muxValue = (RKX12x_HAL_CRU_CORE_Read(hw, hw->sel_con0 + REG_X4(index)) & mask) >> shift;
    ...
    for (c = hw->ops->clkTable; c->name; c++) {
        if (c->type == DUMP_INT) {
            if (c->numParents == 1) {
                ...
                mux = RKX12x_HAL_CRU_CORE_ClkGetMux(hw, clkMux);
            }
            freq = RKX12x_HAL_CRU_CORE_ClkGetFreq(hw, c->clk);
...
HAL_Status RKX12x_HAL_CRU_CORE_SetGlbSrst(struct hwclk *hw, eCRU_GlbSrstType type)
{
    ...
    if (type == GLB_SRST_FST) {
        RKX12x_HAL_CRU_CORE_Write(hw, hw->gbl_srst_fst, GLB_SRST_FST);
    }
...
HAL_Status RKX12x_HAL_CRU_CORE_Register(struct hwclk *hw, struct xferclk *xfer)
{
    ...
    switch (xfer->type) {
    case CLK_RKX120:
        if (xfer->version == RKX120_DES_CLK) {
            hw->ops = &rkx120_des_clk_ops;
        } else if (xfer->version == RKX121_DES_CLK) {
            hw->ops = &rkx121_des_clk_ops;
        } else {
