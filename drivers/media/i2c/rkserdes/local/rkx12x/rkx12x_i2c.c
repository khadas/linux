...
int rkx12x_i2c_reg_update(struct i2c_client *client, u32 reg, u32 mask, u32 val)
{
	...
	ret = rkx12x_i2c_reg_read(client, reg, &value);
	if (ret)
		return ret;

	value &= ~mask;
	value |= (val & mask);
	ret = rkx12x_i2c_reg_write(client, reg, value);
	if (ret)
		return ret;
...
static int rkx12x_i2c_write_array(struct i2c_client *client,
				const struct rkx12x_i2c_regval *regs)
{
	u32 i = 0;
	int ret = 0;

	for (i = 0; (ret == 0) && (regs[i].reg_addr != RKX12X_I2C_REG_NULL); i++) {
		if (regs[i].reg_addr == RKX12X_I2C_REG_DELAY) {
		...
		if (regs[i].val_mask != 0)
			ret = rkx12x_i2c_reg_update(client,
					regs[i].reg_addr, regs[i].val_mask, regs[i].reg_val);
		else
			ret = rkx12x_i2c_reg_write(client,
...
static int rkx12x_i2c_parse_init_seq(struct device *dev,
		const u32 *seq_data, int data_len, struct rkx12x_i2c_init_seq *init_seq)
{
	struct rkx12x_i2c_regval *reg_val = NULL;
	...
	init_seq->reg_init_seq = devm_kcalloc(dev, init_seq->reg_seq_size,
					sizeof(struct rkx12x_i2c_regval), GFP_KERNEL);
	...
	// End register Flag
	init_seq->reg_init_seq[init_seq->reg_seq_size - 1].reg_addr = RKX12X_I2C_REG_NULL;
				...
				struct rkx12x_i2c_init_seq *init_seq)
{
	...
	init_seq->seq_item_size = sizeof(struct rkx12x_i2c_regval);
	...
	ret = rkx12x_i2c_parse_init_seq(dev,
				...
				struct rkx12x_i2c_init_seq *init_seq)
{
	...
	ret = rkx12x_i2c_write_array(client, init_seq->reg_init_seq);
