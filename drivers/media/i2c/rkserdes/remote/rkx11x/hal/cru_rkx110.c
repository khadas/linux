...
static uint32_t RKX11x_HAL_CRU_ClkGetFreq(struct hwclk *hw, uint32_t clockName)
{
    ...
    case RKX110_CPS_PLL_RXPLL:
        hw->pllRate[RKX110_RXPLL] = RKX11x_HAL_CRU_CORE_GetPllFreq(hw, &RXPLL_SETUP);

        return hw->pllRate[RKX110_RXPLL];

    case RKX110_CPS_PLL_CPLL:
        hw->pllRate[RKX110_CPLL] = RKX11x_HAL_CRU_CORE_GetPllFreq(hw, &CPLL_SETUP);
    ...
    case RKX110_CPS_CLK_I2S_SRC_RKLINK_TX:
        freq = RKX11x_HAL_CRU_CORE_MuxGetFreq3(hw, clkMux, hw->pllRate[RKX110_RXPLL],
    ...
    case RKX110_CPS_BUSCLK_RX_PRE0:
        freq = RKX11x_HAL_CRU_CORE_MuxGetFreq2(hw, clkMux, hw->pllRate[RKX110_RXPLL],
    ...
    case RKX110_CPS_BUSCLK_RX_PRE:
        ...
        freq = RKX11x_HAL_CRU_CORE_MuxGetFreq2(hw, clkMux, OSC_24M, pRate);
        break;
    ...
    if (clkDiv) {
        freq /= (RKX11x_HAL_CRU_CORE_ClkGetDiv(hw, clkDiv));
    }
...
static HAL_Status RKX11x_HAL_CRU_ClkSetFreq(struct hwclk *hw, uint32_t clockName, uint32_t rate)
{
    ...
    switch (clockName) {
    case RKX110_CPS_PLL_RXPLL:
        ret = RKX11x_HAL_CRU_CORE_SetPllFreq(hw, &RXPLL_SETUP, rate);
        if (ret != HAL_OK) {
    ...
    case RKX110_CPS_PLL_CPLL:
        ret = RKX11x_HAL_CRU_CORE_SetPllFreq(hw, &CPLL_SETUP, rate);
        if (ret != HAL_OK) {
    ...
    case RKX110_CPS_CLK_CAM1_OUT2IO:
    case RKX110_CPS_CLK_CIF_OUT2IO:
        mux = RKX11x_HAL_CRU_CORE_RoundFreqGetMux3(hw, rate, hw->pllRate[RKX110_RXPLL],
    ...
    /* pre-bus */
    case RKX110_CPS_BUSCLK_RX_PRE0:
        mux = RKX11x_HAL_CRU_CORE_RoundFreqGetMux2(hw, rate, hw->pllRate[RKX110_RXPLL],
    ...
    case RKX110_CPS_BUSCLK_RX_PRE:
        if (rate == OSC_24M) {
            return RKX11x_HAL_CRU_CORE_ClkSetMux(hw, clkMux, 0);
        } else {
            RKX11x_HAL_CRU_ClkSetFreq(hw, RKX110_CPS_BUSCLK_RX_PRE0, rate);

            return RKX11x_HAL_CRU_CORE_ClkSetMux(hw, clkMux, 1);
        }
        break;
    ...
    case RKX110_CPS_CLK_PMA2LINK2PCS_CM:
        ...
        RKX11x_HAL_CRU_CORE_ClkSetMux(hw, clkMux, 0);
    ...
    if (clkDiv) {
        ...
        RKX11x_HAL_CRU_CORE_ClkSetDiv(hw, clkDiv, div);
    }

    if (clkMux) {
        RKX11x_HAL_CRU_CORE_ClkSetMux(hw, clkMux, mux);
    }
...
static void RKX11x_HAL_CRU_Init_SSCG(struct hwclk *hw)
{
    ...
    RKX11x_HAL_CRU_CORE_Write(hw, hw->cru_base + 0x0c, 0x1f000000 | ((amplitude & 0x1f) << 8));
    RKX11x_HAL_CRU_CORE_Write(hw, hw->cru_base + 0x0c, 0x00f00050);
    RKX11x_HAL_CRU_CORE_Write(hw, hw->cru_base + 0x0c, 0x00080000 | ((down_spread & 0x1) << 3));
    RKX11x_HAL_CRU_CORE_Write(hw, hw->cru_base + 0x04, 0x10000000);
    RKX11x_HAL_CRU_CORE_Write(hw, hw->cru_base + 0x0c, 0x00070000);
    CRU_MSG("%s: CPLL enable SSCG\n", hw->name);
#endif
#if RKX110_SSCG_RXPLL_EN
    /* down-spread, 0.8%, 37.5khz */
    RKX11x_HAL_CRU_CORE_Write(hw, hw->cru_base + 0x2c, 0x1f000000 | ((amplitude & 0x1f) << 8));
    RKX11x_HAL_CRU_CORE_Write(hw, hw->cru_base + 0x2c, 0x00f00050);
    RKX11x_HAL_CRU_CORE_Write(hw, hw->cru_base + 0x2c, 0x00080000 | ((down_spread & 0x1) << 3));
    RKX11x_HAL_CRU_CORE_Write(hw, hw->cru_base + 0x24, 0x10000000);
    RKX11x_HAL_CRU_CORE_Write(hw, hw->cru_base + 0x2c, 0x00070000);
                                             ...
                                             uint32_t muxValue, uint32_t divValue)
{
    ...
    RKX11x_HAL_CRU_CORE_Write(hw, RKX110_GRF_GPIO0B_IOMUX_H, GPIO0B7_TEST_CLKOUT);

    /* Enable clock */
    RKX11x_HAL_CRU_CORE_ClkEnable(hw, RKX110_CLK_TESTOUT_TOP_GATE);

    /* Mux, div */
    RKX11x_HAL_CRU_CORE_ClkSetDiv(hw, clkDiv, divValue);
    RKX11x_HAL_CRU_CORE_ClkSetMux(hw, clkMux, muxValue);
...
static uint32_t RKX11x_HAL_CRU_ClkGetExtFreq(struct hwclk *hw, uint32_t clk)
{
    ...
    RKX11x_HAL_CRU_CORE_ClkSetMux(hw, clkMux, 0);
    HAL_SleepMs(2);
    RKX11x_HAL_CRU_CORE_ClkSetMux(hw, clkMux, clk);
    HAL_SleepMs(2);
    freq = RKX11x_HAL_CRU_CORE_Read(hw, CAL_FREQ_REG);
...
struct clkOps rkx110_ser_clk_ops =
{
