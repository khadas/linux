...
static uint32_t RKX11x_HAL_CRU_ClkGetFreq(struct hwclk *hw, uint32_t clockName)
{
    ...
    case RKX111_CPS_CLK_D_LVDS0_RKLINK_TX:
        ...
        freq = RKX11x_HAL_CRU_CORE_MuxGetFreq2(hw, clkMux, pRate0, pRate1);
        break;
...
static uint32_t RKX11x_HAL_CRU_ClkGetExtFreq(struct hwclk *hw, uint32_t clk)
{
    ...
    RKX11x_HAL_CRU_CORE_Write(hw, CAL_FREQ_EN_REG, 1); /* NOTE: 1: disable, 0: enable */
    RKX11x_HAL_CRU_CORE_Write(hw, CAL_FREQ_EN_REG, 0);
    HAL_SleepMs(3);
    freq = RKX11x_HAL_CRU_CORE_Read(hw, CAL_FREQ_REG);
    RKX11x_HAL_CRU_CORE_Write(hw, CAL_FREQ_EN_REG, 1);
...
struct clkOps rkx111_ser_clk_ops =
{
