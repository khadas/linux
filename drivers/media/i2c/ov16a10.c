				    ...
				    struct v4l2_subdev_frame_interval *fi)
{
	...
	if (ov16a10->streaming)
		fi->interval = ov16a10->cur_fps;
	else
...
static const struct ov16a10_mode *ov16a10_find_mode(struct ov16a10 *ov16a10, int fps)
{
	const struct ov16a10_mode *mode = NULL;
	const struct ov16a10_mode *match = NULL;
	int cur_fps = 0;
	int i = 0;

	for (i = 0; i < ov16a10->cfg_num; i++) {
		mode = &supported_modes[i];
		if (mode->width == ov16a10->cur_mode->width &&
		    mode->height == ov16a10->cur_mode->height &&
		    mode->hdr_mode == ov16a10->cur_mode->hdr_mode &&
		    mode->bus_fmt == ov16a10->cur_mode->bus_fmt) {
...
static int ov16a10_s_frame_interval(struct v4l2_subdev *sd,
				   struct v4l2_subdev_frame_interval *fi)
{
	struct ov16a10 *ov16a10 = to_ov16a10(sd);
	const struct ov16a10_mode *mode = NULL;
	...
	mode = ov16a10_find_mode(ov16a10, fps);
	if (mode == NULL) {
	...
	case RKMODULE_SET_HDR_CFG:
		...
		if (i == ov16a10->cfg_num) {
			...
			__v4l2_ctrl_modify_range(ov16a10->vblank, h,
						 ...
						 1, h);
			ov16a10->cur_fps = ov16a10->cur_mode->max_fps;
...
static const struct v4l2_subdev_video_ops ov16a10_video_ops = {
	...
	.s_frame_interval = ov16a10_s_frame_interval,
};
...
static void ov16a10_modify_fps_info(struct ov16a10 *ov16a10)
{
	const struct ov16a10_mode *mode = ov16a10->cur_mode;

	ov16a10->cur_fps.denominator = mode->max_fps.denominator * mode->vts_def /
				      ov16a10->cur_vts;
}
...
static int ov16a10_set_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_CID_VBLANK:
		...
		ov16a10->cur_vts = ctrl->val + ov16a10->cur_mode->height;
		ov16a10_modify_fps_info(ov16a10);
		break;
