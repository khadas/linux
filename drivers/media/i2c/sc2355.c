...
static const struct sc2355_mode *sc2355_find_mode(struct sc2355 *sc2355, int fps)
{
	const struct sc2355_mode *mode = NULL;
	const struct sc2355_mode *match = NULL;
	int cur_fps = 0;
	int i = 0;

	for (i = 0; i < ARRAY_SIZE(supported_modes); i++) {
		mode = &supported_modes[i];
		if (mode->width == sc2355->cur_mode->width &&
		    mode->height == sc2355->cur_mode->height &&
		    mode->bus_fmt == sc2355->cur_mode->bus_fmt) {
...
static int sc2355_s_frame_interval(struct v4l2_subdev *sd,
				   struct v4l2_subdev_frame_interval *fi)
{
	struct sc2355 *sc2355 = to_sc2355(sd);
	const struct sc2355_mode *mode = NULL;
	...
	mode = sc2355_find_mode(sc2355, fps);
	if (mode == NULL) {
...
static const struct v4l2_subdev_video_ops sc2355_video_ops = {
	...
	.s_frame_interval = sc2355_s_frame_interval,
};
