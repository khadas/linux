...
static bool checkreturn pb_decode_varint32_eof(pb_istream_t *stream, uint32_t *dest, bool *eof)
{
	...
	if ((byte & 0x80) == 0)
		...
		do {
			...
			if (bitpos >= 32) {
				/* The varint could have trailing 0x80 bytes,
				 * or 0xFF for negative.
				 */
 ...
 */
static bool checkreturn allocate_field(pb_istream_t *stream, void *pData, size_t data_size,
				       size_t array_size)
{
	void *ptr = *(void **)pData;
...
static bool checkreturn decode_pointer_field(pb_istream_t *stream, pb_wire_type_t wire_type,
					     pb_field_iter_t *iter)
{
		...
		&& PB_LTYPE(type) <= PB_LTYPE_LAST_PACKABLE) {
			...
			while (substream.bytes_left) {
				if ((size_t)*size + 1 > allocated_size) {
					 ...
					 * upwards.
					 */
					allocated_size += (substream.bytes_left - 1) /
							   iter->pos->data_size + 1;

					if (!allocate_field(&substream, iter->pData,
							    iter->pos->data_size, allocated_size)) {
						status = false;
						break;
					}
				}
...
static bool checkreturn pb_dec_fixed_length_bytes(pb_istream_t *stream, const pb_field_t *field,
						  void *dest)
{
	uint32_t size;
	...
	if (size == 0) {
		/* As a special case, treat empty bytes string
		 * as all zeros for fixed_length_bytes.
		 */
