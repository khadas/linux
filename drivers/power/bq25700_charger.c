...
#define DEVICE_ID_MASK ( BIT(7) | BIT(6) | BIT(5) | BIT(4) | BIT(3) | BIT(2) | BIT(1) | BIT(0))
#define DEVICE_ID_SHIFT 0

#define OUTPUT_BAT_VOL 0x2c
#define OUTPUT_BAT_VOL_MASK ( BIT(6) | BIT(5) | BIT(4) | BIT(3) | BIT(2) | BIT(1) | BIT(0) )
#define OUTPUT_BAT_VOL_SHIFT 0

#define OUTPUT_CHG_CUR 0x24
#define OUTPUT_CHG_CUR_MASK  ( BIT(6) | BIT(5) | BIT(4) | BIT(3) | BIT(2) | BIT(1) | BIT(0) )
#define OUTPUT_CHG_CUR_SHIFT 0

#define CHARGE_CURRENT  0x02
#define CHARGE_CURRENT_MASK (BIT(12) | BIT(11) | BIT(10) | BIT(9) | BIT(8) | BIT(7) | BIT(6))
...
#define MIN_SYS_VOTAGE_MASK (BIT(13) | BIT(12) | BIT(11) | BIT(10) | BIT(9) | BIT(8))
#define MIN_SYS_VOTAGE_SHIFT 8

#define INPUT_CURRENT 0x0e
#define INPUT_CURRENT_MASK (BIT(14) | BIT(13) | BIT(12) | BIT(11) | BIT(10) | BIT(9) | BIT(8))
#define INPUT_CURRENT_SHIFT 8

#define INPUT_VOLTAGE 0x0a
#define INPUT_VOLTAGE_MASK (BIT(13) | BIT(12) | BIT(11) | BIT(10) | BIT(9) | BIT(8) | BIT(7) | BIT(6))
#define INPUT_VOLTAGE_SHIFT 6

#define MAX_CHARGE_VOLTAGE 0x04
#define MAX_CHARGE_VOLTAGE_MASK (BIT(14) | BIT(13) | BIT(12) | BIT(11) | BIT(10) | BIT(9) | BIT(8) | \
...
#define OTG_VOLTAGE_MASK (BIT(13) | BIT(12) | BIT(11) | BIT(10) | BIT(9) | BIT(8) | BIT(7) | BIT(6))
#define OTG_VOLTAGE_SHIFT 6

#define OTG_CURRENT 0x08
#define OTG_CURRENT_MASK (BIT(14) | BIT(13) | BIT(12) | BIT(11) | BIT(10) | BIT(9) | BIT(8))
				  ...
				  struct bq25700_state *state)
{
	...
	for (i = 0; i < ARRAY_SIZE(state_fields); i++) {
		ret = bq25700_read(charger, state_fields[i].id);
		if (ret < 0)
			return ret;
	...
	ret = bq25700_read_mask(charger, AC_STAT, AC_STAT_MASK, AC_STAT_SHIFT, &val);
	if (ret < 0)
		return ret;
	state->ac_stat = val;
	ret = bq25700_read_mask(charger, ICO_DONE, ICO_DONE_MASK, ICO_DONE_SHIFT, &val);
	if (ret < 0)
		return ret;
	state->ico_done = val;
	ret = bq25700_read_mask(charger, IN_VINDPM, IN_VINDPM_MASK, IN_VINDPM_SHIFT, &val);
	if (ret < 0)
		return ret;
	state->in_vindpm = val;
	ret = bq25700_read_mask(charger, IN_IINDPM, IN_IINDPM_MASK, IN_IINDPM_SHIFT, &val);
	if (ret < 0)
		return ret;
	state->in_iindpm = val;
	ret = bq25700_read_mask(charger, IN_FCHRG, IN_FCHRG_MASK, IN_FCHRG_SHIFT, &val);
	if (ret < 0)
		return ret;
	state->in_fchrg = val;
	ret = bq25700_read_mask(charger, IN_PCHRG, IN_PCHRG_MASK, IN_PCHRG_SHIFT, &val);
	if (ret < 0)
		return ret;
	state->in_pchrg = val;
	ret = bq25700_read_mask(charger, IN_OTG, IN_OTG_MASK, IN_OTG_SHIFT, &val);
	if (ret < 0)
		return ret;
	state->in_otg = val;
	ret = bq25700_read_mask(charger, F_ACOV, F_ACOV_MASK, F_ACOV_SHIFT, &val);
	if (ret < 0)
		return ret;
	state->fault_acov = val;
	ret = bq25700_read_mask(charger, F_BATOC, F_BATOC_MASK, F_BATOC_SHIFT, &val);
	if (ret < 0)
		return ret;
	state->fault_batoc = val;
	ret = bq25700_read_mask(charger, F_ACOC, F_ACOC_MASK, F_ACOC_SHIFT, &val);
	if (ret < 0)
		return ret;
	state->fault_acoc = val;
	ret = bq25700_read_mask(charger, SYSOVP_STAT, SYSOVP_STAT_MASK, SYSOVP_STAT_SHIFT, &val);
	if (ret < 0)
		return ret;
	state->sysovp_stat = val;
	ret = bq25700_read_mask(charger, F_LATCHOFF, F_LATCHOFF_MASK, F_LATCHOFF_SHIFT, &val);
	if (ret < 0)
		return ret;
	state->fault_latchoff = val;
	ret = bq25700_read_mask(charger, F_OTG_OVP, F_OTG_OVP_MASK, F_OTG_OVP_SHIFT, &val);
	if (ret < 0)
		return ret;
	state->fault_otg_ovp = val;

	ret = bq25700_read_mask(charger, F_OTG_OCP, F_OTG_OCP_MASK, F_OTG_OCP_SHIFT, &val);
	if (ret < 0)
		return ret;
...
//	ret = bq25700_field_write(charger, WDTWR_ADJ, 0);
	ret = bq25700_write_mask(charger, WDTWR_ADJ, WDTWR_ADJ_MASK, WDTWR_ADJ_SHIFT, 0);
	if (ret < 0)
		return ret;
	...
	//		return ret;
	//}
	ret = bq25700_write_mask(charger, CHARGE_CURRENT, CHARGE_CURRENT_MASK, CHARGE_CURRENT_SHIFT, charger->init_data.ichg);
	if (ret < 0)
		return ret;
	ret = bq25700_write_mask(charger, MAX_CHARGE_VOLTAGE, MAX_CHARGE_VOLTAGE_MASK, MAX_CHARGE_VOLTAGE_SHIFT, charger->init_data.max_chg_vol);
	if (ret < 0)
		return ret;
//	ret = bq25700_write_mask(charger, INPUT_CURRENT, INPUT_CURRENT_MASK, INPUT_CURRENT_SHIFT, charger->init_data.input_current);
//	printk("+++++INPUT_CURRENT+++ret=0x%x\n", ret);
//	if (ret < 0)
//		return ret;
	ret = bq25700_write_mask(charger, MIN_SYS_VOTAGE, MIN_SYS_VOTAGE_MASK, MIN_SYS_VOTAGE_SHIFT, charger->init_data.sys_min_voltage);
	if (ret < 0)
		return ret;
	ret = bq25700_write_mask(charger, OTG_VOLTAGE, OTG_VOLTAGE_MASK, OTG_VOLTAGE_SHIFT , charger->init_data.otg_current);
	if (ret < 0)
		return ret;
	ret = bq25700_write_mask(charger, OTG_CURRENT, OTG_CURRENT_MASK, OTG_CURRENT_SHIFT, charger->init_data.otg_current);
	if (ret < 0)
		return ret;
	


	bq25700_read_mask(charger, CHARGE_CURRENT, CHARGE_CURRENT_MASK, CHARGE_CURRENT_SHIFT, &val);
	DBG("    CHARGE_CURRENT: %dmA\n", val*64);
	bq25700_read_mask(charger, MAX_CHARGE_VOLTAGE, MAX_CHARGE_VOLTAGE_MASK, MAX_CHARGE_VOLTAGE_SHIFT, &val);
	DBG("MAX_CHARGE_VOLTAGE: %dmV\n", val*16);
	bq25700_read_mask(charger, INPUT_VOLTAGE, INPUT_VOLTAGE_MASK, INPUT_VOLTAGE_SHIFT, &val);
	DBG("	  INPUT_VOLTAGE: %dmV\n", 3200 + val*64);
	bq25700_read_mask(charger, INPUT_CURRENT, INPUT_CURRENT_MASK, INPUT_CURRENT_SHIFT, &val);
	DBG("	  INPUT_CURRENT: %dmA\n", val*50);
	bq25700_read_mask(charger, MIN_SYS_VOTAGE, MIN_SYS_VOTAGE_MASK, MIN_SYS_VOTAGE_SHIFT, &val);
	DBG("	 MIN_SYS_VOTAGE: %dmV\n", 1024 + val*256);
	/* Configure ADC for continuous conversions. This does not enable it. */

	ret = bq25700_write_mask(charger, EN_LWPWR, EN_LWPWR_MASK, EN_LWPWR_SHIFT, 0);
	if (ret < 0) {
		DBG("error: EN_LWPWR\n");
		return ret;
	}

	ret = bq25700_write_mask(charger, ADC_CONV, ADC_CONV_MASK, ADC_CONV_SHIFT, 1);
	if (ret < 0) {
		DBG("error: ADC_CONV\n");
		return ret;
	}

	ret = bq25700_write_mask(charger, ADC_START, ADC_START_MASK, ADC_START_SHIFT, 1);
	if (ret < 0) {
		DBG("error: ADC_START\n");
		return ret;
	}

	ret = bq25700_write_mask(charger, ADC_FULLSCALE, ADC_FULLSCALE_MASK, ADC_FULLSCALE_SHIFT, 1);
	if (ret < 0) {
		DBG("error: ADC_FULLSCALE\n");
		return ret;
	}

	ret = bq25700_write_mask(charger, EN_ADC_CMPIN, EN_ADC_CMPIN_MASK, EN_ADC_CMPIN_SHIFT, 1);
	if (ret < 0) {
		DBG("error: EN_ADC_CMPIN\n");
		return ret;
	}

	ret = bq25700_write_mask(charger, EN_ADC_VBUS, EN_ADC_VBUS_MASK, EN_ADC_VBUS_SHIFT, 1);
	if (ret < 0) {
		DBG("error: EN_ADC_VBUS\n");
		return ret;
	}

	ret = bq25700_write_mask(charger, EN_ADC_PSYS, EN_ADC_PSYS_MASK, EN_ADC_PSYS_SHIFT, 1);
	if (ret < 0) {
		DBG("error: EN_ADC_PSYS\n");
		return ret;
	}

	ret = bq25700_write_mask(charger, EN_ADC_IIN, EN_ADC_IIN_MASK, EN_ADC_IIN_SHIFT, 1);
	if (ret < 0) {
		DBG("error: EN_ADC_IIN\n");
		return ret;
	}

	ret = bq25700_write_mask(charger, EN_ADC_IDCHG, EN_ADC_IDCHG_MASK, EN_ADC_IDCHG_SHIFT, 1);
	if (ret < 0) {
		DBG("error: EN_ADC_IDCHG\n");
		return ret;
	}

	ret = bq25700_write_mask(charger, EN_ADC_ICHG, EN_ADC_ICHG_MASK, EN_ADC_ICHG_SHIFT, 1);
	if (ret < 0) {
		DBG("error: EN_ADC_ICHG\n");
		return ret;
	}

	ret = bq25700_write_mask(charger, EN_ADC_VSYS, EN_ADC_VSYS_MASK, EN_ADC_VSYS_SHIFT, 1);
	if (ret < 0) {
		DBG("error: EN_ADC_VSYS\n");
		return ret;
	}

	ret = bq25700_write_mask(charger, EN_ADC_VBAT, EN_ADC_VBAT_MASK, EN_ADC_VBAT_SHIFT,  1);
	if (ret < 0) {
...
static void bq25700_enable_charger(struct bq25700_device *charger,
				   u32 input_current)
{
	bq25700_write_mask(charger, INPUT_CURRENT, INPUT_CURRENT_MASK, INPUT_CURRENT_SHIFT, input_current);
	bq25700_write_mask(charger, CHARGE_CURRENT, CHARGE_CURRENT_MASK, CHARGE_CURRENT_SHIFT, charger->init_data.ichg);
}
					     ...
					     union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CONSTANT_CHARGE_CURRENT:
		/* read measured value */
		ret = bq25700_read_mask(bq, OUTPUT_CHG_CUR, OUTPUT_CHG_CUR_MASK, OUTPUT_CHG_CUR_SHIFT, &v);
		if (ret < 0)
			return ret;
	...
	case POWER_SUPPLY_PROP_CONSTANT_CHARGE_VOLTAGE:
		...
		/* read measured value */
		ret = bq25700_read_mask(bq, OUTPUT_BAT_VOL, OUTPUT_BAT_VOL_MASK, OUTPUT_BAT_VOL_SHIFT, &v);
		if (ret < 0)
			return ret;
...
static irqreturn_t bq25700_irq_handler_thread(int irq, void *private)
{
	...
	if (val) {
		...
		bq25700_enable_charger(charger, charger->init_data.input_current_dcp);	
		bq25700_get_chip_state(charger, &state);
		charger->state = state;
...
static void bq25700_charger_evt_worker(struct work_struct *work)
{
	struct bq25700_device *charger = container_of(work,
				struct bq25700_device, ac_work.work);
	bq25700_enable_charger(charger, charger->init_data.input_current_dcp);

}

static int bq25700_init_ac(struct bq25700_device *charger)
{
	charger->ac_charger_wq = alloc_ordered_workqueue("%s",
							WQ_MEM_RECLAIM |
							WQ_FREEZABLE,
							"bq25700-ac-wq");	
	INIT_DELAYED_WORK(&charger->ac_work, bq25700_charger_evt_worker);
	queue_delayed_work(charger->ac_charger_wq, &charger->ac_work,
                           msecs_to_jiffies(10));
}
*/
...
*/
	...
	ret = bq25700_read_mask(charger, DEVICE_ID, DEVICE_ID_MASK, DEVICE_ID_SHIFT, &val);
	if (ret < 0) {
