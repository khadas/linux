...
#define P_SYS_CPU_CFG0		0xc8834648

static void __iomem *aml_sys_cpu_cfg0;

static void enable_cpu_dbg(int cpu, int en)
{
	u32 reg_v;
	if (aml_sys_cpu_cfg0) {
		if (en)
			reg_v = readl_relaxed(aml_sys_cpu_cfg0) | (1 << cpu);
		else
			reg_v = readl_relaxed(aml_sys_cpu_cfg0) & (~(1 << cpu));
		writel_relaxed(reg_v, aml_sys_cpu_cfg0);
	}
}
...
static int etm4_probe(struct amba_device *adev, const struct amba_id *id)
{
	...
	aml_sys_cpu_cfg0 = ioremap(P_SYS_CPU_CFG0, 4);
