...
static int __init dm_setup(char *str)
{
	...
	DMINFO("dm: will configure '%s' on dm-%d\n",
...
parse_fail:
	DMERR("dm: Invalid arguments supplied to dm=.\n");
	return 0;
}
...
dm_create_fail:
	dm_setup_cleanup();
parse_fail:
	DMINFO("dm: starting dm-%d (%s) failed\n",
...
void __init dm_run_setup(void)
{
	if (!dm_early_setup)
		return;
	DMINFO("dm: attempting early device configuration.\n");
	dm_setup_drive();
}
