...
/* ((SYS_MEM_START+SYS_MEM_SIZE)-AUDIO_DSP_MEM_SIZE) */
#define AUDIO_DSP_START_ADDR \
	((unsigned)phys_to_virt(AUDIO_DSP_START_PHY_ADDR))
...
/* ------------------------------------------------ */
/*
* as iec958 ouput maybe be enabled
* before the spdif module inititation finish in the spdif driver,
* in this case,it will cause 958 module not work.
* so add a protocal register to co-work for that
*/
...
#define MDEC_TRIGGER_IRQ(irq) \
do { \
	SET_VREG_MASK(MB0_SEL, 1<<irq); \
	WRITE_VREG(MB0_REG, 1 << irq); \
} while (0)

#if 0	/* MESON_CPU_TYPE >= MESON_CPU_TYPE_MESON8 //>= m8 */
#define SYS_TRIGGER_IRQ(irq)    WRITE_VREG(MB1_REG, 1 << irq);

#define SYS_CLEAR_IRQ(irq)	WRITE_VREG(MB1_CLR, 1 << irq);

#define DSP_TRIGGER_IRQ(irq)	WRITE_VREG(MB2_REG, 1 << irq);
#else	/*  */
#define SYS_TRIGGER_IRQ(irq) \
do { \
	SET_VREG_MASK(MB1_SEL, 1<<irq); \
	WRITE_VREG(MB1_REG, 1 << irq); \
} while (0)

#define SYS_CLEAR_IRQ(irq) \
do { \
	CLEAR_VREG_MASK(MB1_SEL, 1<<irq); \
	WRITE_VREG(MB1_CLR, 1 << irq); \
} while (0)

#define DSP_TRIGGER_IRQ(irq) \
do { \
	SET_VREG_MASK(MB2_SEL, 1<<irq); \
	WRITE_VREG(MB2_REG, 1 << irq); \
} while (0)
#endif	/*  */

#define MDEC_CLEAR_IRQ(irq) \
do { \
	CLEAR_VREG_MASK(MB0_SEL, 1<<irq); \
	WRITE_VREG(MB0_CLR, 1 << irq); \
} while (0)

#define DSP_CLEAR_IRQ(irq) \
do { \
	CLEAR_VREG_MASK(MB2_SEL, 1<<irq); \
...
#define DSP_RD(reg)	  (*((unsigned long *)reg))
